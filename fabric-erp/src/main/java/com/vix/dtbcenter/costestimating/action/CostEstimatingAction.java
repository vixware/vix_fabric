 ...
 * 
 */
package com.vix.dtbcenter.costestimating.action;
...
@Controller
@Scope("prototype")
public class CostEstimatingAction extends BaseAction {

	private static final long serialVersionUID = 1L;
	Logger logger = Logger.getLogger(CostEstimatingAction.class);
	@Autowired
	private CostEstimatingController costEstimatingController;
	private String id;
	private String ids;
	...
	public String goList() {
		try {
			...
			saleOrderCostList = costEstimatingController.doListSaleOrderCostList(params);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return GO_LIST;
	...
	public String goSingleList() {
		try {
			...
			Pager pager = costEstimatingController.doListSaleOrderCost(params, getPager());
			setPager(pager);
			logger.info("");
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String goSaveOrUpdate() {
		try {
			if (StringUtils.isNotEmpty(id) && !id.equals("0")) {
				saleOrderCost = costEstimatingController.doListSaleOrderCostById(id);
				logger.info("");
			} else {
				saleOrderCost = new SaleOrderCost();
				saleOrderCost.setIsTemp("1");
				saleOrderCost = costEstimatingController.doSaveOrUpdateSaleOrderCost(saleOrderCost, null);
			}
	...
	public String saveOrUpdate() {
		...
		try {
			...
			saleOrderCost = costEstimatingController.doSaveOrUpdateSaleOrderCost(saleOrderCost, saleOrderCostItemList);
			if (isSave) {
	...
	public String goCostItemList() {
		try {
			...
			Pager pager = costEstimatingController.doListCostItem(params, getPager());
			setPager(pager);
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		if (costItemidArr != null && !"".equals(costItemidArr) && costItemidArr.length > 0) {
			for (String costItemidid : costItemidArr) {
				...
				if (costItemidid != null && !"".equals(costItemidid)) {
					...
					CostItem costItem = costEstimatingController.doListCostItemById(costItemidid);
					if (costItem != null) {
						try {
							// 将项目费用转化成订单成本明细
							costEstimatingController.convCostItemToSaleOrderCostItem(saleOrderCost, costItem);
							renderText(SAVE_SUCCESS);
						} catch (Exception e) {
							renderText(SAVE_FAIL);
							e.printStackTrace();
	...
	public void getSaleOrderCostItemJson() {
		try {
			...
			if (null != id && !"".equals(id)) {
				// 首先根据ID查询到订单成本
				SaleOrderCost saleOrderCost = costEstimatingController.doListSaleOrderCostById(id);

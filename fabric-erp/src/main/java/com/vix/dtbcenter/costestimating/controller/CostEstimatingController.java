 ...
 * 
 */
package com.vix.dtbcenter.costestimating.controller;
...
import com.vix.dtbcenter.costestimating.domain.CostEstimatingDomain;
 ...
 * @date 2013-6-27
 */
@Controller("costEstimatingController")
@Scope("prototype")
public class CostEstimatingController {

	Logger logger = Logger.getLogger(CostEstimatingController.class);

	@Autowired
	private CostEstimatingDomain costEstimatingDomain;
	...
	public SaleOrderCost doSaveOrUpdateSaleOrderCost(SaleOrderCost saleOrderCost, List<SaleOrderCostItem> saleOrderCostItemList) throws Exception {
		...
		try {
			// 执行保存操作
			saleOrderCost1 = costEstimatingDomain.mergeSaleOrderCost(saleOrderCost);
			if (saleOrderCost1 != null) {
				if (saleOrderCostItemList != null && saleOrderCostItemList.size() > 0) {
					for (SaleOrderCostItem saleOrderCostItem : saleOrderCostItemList) {
						...
						costEstimatingDomain.mergeSaleOrderCostItem(saleOrderCostItem);
					}
				}
			}
			// 保存日志
	...
	public Pager doListSaleOrderCost(Map<String, Object> params, Pager pager) {
		Pager p = null;
		try {
			p = costEstimatingDomain.findSaleOrderCostPagerByHql(params, pager);
			executeLogger(true, "");
		} catch (Exception ex) {
	...
	public Pager doListCostItem(Map<String, Object> params, Pager pager) {
		Pager p = null;
		try {
			p = costEstimatingDomain.findCostItemPagerByHql(params, pager);
			executeLogger(true, "");
		} catch (Exception ex) {
			executeLogger(true, "" + ex.getMessage());
		}
	...
	public Pager doListSaleOrderCostByCon(Map<String, Object> params, Pager pager) {
		Pager p = null;
		try {
			p = costEstimatingDomain.findSaleOrderCostPagerByOrHql(params, pager);
		} catch (Exception ex) {
		}
		return p;
	}
	...
	public SaleOrderCost doListSaleOrderCostById(String id) {
		...
		try {
			// 执行查询操作
			saleOrderCost = costEstimatingDomain.findSaleOrderCostById(id);
	...
	public CostItem doListCostItemById(String id) {
		...
		try {
			// 3.执行查询操作
			costItem = costEstimatingDomain.findCostItemById(id);
	...
	public SaleOrderCostItem convCostItemToSaleOrderCostItem(SaleOrderCost saleOrderCost, CostItem costItem) throws Exception {
		...
		if (costItem != null && saleOrderCost != null) {
			...
			delieryNotification1 = costEstimatingDomain.mergeSaleOrderCostItem(saleOrderCostItem);
		}
	...
	public List<SaleOrderCost> doListSaleOrderCostList(Map<String, Object> params) throws Exception {
		...
		// 3. 执行查询操作
		saleOrderCostList = costEstimatingDomain.findSaleOrderCostList(params);
		// 6. 日志处理 log(); 每个Action中独立定义;记录操作日志、记录性能数据;

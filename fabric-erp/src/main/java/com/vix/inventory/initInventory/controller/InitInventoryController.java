 ...
 * 
 */
package com.vix.inventory.initInventory.controller;
...
import com.vix.inventory.initInventory.domain.InitInventoryDomain;
 ...
 * @date 2013-6-27
 */
@Controller("initInventoryController")
@Scope("prototype")
public class InitInventoryController {

	Logger logger = Logger.getLogger(InitInventoryController.class);

	@Autowired
	private InitInventoryDomain initInventoryDomain;
	...
	public InventoryCurrentStock doSaveInventoryCurrentStock(InventoryCurrentStock inventoryCurrentStock) throws Exception {
		...
		try {
			...
			// 执行保存操作
			inventoryCurrentStock1 = initInventoryDomain.mergeInventoryCurrentStock(inventoryCurrentStock);
	...
	public MasterInventoryCurrentStock doSaveMasterInventoryCurrentStock(MasterInventoryCurrentStock masterInventoryCurrentStock) throws Exception {
		MasterInventoryCurrentStock masterInventoryCurrentStock1 = null;
		try {
			// 处理传入参数 executeParameterProcess(); 抽象方法，需要重载；
			// Map parameters = baExecutor.executeParameterProcess();
			...
			beforeEventTrigger("PO_CREATE_BEFORE");
			// 执行保存操作
			masterInventoryCurrentStock1 = initInventoryDomain.mergeMasterInventoryCurrentStock(masterInventoryCurrentStock);
			// 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
			afterEventTrigger("PO_CREATE_AFTER");
			// 访问功能和业务对象记录操作 visitBORecord(parameter);
			// 日志处理 log(); 每个Action中独立定义;记录操作日志、记录性能数据;
			executeLogger(true, "保存：" + masterInventoryCurrentStock + "成功！");
		} catch (Exception ex) {
			executeLogger(true, "保存：" + masterInventoryCurrentStock + "失败！失败原因：" + ex.getMessage());
			throw new Exception("保存失败", ex);
		}
		return masterInventoryCurrentStock1;
	}

	public List<ObjectExpand> doListObjectExpandList() {
		List<ObjectExpand> objectExpandList = null;
		try {
			objectExpandList = initInventoryDomain.findObjectExpandList();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public List<Specification> doListSpecificationList(Map<String, Object> params) {
		List<Specification> specificationList = null;
		try {
			specificationList = initInventoryDomain.findSpecificationList(params);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public List<SpecificationDetail> doListSpecificationDetail(Map<String, Object> params) {
		List<SpecificationDetail> specificationDetailList = null;
		try {
			specificationDetailList = initInventoryDomain.findSpecificationDetailList(params);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public Pager doListItem(Map<String, Object> params, Pager pager) {
		...
		try {
			...
			beforeEventTrigger("PO_CREATE_BEFORE");
			// 执行保存操作
			p = initInventoryDomain.findItemPagerByHqlConditions(params, pager);
	...
	public Item doListItemById(String id) {
		...
		try {
			...
			// 3.执行查询操作
			item = initInventoryDomain.findItemById(id);
	...
	public MasterInventoryCurrentStock doListMasterInventoryCurrentStockById(String sql, Map<String, Object> params) {
		MasterInventoryCurrentStock item = null;
		try {
			// 1. 处理传入参数 executeParameterProcess(); 抽象方法，需要重载；
			// Map parameters = baExecutor.executeParameterProcess();
			...
			// 3.执行查询操作
			item = initInventoryDomain.findMasterInventoryCurrentStockByHql(sql, params);
			// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
			afterEventTrigger("PO_CREATE_AFTER");
	...
	public SpecificationDetail doListSpecificationDetailById(String id) {
		...
		try {
			specificationDetail = initInventoryDomain.findSpecificationDetailById(id);
		} catch (Exception e) {
			e.printStackTrace();
		}

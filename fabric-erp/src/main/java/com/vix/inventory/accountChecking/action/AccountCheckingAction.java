package com.vix.inventory.accountChecking.action;
...
@Controller
@Scope("prototype")
public class AccountCheckingAction extends BaseAction {

	private static final long serialVersionUID = 1L;
	Logger logger = Logger.getLogger(AccountCheckingAction.class);
	@Autowired
	...
	public void getWimTransvouchItemJson() {
		try {
			...
			if (StringUtils.isNotEmpty(id) && !"0".equals(id)) {
				...
				if (so != null) {
					json = convertListToJson(new ArrayList<WimTransvouchitem>(so.getWimTransvouchitem()), so.getWimTransvouchitem().size(), "wimTransvouch");
				}
			}
			if (!"".equals(json)) {
				renderJson(json);

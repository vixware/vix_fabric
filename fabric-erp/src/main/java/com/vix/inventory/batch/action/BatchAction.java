package com.vix.inventory.batch.action;
...
import com.vix.inventory.batch.controller.BatchController;
...
public class BatchAction extends BaseAction {
	...
	public String goSingleList() {
		try {
			...
			Pager pager = batchService.findPagerByHqlConditions(getPager(), BaseCoder.class, params);
			setPager(pager);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String goSaveOrUpdate() {
		...
		try {
			if (StringUtils.isNotEmpty(billTypeCode)) {
				encodingRulesTableInTheMiddle = batchService.findEntityByAttribute(EncodingRulesTableInTheMiddle.class, "billType", billTypeCode);
	...
	public String saveOrUpdate() {
		...
		boolean isSave = true;
		try {
			encodingRulesTableInTheMiddle = batchController.doSaveEncodingRulesTableInTheMiddle(encodingRulesTableInTheMiddle);
			if (isSave) {
			...
			if (StringUtils.isNotEmpty(nodeId)) {
				if (StringUtils.isNotEmpty(nodeTreeType)) {
					if (nodeTreeType.equals("C")) {
						orgUnitList = batchService.findOrgAndUnitTreeList(nodeTreeType, nodeId);
					}
				}
			} else {
				// id为空 则类型也为空
				Map<String, Object> params = getParams();
				itemCatalogList = batchService.findAllSubEntity(ItemCatalog.class, "parentItemCatalog.id", null, params);
			}

package com.vix.drp.competitiveproducts.action;
...
@Controller
@Scope("prototype")
public class CompetitiveProductsAction extends BaseAction {

	private static final long serialVersionUID = 1L;
	Logger logger = Logger.getLogger(CompetitiveProductsAction.class);
	@Autowired
	...
	public String goList() {
		try {
			...
			competingProductsList = competitiveProductsController.doListCompetingProductsList(params);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return GO_LIST;
	...
	public String goSingleList() {
		try {
			...
			pager = competitiveProductsController.doListCompetingProductsPager(params, pager);
			setPager(pager);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String saveOrUpdate() {
		...
		try {
			...
			initEntityBaseController.initEntityBaseAttribute(competingProducts);
			//处理修改留痕
			billMarkProcessController.processMark(competingProducts, updateField);

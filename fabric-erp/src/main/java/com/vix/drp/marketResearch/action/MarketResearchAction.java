package com.vix.drp.marketResearch.action;
...
public class MarketResearchAction extends BaseAction {
	...
	Logger logger = Logger.getLogger(MarketResearchAction.class);
	@Autowired
	...
	public String goSearchBasicInformationList() {
		try {
			...
			Pager pager = null;
			// 简单搜索
			pager = competitiveProductsController.doListCompetingProductsPager(params, getPager());
			setPager(pager);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void findTreeToJson() {
		try {
			...
			if (null != id && !"".equals(id)) {
				...
				listItemCatalog = competitiveProductsController.findAllItemCatalog("parentItemCatalog.id", "0", params);
			}

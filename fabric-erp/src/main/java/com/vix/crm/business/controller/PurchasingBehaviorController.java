...
import com.vix.crm.business.domain.PurchasingBehaviorDomain;
 ...
 * @date 2013-6-27
 */
@Controller("purchasingBehaviorController")
@Scope("prototype")
public class PurchasingBehaviorController {

	Logger logger = Logger.getLogger(PurchasingBehaviorController.class);

	@Autowired
	private PurchasingBehaviorDomain purchasingBehaviorDomain;
	...
	public Pager doListSalesOrderPager(Map<String, Object> params, Pager pager) throws Exception {
		...
		// 3.执行保存操作
		p = purchasingBehaviorDomain.findSalesOrderPager(params, pager);
	...
	public List<GoodsSaleInformation> doListGoodsSaleInformationList(Map<String, Object> params) throws Exception {
		...
		beforeEventTrigger("PO_CREATE_BEFORE");
		// 3. 执行查询操作
		goodsSaleInformationList = purchasingBehaviorDomain.findGoodsSaleInformationList(params);
		// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");
	...
	public List<MessageTemplate> doListMessageTemplateList(Map<String, Object> params) throws Exception {
		...
		beforeEventTrigger("PO_CREATE_BEFORE");
		// 3. 执行查询操作
		messageTemplateList = purchasingBehaviorDomain.findMessageTemplateList(params);
		// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");
	...
	public List<SalesOrder> doListSalesOrderList(Map<String, Object> params) throws Exception {
		...
		beforeEventTrigger("PO_CREATE_BEFORE");
		// 3. 执行查询操作
		salesOrderList = purchasingBehaviorDomain.findSalesOrderList(params);
		// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");
	...
	public Order doListOrderById(String id) throws Exception {
		...
		// 3.执行查询操作
		order = purchasingBehaviorDomain.findOrderById(id);
		// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");
	...
	public ExpeditingSetting doSaveExpeditingSetting(ExpeditingSetting expeditingSetting) throws Exception {
		...
		// 3.执行查询操作
		expeditingSetting1 = purchasingBehaviorDomain.saveExpeditingSetting(expeditingSetting);
		// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");

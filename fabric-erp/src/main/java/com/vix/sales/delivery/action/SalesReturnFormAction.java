...
@Controller
@Scope("prototype")
public class SalesReturnFormAction extends BaseAction {
	...
	public String goList() {
		try {
			saleReturnFormList = salesReturnController.findSaleReturnFormIndex();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		return GO_LIST;
	...
	public String deleteById() {
		try {
			SaleReturnForm saleReturnForm = salesReturnController.doListEntityById(id);
	...
	public void getSaleReturnFormItemJson() {
		try {
			...
			if (null != id && !"".equals(id)) {
				...
				json = convertListToJson(new ArrayList<SaleReturnFormItem>(sf.getSaleReturnFormItems()), sf.getSaleReturnFormItems().size(), "deliveryDocumentItem");
			}
			if (!"".equals(json)) {
				renderJson(json);
			} else {
	...
	public void getApprovalRecordJson() {
		try {
			...
			if (null != id && !"".equals(id)) {
				SaleReturnForm sf = salesReturnController.findEntityById(id);
				json = convertListToJson(new ArrayList<CMNApprovalRecord>(sf.getApprovalRecords()), sf.getApprovalRecords().size(), "deliveryDocumentItem");
			}
			if (!"".equals(json)) {
				renderJson(json);
			} else {
	...
	public void getAttachmentsJson() {
		try {
			...
			if (null != id && !"".equals(id)) {
				SaleReturnForm sf = salesReturnController.findEntityById(id);
				json = convertListToJson(new ArrayList<Attachments>(sf.getAttachments()), sf.getAttachments().size(), "supplier");
			}
			if (!"".equals(json)) {
				renderJson(json);
			} else {
	...
	public void deleteAttachments() {
		try {
			...
			if (null != afId && !"".equals(afId)) {
				...
				if (!afIdLong.equals("")) {
					Attachments atts = salesReturnController.findAttachmentsEntityById(afIdLong);
					if (null != atts) {
						/** 上传目录 */
						String separator = System.getProperty("file.separator");
						String baseDir = getServletContext().getRealPath(separator + "richContent");
	...
	private void dealBillGroupCode() throws Exception {
		if (null != saleReturnForm && null != saleReturnForm.getId()) {
			...
			p.put("billCode," + SearchCondition.EQUAL, saleReturnForm.getId().toString());
			...
			SaleReturnForm sf = baseHibernateService.findEntityById(SaleReturnForm.class, saleReturnForm.getId());
			Set<String> bgcSet = new HashSet<String>();
			for (SaleReturnFormItem srfi : sf.getSaleReturnFormItems()) {
				if (null != srfi && null != srfi.getGroupCode() && !"".equals(srfi.getGroupCode())) {

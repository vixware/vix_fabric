...
@Controller
@Scope("prototype")
public class SalesReturnFormApprovalOpinionAction extends BaseAction {
	...
	/** 获取采购订单明细json数据 */
	public void getCMNApprovalRecordJson() {
		try {
			String json = "";
			if (null != id && !"".equals(id)) {
				DeliveryDocument dd = baseHibernateService.findEntityById(DeliveryDocument.class, id);
				json = convertListToJson(new ArrayList<CMNApprovalRecord>(dd.getApprovalRecords()), dd.getApprovalRecords().size(), "approvalRecord");
			}
			if (!"".equals(json)) {
				renderJson(json);
			} else {

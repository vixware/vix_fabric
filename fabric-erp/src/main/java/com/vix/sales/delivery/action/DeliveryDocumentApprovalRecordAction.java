...
@Controller
@Scope("prototype")
public class DeliveryDocumentApprovalRecordAction extends BaseAction {
	...
	public String saveOrUpdate() {
		...
		try {
			...
			if (null != id) {
				DeliveryDocument dd = baseHibernateService.findEntityById(
						DeliveryDocument.class, id);
				Employee emp = getEmployee();
				approvalRecord.setCheckPerson(emp.getName());
				approvalRecord = baseHibernateService.merge(approvalRecord);
				dd.getApprovalRecords().add(approvalRecord);
				baseHibernateService.merge(dd);
	...
	public String deleteById() {
		try {
			CMNApprovalRecord ddi = baseHibernateService.findEntityById(
					CMNApprovalRecord.class, id);
			if (null != ddi) {

...
@Controller
@Scope("prototype")
public class VixntContractStageGroupTypeAction extends VixntBaseAction{
	...
	public void goSingleList(){
		try {
			...
			pager = vixntBaseService.findPagerByHqlConditions(pager, ContractStageGroupType.class, params);
	...
	public String goSaveOrUpdate(){
		try {
			if(StringUtils.isNotEmpty(id)){
				contractStageGroupType = vixntBaseService.findEntityById(ContractStageGroupType.class, id);
			}else{
				contractStageGroupType = new ContractStageGroupType();
				contractStageGroupType.setCode(VixUUID.createCode(12));
				contractStageGroupType.setDisabled("1");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void saveOrUpdate(){
		boolean isSave = true;
		try {
			if(StringUtils.isNotEmpty(contractStageGroupType.getId())){
				isSave = false;
			}
			initEntityBaseController.initEntityBaseAttribute(contractStageGroupType);
			contractStageGroupType = vixntBaseService.merge(contractStageGroupType);
			if (isSave) {
	...
	public void deleteById(){
		try {
			if(StringUtils.isNotEmpty(id)){
				contractStageGroupType = vixntBaseService.findEntityById(ContractStageGroupType.class, id);
				if(contractStageGroupType != null){
					vixntBaseService.deleteByEntity(contractStageGroupType);

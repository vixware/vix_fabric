...
@Controller
@Scope("prototype")
public class VixntContractGroupTypeAction extends VixntBaseAction{
	...
	public void goSingleList(){
		try {
			...
			pager = vixntBaseService.findPagerByHqlConditions(pager, ContractGroupType.class, params);
	...
	public String goSaveOrUpdate(){
		try {
			if(StringUtils.isNotEmpty(id)){
				contractGroupType = vixntBaseService.findEntityById(ContractGroupType.class, id);
			}else{
				contractGroupType = new ContractGroupType();
				contractGroupType.setCode(VixUUID.createCode(12));
				contractGroupType.setDisabled("1");
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void saveOrUpdate(){
		boolean isSave = true;
		try {
			if(StringUtils.isNotEmpty(contractGroupType.getId())){
				isSave = false;
			}
			initEntityBaseController.initEntityBaseAttribute(contractGroupType);
			contractGroupType = vixntBaseService.merge(contractGroupType);
			if (isSave) {
		...
		try {
			if(StringUtils.isNotEmpty(id)){
				...
				if(contractGroupType != null){
					vixntBaseService.deleteByEntity(contractGroupType);

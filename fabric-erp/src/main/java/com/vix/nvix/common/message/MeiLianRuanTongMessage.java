...
// 连接超时及读取超时设置
// String encode = "GBK";
// //页编码和短信内容编码为GBK。重要说明：如提交短信后收到乱码，请将GBK改为UTF-8测试。如本程序页面为编码格式为：ASCII/GB2312/GBK则该处为GBK。如本页面编码为UTF-8或需要支持繁体，阿拉伯文等Unicode，请将此处写为：UTF-8
// String username = "zichen"; // 用户名
// String password_md5 = "5d93ceb70e2bf5daa84ec3d0cd2c731a"; // 密码
// String mobile = "18801009650"; 
// 手机号,只发一个号码：13800000001。发多个号码：13800000001,13800000002,...N使用半角逗号分隔。
// String apikey = "d1647838edd30e98d6333c153e5d5e8b"; // apikey秘钥（请登录
// 短信平台-->账号管理-->我的信息 中复制apikey）
// String content = "您好，您的验证码是：12345【点点医生】"; //
// 要发送的短信内容，特别注意：签名必须设置，网页验证码应用需要加添加【图形识别码】。
// 把发送链接存入buffer中，如连接超时，可能是您服务器不支持域名解析，请将下面连接中的：【m.5c.com.cn】修改为IP：【115.28.23.78】
// http://m.5c.com.cn

public class MeiLianRuanTongMessage {
	public static String sendSms(String mesageurl, String username, String password, String mobile, String apikey, String content) throws Exception {
		...
		password = md.encodeString(password);
		String contentUrlEncode = URLEncoder.encode(content, encode); // 对短信内容做Urlencode编码操作。注意：如
		buffer.append(mesageurl + "/api/send/index.php?username=" + username + "&password_md5=" + password + "&mobile=" + mobile + "&apikey=" + apikey + "&content=" + contentUrlEncode + "&encode=" + encode);

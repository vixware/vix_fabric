...
@Controller
@Scope("request")
public class VixntWxPayRechargeAction extends WxBaseAction {
				...
				if (null == wxpWeixinSite || StrUtils.isEmpty(wxpWeixinSite.getAppId()) || StrUtils.isEmpty(wxpWeixinSite.getPrivateKey()) || StrUtils.isEmpty(wxpWeixinSite.getMchID())) {
					...
					if (null != rechargeRecord) {
						if (StrUtils.isNotEmpty(payAmount) && Double.parseDouble(payAmount) > 0) {
							...
							// 测试写死
							// int totalFee = 1;
							ScanPayReqData scanPayReqData = new ScanPayReqData("", "办卡结算", "", String.valueOf(rechargeRecord.getCode()), totalFee, "", getIpAddr(getRequest()), "", "", "");
							// 账号信息
	...
	public void checkPay() {
		try {
			if (StrUtils.isNotEmpty(rechargeRecordId)) {
				rechargeRecord = vixntBaseService.findEntityById(RechargeRecord.class, rechargeRecordId);
				if (rechargeRecord != null) {
					ScanPaySuccessReturnDataEntity entity = null;
					Map<String, Object> p = new HashMap<String, Object>();
					p.put("rechargeRecord.id," + SearchCondition.EQUAL, rechargeRecord.getId());

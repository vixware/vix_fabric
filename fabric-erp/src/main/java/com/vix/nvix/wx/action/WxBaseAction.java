...
import com.vix.nvix.wx.util.MsgReplyMap;
import com.vix.wechat.OAuthTokenData;
import com.vix.wechat.base.util.WxQyUtil;

@Controller
@Scope("request")
public class WxBaseAction extends VixntBaseAction {
	...
	public WxpWeixinSite getParkingCarWxpWeixinSite() {
		try {
	...
	public void checkWxVisitUser(String forward) {
		...
		WxpWeixinSite wxpWeixinSite = getParkingCarWxpWeixinSite();
	...
	public void wxOAuthCallback() {
		System.out.println("wxOAuthCallback:" + getRequest().getRequestURL().toString());
	...
	public String getAppId() {
		String appId = "";
		try {
			Object objCcA = getRequest().getParameter("app_id");
			if (null != objCcA && !"".equals(objCcA.toString())) {
				appId = objCcA.toString();
				getSession().setAttribute("appId", appId);
			} else {
				Object objCc = getSession().getAttribute("appId");
				if (null != objCc && !"".equals(objCc.toString())) {
					appId = objCc.toString();
				}
			}
		} catch (Exception ex) {
			ex.printStackTrace();
	...
	public WxpWeixin getWeixin() {
		try {
			String appId = "";
			if (StrUtils.objectIsNotNull(getAppId())) {
				appId = getAppId();
			} else {
				appId = getRequest().getParameter("app_id");
				getSession().setAttribute("appId", appId);
			}
			if (StrUtils.objectIsNull(appId)) {
				appId = (String) getSession().getAttribute("appId");
			}
			System.out.println("getWeixin:appId = " + appId);
			if (StrUtils.objectIsNotNull(appId)) {
				WxpWeixin weixin = this.vixntBaseService.findEntityByAttribute(WxpWeixin.class, "appId", appId);
				if (null != weixin) {
					return weixin;
				}
			} /*
				 * else{ String domain = getRequest().getServerName(); WxpWeixin
				 * weixin = workService.findEntityByAttribute(WxpWeixin.class,
				 * "domain", domain); if(null != weixin){
				 * getSession().setAttribute("appId", weixin.getAppId()); return
				 * weixin; } }
				 */
		} catch (Exception e1) {
			e1.printStackTrace();
	...
	private MsgReplyMap loadWeixinReplyMapByAppid(String appId) {
		try {
			WxpWeixin weixin = vixntBaseService.findEntityByAttribute(WxpWeixin.class, "appId", appId);
			return this.loadWeixinSiteReplyMap(weixin);
		} catch (Exception e) {
			e.printStackTrace();
			return null;
		}
	...
	private MsgReplyMap loadWeixinSiteReplyMap(WxpWeixin weixin) {
		if (wxSiteReplyMap == null) {
			wxSiteReplyMap = new HashMap<String, MsgReplyMap>();
		}
		MsgReplyMap replyMap = new MsgReplyMap(weixin.getAppId(), menuService);

		wxSiteReplyMap.put(weixin.getAppId(), replyMap);

		return replyMap;
	}

	/**
	 * reply的缓存map，<appid,reply>
	 */
	protected static HashMap<String, MsgReplyMap> wxSiteReplyMap;

	public void reloadWeixinReplyMap(String appId) {
		if (wxSiteReplyMap == null || !wxSiteReplyMap.containsKey(appId)) {
			this.loadWeixinReplyMapByAppid(appId);
		} else {
			try {
				wxSiteReplyMap.get(appId).loadAllReply();
			} catch (Exception e) {
				e.printStackTrace();
			}
		}

...
@Controller
@Scope("prototype")
public class VixntMembershipSubdivisionAction extends VixntBaseAction {
	...
	public void goMembershipSubdivisionDetailList() {
		try {
			Map<String, Object> params = new HashMap<String, Object>();
			Pager pager = getPager();

			if (StringUtils.isNotEmpty(membershipSubdivisionId)) {
				params.put("membershipSubdivision.id," + SearchCondition.EQUAL, membershipSubdivisionId);
				pager = vixntBaseService.findPagerByHqlConditions(pager, MembershipSubdivisionDetail.class, params);
			}
			renderDataTable(pager);
		} catch (Exception e) {
			e.printStackTrace();
			...
			if (StringUtils.isNotEmpty(id)) {
				...
				if (StringUtils.isNotEmpty(parentId)) {
					...
					if (channelDistributor != null) {
						membershipSubdivision.setChannelDistributor(channelDistributor);
					}
				}
	...
	public void saveOrUpdate() {
		...
		try {
			...
			if (isSave) {
				renderText(membershipSubdivision.getId());
			} else {
				renderText(membershipSubdivision.getId());
			}
		} catch (Exception e) {
	...
	public void saveOrUpdateMembershipSubdivisionDetail() {
		boolean isSave = true;
		try {
	...
	public String goSaveOrUpdateMembershipSubdivision() {
		try {
			if (StringUtils.isNotEmpty(id)) {
				...
				if (StringUtils.isNotEmpty(membershipSubdivisionId)) {
					membershipSubdivision = membershipSubdivisionController.doListMembershipSubdivisionById(membershipSubdivisionId);
					if (membershipSubdivision != null) {
						membershipSubdivisionDetail.setMembershipSubdivision(membershipSubdivision);
					}
				}
			}
		} catch (Exception e) {
	...
	public void deleteMembershipSubdivisionDetail() {
		try {
			if (StringUtils.isNotEmpty(id)) {
				membershipSubdivisionDetail = membershipSubdivisionController.doListMembershipSubdivisionDetailById(id);
				if (membershipSubdivisionDetail != null) {
	...
	public void goCustomerListContent() {
		try {
			Map<String, Object> params = getParams();
			Pager pager = getPager();
			// 排序
			...
			if (StringUtils.isNotEmpty(id)) {
				...
				memberNum = pager.getTotalCount();
			}
			renderDataTable(pager);
			/*
			 * customerAccountGroupListViewList =
			 * membershipSubdivisionController.doListCustomerAccountGroupListViewList(params
			 * ); if (customerAccountGroupListViewList != null &&
			 * customerAccountGroupListViewList.size() > 0) { memberNum =
			 * customerAccountGroupListViewList.size(); }
			 */
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void deleteById() {
		try {
			membershipSubdivision = vixntBaseService.findEntityById(MembershipSubdivision.class, id);
			if (membershipSubdivision != null) {
				Map<String, Object> params = getParams();
				params.put("membershipSubdivision.id," + SearchCondition.EQUAL, membershipSubdivision.getId());
				List<MembershipSubdivisionDetail> membershipSubdivisionDetailList = this.vixntBaseService.findAllByConditions(MembershipSubdivisionDetail.class, params);

				if (membershipSubdivisionDetailList != null && membershipSubdivisionDetailList.size() > 0) {
					for (MembershipSubdivisionDetail membershipSubdivisionDetail : membershipSubdivisionDetailList) {
						if (membershipSubdivisionDetail != null) {
							vixntBaseService.deleteByEntity(membershipSubdivisionDetail);
						}
					}
				}
				
				vixntBaseService.deleteByEntity(membershipSubdivision);

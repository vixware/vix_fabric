...
@Controller
@Scope("prototype")
public class NvixntItemAction extends VixntBaseAction {
	...
	public String goSaveOrUpdateService() {
		try {
			Map<String, Object> params = new HashMap<String, Object>();
			measureUnitList = itemService.findAllByConditions(MeasureUnit.class, params);
			currencyTypeList = itemService.findAllByEntityClass(CurrencyType.class);
	...
	public void saveOrUpdate() {
		...
		try {
			...
			if (null != itemPurchaseProperties) {
				...
				item.setItemPurchaseProperties(itemPurchaseProperties);
			}
			/** 库存 */
			if (null != itemInventoryProperties) {
				...
				item.setItemInventoryProperties(itemInventoryProperties);
			}
			item = itemService.merge(item);

...
import com.vix.common.id.exception.InitSequenceGeneratorException;
import com.vix.common.id.storer.DBSequenceStorer;
...
public class DefaultSequenceGenerator implements SequenceGenerator {
	...
	/**
	 * cache大小，用于确定预分配序号数;cache越大，ID生成效率越高，但是当系统 关闭时，可能造成的ID浪费也会更多.
	 */
	 ...
	 * 用于存储／读取　已分配出去的最大序号
	 */
	protected SequenceStorer sequenceStorer = new DBSequenceStorer();

	protected long currCount = 0L;// 当前实际已分配序号最大值
	protected long maxCount = cache + currCount;// 可以分配最大序号值（
	protected String id = null;// 序号的标致符号

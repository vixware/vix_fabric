...
public class Utils {
	...
	private static PropertiesHelper pHelper = PropertiesFactory.getPropertiesHelper(PropertiesFile.VIX);
	...
	public static boolean isTheStyle(String pStr, String pStyle) {
		for (int i = 0; i < pStr.length(); i++) {
			...
			if (pStyle.equals(VixConstants.S_STYLE_N)) {
				if (!Character.isDigit(c))
					return false;
			} else if (pStyle.equals(VixConstants.S_STYLE_L)) {
				if (!Character.isLetter(c))
					return false;
			} else if (pStyle.equals(VixConstants.S_STYLE_NL)) {
...
//			System.out.println("SaUtil.md5Encode   @@@:"+text);
			e.printStackTrace();
			return null;
		}
	}
	/* 有问题，不是md5，相同字符串每次返回值不一样
	...
	@SuppressWarnings("restriction")
	public static byte[] decryptBasedDesByByte(String cryptData) {
		...
		try {
			...
			cipher.init(Cipher.DECRYPT_MODE, key, sr);
			// 把字符串解码为字节数组，并解密
			decryptedData = new sun.misc.BASE64Decoder().decodeBuffer(cryptData);
		} catch (Exception e) {
			throw new RuntimeException("解密错误，错误信息：", e);
		}
		return decryptedData;
	...
	public static Long checkDoubleToLong(Double ddouble){
		Long l = new Double(ddouble).longValue();
		return l;
	}
}

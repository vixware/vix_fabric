...
@Controller
@Scope("prototype")
public class SupplierTenderAction extends BaseAction {
	...
	public String goList() {
		try {
			supplierTenderList = supplierTenderController
					.findSupplierTenderIndex();
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		return GO_LIST;
	...
	public String goSrmTenderList() {
		try {
			...
			Supplier sTemp = supplierTenderController.findSupplierById(useCache
					.getSupplier().getId());
			params.put("id," + SearchCondition.EQUAL, sTemp.getId());
			Pager pager = supplierTenderController.goSingleList(params,
					getPager());
			pager.setResultList(new ArrayList<PurchaseTender>(sTemp
					.getPurchaseTenders()));
	...
	public String goSearchList() {
		try {
			...
			if ("0".equals(i)) {
				...
				pager = supplierTenderController.goSearchList(params,
						getPager());
			}
			// 高级搜索
			else {
				...
				pager = supplierTenderController.goSingleList(params,
						getPager());
			}
	...
	public String goSaveOrUpdate() {
		try {
			if (null != id && !"".equals(id)) {
				purchaseTender = supplierTenderController
						.doListPurchaseTenderById(id);
	...
	public String saveOrUpdate() {
		...
		try {
			...
			supplierTender = supplierTenderController
					.doSaveSupplierTender(supplierTender);
	...
	public void getSupplierTendersJson() {
		try {
			...
			if (null != id && !"".equals(id)) {
				PurchaseTender pt = supplierTenderController
						.findPurchaseTenderById(id);
				...
				json = convertListToJson(
						new ArrayList<SupplierTender>(pt.getSupplierTenders()),
						pt.getSupplierTenders().size(), "purchaseTender");
			}
			if (!"".equals(json)) {
				renderJson(json);
			} else {

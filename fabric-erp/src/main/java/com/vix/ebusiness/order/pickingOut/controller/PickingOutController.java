package com.vix.ebusiness.order.pickingOut.controller;
...
import com.vix.ebusiness.order.pickingOut.domain.PickingOutDomain;
 ...
 * @date 2013-6-27
 */
@Controller("pickingOutController")
@Scope("prototype")
public class PickingOutController {

	@Autowired
	private PickingOutDomain pickingOutDomain;
	...
	public Pager doListOrder(Map<String, Object> params, Pager pager) throws Exception {
		...
		// 3.执行保存操作
		p = pickingOutDomain.findOrderPagerByHqlConditions(params, pager);
	...
	public List<Order> doListOrderList(Map<String, Object> params) throws Exception {
		...
		beforeEventTrigger("PO_CREATE_BEFORE");
		// 3. 执行查询操作
		orderBatch = pickingOutDomain.findOrderList(params);
		// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");
	...
	public Order doListOrderById(String id) throws Exception {
		...
		beforeEventTrigger("PO_CREATE_BEFORE");
		// 3. 执行查询操作
		order = pickingOutDomain.findOrderById(id);
		// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");

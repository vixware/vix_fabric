package com.vix.ebusiness.statisticalAnalysis.hql;
...
@Component
@Scope("prototype")
public class StatisticalAnalysisHqlProvider extends HqlProvider {
	...
	public StringBuilder findEquipmentCoinNumberList(Map<String, Object> params) {
		StringBuilder hql = new StringBuilder();
		hql.append("SELECT t.bn as 名称,SUM(t.payment) as 金额  FROM SaleOrderItem t where 1=1 and t.bn is not null ");
		if (params != null) {
			if (params.containsKey("tenantId")) {
				String tenantId = (String) params.get("tenantId");
				if (tenantId == null) {
	...
	public StringBuilder findSalesOrder(Map<String, Object> params) {
		StringBuilder hql = new StringBuilder();
		hql.append("SELECT  ").append(entityAsName()).append(".createTime,sum(").append(entityAsName()).append(".payment) FROM SalesOrder ").append(entityAsName()).append(" WHERE ").append(entityAsName()).append(".payment IS NOT NULL ");

		if (params != null) {
			if (params.containsKey("tenantId")) {
				String tenantId = (String) params.get("tenantId");
				if (tenantId == null) {
					hql.append(" and ").append(entityAsName()).append(".tenantId is null ");
					params.remove("tenantId");
				} else {
					hql.append(" and ").append(entityAsName()).append(".tenantId ='" + tenantId + "'");
				}
			}
			if (params.containsKey("date")) {
				String date = (String) params.get("date");
				if (date != null) {
					hql.append(" and ").append(entityAsName()).append(".createTime  BETWEEN '" + date + " 00:00:00' ");

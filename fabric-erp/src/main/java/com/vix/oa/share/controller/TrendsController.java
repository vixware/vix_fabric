...
import com.vix.oa.share.doman.TrendsDomain;
...
@Scope("prototype")
public class TrendsController extends BaseAction {
	...
	Logger logger = Logger.getLogger("TrendsController");

	@Autowired
	private TrendsDomain trendsDomain;
	
	/** 获取列表数据 */
	public Pager goSingleList(Map<String, Object> params, Pager pager)throws Exception {
		Pager p = trendsDomain.findPagerByHqlConditions(params, pager);
		return p;
	}
	
	/** 获取搜索列表数据 */
	public Pager goSearchList(Map<String, Object> params, Pager pager)throws Exception {
		Pager p = trendsDomain.findPagerByOrHqlConditions(params, pager);
		return p;
	}
	...
	public Pager doSubSingleList(Map<String, Object> params, Pager pager) {
		...
		try {
			...
			// 3.执行查询操作
			p = trendsDomain.findPagerByHqlConditions(params, pager);
	...
	public List<Trends> doListSalesOrderIndex() {
		...
		try {
			...
			beforeEventTrigger("PO_CREATE_BEFORE");
			// 3. 执行查询操作
			trends = trendsDomain.findSalesOrderIndex();
			// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
			afterEventTrigger("PO_CREATE_AFTER");
	...
	public Trends doListEntityById(String id) throws Exception {
		...
		try {
			...
			// 3.执行查询操作
			trends = trendsDomain.findEntityById(id);
	 ...
	 */
	public Trends doSaveSalesOrder(Trends trends)
			throws Exception {
		Trends trends1 = null;
		try {
			// 1. 处理传入参数 executeParameterProcess(); 抽象方法，需要重载；
			...
			// 3. 执行保存操作
			trends1 = trendsDomain.merge(trends);
			// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
			afterEventTrigger("PO_CREATE_AFTER");
			// 5. 访问功能和业务对象记录操作 visitBORecord(parameter);
			// 6. 日志处理 log(); 每个Action中独立定义;记录操作日志、记录性能数据;
	...
	public Trends findEntityById(String id) throws Exception {
		return trendsDomain.findEntityById(id);
	}
	...
	public void doDeleteByEntity(Trends trends) {
		try {
			...
			// 3. 执行删除操作
			trendsDomain.deleteByEntity(trends);
			// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
			afterEventTrigger("PO_CREATE_AFTER");

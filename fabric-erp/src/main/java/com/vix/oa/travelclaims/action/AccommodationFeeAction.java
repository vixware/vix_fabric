...
@Controller
@Scope("prototype")
public class AccommodationFeeAction extends BaseAction {
	...
	/** 获取列表数据 */
	private List<AccommodationFee> accommodationFeeList;
	...
	public String goListContent() {
		try {
			if (StringUtils.isNotEmpty(id) && !id.equals("0")) {
				...
				accommodationFeeList = baseHibernateService.findAllByConditions(AccommodationFee.class, params);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String goSaveOrUpdate() {
		try {
			...
			if (StringUtils.isNotEmpty(id) && !id.equals("0")) {
				accommodationFee = baseHibernateService.findEntityById(AccommodationFee.class, id);
			} else {
					...
					if (employee != null) {
						if (employee.getOrgPositions().size() > 0) {
							for (OrgPosition orgPosition : employee.getOrgPositions()) {
								accommodationFee.setOrgPosition(orgPosition);
							}
						}
					}
				}
				accommodationFee = baseHibernateService.merge(accommodationFee);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String deleteById() {
		try {
			AccommodationFee accommodationFee = baseHibernateService.findEntityById(AccommodationFee.class, id);
			if (null != accommodationFee) {
	...
	public List<AccommodationFee> getAccommodationFeeList() {
		return accommodationFeeList;
	}

	public void setAccommodationFeeList(List<AccommodationFee> accommodationFeeList) {
		this.accommodationFeeList = accommodationFeeList;
	}

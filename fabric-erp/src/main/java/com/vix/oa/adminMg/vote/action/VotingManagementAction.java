...
/**
 * 
 * @ClassName: VotingManagementAction
 * @Description: 行政办公——投票管理 
 * @author chenzhengwen
 * @author w_a_533@163.com 
 * @date 2014-3-11 下午1:14:48
 */
@Controller
@Scope("prototype")
public class VotingManagementAction extends BaseAction {
	...
	public void updateIsPublish(){
		try{
			...
			if(pb.getPublishType()==1){
				publishType =0;
			}else if(pb.getPublishType()==0){
				publishType = 1;
			}
			pb.setPublishType(publishType);
			votingManagement = votingManagementController.doSaveSalesOrder(pb);
		}catch(Exception e){
			e.printStackTrace();
		}
	}
	...
	/**投票结果*/
	public String goVotingOrResult() {
		return "goVotingOrResult";
	}	
	
	/**投票结果*/
	public String votingOrResult() {
		return "votingOrResult";
	}	

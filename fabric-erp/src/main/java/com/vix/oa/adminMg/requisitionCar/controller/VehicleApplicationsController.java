...
import com.vix.oa.adminMg.requisitionCar.domain.VehicleApplicationsDomain;
 ...
 *  车辆使用申请管理
 */
@Controller("vehicleApplicationsController")
@Scope("prototype")
public class VehicleApplicationsController extends BaseAction {
	...
	Logger logger = Logger.getLogger("vehicleApplicationsController");

	@Autowired
	private VehicleApplicationsDomain vehicleApplicationsDomain;
	/**
	...
	public Pager doCarUseSingleList(Map<String, Object> params, Pager pager) {
		...
		try {
			...
			// 3.执行查询操作
			p = vehicleApplicationsDomain.findPagerByVehicleHqlCarUse(params, pager);
	...
	public List<CarUse> doListCarUseIndex() {
		...
		try {
			...
			beforeEventTrigger("PO_CREATE_BEFORE");
			// 3. 执行查询操作
			carUse = vehicleApplicationsDomain.findCarUseIndex();
			// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
			afterEventTrigger("PO_CREATE_AFTER");
	...
	public CarUse doListCarUseById(String id) throws Exception {
		...
		try {
			...
			// 3.执行查询操作
			carUse = vehicleApplicationsDomain.findCarUseById(id);
	...
	public CarUse doSaveCarUse(CarUse carUse) {
		...
		try {
			...
			// 3. 执行保存操作
			carUse1 = vehicleApplicationsDomain.merge(carUse);			
			// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
			afterEventTrigger("PO_CREATE_AFTER");
			// 5. 访问功能和业务对象记录操作 visitBORecord(parameter);
			// 6. 日志处理 log(); 每个Action中独立定义;记录操作日志、记录性能数据;
	...
	public CarUse findEntityById(String id) throws Exception {
		return vehicleApplicationsDomain.findCarUseById(id);
	}
	...
	public void doDeleteByCarUse(CarUse carUse) {
		try {
			...
			// 3. 执行删除操作
			vehicleApplicationsDomain.deleteByCarUse(carUse);
			// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
			afterEventTrigger("PO_CREATE_AFTER");
	...
	public Pager goSingleList(Map<String, Object> params, Pager pager)throws Exception {
		Pager p = vehicleApplicationsDomain.findPagerByHqlConditions(params, pager);
		return p;
	}
	/** 获取汽车列表数据 */
	public Pager goSingleCarList(Map<String, Object> params, Pager pager)throws Exception {
		Pager p = vehicleApplicationsDomain.findPagerByHqlConditionsCar(params, pager);
		return p;
	}

	/** 获取列表数据 */
	public Pager goSearchList(Map<String, Object> params, Pager pager)throws Exception {
		Pager p = vehicleApplicationsDomain.findPagerByOrHqlConditions(params, pager);
		return p;
	}

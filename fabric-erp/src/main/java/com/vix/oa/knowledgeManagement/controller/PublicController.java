...
import com.vix.oa.knowledgeManagement.domain.PublicDomain;
...
public class PublicController extends BaseAction {
	...
	Logger logger = Logger.getLogger("PublicController");
	...
	public Pager goSearchList(Map<String, Object> params, Pager pager)
			throws Exception {
		Pager p = publicDomain.findPagerByOrHqlConditions(params, pager);
		return p;
	}
	...
	public Pager goSingleList(Map<String, Object> params, Pager pager)
			throws Exception {
		Pager p = publicDomain.findPagerByHqlConditions(params, pager);
		return p;
	}
	 ...
	 */
	public List<PublicCabinet> doListSalesOrderIndex() {
		List<PublicCabinet> publicCabinet = null;
		...
		try {
			...
			// 3. 执行查询操作
			publicCabinet = publicDomain.findSalesOrderIndex();
	 ...
	 */
	public PublicCabinet doSaveSalesOrder(PublicCabinet publicCabinet)
			throws Exception {
		try {
			// 1. 处理传入参数 executeParameterProcess(); 抽象方法，需要重载；
			// Map parameters = baExecutor.executeParameterProcess();
			...
			// 3. 执行保存操作
			publicCabinet = publicDomain.merge(publicCabinet);
			// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
			afterEventTrigger("PO_CREATE_AFTER");
			// 5. 访问功能和业务对象记录操作 visitBORecord(parameter);
			// 6. 日志处理 log(); 每个Action中独立定义;记录操作日志、记录性能数据;
			executeLogger(true, "保存公共文件柜：" + publicCabinet + "成功！");
		} catch (Exception ex) {
			executeLogger(true,
					"保存公共文件柜：" + publicCabinet + "失败！失败原因：" + ex.getMessage());
	...
	public Pager doSubSingleList(Map<String, Object> params, Pager pager) {
		...
		try {
			...
			beforeEventTrigger("PO_CREATE_BEFORE");
			// 3.执行查询操作
			p = publicDomain.findPagerByHqlConditions(params, pager);

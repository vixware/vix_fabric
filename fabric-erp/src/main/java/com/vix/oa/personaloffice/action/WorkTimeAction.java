...
@Controller
@Scope("prototype")
public class WorkTimeAction extends BaseAction{
...
/*			String code = getRequestParameter("code");
			...
			String workLength = getRequestParameter("workLength");
			if (StrUtils.isNotEmpty(workLength)) {
				params.put("workLength," + SearchCondition.EQUAL, workLength);
			}

			pager = this.baseHibernateService.findPagerByHqlConditions(pager,
					WorkTime.class, params);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String goSaveOrUpdate() {
		try {
			if (StringUtils.isNotEmpty(id) && !id.equals("0")) {
				workTime = baseHibernateService.findEntityById(
						WorkTime.class, id);
			}
		} catch (Exception e) {
			e.printStackTrace();
	...
	public String saveOrUpdate() {
		...
		try {
			if (null != workTime
					&& StrUtils.objectIsNotNull(workTime.getId())) {
				isSave = false;
				workTime.setUpdateTime(new Date());
			} else {
				workTime.setCreateTime(new Date());
				loadCommonData(workTime);// 载入数据公司码
			}
			System.out.println(SecurityUtil.getCurrentUserTenantId());
			workTime = baseHibernateService.merge(workTime);
			if (isSave) {
	...
	public String deleteById() {
		try {
			WorkTime pb = baseHibernateService.findEntityById(
					WorkTime.class, id);
			if (null != pb) {

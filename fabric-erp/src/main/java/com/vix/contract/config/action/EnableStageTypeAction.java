...
@Controller
@Scope("prototype")
public class EnableStageTypeAction extends BaseAction {
	...
	public String goSingleList(){
		try {
			...
			Pager pager = baseHibernateService.findPagerByHqlConditions(getPager(), EnableStageType.class, params);
			setPager(pager);
		}catch(Exception e){
			e.printStackTrace();
		}
	...
	public String goSaveOrUpdate() {
		try {
			if(null != id && !"".equals(id)){
				enableStageType = baseHibernateService.findEntityById(EnableStageType.class,id);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public String saveOrUpdate() {
		boolean isSave = true;
		try {
			if(null != enableStageType.getId()){
				isSave = false;
			}
			enableStageType = baseHibernateService.merge(enableStageType);

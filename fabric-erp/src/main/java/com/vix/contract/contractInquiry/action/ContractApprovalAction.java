...
/**
 * 
 * @ClassName: ContractApprovalAction
 * @Description: 合同审批
 * @author bobchen
 * @date 2015年12月30日 上午10:15:45
 *
 */
@Controller
@Scope("prototype")
public class ContractApprovalAction extends BaseAction {
	...
	public void updateIsPublish() {
		try {
			...
			if (ci.getIsPublish() == 1) {
				isPublish = 0;
			} else if (ci.getIsPublish() == 0) {
				isPublish = 1;
			}
			ci.setIsPublish(isPublish);
			contractInquiry = contractInquiryController.doSaveContract(ci);
		} catch (Exception e) {
			e.printStackTrace();
		}
	}

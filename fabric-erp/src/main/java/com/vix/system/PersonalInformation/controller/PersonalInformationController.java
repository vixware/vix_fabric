 ...
 * 
 */
package com.vix.system.PersonalInformation.controller;
...
import com.vix.system.PersonalInformation.domain.PersonalInformationDomain;
...
public class PersonalInformationController {
	...
	Logger logger = Logger.getLogger(PersonalInformationController.class);
	...
	public PersonalEmail doSavePersonalEmail(PersonalEmail personalEmail) throws Exception {
		...
		// 执行保存操作
		personalEmail1 = personalInformationDomain.mergePersonalEmail(personalEmail);
	...
	public UserAccount doSaveUserAccount(UserAccount userAccount) throws Exception {
		...
		beforeEventTrigger("PO_CREATE_BEFORE");
		// 执行保存操作
		userAccount1 = personalInformationDomain.mergeUserAccount(userAccount);
		// 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");
		// 访问功能和业务对象记录操作 visitBORecord(parameter);
		// 日志处理 log(); 每个Action中独立定义;记录操作日志、记录性能数据;
	...
	public Pager doListPersonalEmail(Map<String, Object> params, Pager pager) throws Exception {
		...
		// 执行保存操作
		p = personalInformationDomain.findPersonalEmailPager(params, pager);
	...
	public void doDeleteByEntity(PersonalEmail personalEmail) throws Exception {
		...
		// 3. 执行删除操作
		personalInformationDomain.deleteByEntity(personalEmail);
	...
	public void doDeleteModuleCategoryByIds(List<String> ids) throws Exception {
		...
		// 3. 执行删除操作
		personalInformationDomain.deleteModuleCategoryByIds(ids);
		// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");
	...
	public PersonalEmail doListPersonalEmailById(String id) throws Exception {
		...
		// 3.执行查询操作
		personalEmail = personalInformationDomain.findPersonalEmailById(id);
		// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");
	...
	public List<PersonalEmail> doListPersonalEmailIndex() throws Exception {
		...
		beforeEventTrigger("PO_CREATE_BEFORE");
		// 3. 执行查询操作
		personalEmail = personalInformationDomain.findPersonalEmailIndex();
		// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");

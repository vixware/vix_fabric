package com.vix.chain.customerLog.controller;
...
import com.vix.chain.customerLog.domain.CustomerLogDomain;
...
public class CustomerLogController {
	...
	@Autowired
	private CustomerLogDomain customerLogDomain;
	...
	public Pager doListZoVipCardLog(Map<String, Object> params, Pager pager) throws Exception {
		...
		// 3.执行保存操作
		p = customerLogDomain.findZoVipCardLogPager(params, pager);
	 ...
	 * @throws Exception
	 */
	public List<ZoVipCardLog> doListZoVipCardLog() throws Exception {
		List<ZoVipCardLog> zoVipCardLog = null;
		...
		beforeEventTrigger("PO_CREATE_BEFORE");
		// 3. 执行查询操作
		zoVipCardLog = customerLogDomain.findZoVipCardLog();
		// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");
	...
	public Employee doListEmployeeById(String id) throws Exception {
		...
		beforeEventTrigger("PO_CREATE_BEFORE");
		// 3. 执行查询操作
		employee = customerLogDomain.findEmployee(id);
		// 4. 触发事件 afterEventSaveOrder(parameter); 抽象方法，需要重载；
		afterEventTrigger("PO_CREATE_AFTER");

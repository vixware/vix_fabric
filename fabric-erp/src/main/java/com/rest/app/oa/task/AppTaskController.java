...
/**
 * 
 * @ClassFullName com.rest.app.oa.task.AppTaskController
 *
 * @author bjitzhang
 *
 * @date 2016年1月26日
...
@Controller
@RequestMapping(value = "restService/app/task")
public class AppTaskController extends BaseRestController {
	...
	@RequestMapping(value = "findTaskListCount.rs", method = RequestMethod.POST)
	public void findTaskListCount(HttpServletRequest request, HttpServletResponse response) throws Exception {
		Map<String, Object> params = new HashMap<String, Object>();
		List<VixTask> vixTaskList = traceabilityService.findAllByConditions(VixTask.class, params);
		if (vixTaskList != null && vixTaskList.size() > 0) {
			renderResult(response, vixTaskList.size());
		} else {

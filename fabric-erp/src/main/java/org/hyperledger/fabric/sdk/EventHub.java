...
public class EventHub {
	 ...
	 *
	 * @return Time in milli seconds disconnect occurred. Zero if never
	 *         disconnected
	 */
	 ...
	 *
	 * @return Time in milli seconds the event hub last connected. Zero if never
	 *         connected.
	 */
	 ...
	 *
	 * @return Last attempt time to connect the event hub in milli seconds. Zero
	...
	public EventHub(String name, String grpcURL, ExecutorService executorService, Properties properties) throws InvalidArgumentException {
		...
		this.properties = properties == null ? null : (Properties) properties.clone(); // keep
																						// our
																						// own
																						// copy.
	}
	...
	private StreamObserver<PeerEvents.Event> eventStream = null; // Saved here
																	// to avoid
																	// potential
																	// garbage
																	// collection
		...
		final StreamObserver<PeerEvents.Event> eventStreamLocal = new StreamObserver<PeerEvents.Event>() {
			...
			public void onNext(PeerEvents.Event event) {
				...
				if (event.getEventCase() == PeerEvents.Event.EventCase.BLOCK) {
					try {
						eventQue.addBEvent(new BlockEvent(EventHub.this, event)); // add
																					// to
																					// channel
																					// queue
			...
			public void onError(Throwable t) {
				...
				logger.error(format("%s terminated is %b shutdown is %b has error %s ", EventHub.this.toString(), isTerminated, isChannelShutdown, t.getMessage()), new EventHubException(t));
				threw.add(t);
				finishLatch.countDown();

				// logger.error("Error in stream: " + t.getMessage(), new
				// EventHubException(t));
	...
	private void blockListen(TransactionContext transactionContext) throws CryptoException {
		...
		PeerEvents.Register register = PeerEvents.Register.newBuilder().addEvents(PeerEvents.Interest.newBuilder().setEventType(PeerEvents.EventType.BLOCK).build()).build();
		ByteString blockEventByteString = PeerEvents.Event.newBuilder().setRegister(register).setCreator(transactionContext.getIdentity().toByteString()).build().toByteString();
		PeerEvents.SignedEvent signedBlockEvent = PeerEvents.SignedEvent.newBuilder().setEventBytes(blockEventByteString).setSignature(transactionContext.signByteString(blockEventByteString.toByteArray())).build();
	...
	void setChannel(Channel channel) throws InvalidArgumentException {
		...
		if (null != this.channel) {
			throw new InvalidArgumentException(format("Can not add event hub  %s to channel %s because it already belongs to channel %s.", name, channel.getName(), this.channel.getName()));
		}
	...
	/**
	 * Default reconnect event hub implementation. Applications are free to
	 * replace
	 */
	...
	protected EventHubDisconnected disconnectedHandler = new EventHub.EventHubDisconnected() {
		...
		public synchronized void disconnected(final EventHub eventHub) throws EventHubException {
			...
			if (eventHub.connectedTime == 0) { // means event hub never
												// connected
				logger.error(format("%s failed on first connect no retries", eventHub.toString()));

				eventHub.setEventHubDisconnectedHandler(null); // don't try
																// again
	 ...
	 *
	 * @param newEventHubDisconnectedHandler
	 *            New handler to replace. If set to null no retry will take
	 *            place.

...
public class CSCCProposalBuilder extends ProposalBuilder {
	...
	private static final Chaincode.ChaincodeID CHAINCODE_ID_CSCC = Chaincode.ChaincodeID.newBuilder().setName(CSCC_CHAIN_NAME).build();
	...
	public FabricProposal.Proposal build() throws ProposalException {
		...
		if (!"".equals(context.getChannel().getName())) { // if not "" this is
															// not the system
															// channel

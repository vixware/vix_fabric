...
public class ProposalBuilder {
	...
	// The channel that is being targeted . note blank string means no specific
	// channel
	...
	public ProposalBuilder context(TransactionContext context) {
		...
		if (null == channelID) {
			channelID = context.getChannel().getName(); // Default to context
														// channel.
		}
		return this;
	}
	...
	public ProposalBuilder request(TransactionRequest request) {
		...
		ccType(request.getChaincodeLanguage() == TransactionRequest.Type.JAVA ? Chaincode.ChaincodeSpec.Type.JAVA : Chaincode.ChaincodeSpec.Type.GOLANG);
	...
	private FabricProposal.Proposal createFabricProposal(String channelID, Chaincode.ChaincodeID chaincodeID) {
		...
		Common.ChannelHeader chainHeader = createChannelHeader(HeaderType.ENDORSER_TRANSACTION, context.getTxID(), channelID, context.getEpoch(), context.getFabricTimestamp(), chaincodeHeaderExtension);
	...
	private ChaincodeInvocationSpec createChaincodeInvocationSpec(Chaincode.ChaincodeID chaincodeID, ChaincodeSpec.Type langType) {
		...
		if (argList != null && argList.size() > 0) {
			// If we already have an argList then the Builder subclasses have
			// already set the arguments
			// for chaincodeInput. Accept the list and pass it on to the
			// chaincodeInput builder
			// TODO need to clean this logic up so that common protobuf struct
			// builds are in one place
			allArgs = argList;
		} else if (request != null) {
			// if argList is empty and we have a Request, build the
			// chaincodeInput args array from the Request args and argbytes
			// lists
			...
			// TODO currently assume that chaincodeInput args are strings
			// followed by byte[].
			// Either agree with Fabric folks that this will always be the case
			// or modify all Builders to expect
			// a List of Objects and determine if each list item is a string or
			// a byte array

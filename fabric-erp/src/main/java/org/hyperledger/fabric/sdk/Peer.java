...
/**
 * The Peer class represents a peer to which SDK sends deploy, or query
 * proposals requests.
 */
public class Peer {
	...
	public Peer(String name, String grpcURL, Properties properties) throws InvalidArgumentException {
	...
	public void setChannel(Channel channel) throws InvalidArgumentException {
		...
		if (null != this.channel) {
			throw new InvalidArgumentException(format("Can not add peer %s to channel %s because it already belongs to channel %s.", name, channel.getName(), this.channel.getName()));
		}
	 ...
	 *
	 * @param otherPeer
	 *            the peer instance to compare against
	...
	public ListenableFuture<FabricProposalResponse.ProposalResponse> sendProposalAsync(FabricProposal.SignedProposal proposal) throws PeerException, InvalidArgumentException {
		checkSendProposal(proposal);

		logger.debug(format("peer.sendProposalAsync name: %s, url: %s", name, url));

		EndorserClient localEndorserClient = endorserClent; // work off thread
															// local copy.

 ...
 * @author zhanghaibing
 * 
 * @类全名 org.hyperledger.fabric.sdk.transaction.InstallProposalBuilder
 *
 * @date 2018年4月27日
 */
public class InstallProposalBuilder extends LSCCProposalBuilder {
	...
	private void createNetModeTransaction() throws IOException {
		...
		if (chaincodeSource != null) {
			...
			logger.info(format("Installing '%s'  %s chaincode from directory: '%s' with source location: '%s'. chaincodePath:'%s'", chaincodeID, dplang, projectSourceDir.getAbsolutePath(), targetPathPrefix, chaincodePath));

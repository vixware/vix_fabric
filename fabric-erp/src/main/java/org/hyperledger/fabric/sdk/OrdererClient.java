...
class OrdererClient {
	...
	/**
	 * Construct client for accessing Orderer server using the existing
	 * managedChannel.
	 */
	...
	DeliverResponse[] sendDeliver(Common.Envelope envelope) throws TransactionException {
		...
		try {
			...
			try {
				if (!finishLatch.await(ordererWaitTimeMilliSecs, TimeUnit.MILLISECONDS)) {
					TransactionException ex = new TransactionException(format("Channel %s sendDeliver time exceeded for orderer %s, timed out at %d", channelName, name, ordererWaitTimeMilliSecs));

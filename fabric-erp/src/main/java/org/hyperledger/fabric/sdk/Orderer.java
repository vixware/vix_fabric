...
/**
 * The Orderer class represents a orderer to which SDK sends deploy, invoke, or
 * query requests.
 */
public class Orderer {
	...
	public Orderer(String name, String url, Properties properties) throws InvalidArgumentException {
	...
	public void setChannel(Channel channel) throws InvalidArgumentException {
		...
		if (null != this.channel && this.channel != channel) {
			throw new InvalidArgumentException(format("Can not add orderer %s to channel %s because it already belongs to channel %s.", name, channel.getName(), this.channel.getName()));
		}

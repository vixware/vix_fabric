 ...
 * with environment variable and then overridden with a java system property.
 * Property hierarchy goes System property overrides environment variable which
 * overrides config file for default values specified here.
 */
...
public class Config {
	...
	private Config() {
		...
		try {
			...
			// TODO remove this once we have implemented MSP and get the peer
			// certs from the channel
	...
	/**
	 * Get the configured security level. The value determines the elliptic
	...
	/**
	 * Get the name of the configured hash algorithm, used for digital
	 * signatures.
	 *
	...
	/**
	 * getProposalConsistencyValidation determine if validation of the proposals
	 * should be done before sending to the orderer.
	 *
	 * @return if true proposals will be checked they are consistent with each
	 *         other before sending to the Orderer
	 */
	...
	/**
	 * The directory where diagnostic dumps are to be place, null if none should
	 * be done.
	 *
	 * @return The directory where diagnostic dumps are to be place, null if
	 *         none should be done.
	 */

...
public final class ProtoUtils {
	 ...
	 *
	 * @param type
	 *            header type. See {@link ChannelHeader.Builder#setType}.
	 * @param txID
	 *            transaction ID. See {@link ChannelHeader.Builder#setTxId}.
	 * @param channelID
	 *            channel ID. See {@link ChannelHeader.Builder#setChannelId}.
	 * @param epoch
	 *            the epoch in which this header was generated. See
	 *            {@link ChannelHeader.Builder#setEpoch}.
	 * @param timeStamp
	 *            local time when the message was created. See
	 *            {@link ChannelHeader.Builder#setTimestamp}.
	 * @param chaincodeHeaderExtension
	 *            extension to attach dependent on the header type. See
	 *            {@link ChannelHeader.Builder#setExtension}.
	...
	public static ChannelHeader createChannelHeader(HeaderType type, String txID, String channelID, long epoch, Timestamp timeStamp, ChaincodeHeaderExtension chaincodeHeaderExtension) {
		...
		if (isDebugLevel) {
			logger.debug(format("ChannelHeader: type: %s, version: 1, Txid: %s, channelId: %s, epoch %d", type.name(), txID, channelID, epoch));
	...
	public static ChaincodeDeploymentSpec createDeploymentSpec(Type ccType, String name, String chaincodePath, String chaincodeVersion, List<String> args, byte[] codePackage) {
		...
		ChaincodeDeploymentSpec.Builder chaincodeDeploymentSpecBuilder = ChaincodeDeploymentSpec.newBuilder().setChaincodeSpec(chaincodeSpec) // .setEffectiveDate(context.getFabricTimestamp())

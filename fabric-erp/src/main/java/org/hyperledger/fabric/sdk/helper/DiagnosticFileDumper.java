...
public class DiagnosticFileDumper implements Runnable {
	...
	// LogFactory.getLog(DiagnosticFileDumper.class);
	...
	public String createDiagnosticFile(byte[] bytes, String prefix, String ext) {
		...
		if (null != bytes) {
			...
			fileName = prefix + dateFormatGmt.format(new Date()) + "P" + pid + "_" + Thread.currentThread().getId() + "_" + counter.addAndGet(1) + "." + ext;
			fileName = fileName.replaceAll("\\:", "-"); // colon is bad for
														// windows.

			new QueEntry(fileName, bytes); // Add to Que let process by async
											// thread.
	...
	public void run() {
		...
		while (true) {
			...
			try {
				...
				this.queEntries.drainTo(entries); // got one, see if there are
													// more.

				if (cantWrite()) {
					return; // IF the directory is missing just assume user does
							// not want diagnostic files created anymore.
				}

				entries.forEach(queEntry -> {

					try {
						final AsynchronousFileChannel channel = AsynchronousFileChannel.open(Paths.get(dirAbsolutePath, queEntry.fileName), StandardOpenOption.WRITE, StandardOpenOption.CREATE, StandardOpenOption.TRUNCATE_EXISTING);

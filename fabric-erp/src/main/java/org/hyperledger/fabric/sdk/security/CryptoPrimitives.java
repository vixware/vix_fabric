...
public class CryptoPrimitives implements CryptoSuite {
	...
	// Following configuration settings are hardcoded as they don't deal with
	// any interactions with Fabric MSP and BCCSP components
	// If you wish to make these customizable, follow the logic from
	// setProperties();
	...
	// *
	// * @param sigAlg the name of the signature algorithm. See the list of
	// valid names in the JCA Standard Algorithm Name documentation
	// */
	...
	// /**
	// * returns the signature algorithm used by this instance of
	// CryptoPrimitives.
	// * Note that fabric and fabric-ca have not yet standardized on which
	// algorithms are supported.
	// * While that plays out, CryptoPrimitives will try the algorithm specified
	// in the certificate and
	// * the default SHA256withECDSA that's currently hardcoded for fabric and
	// fabric-ca
	// *
	...
	/**
	 * addCACertificateToTrustStore adds a CA cert to the set of certificates
	 * used for signature validation
	 *
	 * @param caCertPem
	 *            an X.509 certificate in PEM format
	 * @param alias
	 *            an alias associated with the certificate. Used as shorthand
	 *            for the certificate during crypto operations
	 * @throws CryptoException
	 * @throws InvalidArgumentException
	 */
	...
	public void loadCACertificates(Collection<Certificate> certificates) throws CryptoException {
		...
		try {
			...
			// Note: This can currently never happen (as cert<>null and
			// alias<>null)
	 ...
	 * 
	 * @see
	 * org.hyperledger.fabric.sdk.security.CryptoSuite#loadCACertificatesAsBytes
	...
	public void setHashAlgorithm(String algorithm) throws InvalidArgumentException {
		if (Utils.isNullOrEmpty(algorithm) || !(algorithm.equalsIgnoreCase("SHA2") || algorithm.equalsIgnoreCase("SHA3"))) {
	...
	// } catch (Exception e) {
	// String emsg = "Failed to decode public key: " + data + ". error : " +
	// e.getMessage();
	// logger.error(emsg);
	// throw new CryptoException(emsg, e);
	// }
	...
	// public byte[] eciesDecrypt(KeyPair keyPair, byte[] data) throws
	// CryptoException {
	// try {
	// int ephemeralKeyLength = (int) (Math.floor((this.securityLevel + 7) / 8)
	// * 2 + 1);
	// int mkLen = this.securityLevel >> 3;
	// int encryptedMessageLength = data.length - ephemeralKeyLength - mkLen;
	//
	// byte[] ephemeralPublicKeyBytes = Arrays.copyOfRange(data, 0,
	// ephemeralKeyLength);
	// byte[] encryptedMessage = Arrays.copyOfRange(data, ephemeralKeyLength,
	// ephemeralKeyLength + encryptedMessageLength);
	// byte[] tag = Arrays.copyOfRange(data, ephemeralKeyLength +
	// encryptedMessageLength, data.length);
	//
	// // Parsing public key.
	...
	// KeyFactory.getInstance(ASYMMETRIC_KEY_TYPE, SECURITY_PROVIDER);
	//
	// PublicKey ephemeralPublicKey = asymmetricKeyFactory.generatePublic(new
	// ECPublicKeySpec(
	...
	// HKDFBytesGenerator(getHashDigest());
	//
	...
	// byte[] encrypted = Arrays.copyOfRange(encryptedMessage, 16,
	...
	// }
	//
	// private byte[] aesDecrypt(byte[] encryptionKey, byte[] iv, byte[]
	// encryptedMessage)
	...
	// SYMMETRIC_KEY_TYPE), new IvParameterSpec(iv));
	...
	// }
	//
	// public byte[][] ecdsaSign(PrivateKey privateKey, byte[] data) throws
	// CryptoException {
	// try {
	...
	// params.getG(), params.getN(),
	// params.getH());
	//
	// ECDSASigner signer = new ECDSASigner(new HMacDSAKCalculator(new
	// SHA512Digest()));
	// ECPrivateKeyParameters privKey = new
	// ECPrivateKeyParameters(((ECPrivateKey) privateKey).getS(), ecParams);
	// signer.init(true, privKey);
	// BigInteger[] sigs = signer.generateSignature(encoded);
	// return new byte[][] {sigs[0].toString().getBytes(UTF_8),
	// sigs[1].toString().getBytes(UTF_8)};
	// } catch (Exception e) {
	// throw new CryptoException("Could not sign the message using private key",
	// e);
	// }
	//
	// }
	...
	private byte[] ecdsaSignToBytes(ECPrivateKey privateKey, byte[] data) throws CryptoException {
		try {
			...
			ECDomainParameters ecParams = new ECDomainParameters(params.getCurve(), params.getG(), curveN, params.getH());
	 ...
	 *             if the supplied private key is not of type
	 *             {@link ECPrivateKey}.
	 */
	@Override
	...
	/*
	 * code for signing using JCA/JSSE methods only . Still needed ? public
	 * byte[] sign(PrivateKey key, byte[] data) throws CryptoException { byte[]
	 * signature;
	 * 
	 * if (key == null || data == null) throw new
	 * CryptoException("Could not sign. Key or plain text is null", new
	 * NullPointerException());
	 * 
	 * try { Signature sig = Signature.getInstance(DEFAULT_SIGNATURE_ALGORITHM);
	 * sig.initSign(key); sig.update(data); signature = sig.sign();
	 * 
	 * // TODO see if BouncyCastle handles sig malleability already under // the
	 * covers
	 * 
	 * return signature; } catch (NoSuchAlgorithmException | InvalidKeyException
	 * | SignatureException e) { throw new
	 * CryptoException("Could not sign the message.", e); }
	 * 
	 * }
	 */
	 ...
	 *
	 * @param subject
	 *            The subject to be added to the certificate
	 * @param pair
	...
	// return privateKey;
	// } catch (Exception exp) {
	// throw new CryptoException("Unable to convert byte[] into PrivateKey",
	// exp);
	// }
	// }
	...
	/**
	 * Resets curve name, hash algorithm and cert factory. Call this method when
	 ...
	 * 
	 * @see
	 * org.hyperledger.fabric.sdk.security.CryptoSuite#setProperties(java.util.
	 * Properties)
	 */
	@Override

...
public class HFCAClient {
	...
	Config config = Config.getConfig(); // Load config so enable logging
										// setting.
	...
	// TODO require use of CryptoPrimitives since we need the
	// generateCertificateRequests methods
	 ...
	 * @param properties
	 *            PEM used for SSL .. not implemented.
	 *            <p>
	 ...
	 */
	protected HFCAClient(String name, String url, Properties properties) throws MalformedURLException {
		...
		if (properties != null) {
			this.properties = (Properties) properties.clone(); // keep our own
																// copy.
		} else {
	 ...
	 *
	 * @param request
	 *            Registration request with the following fields: name, role.
	 * @param registrar
	 *            The identity of the registrar (i.e. who is performing the registration).
	...
	public String register(RegistrationRequest request, User registrar) throws RegistrationException, InvalidArgumentException {
		...
		try {
			...
			System.out.println("获取用户的secret:" + secret);
			return secret;
		} catch (Exception e) {
	 ...
	 *
	 * @param user
	 *            Identity name to enroll
	 * @param secret
	 *            Secret returned via registration
	 * @return enrollment
	 ...
	 *            Secret returned via registration
	 * @param req
	 *            Enrollment request with the following fields: hosts, profile, csr, label, keypair
	 * @return enrollment
	 * @throws EnrollmentException
	 * @throws InvalidArgumentException
	 */
	 ...
	 *            User to be re-enrolled
	 * @param req
	 *            Enrollment request with the following fields: hosts, profile, csr, label
	 * @return enrollment
	 * @throws EnrollmentException
	 * @throws InvalidArgumentException
	 */
	...
	public Enrollment reenroll(User user, EnrollmentRequest req) throws EnrollmentException, InvalidArgumentException {
		...
		try {
			...
			PublicKey publicKey = cryptoPrimitives.bytesToCertificate(user.getEnrollment().getCert().getBytes(StandardCharsets.UTF_8)).getPublicKey();
	 ...
	 *
	 * @param revoker
	 *            admin user who has revoker attribute configured in CA-server
	 * @param enrollment
	 *            the user enrollment to be revoked
	 * @param reason
	 *            revoke reason, see RFC 5280
	 ...
	 * @param body
	 *            Body to be sent with the post.
	 * @param credentials
	 *            Credentials to use for basic auth.
	 * @return Body of post returned.
	 * @throws Exception
	 */
	public String httpPost(String url, String body, UsernamePasswordCredentials credentials) throws Exception {
	...
	protected JsonObject httpPost(String url, String body, String authHTTPCert) throws Exception {
		...
		if (status >= 400) {
			Exception e = new Exception(format("POST request to %s failed request body %s with status code: %d. Response: %s", url, body, status, responseBody));
			logger.error(e.getMessage());
			throw e;
		}
		...
		if (!success) {
			EnrollmentException e = new EnrollmentException(format("POST request to %s failed request body %s Body of response did not contain success", url, body), new Exception());
			logger.error(e.getMessage());
			throw e;
		}
		...
		if (messages != null && !messages.isEmpty()) {
			JsonObject jo = messages.getJsonObject(0);
			String message = format("POST request to %s failed request body %s response message [code %d]: %s", url, body, jo.getInt("code"), jo.getString("message"));
			logger.info(message);
		}

...
/**
 * Update channel scenario See
 * http://hyperledger-fabric.readthedocs.io/en/master/configtxlator.html for
 * details.
 */
public class UpdateChannelIT {
	...
	private static final String ORIGINAL_BATCH_TIMEOUT = "\"timeout\": \"2s\""; // Batch
																				// time
																				// out
																				// in
																				// configtx.yaml
	private static final String UPDATED_BATCH_TIMEOUT = "\"timeout\": \"5s\""; // What
																				// we
																				// want
																				// to
																				// change
	...
	public void setup() {
		...
		try {
			...
			// responseAsString is JSON but use just string operations for this
			// test.
			...
			// Now send to configtxlator multipart form post with original
			// config bytes, updated config bytes and channel name.
			...
			// To change the channel we need to sign with orderer admin certs
			// which crypto gen stores:
			...
			final SampleUser ordererAdmin = sampleStore.getMember(sampleOrgName + "OrderAdmin", sampleOrgName, "OrdererMSP", Util.findFileSk(Paths.get("src/test/fixture/sdkintegration/e2e-2Orgs/channel/crypto-config/ordererOrganizations/example.com/users/Admin@example.com/msp/keystore/").toFile()), Paths.get("src/test/fixture/sdkintegration/e2e-2Orgs/channel/crypto-config/ordererOrganizations/example.com/users/Admin@example.com/msp/signcerts/Admin@example.com-cert.pem").toFile());
			...
			if (responseAsString.contains(ORIGINAL_BATCH_TIMEOUT)) { // Should
																		// not
																		// have
																		// been
																		// there

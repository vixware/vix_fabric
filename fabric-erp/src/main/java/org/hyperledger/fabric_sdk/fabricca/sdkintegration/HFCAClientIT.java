...
package org.hyperledger.fabric_sdk.fabricca.sdkintegration;
...
import org.hyperledger.fabric_sdk.fabricca.sdk.MockHFCAClient;
...
public class HFCAClientIT {
	...
	// Common prefix for all test users (the suffix will be the current user
	// count)
	...
	public void testReenrollAndRevoke() throws Exception {
		...
		// trying to reenroll should be ok (revocation above is only for a
		// particular enrollment of this user)
	...
	public void testUserRevoke() throws Exception {
		...
		if (!user.isRegistered()) {
			...
			user.setEnrollmentSecret(client.register(rr, admin)); // Admin can
																	// register
																	// other
																	// users.
			if (!user.getEnrollmentSecret().equals(password)) {
		...
		// trying to reenroll the revoked user should fail with an
		// EnrollmentException
		client.reenroll(user);
	}
	...
	private void sleepALittle() {
		// Seems to be an odd that calling back too quickly can once in a while
		// generate an error on the fabric_ca
		// try {
		// Thread.sleep(5000);

...
var _tab_delay_reload = true;

//当前显示标签页id,默认#tab_home
var _default_tab_page_id = '#tab_home ';
...
function _tab_load(tabLinkA, tabPageContainerId){
	var jLinkA = $(tabLinkA);
	var pageId = jLinkA.attr(_tab_link_page_id);
	if(pageId==''){
		return;
...
function _load_tab_page_content(_tab_page_tabA){
	if(!_tab_page_tabA && _activating_tab_page_id && _activating_tab_page_id.length>0)
	{
		if(_activating_tab_page_id.substring(_activating_tab_page_id.length-1)==' ')
			_activating_tab_page_id = _activating_tab_page_id.substring(0,_activating_tab_page_id.length-1);
		
		var tabShownLinkA =$('#'+itn_tabContainerId_default+' ul li a['+_tab_link_page_id+'="'+_activating_tab_page_id+'"]');
		if(tabShownLinkA.length>0)
			_tab_page_tabA = tabShownLinkA;
	}
	
	_tab_load_page_content(_tab_page_tabA);
}
...
$(function(){
	$('#mainContent').delegate('#'+itn_tabContainerId_default+' ul.nav-tabs>li>a','click',function(event){
		$(this).tab('show');
	});

	$('#mainContent').delegate('#'+itn_tabContainerId_default+' ul.nav-tabs>li>a','show',function(event){
...
(function ($) {
	$.fn.tab = function(option){
		...
		var thisTab,thisLink;
		if($(this).is('li')){
			thisTab = $(this);
			thisLink = thisTab.find('a:first');
		}else{
			thisLink = $(this);
			thisTab = thisLink.parent();
		}
		
		
		if(option=='show')
		{
			thisLink.trigger('show');
			...
			var thisPageId = thisLink.attr(_tab_link_page_id);
			$(thisPageId).show();
			
			thisLink.trigger('shown');
		}
		else if(option=='hide')
		{
			thisLink.trigger('hide');

			thisTab.removeClass('current');
			var pageId = thisLink.attr(_tab_link_page_id);
			$(pageId).hide();
			
			thisLink.trigger('hidden');
		}
		else if(option=='close')
		{
			thisLink.trigger('close');
			
			thisLink.trigger('closed');
		}
	};

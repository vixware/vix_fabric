...
Rreport.prototype.configEditModel=function(){
	...
	$("#"+this.config.contentDiv).append(
		'<div id="vreporttoolbar" class="ui-widget-header ui-corner-all">'+
		 ...
		 '</div>'+
		 '<div id="report-win" title="报表配置" style="display:none;">'+
		    '<span>展现：</span><select id="report_view_type"><option value="w">横向</option><option value="h">纵向</option></select><br>'+
...
Rreport.prototype.setMovePath = function(nodeId,x,y){
			...
			if((y>node.config.y)&&(y<(node.config.y+node.config.h))){
				if(nId!=nodeId){
					if(this.config.viewType=='w'){
						x = node.config.x;
						y = node.config.y+(node.config.h-Rreport.conf.titleHeight)/2+Rreport.conf.titleHeight;
	...
	if(this.config.viewType=='w'){
		...
		var s_y = startNode.config.y + (startNode.config.h-Rreport.conf.titleHeight)/2+Rreport.conf.titleHeight;
		startP = "M "+s_x+" "+s_y;
...
Rreport.prototype.hideNode = function(nodeId){
	var node = this.getNodeById(nodeId);
	node.c1.hide();
	node.c2.hide();
	node.title.hide();
	node.content.hide();
	node.titleRemark.hide();
	
	for(var linkId in this.linkArr){
		if(this.linkArr[linkId].config.from==nodeId){
			this.hideLink(linkId);
			var to = this.linkArr[linkId].config.to;
...
Rreport.prototype.showNode = function(nodeId){
	var node = this.getNodeById(nodeId);
	node.c1.show();
	node.c2.show();
	node.title.show();
	node.content.show();
	node.titleRemark.show();
	for(var linkId in this.linkArr){
		if(this.linkArr[linkId].config.from==nodeId){
			this.showLink(linkId);
			var to = this.linkArr[linkId].config.to;
			for(var i=0;i<to.length;i++){
				this.showNode(to[i]);
			}
...
Rreport.prototype.hideLink = function(linkId){
	var link = this.linkArr[linkId]
	link.l.hide();
}

Rreport.prototype.showLink = function(linkId){
	var link = this.linkArr[linkId];
	link.l.show();
}
...
Rlink.prototype.getPath = function(){
	...
	if(report.config.viewType=='w'){
		...
		for(var i=0;i<config.to.length;i++){
			...
			var threeP = "L "+(s_x+length)+" "+(targetNode.config.y+(targetNode.config.h-Rreport.conf.titleHeight)/2+Rreport.conf.titleHeight);
			var fourP = "L "+targetNode.config.x+" "+(targetNode.config.y+(targetNode.config.h-Rreport.conf.titleHeight)/2+Rreport.conf.titleHeight);
...
function Rnode(config,report){
	...
	config.th = config.th||Rreport.conf.titleHeight;//标题高度
...
Rnode.prototype.resetSize=function(){
	with(this.config){
		...
		this.config.h = Rreport.conf.titleHeight+k*20;
		...
		this.titleRemark.attr({x:x+w-Rreport.conf.titleHeight,y:y+20});
...
Rnode.prototype.afterAjaxCrete = function(){
	...
	with(this.config){
		...
		this.titleRemark = report.paper.text(x+w-Rreport.conf.titleHeight,y+20,titleRemark).attr({
			"font-size":Rreport.conf.titleFontSize+'px',
			"font-weight":'bold',
			"cursor":'pointer',
			"fill":'#ffffff'
		});
		this.titleRemark.click(function(e){
			var report = this.report;
			var nodeId = this.config.id;
			if(this.isHide==true){
				this.isHide = false;
				this.c1.show();
				this.content.show();
				for(var linkId in report.linkArr){
					if(report.linkArr[linkId].config.from==nodeId){
						report.showLink(linkId);
						var to = report.linkArr[linkId].config.to;
						for(var i=0;i<to.length;i++){
							report.showNode(to[i]);
						}
					}
				}
			}else{
				...
				for(var linkId in report.linkArr){
					if(report.linkArr[linkId].config.from==nodeId){
						report.hideLink(linkId);
						var to = report.linkArr[linkId].config.to;
						for(var i=0;i<to.length;i++){
							report.hideNode(to[i]);
						}
					}
				}
			}
		...
		this.content = report.paper.text(x+10,y+th+h/2-20,content).attr({
			"font-size":Rreport.conf.bodyFontSize+'px',
			"font-weight":'normal',
			"fill":'#000000',
			"text-anchor":'start'
		});

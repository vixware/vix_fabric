...
function openOwnerOptions(id, _f41, _f42, _f43, _f44, _f45) {
	...
	jQuery("#" + _f42).show();
	$(".userminmax").attr('rel',id);
	scrollInViewPart(_f42);
}

function selectMember(){
	var opts = $('.userminmax input');
	var name = "";
	opts.each(function(){
	...
	var linkid = $(".userminmax").attr('rel');
	$(".userminmax").attr('rel','');
	
	if(name!=""){
		name = name.substring(1);
		$('#owner_span_'+linkid).text(name);
	}else{
		$('#owner_span_'+linkid).text('任意成员');
	}
...
//自定义列
function fieldToggle(_109d) {
	$("#common_table tbody tr td:nth-child(" + _109d + "),#common_table tbody tr th:nth-child(" + _109d + ")").toggle();
}
function reorderTaskList(){
	$("#customcolumn ul li input[type='checkbox']").attr("checked","checked");
	$("#common_table tbody tr td,#common_table tbody tr th").show();
}
$(function(){
	$("#customcolumn ul li input[type='checkbox']").each(function(index, element) {
		var $_this = $(this);
		if($_this.attr("checked")!="checked"){
			$("#common_table tbody tr td:nth-child(" + $_this.attr('id') + "),#common_table tbody tr th:nth-child(" + $_this.attr('id') + ")").hide();
		}
	});
	$('#common_table .lastLine').each(function(index, element) {
       $(element).attr('colspan', $('tr:eq(0) th:visible',$(element).closest('table')).length -2);
    });

	$("#customcolumn ul li").click(function() {
		var showId = $("input[type='checkbox']",this).attr("id");
		if($("input[type='checkbox']",this).attr("checked")!="checked"){
			$("input[type='checkbox']",this).attr("checked","checked");
			$("#common_table tbody tr td:nth-child(" + showId + "),#common_table tbody tr th:nth-child(" + showId + ")").show();
		}
		else if($("input[type='checkbox']",this).attr("checked")=="checked"){
			$("input[type='checkbox']",this).attr("checked",false);
			$("#common_table tbody tr td:nth-child(" + showId + "),#common_table tbody tr th:nth-child(" + showId + ")").hide();
		}
		
		$('#common_table .lastLine').each(function(index, element) {
           $(element).attr('colspan', $('tr:eq(0) th:visible',$(element).closest('table')).length -2);
        });
		//$('#common_table .lastLine').attr('colspan',$('#common_table .task_list:eq(0) tr:eq(0) th:visible').length-2);
...
function showAddToDoId(obj){
	var $_obj = $(obj);
	var pos = $_obj.offset();
	var pt = Math.round(eval(pos.top));
	var pl = Math.round(eval(pos.left));
	if(pl<350){
		console.log(1)
		pt = pt-236;
		pl = pl+38;
		$('#addtodoid .zt').css({"right":"auto","left":-15});
		$('#addtodoid .zt .zprarrow').css({"background-position":"-843px -642px"});
	}else{
		console.log(2)
		pt = pt-226;
		pl = pl-330;
		$('#addtodoid .zt').css({"right":-321,"left":"auto"});
		$('#addtodoid .zt .zprarrow').css({"background-position":"-878px -642px"});
	}
	$("#addtodo").css({"top": (pt) + "px","left": (pl) + "px"});
	$('#addtodoid').show();
}
...
function updateValues (idName,num){
	...
	if(num == 0){
		$("#"+idName+id).attr("title","0% 完成");
		$("#"+idName+id+" .greenbar").css("width",0);
		$("#"+idName+id+" .redbar").css("width",50);
	}
	else{
		$("#"+idName+id).attr("title",num+"% 完成");
		$("#"+idName+id+" .greenbar").css("width",num/10*5);
...
function updateDateValues(idName,div){
	$("#"+idName+$("#"+div).attr("rel")).html($("#durtext").val()+'&nbsp;天<span class="morearrow" style="visibility: hidden;">▼</span>');
}

$(function(){
	$('#common_table table tr').hover(function(){
		$('span.morearrow',this).css('visibility','visible');
	},function(){
		$('span.morearrow',this).css('visibility','hidden');
	});
	$('.ulstyle li').hover(function(){
		$(this).attr('class','pt5 pb5 pl5 taskDescRowHover bdrbtm4 pointer');
	},function(){
		$(this).attr('class','pt5 pb5 pl5 taskDescRow bdrbtm4');
	});
	
	if($('.removeAllBox').length > 0){
		if($('.removeAll').attr("checked")=="checked"){
			$(".removeLine").attr("checked","checked");
		}
		$('.removeAll').click(function(){
			if($(this).attr("checked")=="checked"){
				var cbrem = $(this).parents(".cbrem");
				$(".removeLine",cbrem).attr("checked","checked");
			}
			else{
				var cbrem = $(this).parents(".cbrem");
				$(".removeLine",cbrem).attr("checked",false);
			}
		});
		
		$('.removeLine').click(function(){
			var cbrem = $(this).parents(".cbrem");
			var iptl = $(".removeLine",cbrem).length;
			var chel = $(".removeLine:checked",cbrem).length;
			if(iptl==chel){
				$(".removeAll",cbrem).attr("checked","checked");
			}else{
				$(".removeAll",cbrem).attr("checked",false);
			}
		});
	}
...
function removeLine(){
	$('input.removeAll').attr("checked",false);
	$(".cbrem .removeLine:checked").each(function(){
		var $_this = $(this);

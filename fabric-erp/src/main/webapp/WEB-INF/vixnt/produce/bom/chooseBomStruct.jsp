		...
		<div class="widget-body no-padding">
			<div style="margin: 5px;">
				<div class="form-group" style="margin: 0 5px;">
					<div class="col-md-4">
						<input type="text" value="" placeholder="BOM结构名称" class="form-control" id="searchName">
					</div>
					<button onclick="chooseBomStructTable.ajax.reload();" type="button" class="btn btn-info">
						<i class="glyphicon glyphicon-search"></i> 检索
					</button>
					<button type="button" class="btn btn-default" onclick="$('#searchName').val('');chooseBomStructTable.ajax.reload();">
						<i class="glyphicon glyphicon-repeat"></i> 清空
					</button>
				</div>
			</div>
			<table id="chooseBomStructTableId" class="table table-striped table-bordered table-hover" width="100%">
				<thead>
					<tr>
						<th width="10%">
...
<script type="text/javascript">
	...
	var chooseBomStructColumns = [ {
		"data" : function(data) {
			if (chooseType == 'single') {
				return "<input id='entityId' name='entityId' value='" + data.id + "' type='radio' onchange=\"radioChange('${entityName}','" + data.id + "','" + data.configItemBomName + "');\"/>";
			}
			return "";
		}
	}, {
	...
	var chooseItemTable = initDataTable("chooseBomStructTableId", "${nvix}/nvixnt/produce/nvixBomStructAction!goChooseBomStructListContent.action", chooseBomStructColumns, function(page, pageSize, orderField, orderBy) {

	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-tasks"></i>任务与计划 <span >> 任务详情</span>
				<%-- <s:if test="source =='taskandplan'">
					<i class="fa-fw fa fa-tasks"></i>
					<a style="cursor: pointer;" onclick="loadContent('mid_task','${nvix}/nvixnt/taskAndPlanAction!goList.action');">任务与计划 </a>
					<span style="cursor: pointer;" onclick="loadContent('mid_task','${nvix}/nvixnt/taskAndPlanAction!goProjectTaskDetails.action');">> 任务详情</span>
				</s:if>
				<s:else>
					<i class="fa-fw fa fa-tasks"></i>
					<a style="cursor: pointer;" onclick="loadContent('mid_task','${nvix}/nvixnt/taskAndPlanAction!goList.action');">任务与计划 </a>
					<span style="cursor: pointer;" onclick="loadContent('mid_projectstask','${nvix}/nvixnt/taskAndPlanAction!goProjectTaskManagement.action');">> 项目任务管理</span>
					<span style="cursor: pointer;" onclick="loadContent('mid_projectstask','${nvix}/nvixnt/taskAndPlanAction!goProjectTaskDetails.action');">> 任务详情</span>
				</s:else> --%>
			</h1>
		</div>
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				<c:if test="${source == 'taskandplan'}">
					<button class="btn btn-default" type="button" onclick="loadContent('', '${nvix}/nvixnt/taskAndPlanAction!goTask.action');">
						<i class="fa fa-rotate-left"></i> 返回
					</button>
				</c:if>
				<c:if test="${source == 'task'}">
					<button class="btn btn-default" type="button" onclick="loadContent('', '${nvix}/nvixnt/taskAndPlanAction!goMyTask.action');">
						<i class="fa fa-rotate-left"></i> 返回
					</button>
				</c:if>
				...
				<c:if test="${source == 'taskhome'}">
					<button class="btn btn-default" type="button" onclick="loadContent('mid_task','${nvix}/nvixnt/taskAndPlanAction!goList.action');">
						<i class="fa fa-rotate-left"></i> 返回
					</button>
				</c:if>
				<c:if test="${source == 'project'}">
					<button class="btn btn-default" type="button" onclick="loadContent('mid_task','${nvix}/nvixnt/nvixProjectAction!goProjectTracing.action');">
						<i class="fa fa-rotate-left"></i> 返回
					</button>
				</c:if>
				<c:if test="${source == 'projectTask'}">
					<button class="btn btn-default" type="button" onclick="loadContent('mid_task','${nvix}/nvixnt/taskAndPlanAction!goProjectTask.action');">
						<i class="fa fa-rotate-left"></i> 返回
					</button>
				</c:if>
				<c:if test="${source == 'teamtask'}">
					<button class="btn btn-default" type="button" onclick="loadContent('mid_task','${nvix}/nvixnt/taskAndPlanAction!goTeamTask.action');">
						<i class="fa fa-rotate-left"></i> 返回
					</button>
				</c:if>
			</div>
		...
		<div class="row">
			<article class="col-sm-12 col-md-12 col-lg-12">
				<div class="jarviswidget" id="">
					<header>
						<span class="widget-icon"><i class="fa-fw fa fa-tasks"></i></span>
							...
							<div class="col-sm-12 col-md-12 col-lg-7">
								...
								<div class="row">
									<div class="col-sm-12 col-md-12 col-lg-12">
										<%-- <button type="button" class="btn btn-default" onclick="loadContent('','${nvix}/nvixnt/taskAndPlanAction!goShowTaskEmployeeList.action?taskId=${vixTask.id }');">
											<span class="glyphicon glyphicon-list text-muted"></span>（${employeeNum }）
										</button> --%>
										<button type="button" class="btn btn-default" onclick="loadContent('','${nvix}/nvixnt/taskAndPlanAction!goSaveOrUpdateTaskAttachment.action?id=${vixTask.id }&source=${source}');">
											<span class="glyphicon glyphicon-paperclip text-muted"></span> （${uploaderNum }）
										</button>
										<button type="button" class="btn btn-default" onclick="loadContent('','${nvix}/nvixnt/taskAndPlanAction!goSaveOrUpdateTaskDiscuss.action?id=${vixTask.id }&source=${source}');">
																				...
																				<li>
																					<a href="javascript:void(0);">
																						<s:if test="entity.headImgUrl !=null">
																							<img src="${entity.headImgUrl }" alt="">
																						</s:if> <s:else>
																							<img src="${nvix}/vixntcommon/base/images/touxiang.png" alt="">
																						</s:else>${entity.name } 
																				...
																				<table id="executionFeedbackTableId" class="table table-striped table-bordered table-hover" width="100%">
																					<thead>
																						<tr>
																							...
																							<th data-class="expand" width="20%">进度</th>
																							<th data-class="expand" width="55%">内容</th>
																							<th data-class="expand" width="10%">时间</th>
																							<th width="10%">操作</th>
																						</tr>
																					</thead>
																				</table>
...
<script type="text/javascript">
	...
	var vixTaskTable = initDataTable("vixTaskTableId", "${nvix}/nvixnt/taskAndPlanAction!goSubTaskList.action?id=${vixTask.id}", vixTaskColumns, function(page, pageSize, orderField, orderBy) {
		return {
		"page" : page,
		"pageSize" : pageSize,
	...
	var productCategoryColumns = [ {
		...
		"data" : function(data) {
			if(data.schedule != null){
				return "<div data-progressbar-value='"+data.schedule+"' class='progress progress-xs'><div class='progress-bar'></div></div>";
			}else{
		...
		"data" : function(data) {
			return moment(data.feedbackTime).format('YYYY-MM-DD HH:mm:ss');
		}
	}, {
		"data" : function(data) {
	...
	var executionFeedbackTable = initDataTable("executionFeedbackTableId", "${nvix}/nvixnt/taskAndPlanAction!goExecutionFeedbackList.action?id=${vixTask.id}", productCategoryColumns, function(page, pageSize, orderField, orderBy) {
		return {
		"page" : page,
		"pageSize" : pageSize,
		"orderField" : orderField,
	...
	function goSaveOrUpdate(id, parentId, title) {
		openSaveOrUpdateWindow('${nvix}/nvixnt/taskAndPlanAction!goSaveOrUpdate.action?id=' + id + "&parentId=" + parentId, "taskForm", title, 900, 650, vixTaskTable);
	};
	//新增反馈
	...
	function deleteTaskEmployee(id, taskId) {
		$.ajax({
		url : '${nvix}/nvixnt/taskAndPlanAction!checkVixTaskEmployeeNum.action?taskId=' + taskId,
		cache : false,

	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				<button class="btn btn-success" type="button" onclick="accept();">
					<i class="fa fa-save"></i> 签收背书
				</button>
				<button class="btn btn-success" type="button" onclick="reject();">
					<i class="fa fa-save"></i> 拒签背书
				</button>
...
<script type="text/javascript">
	...
	function accept() {
		if ($("#fabricBillForm").validationEngine('validate')) {
			$("#fabricBillForm").ajaxSubmit({
			type : "post",
			url : "${nvix}/nvixnt/vixntFabricBillAction!accept.action?id=" + $('#fabricBillId').val(),
			dataType : "text",
	...
	function reject() {
		if ($("#fabricBillForm").validationEngine('validate')) {
			$("#fabricBillForm").ajaxSubmit({
			type : "post",
			url : "${nvix}/nvixnt/vixntFabricBillAction!reject.action?id=" + $('#fabricBillId').val(),
			dataType : "text",
			success : function(data) {
				showMessage(data, 'success');
				loadContent('', '${nvix}/nvixnt/vixntFabricBillAction!goList.action');

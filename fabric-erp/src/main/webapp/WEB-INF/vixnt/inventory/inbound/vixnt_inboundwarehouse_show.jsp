	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				...
				<button class="btn btn-success" type="button" onclick="loadContent('','${nvix}/nvixnt/vixntInboundWarehouseAction!goSourceList.action?id='+$('#stockRecordsId').val());">
					<i class="fa fa-retweet"></i> 追溯
				</button>
				<button class="btn bg-color-blueDark txt-color-white" type="button" onclick="">
...
<script type="text/javascript">
	...
	function saveOrUpdate() {
		if ($("#stockRecordsForm").validationEngine('validate')) {
			...
			type : "post",
			url : "${nvix}/nvixnt/vixntInboundWarehouseAction!update.action",
			dataType : "text",
			success : function(result) {
				showMessage(result);
				loadContent('', '${nvix}/nvixnt/vixntInboundWarehouseAction!goList.action');
			}
			});
		} else {
			return false;
	...
	function goSaveOrUpdateStockRecordLines(id, title) {
		if (id != null && id != '' & id != 'undefined') {
			openWindowForShow('${nvix}/nvixnt/vixntInboundWarehouseAction!goSaveOrUpdateStockRecordLines.action?id=' + id, title, 950, 675);
		} else {
			if ($("#stockRecordsForm").validationEngine('validate')) {
				$("#stockRecordsForm").ajaxSubmit({
				type : "post",
				...
				success : function(stockRecordsId) {
					//有问题
					var result = stockRecordsId.split(":");
					if (result.length > 1) {
						$("#stockRecordsId").val(result[0]);
						openWindowForShow('${nvix}/nvixnt/vixntInboundWarehouseAction!goSaveOrUpdateStockRecordLines.action?id=' + id + "&stockRecordsId=" + result[0], title, 950, 675);
					}
				}
				});
			} else {
	...
	function saveOrUpdateStockRecordLines() {
		if ($("#stockRecordsId").val() != '' && $("#stockRecordsId").val() != null) {
			$.post('${nvix}/nvixnt/vixntInboundWarehouseAction!saveOrUpdateStockRecordLines.action', {
	...
	function audit() {
		if ($("#stockRecordsForm").validationEngine('validate')) {
			$("#stockRecordsForm").ajaxSubmit({
			type : "post",
			url : "${nvix}/nvixnt/vixntInboundWarehouseAction!audit.action",
			dataType : "text",
			success : function(result) {
	...
	function goBeforeOrAfter(tag) {
		goSaveOrUpdateEntity('${nvix}/nvixnt/vixntInboundWarehouseAction!goBeforeOrAfterStockRecords.action?str=' + tag + '&id=' + $('#stockRecordsId').val());
	}

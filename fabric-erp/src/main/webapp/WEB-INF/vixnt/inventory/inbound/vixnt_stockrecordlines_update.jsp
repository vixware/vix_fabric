				...
				<div>
					<div class="widget-body">
						<fieldset>
							...
							<input id="measureUnitId" name="stockRecordLines.measureUnit.id" value="${stockRecordLines.measureUnit.id}" type="hidden" /> 
							<input type="hidden" id="stockRecordLinesStockRecordsId" name="stockRecordLines.stockRecords.id" value="${stockRecordLines.stockRecords.id}" /> <input type="hidden" id="itemcode" name="stockRecordLines.itemcode" value="${stockRecordLines.itemcode }" />
							...
							<div class="form-group">
								...
								<div class="col-md-3">
									<input id="measureUnitName" name="stockRecordLines.measureUnit.name" value="${stockRecordLines.measureUnit.name}" data-prompt-position="topLeft" class="form-control" type="text" />
								</div>
							</div>
							<div class="form-group">
								<label class="col-md-3 control-label">货位:</label>
								<div class="col-md-9">
							...
							<div class="form-group">
								...
								<div class="col-md-9">
									<input id="supplierId" name="stockRecordLines.supplier.id" value="${stockRecordLines.supplier.id}" type="hidden" /> <input id="supplierName" name="stockRecordLines.supplier.name" value="${stockRecordLines.supplier.name}" onfocus="goChooseSupplier();" data-prompt-position="topLeft"
										class="form-control" type="text" />
								</div>
							</div>
							<div class="form-group">
							...
							<div class="form-group">
								...
								<div class="col-md-9">
									<div class="input-group">
										<input id="massunitEndTime" name="stockRecordLines.massunitEndTime" value="${stockRecordLines.massunitEndTimeStr}" data-prompt-position="topLeft" class="form-control" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" type="text" /> <span class="input-group-addon"
											onclick="WdatePicker({dateFmt:'yyyy-MM-dd',el:'massunitEndTime'});"><i class="fa fa-calendar"></i></span>
									</div>
								</div>
							</div>
						</fieldset>
...
<script type="text/javascript">
	...
	function updateStockRecordLines() {
		if ($('#stockRecordLinesForm').validationEngine('validate')) {
			...
			//入库单主体ID
			if(!$("#stockRecordLinesStockRecordsId").val()){
				$("#stockRecordLinesStockRecordsId").val($("#stockRecordsId").val());
			}
			$("#stockRecordLinesForm").ajaxSubmit({
			type : "post",
			url : "${nvix}/nvixnt/vixntInboundWarehouseAction!updateStockRecordLines.action?id=" + $("#stockRecordLinesStockRecordsId").val(),
			...
			success : function(result) {
				...
				clearStockInDetailForm();
				$.ajax({
				url : '${nvix}/nvixnt/vixntInboundWarehouseAction!getStockRecordTotal.action?id=' + $("#stockRecordsId").val(),
				cache : false,

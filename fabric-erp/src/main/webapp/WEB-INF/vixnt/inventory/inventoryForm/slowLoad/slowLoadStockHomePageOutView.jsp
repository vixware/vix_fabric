...
<div id="slowLoadResultViewOut" style="height: 300px"></div>
...
function slowLoadResultViewOut(DivTestId,chancevar) {
	$.ajax({    
		url: "${nvix}/nvixnt/vixntStockQueryHomePageAction!queryDataViewStockInOut.action",
	 	type: "POST",
	 	data: {queryTime:'-Lately-Day{30}',inOrOutStock:'out'},
	    dataType: "json",
...
'',
		  ...
		  /** 下面是用js对数据块中今日,昨日,环比...等进行显示html操作     开始 **/
		    var ytdayOutNum = Number( valueArrNumberVar[(valueArrNumberVar.length-2)]  ).toFixed(2);
		    $("#ytdayOutNum").html(ytdayOutNum);//昨日出库-物料总数  
		    var todayOutNum = Number( valueArrNumberVar[(valueArrNumberVar.length-1)]  ).toFixed(2);
		    $("#todayOutNum").html(todayOutNum);//今日出库-物料总数
		    var perNum = getMomStr(todayOutNum,ytdayOutNum);//环比百分数
			   if(perNum<0.0){
				   $("#iclasspertodayOutNum").html("<i class='fa fa-arrow-down tmyColorA '></i>"+Number( (-(perNum))  ).toFixed(2)+"%");  
			   }else{
				   $("#iclasspertodayOutNum").html("<i class='fa fa-arrow-up tmyColorB '></i>"+Number( perNum  ).toFixed(2)+"%");  
			   }
...
$(document).ready(function() {
	slowLoadResultViewOut('slowLoadResultViewOut','2');
})
</script>

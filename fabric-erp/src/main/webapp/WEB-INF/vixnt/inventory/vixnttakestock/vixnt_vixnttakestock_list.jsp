	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				<a href="#" class="btn btn-primary" onclick="goChooseWarehouseAndShelf('');"><i class="glyphicon glyphicon-plus"></i>&nbsp;生成盘点单</a>
			</div>
		</div>
	</div>
	<!-- row -->
...
<script type="text/javascript">
	...
	var vixntTakeStockTable = initDataTable("vixntTakeStockTableId", "${nvix}/nvixnt/vixntTakeStockAction!goSingleList.action", vixntTakeStockColumns, function(page, pageSize, orderField, orderBy) {
		var title = $("#title").val();
	...
	function goChooseWarehouseAndShelf() {
		openSaveOrUpdateWindow('${nvix}/nvixnt/vixntTakeStockAction!goChooseWarehouseAndShelf.action', "warehouseform", "盘点", 500, 325, null, null, function(id) {
			var result = id.split(":");
			if (result.length > 1) {
				goSaveOrUpdate(result[0]);
			}
		});
	};
	...
	function goSaveOrUpdate(id) {
		$.ajax({
		url : '${nvix}/nvixnt/vixntTakeStockAction!goSaveOrUpdate.action?id=' + id,
		cache : false,
	...
	function deleteById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/vixntTakeStockAction!deleteById.action?id=' + id, '是否删除盘点单?', vixntTakeStockTable);
	};
	//导出盘点单
	function exportExcel(id) {
		form = document.getElementById("exportMD");
		form.action = "${nvix}/nvixnt/vixntTakeStockAction!exportTakeStockExcel2.action?id=" + id;
		form.submit();
	};
	//导出盘点对比表
	function exportExcel1(id) {
		form = document.getElementById("exportMD");
		form.action = "${nvix}/nvixnt/vixntTakeStockAction!exportTakeStockExcel1.action?id=" + id;
		form.submit();
	};
	function importXlsFile() {
		$.ajaxFileUpload({
		url : '${nvix}/nvixnt/vixntTakeStockAction!importFile.action?id=' + $('#stockTakingId').val(),// 用于文件上传的服务器端请求地址

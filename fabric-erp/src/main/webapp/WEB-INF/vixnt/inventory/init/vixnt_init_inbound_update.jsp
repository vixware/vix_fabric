	...
	<div class="row">
		...
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				...
				<button class="btn btn-default" type="button" onclick="loadContent('inv_vixntinboundwarehouse','${nvix}/nvixnt/vixntInitInventoryAction!goList.action');">
		...
		<div class="widget-body">
			<form class="form-horizontal" id="stockRecordsForm">
				<fieldset>
					...
					<input type="hidden" id="stockRecordsFlag" name="stockRecords.flag" value="${stockRecords.flag}" /> 
					<input type="hidden" id="stockRecordstype" name="stockRecords.type" value="${stockRecords.type}" />
					<input type="hidden" id="warehouseId" name="stockRecords.invWarehouse.id" value="${stockRecords.invWarehouse.id}" />
					<input type="hidden" id="isInventory" name="stockRecords.isInventory" value="${stockRecords.isInventory}" />
...
<script type="text/javascript">
	...
	var stockRecordLinesTable = initDataTable("stockRecordLinesTableId", "${nvix}/nvixnt/vixntInitInventoryAction!goStockRecordLinesList.action", stockRecordLinesColumns, function(page, pageSize, orderField, orderBy) {
	...
	function saveOrUpdate() {
		if ($("#stockRecordsForm").validationEngine('validate')) {
			...
			type : "post",
			url : "${nvix}/nvixnt/vixntInitInventoryAction!update.action",
			dataType : "text",
			success : function(id) {
				loadContent('', '${nvix}/nvixnt/vixntInitInventoryAction!goList.action');
			}
			});
		} else {
			return false;
	...
	function goSaveOrUpdateStockRecordLines(id, title) {
		if (id != null && id != '' & id != 'undefined') {
			openWindowForShow('${nvix}/nvixnt/vixntInitInventoryAction!goSaveOrUpdateStockRecordLines.action?id=' + id, title, 950, 675);
		} else {
			if ($("#stockRecordsForm").validationEngine('validate')) {
				$("#stockRecordsForm").ajaxSubmit({
				type : "post",
				url : "${nvix}/nvixnt/vixntInitInventoryAction!saveOrUpdate.action",
				dataType : "text",
				success : function(salesOrderId) {
					var result = salesOrderId.split(":");
					if (result.length > 1) {
						$("#stockRecordsId").val(result[0]);
						openWindowForShow('${nvix}/nvixnt/vixntInitInventoryAction!goSaveOrUpdateStockRecordLines.action?id=' + id + "&stockRecordsId=" + result[0], title, 950, 675);
					}
				}
				});
			} else {
	...
	function saveOrUpdateStockRecordLines() {
		if ($("#stockRecordsId").val() != '' && $("#stockRecordsId").val() != null) {
			$.post('${nvix}/nvixnt/vixntInitInventoryAction!saveOrUpdateStockRecordLines.action', {
	...
	function deleteWimStockrecordlinesById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/vixntInitInventoryAction!deleteWimStockrecordlinesById.action?id=' + id, '是否删除入库单明细?', stockRecordLinesTable);
	};
</script>
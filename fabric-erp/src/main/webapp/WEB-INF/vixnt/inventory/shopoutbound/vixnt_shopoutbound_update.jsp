	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-cubes"></i> 门店管理 <span>&gt; 门店业务管理</span> <span>&gt; 门店出库管理</span>
			</h1>
		</div>
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				...
				<button class="btn btn-default" type="button" onclick="loadContent('inv_vixntoutbound','${nvix}/nvixnt/vixntShopOutBoundAction!goList.action');">
	...
	<div class="jarviswidget" id="" data-widget-colorbutton="false" data-widget-editbutton="false">
		...
		<div class="widget-body">
			<form class="form-horizontal" id="stockRecordsForm">
				<fieldset>
					<input type="hidden" id="stockRecordsId" name="stockRecords.id" value="${stockRecords.id}" /> <input type="hidden" id="warehouseId" name="stockRecords.invWarehouse.id" value="${stockRecords.invWarehouse.id}" /> <input type="hidden" id="stockRecordstype" name="stockRecords.type"
						value="${stockRecords.type}" /> <input type="hidden" id="channelDistributorId" name="stockRecords.channelDistributor.id" value="${stockRecords.channelDistributor.id}" />
...
<script type="text/javascript">
	...
	var stockRecordLinesTable = initDataTable("stockRecordLinesTableId", "${nvix}/nvixnt/vixntShopOutBoundAction!goStockRecordLinesList.action", stockRecordLinesColumns, function(page, pageSize, orderField, orderBy) {
		var id = $("#stockRecordsId").val();
		var searchProductName = $("#searchitemname").val();
	...
	function saveOrUpdate() {
		if ($("#stockRecordsForm").validationEngine('validate')) {
			...
			type : "post",
			url : "${nvix}/nvixnt/vixntShopOutBoundAction!saveOrUpdate.action",
			dataType : "text",
			success : function(id) {
				loadContent('', '${nvix}/nvixnt/vixntShopOutBoundAction!goList.action');
			}
			});
		} else {
			return false;
	...
	function goSaveOrUpdateStockRecordLines(id) {
		if (id != null && id != '') {
			openWindowForShow('${nvix}/nvixnt/vixntShopOutBoundAction!goSaveOrUpdateStockRecordLines.action?id=' + id, "修改", 850, 675);
		} else {
			if ($("#stockRecordsForm").validationEngine('validate')) {
				$("#stockRecordsForm").ajaxSubmit({
				type : "post",
				...
				success : function(salesOrderId) {
					...
					openWindowForShow('${nvix}/nvixnt/vixntShopOutBoundAction!goSaveOrUpdateStockRecordLines.action?id=' + id + "&salesOrderId=" + salesOrderId, "新增", 850, 675);
				}
				});
			} else {
				return false;
	...
	function saveOrUpdateStockRecordLines() {
		if ($("#stockRecordsId").val() != '' && $("#stockRecordsId").val() != null) {
			$.post('${nvix}/nvixnt/vixntShopOutBoundAction!saveOrUpdateStockRecordLines.action', {

	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa fa-lg fa-fw fa-cube"></i> 商品管理 <span>&gt; 商品组装</span>
			</h1>
		</div>
...
<script type="text/javascript">
	...
	var stockRecordsTable = initDataTable("stockRecordsTableId", "${nvix}/nvixnt/vixntProductAssemblyAction!goSingleList.action", stockRecordsColumns, function(page, pageSize, orderField, orderBy) {
	...
	function goSaveOrUpdate(id) {
		$.ajax({
		url : '${nvix}/nvixnt/vixntProductAssemblyAction!goSaveOrUpdate.action?id=' + id,
		cache : false,
	...
	function deleteById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/vixntProductAssemblyAction!deleteById.action?id=' + id, '是否删除商品?', stockRecordsTable);
	};

	function goChooseItem() {
		chooseListData('${nvix}/nvixnt/vixntProductAssemblyAction!goChooseItem.action', 'multi', '选择商品', null, function() {
			var emp = chooseMap.valueSetWithClear().split(":");
			if (emp != '' && emp.length == 2) {
				$.ajax({
				url : '${nvix}/nvixnt/vixntProductAssemblyAction!bingItem.action?itemIds=' + emp[0],
				cache : false,
				success : function(id) {
					goSaveOrUpdate(id);
				}

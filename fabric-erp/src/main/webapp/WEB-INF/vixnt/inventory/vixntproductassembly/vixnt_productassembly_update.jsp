	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa fa-lg fa-fw fa-cube"></i> 商品管理 <span onclick="">&gt; 商品组装</span>
			</h1>
		</div>
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				...
				<button class="btn btn-default" type="button" onclick="loadContent('', '${nvix}/nvixnt/vixntProductAssemblyAction!goList.action');">
	...
	<div class="jarviswidget">
		...
		<div class="widget-body">
			<form class="form-horizontal" id="itemForm">
				<fieldset>
					<input type="hidden" id="itemId" name="item.id" value="${item.id}" /> <input type="hidden" id="isBinding" name="item.isBinding" value="${item.isBinding}" />
...
<script type="text/javascript">
	...
	"orderable" : false,
	"data" : function(data) {
		var update = "<a href='javascript:void(0);' class='btn btn-xs btn-default' onclick=\"goUpdateItemNum('" + data.id + "','修改');\" title='修改'><span class='txt-color-blue pull-right'><i class='fa fa-pencil'></i></span></a>";
		var del = "<a href='javascript:void(0);' class='btn btn-xs btn-default' onclick=\"deleteItemById('" + data.id + "');\" title='删除'><span class='txt-color-red pull-right'><i class='fa fa-times'></i></span></a>";
		return update + "&nbsp;&nbsp;" + del;
	}
	} ];
	var stockRecordLinesTable = initDataTable("stockRecordLinesTableId", "${nvix}/nvixnt/vixntProductAssemblyAction!goItemList.action", stockRecordLinesColumns, function(page, pageSize, orderField, orderBy) {
	...
	function saveOrUpdate() {
		if ($("#itemForm").validationEngine('validate')) {
			$("#itemForm").ajaxSubmit({
			type : "post",
			url : "${nvix}/nvixnt/vixntProductAssemblyAction!saveOrUpdate.action",
			dataType : "text",
			success : function(id) {
				loadContent('', '${nvix}/nvixnt/vixntProductAssemblyAction!goList.action');
			}
			});
		} else {
			return false;
	...
	function goUpdateItemNum(id, title) {
		if (id != null && id != '') {
			openSaveOrUpdateWindow('${nvix}/nvixnt/vixntProductAssemblyAction!goUpdateItemNum.action?id=' + id, "numForm", title, 650, 250, stockRecordLinesTable);
		}
	};
	//删除
	function deleteItemById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/vixntProductAssemblyAction!deleteItemById.action?id=' + id, '是否删除商品?', stockRecordLinesTable);
	};
</script>
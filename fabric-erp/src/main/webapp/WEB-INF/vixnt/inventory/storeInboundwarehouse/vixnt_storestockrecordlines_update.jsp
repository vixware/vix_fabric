					...
					<div class="widget-body">
						<fieldset>
							...
							<div class="form-group">
								...
								<div class="col-md-9">
									<input id="invShelfId" name="stockRecordLines.invShelf.id" value="${stockRecordLines.invShelf.id}" type="hidden" /> 
									<input id="invShelfName" name="stockRecordLines.invShelf.name" value="${stockRecordLines.invShelf.name}" onfocus="chooseLocation();" data-prompt-position="topLeft" class="form-control" type="text" />
								</div>
							</div>
							<div class="form-group">
...
<script type="text/javascript">
	...
	var chooseEcProductTable = initDataTable("chooseEcProduct", "${nvix}/nvixnt/vixntStoreInboundWarehouseAction!goChooseStoreItemList.action", chooseEcProductColumns, function(page, pageSize, orderField, orderBy) {
	...
	$('#chooseEcProduct tbody').on('click', 'tr', function() {
		...
		if (data != undefined) {
			...
			$("#measureUnitName").val(data.measureUnit.name);
			$("#quantity").val("1");
	...
	function updateStockRecordLines() {
		if ($('#stockRecordLinesForm').validationEngine('validate')) {
			...
			type : "post",
			url : "${nvix}/nvixnt/vixntStoreInboundWarehouseAction!updateStockRecordLines.action",
			...
			success : function(result) {
				...
				clearStockInDetailForm();
				$.ajax({
				url : '${nvix}/nvixnt/vixntStoreInboundWarehouseAction!getStockRecordTotal.action?id=' + $("#stockRecordsId").val(),
				cache : false,

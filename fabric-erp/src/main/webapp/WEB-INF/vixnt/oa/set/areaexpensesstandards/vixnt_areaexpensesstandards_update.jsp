...
<form id="transportForm" class="form-horizontal" style="padding: 20px 15px;" action="${nvix}/nvixnt/nvixAreaExpensesStandardsAction!saveOrUpdate.action">
	...
	<fieldset>
		...
		<div class="form-group">
			...
			<div class="col-md-4">
				<select id="transportId" name="areaExpensesStandards.transport.id" data-prompt-position="centerRight" class="form-control validate[required]">
					<option value="">------请选择------</option>
					<c:forEach items="${transportList}" var="entity">
						<option value="${entity.id}" <c:if test="${areaExpensesStandards.transport.id == entity.id}">selected="selected"</c:if>>${entity.name}</option>
					</c:forEach>
				</select>
			</div>

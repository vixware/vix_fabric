	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa fa-tasks fa-fw"></i> 办公 <span>> 日报附件 </span>
			</h1>
		</div>
				...
				<div class="tab-content">
					<div class="col-sm-12 col-md-12 list-warp tab-pane active" id="t1">
						<div class="file-toolbar clearfix">
							<span class="smart-form pull-left"><label class="checkbox"><input type="checkbox" name="checkbox"><i></i>&nbsp;</label></span> <a data-toggle="modal" href="#uploadFile" class="btn btn-primary" onclick="goAddUploader('${workLog.id }')"><i class="glyphicon glyphicon-upload"></i> 上传文件</a> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span
								class="pull-right file-toolbar-buttons listbtn"> <a href="javascript:;" data-type="large"><i class="glyphicon glyphicon-th-large"></i></a> <a href="javascript:;" data-type="list" class="active"><i class="glyphicon glyphicon-th-list"></i></a>
							</span>
						</div>
...
<script type="text/javascript">
	...
	function goAddUploader(id) {
		openSaveOrUpdateWindow('${nvix}/nvixnt/dailyJournalAction!goAddUploader.action?parentId=' + id, "uploaderForm", "添加附件", 550, 350, null, null, function() {
			loadContent('', '${nvix}/nvixnt/dailyJournalAction!goWorkLogAttachment.action?id=${workLog.id }');
		});
	};
	function deleteUploaderByIds() {
		...
		deleteEntityByConfirm('${nvix}/nvixnt/taskAndPlanAction!deleteUploaderByIds.action?ids=' + chk_value, '是否删除附件?', null, null, function() {
			loadContent('', '${nvix}/nvixnt/dailyJournalAction!goWorkLogAttachment.action?id=${workLog.id }');
		});
	};
	function deleteUploaderById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/dailyJournalAction!deleteUploaderById.action?id=' + id, '是否删除附件?', null, null, function() {
			loadContent('', '${nvix}/nvixnt/dailyJournalAction!goWorkLogAttachment.action?id=${workLog.id }');
		});
	};
	function downloadUploader(id) {
		var url = "${nvix}/nvixnt/dailyJournalAction!downloadUploader.action?id=" + id;
		window.open(url);
	};
</script>
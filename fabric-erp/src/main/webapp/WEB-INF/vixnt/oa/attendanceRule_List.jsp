...
<script type="text/javascript">
	...
	var attendanceColumns = [
		...
		{"name":"createTime","data" : function(data) {return data.createTimeStr;}},
		{"name":"isEnable","data" : function(data) {return data.isEnable == null ? '' : data.isEnable == '0' ? '<span style="color: red;">禁用</span>' : '<span style="color: blue;">启用</span>';}},
	...
	var attendanceTable = initDataTable("attendance", "${nvix}/nvixnt/attendanceRuleAction!getRulesJson.action", attendanceColumns, function(page, pageSize, orderField, orderBy) {
		var searchRuleType = $("#searchRuleType").val();
		searchRuleType = Url.encode(searchRuleType);
		return {"page" : page, "pageSize" : pageSize, "orderField" : orderField, "orderBy" : orderBy, "searchRuleType" : searchRuleType};
	});

	//新增
	function saveOrUpdate(id){
		goSaveOrUpdateEntity('${nvix}/nvixnt/attendanceRuleAction!goSaveOrUpdate.action?id=' + id);
	}
	//删除
	function deletesById(id){
		deleteEntityByConfirm('${nvix}/nvixnt/attendanceRuleAction!deleteById.action?id='+id,'是否删除该考勤类型?',attendanceRuleTable);
	}
</script>
	...
	<div class="row hidden-mobile">
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-puzzle-piece"></i>工作日设置
			</h1>
		</div>
									...
									<div class="table-responsive">
										<div id="" class="input-group input-group-lg">
											<form class="navbar-form navbar-left" role="search">
												...
												<button onclick="workDayTable.ajax.reload();" type="button" class="btn btn-info">
													<i class="glyphicon glyphicon-search"></i> 检索
												</button>
												<button onclick="$('#searchName').val('');workDayTable.ajax.reload();" type="button" class="btn btn-default">
													<i class="glyphicon glyphicon-repeat"></i> 清空
												</button>
											</form>
										</div>
										<table id="workDay" class="table table-striped table-bordered table-hover" width="100%">
											<thead>
												<tr>
													<th>编号</th>
													<th>编码</th>
...
<script type="text/javascript">
	...
	var workDayTable = initDataTable("workDay", "${nvix}/nvixnt/workDayAction!getWorkDaysJson.action", workDayColumns, function(page, pageSize, orderField, orderBy) {
		...
		"orderBy" : orderBy,
		"searchRuleType" : searchName
		};
	});

	//新增
	function saveOrUpdate(id) {
		openSaveOrUpdateWindow('${nvix}/nvixnt/workDayAction!goSaveOrUpdate.action?id=' + id, 'workDayForm', '工作日设定', 850, 750, workDayTable, null, function() {
			workDayTable.ajax.reload();
		});
	}
	//删除

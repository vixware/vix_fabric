	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-user"></i> 系统管理 <span>> 角色管理 </span>
			</h1>
		</div>
			...
			<div class="widget-body no-padding">
				<div>
					<form role="search" class="navbar-form navbar-left">
						<input type="hidden" id="homeTemplateId" name="" value="" />
...
<script type="text/javascript">
	...
	"data" : function(data) {
		...
		var chooseHomeTemplate = "<a href='javascript:void(0);' class='btn btn-xs btn-default' onclick=\"goChooseHomeTemplate('" + data.id + "');\" title='绑定模板'><span class='txt-color-blue pull-right'><i class='fa fa-link'></i></span></a>";
		var del = "<a href='javascript:void(0);' class='btn btn-xs btn-default' onclick=\"deleteById('" + data.id + "');\" title='删除'><span class='txt-color-red pull-right'><i class='fa fa-times'></i></span></a>";
		return update + "&nbsp;&nbsp;" + authority + "&nbsp;&nbsp;" + chooseHomeTemplate + "&nbsp;&nbsp;" + del;
	}
	} ];

	var wxpRoleTable = initDataTable("wxpRoleTableId", "${nvix}/nvixnt/nvixntRoleAction!goSingleList.action", wxpRoleColumns, function(page, pageSize, orderField, orderBy) {
	...
	function goChooseHomeTemplate(id) {
		chooseListData('${nvix}/nvixnt/nvixntRoleAction!goChooseHomeTemplate.action', 'single', '选择模板', 'homeTemplate', function() {
			$.ajax({
			url : '${nvix}/nvixnt/nvixntRoleAction!bindHomeTemplateToRole.action?roleId=' + id + "&homeTemplateId=" + $('#homeTemplateId').val(),
			cache : false,
			success : function(result) {
				showMessage(result, 'success');
				wxpRoleTable.ajax.reload();
			}
			});
		});
	};
	...
	function saveOrUpdate(id) {
		openSaveOrUpdateWindow('${nvix}/nvixnt/nvixntRoleAction!goSaveOrUpdate.action?id=' + id, "wxpRoleForm", "新增角色", 750, 350, wxpRoleTable);
	};
	//删除
	...
	function toAuthority(id) {
		...
		$.ajax({
		url : '${nvix}/nvixnt/nvixntRoleAction!goRoleAuthority.action',
		data : "id=" + id,
		cache : false,

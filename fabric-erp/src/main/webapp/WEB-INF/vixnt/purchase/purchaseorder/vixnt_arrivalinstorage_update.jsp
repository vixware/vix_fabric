	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa fa-fw fa-shopping-cart"></i> 供应链管理 <span onclick="">&gt; 采购管理</span> <span onclick="">&gt; 采购订单</span>
			</h1>
		</div>
					...
					<div class="form-group">
						...
						<div class="col-md-3">
							<div class="input-group">
								<input id="createTime" name="purchaseOrder.createTime" value="${purchaseOrder.startTimeTimeStr}" data-prompt-position="topLeft" class="form-control validate[required]" onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})" type="text" /> <span class="input-group-addon"
...
<script type="text/javascript">
	...
	function saveOrUpdate() {
		if ($("#purchaseOrderForm").validationEngine('validate')) {
			...
			type : "post",
			url : "${nvix}/nvixnt/purchase/vixntPurchaseOrderAction!inbond.action",
			dataType : "text",
			success : function(id) {
	...
	function goSaveOrUpdatePurchaseOrderLineItem(id, title) {
		if (id != null && id != '' && id != 'undefined') {
			openWindowForShow('${nvix}/nvixnt/purchase/vixntPurchaseOrderAction!goSaveOrUpdatePurchaseOrderLineItem.action?id=' + id, title, 850, 675);
		} else {
			if ($("#purchaseOrderForm").validationEngine('validate')) {
				$("#purchaseOrderForm").ajaxSubmit({
				type : "post",
				...
				success : function(purchaseOrderId) {
					...
					openWindowForShow('${nvix}/nvixnt/purchase/vixntPurchaseOrderAction!goSaveOrUpdatePurchaseOrderLineItem.action?id=' + id + "&purchaseOrderId=" + purchaseOrderId, title, 850, 675);
				}
				});
			} else {
				return false;

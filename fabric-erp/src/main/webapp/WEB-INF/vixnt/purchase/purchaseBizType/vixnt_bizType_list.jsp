	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-shopping-cart"></i> 采购管理<span>> 业务类型 </span>
			</h1>
		</div>
			...
			<div class="widget-body no-padding">
				<div>
					<form role="search" class="navbar-form navbar-left">
						...
						<button onclick="bizTypeTable.ajax.reload();" type="button" class="btn btn-info">
							<i class="glyphicon glyphicon-search"></i> 检索
						</button>
						<button onclick="$('#searchName').val('');bizTypeTable.ajax.reload();" type="button" class="btn btn-default">
							<i class="glyphicon glyphicon-repeat"></i> 清空
						</button>
					</form>
				</div>
				<table id="bizTypeTableId" class="table table-striped table-bordered table-hover" width="100%"></table>
			</div>
		</div>
	</div>
</div>
<script type="text/javascript">
	...
	var bizTypeTable = initDataTable("bizTypeTableId", "${nvix}/nvixnt/purchase/vixntPurchaseBizTypeAction!goSingleList.action", bizTypeColumns, function(page, pageSize, orderField, orderBy) {
	...
	function saveOrUpdate(id, title) {
		openSaveOrUpdateWindow('${nvix}/nvixnt/purchase/vixntPurchaseBizTypeAction!goSaveOrUpdate.action?id=' + id, "bizTypeForm", title, 850, 350, bizTypeTable);
	};

	function deleteById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/purchase/vixntPurchaseBizTypeAction!deleteById.action?id=' + id, '是否删除会员等级?', bizTypeTable);
	};
</script>
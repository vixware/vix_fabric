...
<fieldset>
	...
	<div class="col-sm-5" style="text-align: left; padding: 0px 0px;">
		<form id="cardEntityDetailForm" class="form-horizontal" style="padding: 5px 5px;">
			<div class="jarviswidget">
				...
				<div>
					<div class="widget-body">
						<fieldset>
							<input type="hidden" id="cardEntityDetailId" name="cardEntityDetail.id" value="${cardEntityDetail.id }" /> 
							<input type="hidden" id="cardEntityId" name="cardEntityDetail.cardEntity.id" value="${cardEntityDetail.cardEntity.id }" />
							<input type="hidden" id="itemId" name="cardEntityDetail.item.id" value="${cardEntityDetail.item.id }" />
							<legend></legend>
							<div class="form-group">
								<label class="col-md-3 control-label"><span class="text-danger">*</span>编码:</label>
								<div class="col-md-9">
									<input id="itemcode" name="cardEntityDetail.item.code" value="${cardEntityDetail.item.code}" data-prompt-position="topLeft" class="form-control validate[required]" type="text" />
								</div>
							</div>
							<div class="form-group">
								<label class="col-md-3 control-label"><span class="text-danger">*</span>名称:</label>
								<div class="col-md-9">
									<input id="itemname" name="cardEntityDetail.item.name" value="${cardEntityDetail.item.name}" data-prompt-position="topLeft" class="form-control validate[required]" type="text" />
								</div>
							</div>
							<div class="form-group">
								<label class="col-md-3 control-label"><span class="text-danger">*</span>次数:</label>
								<div class="col-md-6">
									<s:if test="null == cardEntityDetail">
										<input id="num" name="cardEntityDetail.num" value="1" data-prompt-position="topLeft" class="form-control validate[required]" type="text" />
									</s:if>
									<s:else>
										<input id="num" name="cardEntityDetail.num" value="${cardEntityDetail.num}" data-prompt-position="topLeft" class="form-control validate[required]" type="text" />
									</s:else>
								</div>
							</div>
							<legend></legend>
...
<script type="text/javascript">
	...
	function updateStoredValueRuleSetDetail() {
		if ($('#cardEntityDetailForm').validationEngine('validate')) {
			/** 打开遮盖层 */
			var loadIndex = layer.load(2);
			$("#cardEntityDetailForm").ajaxSubmit({
			type : "post",
			url : "${nvix}/nvixnt/cardAction!updateCardEntityDetail.action?id="+$("#cardEntityId").val(),

	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa fa-lg fa-fw fa-user"></i>人事事务管理<span>> 离退休</span>
			</h1>
		</div>
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				...
				<button class="btn btn-default" type="button" onclick="loadContent('','${nvix}/nvixnt/hr/nvixHrManageAction!goPrepareRetire.action');">
	...
	<div class="jarviswidget">
		...
		<div class="widget-body">
			<form class="form-horizontal" id="regularForm">
			    ...
			    <input type="hidden" id="types" name="regular.types" value="retire" />
				<fieldset>
					...
					<div class="form-group">
						<label class="col-md-2 control-label"><span class="text-danger">*</span>离退休类型:</label>
						<div class="col-md-3">
							<select id="becomeFullMemberState" name="regular.becomeFullMemberState" class="form-control validate[required]">
							    <option value="">请选择</option>
								<option value="1" <c:if test='${regular.becomeFullMemberState == "1"}'>selected="selected"</c:if>>离休</option>
								<option value="2" <c:if test='${regular.becomeFullMemberState == "2"}'>selected="selected"</c:if>>退休</option>
							</select>
						</div>
					</div>
					<div class="form-group">
					...
					<div class="form-group">
						<label class="col-md-2 control-label"><span class="text-danger">*</span>入司日期:</label>
						<div class="col-md-3">
							<div class="input-group">
								<input id="infoOrgDate" name="regular.infoOrgDate" value="${regular.infoOrgTimeStr}" data-prompt-position="topLeft" class="form-control validate[custom[date]]" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" type="text" /> 
								<span class="input-group-addon" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',el:'leaveStartTimeStr'});">
								<i class="fa fa-calendar"></i></span>
							</div>
						</div>
						<label class="col-md-2 control-label"><span class="text-danger">*</span>任职日期:</label>
						<div class="col-md-3">
							<div class="input-group">
								<input id="employmentDate" name="regular.employmentDate" value="${regular.employmentTimeStr}" data-prompt-position="topLeft" class="form-control validate[custom[date]]" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" type="text" /> 
					...
					<div class="form-group">
						<label class="col-md-2 control-label">离退休原因:</label>
						<div class="col-md-8">
...
<script type="text/javascript">
	...
	function saveOrUpdate() {
		if ($("#regularForm").validationEngine('validate')) {
			...
			dataType : "text",
			success : function(id) {
				loadContent('', '${nvix}/nvixnt/hr/nvixHrManageAction!goPrepareRetire.action');
			}
			});
		} else {
			return false;

			...
			<article style="width: 260px; float: left; margin-right: 20px">
				<div class="tree-box">
					...
					<script type="text/javascript">
						...
						async : {
						enable : true,
						url : "${nvix}/nvixnt/hr/nvixntPostAction!findOrgAndUnitTreeToJson.action",
							...
							<div>
								<form role="search" class="navbar-form navbar-left">
									<div class="form-group">
										岗位名称: <input type="text" value="" class="form-control" id="posName">
									</div>
...
<script type="text/javascript">
	...
	var organizationUnitTable = initDataTable("organizationUnitTableId", "${nvix}/nvixnt/hr/nvixntPostAction!goSingleList.action", organizationUnitColumns, function(page, pageSize, orderField, orderBy) {
	...
	function goSaveOrUpdate(id, title) {
		if (id != '') {
			openSaveOrUpdateWindow('${nvix}/nvixnt/hr/nvixntPostAction!goSaveOrUpdate.action?id=' + id, "organizationUnitForm", title, 750, 450, organizationUnitTable, null, function() {
			...
			if ($('#selectId').val().substring($('#selectId').val().length - 1, $('#selectId').val().length) == 'O') {
				openSaveOrUpdateWindow('${nvix}/nvixnt/hr/nvixntPostAction!goSaveOrUpdate.action?id=' + id + '&treeId=' + $('#selectId').val(), "organizationUnitForm", title, 750, 450, organizationUnitTable, null, function() {
					var treeNode = zTree.getNodeByTId($("#selectIdTreeId").val());
					if (null != treeNode) {
						treeNode.isParent = true;
					}
	...
	function deleteEntityById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/hr/nvixntPostAction!deleteEntityById.action?id=' + id, "是否删除该岗位!", organizationUnitTable, "删除岗位");
	};
	...
	function createDefaultBvRel() {
		...
		$.ajax({
		url : "${nvix}/nvixnt/hr/nvixntPostAction!checkBizViewHasDefaultRelation.action",
		cache : false,
		async : false,
		dataType : "json",
		...
		if (confirm(confirmMsg)) {
			$.post('${nvix}/nvixnt/hr/nvixntPostAction!createBizViewHasDefaultRelation.action', "", function(result) {
				showMessage(result);
				setTimeout("", 1000);
			});
		}

	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa fa-table fa-fw "></i>客户关系管理 <span>>客户管理  </span><span>>客户公海 </span>
			</h1>
		</div>
		<div class="col-lg-8 text-align-right">
			<div class="col-xs-12 col-sm-6 col-md-6 col-lg-12 text-align-right">
				<div class="btn-group">
					<button class="btn btn-primary" type="button" onclick="receiveAll();">
						<i class="glyphicon glyphicon-user"></i>&nbsp;分配客户
					</button>
				</div>
			</div>
		</div>
									...
									<form id="advanceSearchForm" class="form-horizontal">
										<fieldset>
											...
											<div class="form-group">
												...
												<div class="col-md-4"> 
													<input type="text" value="" id="employee" placeholder="所有者" class="form-control"/>
												</div>
														...
														<th width="5%">
															<div class="btn-group">
																...
																<ul class="dropdown-menu js-status-update pull-left">
																	<li><a id="all" onclick="changeDataTableSelect('customerAccount','all');" href="javascript:void(0);">全选</a></li>
																	<li><a id="reverse" onclick="changeDataTableSelect('customerAccount','reverse');" href="javascript:void(0);">反选</a></li>
																	<li><a id="cancle" onclick="changeDataTableSelect('customerAccount','cancle');" href="javascript:void(0);">取消</a></li>
																</ul>
															</div>
														</th>
														<th>客户编码</th>
...
<script type="text/javascript">
	...
	var customerTable = initDataTable("customerAccount", "${nvix}/nvixnt/nvixCustomerAction!goCustomerListContentJson.action", customerColumns, function(page, pageSize, orderField, orderBy) {
	...
	function receiveAll(){
		...
		if (emp != '' && emp.length == 2 && emp[0] != null && emp[0] != '') {
			chooseListData('${nvix}/nvixnt/nvixntUserAccountAction!goEmployeeChoose.action', 'single', '选择人员', 'employee',function(){
				var employeeId = $("#employeeId").val();
				if(!employeeId){
					layer.alert("请选择销售员");
					...
					$.ajax({
						type : "post",
						url : "${nvix}/nvixnt/nvixCustomerAction!receiveAll.action",
						data : {
							"ids" : emp[0],
							"employeeId" : employeeId
						},
	...
	function view(id,syncTag){
		$.ajax({
			url : '${nvix}/nvixnt/nvixCustomerAction!view.action?id=' + id + "&customerAccountType=" + syncTag + "&source=view",
			cache : false,

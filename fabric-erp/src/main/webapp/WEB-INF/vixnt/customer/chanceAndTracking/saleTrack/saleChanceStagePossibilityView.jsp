	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa fa-lg fa-fw fa-user"></i> 客户关系管理 <span>> 销售跟踪 </span><span>> 机会可能性/阶段统计 </span>
			</h1>
			<input type="hidden" value="ThisQuarterOT" id="queryTime">
			<input type="hidden" value="findSaleChanceStagePossibility" id="queryType">  
		</div>
	</div>
					...
					<div class="row">
						<div class="col-lg-12">
							<ul class="list-group no-margin chooseSort">    
								...
								<li class="list-group-item"> <span class="btn bg-color-red txt-color-white badge pull-right"><i class="fa fa-times"></i></span><span id='alreadySltSupplier'>机会可能性/阶段统计</span></li>
							</ul>
						</div>
					</div>
				</div>
				<div class="well padding-10">
					...
					<script type="text/javascript">
						var testdataBsSuggest = $("#sltSupplier").bsSuggest({
						     ...
						     data:{
						    	 "value" : [
									{"id" : "findSaleChanceStagePossibility","word" : "机会可能性/阶段统计"},
						   		]
						     }
						 });
						 /**选择供应商时**/
...
function queryLegendArrToSaleStage() {
	$.ajax({
		url : "${nvix}/nvixnt/nvixSaleChanceStatisticsAction!queryLegendArrToSaleStage.action",
		type: "POST",
		dataType: "json",
		success : function(json){
			...
			for(var i=0 ;i<legendArrName.length;i++){
				$("#viewABC").append("<div id='viewABC"+i+"'style='height: 170px'></div>");
				stateInt = 2;
			}
...
function queryMaxNum() {
	...
	if(stateInt ==2){
		$.ajax({
			...
			dataType: "json",
			async:false,
			data: {queryTime:$('#queryTime').val(),type:'findMaxNum'},
			success : function(json) {
				num = json.maxNum;
				if( num < 1 ){
					num = 1;
...
var qCondition = new queryCondition("ThisQuarterOT", "findSaleChanceStagePossibility");//不能删除，后面用
/** 点击时间按钮 **/

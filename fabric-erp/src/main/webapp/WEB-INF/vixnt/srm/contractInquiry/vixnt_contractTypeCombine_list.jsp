	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-book"></i> 合同管理 <span>> 合同类型集合 </span>
			</h1>
		</div>
			...
			<div class="widget-body no-padding">
				<div>
					<form role="search" class="navbar-form navbar-left">
						...
						<button onclick="contractTypeCombineTable.ajax.reload();" type="button" class="btn btn-info">
							<i class="glyphicon glyphicon-search"></i> 检索
						</button>
						<button onclick="$('#searchName').val('');contractTypeCombineTable.ajax.reload();" type="button" class="btn btn-default">
...
<script type="text/javascript">
	var contractTypeCombineColumns = [ {
	"orderable" : false,
	"title" : "编号",
	"width" : "10%",
	"data" : function(data) {
	...
	var contractTypeCombineTable = initDataTable("areaLevelTableId", "${nvix}/nvixnt/contract/vixntContractTypeCombineAction!goSingleList.action", contractTypeCombineColumns, function(page, pageSize, orderField, orderBy) {
	...
	function saveOrUpdate(id, title) {
		openSaveOrUpdateWindow('${nvix}/nvixnt/contract/vixntContractTypeCombineAction!goSaveOrUpdate.action?id=' + id, "contractTypeCombineForm", title, 750, 375, contractTypeCombineTable);
	};
	function deleteById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/contract/vixntContractTypeCombineAction!deleteById.action?id=' + id, '是否删除会员标签?', contractTypeCombineTable);
	};
	pageSetUp();
</SCRIPT>
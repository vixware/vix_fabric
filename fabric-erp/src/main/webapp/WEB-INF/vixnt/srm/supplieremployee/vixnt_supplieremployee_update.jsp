	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-truck"></i> 供应商管理 <span>> 员工管理 </span>
			</h1>
		</div>
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				...
				<button class="btn btn-default" type="button" onclick="loadContent('','${nvix}/nvixnt/vixntSupplierEmployeeAction!goList.action');">
	...
	<div class="jarviswidget">
		...
		<div class="widget-body">
			<form class="form-horizontal" id="employeeForm">
				<input type="hidden" id="employeeId" name="emp.id" value="${emp.id}" /> <input type="hidden" id="supplierId" name="emp.supplier.id" value="${emp.supplier.id}" />
				<fieldset>
					<legend>基本信息:</legend>
					<div class="form-group">
						...
						<div class="col-md-3">
							<select id="userType" name="emp.userType" class="form-control">
								<option value="1" <c:if test='${emp.userType eq 1}'>selected="selected"</c:if>>员工</option>
							</select>
						</div>
...
<script type="text/javascript">
	...
	function saveOrUpdate() {
		if ($("#employeeForm").validationEngine('validate')) {
			...
			type : "post",
			url : "${nvix}/nvixnt/vixntSupplierEmployeeAction!saveOrUpdate.action",
			dataType : "text",
			success : function(id) {
				loadContent('', '${nvix}/nvixnt/vixntSupplierEmployeeAction!goList.action');
			}
			});
		} else {
			return false;
	...
	function goChooseRole() {
		chooseListData('${nvix}/nvixnt/nvixntRoleAction!goChooseSupplierRole.action', 'single', '选择角色', 'role');
	};

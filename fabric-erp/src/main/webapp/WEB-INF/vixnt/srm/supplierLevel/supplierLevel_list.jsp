	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-truck"></i> 供应商管理<span>> 供应商等级 </span>
			</h1>
		</div>
			...
			<div class="widget-body no-padding">
				<div>
					<form role="search" class="navbar-form navbar-left">
						...
						<button onclick="supplierLevelTable.ajax.reload();" type="button" class="btn btn-info">
							<i class="glyphicon glyphicon-search"></i> 检索
						</button>
						<button onclick="$('#searchName').val('');supplierLevelTable.ajax.reload();" type="button" class="btn btn-default">
							<i class="glyphicon glyphicon-repeat"></i> 清空
						</button>
					</form>
				</div>
				<table id="supplierLevelTableId" class="table table-striped table-bordered table-hover" width="100%"></table>
			</div>
		</div>
	</div>
</div>
<script type="text/javascript">
	var supplierLevelColumns = [ {
	"orderable" : false,
	"title" : "编号",
	"width" : "10%",
	"data" : function(data) {
	...
	var supplierLevelTable = initDataTable("supplierLevelTableId", "${nvix}/nvixnt/vixntSupplierLevelAction!goSingleList.action", supplierLevelColumns, function(page, pageSize, orderField, orderBy) {
	...
	function saveOrUpdate(id, title) {
		openSaveOrUpdateWindow('${nvix}/nvixnt/vixntSupplierLevelAction!goSaveOrUpdate.action?id=' + id, "supplierLevelForm", title, 750, 375, supplierLevelTable);
	};

	function deleteById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/vixntSupplierLevelAction!deleteById.action?id=' + id, '是否删除会员等级?', supplierLevelTable);
	};
</script>
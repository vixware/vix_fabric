	...
	<div class="row">
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-truck"></i> 供应商管理<span>> 供应商建档 </span>
			</h1>
		</div>
			...
			<div class="page-title">
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu pull-right">
						<li><a href="javascript:void(0);" onclick="chooseSupplier();">筛选待建档供应商</a></li>
					</ul>
				</div>
			</div>
		</div>
...
<script type="text/javascript">
	...
	"orderable" : false,
	"data" : function(data) {
		var update = "<a href='javascript:void(0);' class='btn btn-xs btn-default' onclick=\"goSaveOrUpdate('" + data.id + "');\" title='建档'><span class='txt-color-blue pull-right'><i class='fa fa-pencil'></i></span></a>";
		var assess = "<a href='javascript:void(0);' class='btn btn-xs btn-default' onclick=\"assess('" + data.id + "');\" title='提交评估'><span class='txt-color-blue pull-right'><i class='fa fa-upload'></i></span></a>";
		var del = "<a href='javascript:void(0);' class='btn btn-xs btn-default' onclick=\"deleteById('" + data.id + "');\" title='删除'><span class='txt-color-red pull-right'><i class='fa fa-times'></i></span></a>";
		return update + "&nbsp;&nbsp;"+ assess +"&nbsp;&nbsp;"  + del;
	}
	} ];
	...
	function deleteById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/vixntSupplierAction!deleteById.action?id=' + id, '是否删除供应商?', supplierTable);
	};
	function goSaveOrUpdate(id) {
		$.ajax({
		url : '${nvix}/nvixnt/vixntSupplierAction!goSaveOrUpdateBuildingSupplier.action?id=' + id,
		cache : false,
	...
	function chooseSupplier(){
		chooseListData('${nvix}/nvixnt/vixntSupplierAction!goChooseSupplier.action', 'single', '选择供应商', 'supplier',function(){
			$.ajax({
				url : '${nvix}/nvixnt/vixntSupplierAction!goSaveOrUpdateBuildingSupplier.action?id=' + $("#supplierId").val(),
				cache : false,
				success : function(html) {
					$("#mainContent").html(html);
				}

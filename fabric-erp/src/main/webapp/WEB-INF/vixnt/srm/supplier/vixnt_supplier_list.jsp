	...
	<div class="row">
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-6">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-truck"></i> 供应商管理<span>> 供应商清单 </span>
			</h1>
		</div>
...
var supplierColumns = [ {
	...
	"data" : function(data) {
		...
		var employee = "<a href='javascript:void(0);' class='btn btn-xs btn-default' onclick=\"goSaveOrUpdateEmployee('" + data.id + "');\" title='添加管理员'><span class='txt-color-blue pull-right'><i class='fa fa-user'></i></span></a>";
		return update + "&nbsp;&nbsp;" + employee + "&nbsp;&nbsp;" + del;
	}
	} ];

	var supplierTable = initDataTable("supplierTableId", "${nvix}/nvixnt/vixntSupplierAction!goSingleList.action", supplierColumns, function(page, pageSize, orderField, orderBy) {
	...
	function goSaveOrUpdate(id) {
		$.ajax({
		url : '${nvix}/nvixnt/vixntSupplierAction!goSaveOrUpdate.action?id=' + id,
		cache : false,
	...
	function goSaveOrUpdateEmployee(id) {
		$.ajax({
		url : '${nvix}/nvixnt/vixntSupplierAction!goSaveOrUpdateEmployee.action?parentId=' + id,
		cache : false,
		success : function(html) {
			$("#mainContent").html(html);
		}

	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-cube"></i> 物料管理 <span>&gt; 新增物料</span>
			</h1>
		</div>
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				...
				<button class="btn btn-default" type="button" onclick="loadContent('', '${nvix}/nvixnt/mdm/nvixntMaterialAction!goList.action');">
			...
			<form class="form-horizontal" id="itemsForm">
				...
				<fieldset>
					...
					<div class="form-group">
						...
						<div class="col-md-3">
							<s:if test="item.itemType != null && item.itemType != '' && item.itemType != ''">
								<input id="itemType" type="hidden" name="item.itemType" value="${item.itemType}"/>
								<s:if test="item.itemType=='material'">
									<input type="text" readonly="readonly" value="原材料" class="form-control"/>
								</s:if>
								<s:if test="item.itemType=='purchasepart'">
									<input type="text" readonly="readonly" value="外部零件" class="form-control"/>
								</s:if>
								<s:if test="item.itemType=='purchasebackuppart'">
									<input type="text" readonly="readonly" value="外购备件" class="form-control"/>
								</s:if>
								<s:if test="item.itemType=='machiningpart'">
									<input type="text" readonly="readonly" value="加工部件" class="form-control"/>
								</s:if>
								<s:if test="item.itemType=='semifinished'">
									<input type="text" readonly="readonly" value="半成品" class="form-control"/>
								</s:if>
								<s:if test="item.itemType=='assemblypart'">
									<input type="text" readonly="readonly"  value="组装件" class="form-control"/>
								</s:if>
								<s:if test="item.itemType=='finishedgoods'">
									<input type="text" readonly="readonly"  value="产成品" class="form-control"/>
								</s:if>
								<s:if test="item.itemType=='equipment'">
									<input type="text" readonly="readonly" value="设备" class="form-control"/>
								</s:if>
							</s:if>
							<s:else>
								<select id="itemType" name="item.itemType" onchange="promotionTypeChange();" data-prompt-position="topLeft" class="form-control validate[required]" >
									<option value="">----请选择----</option>
									<s:iterator value="promotionTypeMap.entrySet().iterator()" var="key">
										<option value="<s:property value='key'/>" <s:if test="item.itemType == key">selected="selected"</s:if>><s:text name="getText(value)"/></option>
									</s:iterator>
								</select>
							</s:else>
						</div>
					...
					<div class="form-group">
					<label class="col-md-2 control-label">物料描述:</label>
						<div class="col-md-8">
					...
					<div class="form-group">
						...
						<div class="col-md-3">
							<div class="row">
								<div class="col-sm-12">
									<div class="input-group">
										<input id="invShelfId" name="itemPurchaseProperties.invShelf.id" value="${itemPurchaseProperties.invShelf.id}" type="hidden" /> <input id="invShelfName" type="text" value="${itemPurchaseProperties.invShelf.name}" data-prompt-position="topLeft" class="form-control" onfocus="chooseShelf('itemPurchase');" readonly="readonly" />
										<div class="input-group-btn">
								...
								<div class="col-sm-12">
									<div class="input-group">
										<input id="defaultInvShelfId" name="itemInventoryProperties.defaultInvShelf.id" value="${itemInventoryProperties.defaultInvShelf.id}" type="hidden" /> <input id="defaultInvShelfName" type="text" value="${itemInventoryProperties.defaultInvShelf.name}" data-prompt-position="topLeft" class="form-control"
											onfocus="chooseShelf('itemInventory');" readonly="readonly" />
										<div class="input-group-btn">
				...
				<div class="form-actions" style="margin-top: 15px;">
					<div class="row">
						<div class="col-md-12">
							...
							<button type="button" class="btn btn-default" onclick="loadContent('', '${nvix}/nvixnt/mdm/nvixntMaterialAction!goList.action?syncTag=' +$('#type').val());">返回</button>
						</div>
					</div>
				</div>
			</form>
...
<script type="text/javascript">
	...
	/* 	} */
		if ($('#itemsForm').validationEngine('validate')) {
			...
			type : "post",
			url : "${nvix}/nvixnt/mdm/nvixntMaterialAction!saveOrUpdate.action",
			contentType : "application/x-www-form-urlencoded; charset=utf-8",
			success : function(result) {
				var r = result.split(":");
				showMessage(r[1]);
				loadContent('mdm_items', '${nvix}/nvixnt/mdm/nvixntMaterialAction!goList.action');
			}
			});
		}
	}
	...
	function chooseItemCatagory() {
		...
		chooseListData('${nvix}/nvixnt/mdm/nvixntMaterialAction!goChooseMultiItemCatalog.action', 'multi', '选择分类', null, function() {
	...
	function checkItemCode() {
		$.ajax({
		url : '${nvix}/nvixnt/mdm/nvixntMaterialAction!checkItemCode.action?itemCode=' + $('#itemCode').val(),
		cache : false,
		success : function(result) {
			if (result != '') {
				layer.alert(result);
	...
	//加载库位
	function chooseShelf(tag) {
		var wareHouseId = '';
		if (tag == 'itemPurchase') {
			wareHouseId = $("#recieveWarehouseId").val();
		}
		if (tag == 'itemInventory') {
			wareHouseId = $("#defaultWarehouseId").val();
		}
		if (wareHouseId == '') {
			layer.alert("请选仓库!");
		} else {
			chooseListData('${nvix}/nvixnt/vixntTakeStockAction!goChooseShelf.action?wareHouseId=' + wareHouseId, 'single', '选择货位', tag == 'itemPurchase' ? 'invShelf' : 'defaultInvShelf');
		}
	}
	//加载主计量单位
	function loadItemMeasureUnit() {
		...
		if (mug != "") {
			$.ajax({
			url : "${nvix}/nvixnt/mdm/nvixntMaterialAction!loadMeasureUnit.action?type=itemMeasureUnit&id=" + $("#id").val() + "&measureUnitGroupId=" + mug,
			cache : false,
			...
			$.ajax({
			url : "${nvix}/nvixnt/mdm/nvixntMaterialAction!loadMeasureUnit.action?type=itemAssitMeasureUnit&id=" + $("#id").val() + "&measureUnitGroupId=" + mug,
			cache : false,
			success : function(html) {
	...
	function loadPurchaseMeasureUnit() {
		...
		if (mug != "") {
			$.ajax({
			url : "${nvix}/nvixnt/mdm/nvixntMaterialAction!loadMeasureUnit.action?type=purBaseUnit&id=" + $("#id").val() + "&measureUnitGroupId=" + mug,
			cache : false,
			success : function(html) {
			...
			$.ajax({
			url : "${nvix}/nvixnt/mdm/nvixntMaterialAction!loadMeasureUnit.action?type=purAssitUnit&id=" + $("#id").val() + "&measureUnitGroupId=" + mug,
			cache : false,
			success : function(html) {
			...
			$.ajax({
			url : "${nvix}/nvixnt/mdm/nvixntMaterialAction!loadMeasureUnit.action?type=poUnit&id=" + $("#id").val() + "&measureUnitGroupId=" + mug,
			cache : false,
			success : function(html) {
				$("#poUnitId").html(html);
			}
	...
	specificationDataTableColumns.push({"title":"操作","width":"5%","orderable" :false,"data":function(data){
  	...
  	var specificationDataTable = initDataTableNoPaging("specificationDataTable","${nvix}/nvixnt/mdm/nvixntMaterialAction!loadSpecificationDetailJson.action",specificationDataTableColumns,function(orderField,orderBy){
		var id = $("#id").val();
		var sku = $("#skuCode").val();
	...
	/** 添加规格组 */
  	function addSpecificationGroup(id,productCategoryId,specificationDetailIds){
		...
		var data = specificationDataTable.rows().data();
  		...
  		layer.confirm("是否根据选中的规格生成规格明细?", {title:"提示信息"}, function(index){
			...
			var loadIndex = layer.load(2);
			$.ajax({
				url:'${nvix}/nvixnt/mdm/nvixntMaterialAction!addSpecificationGroup.action?specificationDetailIds='+specificationDetailIds+"&productCategoryId="+ productCategoryId +"&id="+id+"&specData="+specData,
				cache: false,
  	...
  	function deleteSpecificationGroup(id){
  		layer.confirm("是否删除规则明细?", {title:"提示信息"}, function(index){
  			...
  			var loadIndex = layer.load(2);
  			$.ajax({
  				url:'${nvix}/nvixnt/mdm/nvixntMaterialAction!deleteSpecificationGroup.action?sgId='+id+'&ecProductId='+$("#id").val()+'&productCategoryId='+$("#itemCatalogIds").val(),
  				cache: false,
  				success: function(json){
  					/** 关闭遮盖层 */
  					layer.close(loadIndex);
  	...
  	function changeSpecCellData(input,index,id){
  		if(index == 1){
  			/** 校验sku */
  			$.ajax({
  				url:'${nvix}/nvixnt/mdm/nvixntMaterialAction!checkSkuIsExist.action?ecProductId=' + $("#id").val() + '&sku='+ $(input).val(),
  				cache: false,
  				success: function(result){
  	...
  	function saveOrUpdateEcProductImage(id){
		...
		if(ecProductId == ""){
				...
				$("#itemsForm").ajaxSubmit({
					...
					success : function(result) {
						...
						if(r[0] == '0'){
							...
							$("#id").val(r[0]);
							showMessage(r[1]);
							openSaveOrUpdateWindow('${nvix}/nvixnt/mdm/nvixntMaterialAction!goSaveOrUpdateImage.action?itemImageId='+id+'&id='+r[0],"itemImageForm","商品图片",480,340,ecProductImageDataTable);
						}
			...
			openSaveOrUpdateWindow('${nvix}/nvixnt/mdm/nvixntMaterialAction!goSaveOrUpdateImage.action?itemImageId='+id+'&id='+ecProductId,"itemImageForm","商品图片",480,340,ecProductImageDataTable);
		} 	
	}
                    	
function deleteImageById(id){
deleteEntityByConfirm('${nvix}/nvixnt/mdm/nvixntMaterialAction!deleteImageById.action?id='+id,'是否删除商品图片?',ecProductImageDataTable);
}
function fileToChange(){
var id = $("#id").val();
if(id){
	uploadFileOrImage("itemFileUploadForm","${nvix}/nvixnt/mdm/nvixntMaterialAction!uploadItemImage.action?id="+id,"itemImageFileToUpload","image",function(data){
		var d = data.split(":");
		if($.trim(d[0]) == '0'){
			layer.alert(d[1]);
		}else{
		...
		$("#itemsForm").ajaxSubmit({
			...
			success : function(result) {
				...
				if(r[0] == '0'){
					...
					$("#id").val(r[0]);
					showMessage(r[1]);
					uploadFileOrImage("itemFileUploadForm","${nvix}/nvixnt/mdm/nvixntMaterialAction!uploadItemImage.action?id="+r[0],"itemImageFileToUpload","image",function(data){
						var d = data.split(":");
						if($.trim(d[0]) == '0'){
							layer.alert(d[1]);
						}else{
	...
	var ecProductImageColumns = [
	                       	...
	                       	var ecProductImageDataTable = initDataTableNoPaging("ecProductImageDataTable","${nvix}/nvixnt/mdm/nvixntMaterialAction!getItemImageList.action",ecProductImageColumns,function(orderField,orderBy){
	                    		var id = $("#id").val();

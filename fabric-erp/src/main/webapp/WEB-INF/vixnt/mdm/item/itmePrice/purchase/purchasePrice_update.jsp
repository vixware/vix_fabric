...
<div id="content">
	<div class="row">
		...
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				...
				<button class="btn btn-default" type="button" onclick="loadContent('mdm_salePrice','${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!goList.action');">
	...
	<div class="jarviswidget">
		...
		<div class="widget-body">
			<form id="salePriceForm" class="form-horizontal" action="${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!saveOrUpdate.action">
				<input type="hidden" id="id" name="priceCondition.id" value="${priceCondition.id}" /> 
				<fieldset>
					<div class="form-group">
						<label class="col-md-2 control-label"><span class="text-danger">*</span>采购组织:</label>
						<div class="col-md-3">
							<div class="row">
								<div class="col-sm-12">
												...
												<div class="tab-pane no-padding active" id="priceTab">
													<div id="itemPriceSearchForm" style="margin:5px;">
														<div class="form-group" style="margin: 0 5px;">
															...
															<button onclick="$('#priceSearchName').val('');priceTable.ajax.reload();" type="button" class="btn btn-default">
				...
				<div class="form-actions" style="margin-top: 15px;">
					<div class="row">
						<div class="col-md-12">
							...
							<button type="button" class="btn btn-default" onclick="loadContent('mdm_salePrice','${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!goList.action');"><i class="fa fa-rotate-left"></i>返回</button>
						</div>
					</div>
				</div>
			</form>
...
<script type="text/javascript">
	var priceColumns = [
		...
		{"orderable" : false,"title" : "条件类型","width" : "10%","data" : function(data){return data.conditionType;}}, 
		/* {"title":"条件类型","width":"7%","name":"conditionType","data" : function(data) {
		...
		{"orderable" : false,"title":"采购区域","width":"10%","data" : function(data) {return data.regionalName;}},
		...
		{"orderable" : false,"title":"供应商","width":"10%","data" : function(data) {return data.supplierName;}},
		{"orderable" : false,"title":"商品名称","width":"10%","data" : function(data) {return data.itemName;}},
	...
	var priceTable = initDataTable("priceTable","${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!goPriceAreaList.action",priceColumns,function(page,pageSize,orderField,orderBy){
	...
	var countPriceColumns = [
   	...
   	var countPriceTable = initDataTable("countTable","${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!goCountAreaList.action",countPriceColumns,function(page,pageSize,orderField,orderBy){
		...
		if($('#salePriceForm').validationEngine('validate')){
			...
			if(!tag){
				...
				$("#salePriceForm").ajaxSubmit({
					type: "post",
					url:"${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!saveOrUpdate.action",
					contentType: "application/x-www-form-urlencoded; charset=utf-8", 
					success : function(result) {
						showMessage(result);
						loadContent('mdm_salePrice','${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!goList.action');
					}
				});
			}
		}
	...
	function saveOrUpdatePriceConditionDetail(id,type){
		...
		if(priceConditionId){
			...
			if(!tag){
				...
				if(type == "price"){
					openSaveOrUpdateWindow('${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!goSaveOrUpdatePriceConditionPriceArea.action?priceConditionId='+priceConditionId+'&id='+id,'priceConditionPriceAreaForm','价格区间',720,480,priceTable,function(){
					...
					openSaveOrUpdateWindow('${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!goSaveOrUpdatePriceConditionCountArea.action?priceConditionId='+priceConditionId+'&id='+id,'priceConditionCountAreaForm','数量区间',720,480,countPriceTable,function(){
			...
			if ($("#salePriceForm").validationEngine('validate')) {
				...
				if(!tag){
					...
					$("#salePriceForm").ajaxSubmit({
						type : "post",
						url : "${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!saveOrUpdateInner.action",
						dataType : "text",
						success : function(data) {
							...
							if(d[1]){
								$("#id").val(d[1]);
								if(type == "price"){
									openSaveOrUpdateWindow('${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!goSaveOrUpdatePriceConditionPriceArea.action?priceConditionId='+d[1]+'&id='+id,'priceConditionPriceAreaForm','价格区间',720,480,priceTable,function(){
										var startTotalAmount = $("#startTotalAmount").val();
										var endTotalAmount = $("#endTotalAmount").val();
										if(Number(endTotalAmount) <= Number(startTotalAmount)){
											layer.alert("结束价格不能小于开始价格!");
									...
									openSaveOrUpdateWindow('${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!goSaveOrUpdatePriceConditionCountArea.action?priceConditionId='+d[1]+'&id='+id,'priceConditionCountAreaForm','数量区间',720,480,countPriceTable,function(){
										var startCount = $("#startCount").val();
										var endCount = $("#endCount").val();
										if(Number(endCount) <= Number(startCount)){
											layer.alert("结束数量不能小于开始数量!");
	...
	function deletePriceConditionEntity(id,type) {
		if(type == 'price'){
			deleteEntityByConfirm('${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!deletePriceConditionPriceArea.action?id=' + id, '是否删除该价格区间?', priceTable);
		}else if(type == 'count'){
			deleteEntityByConfirm('${nvix}/nvixnt/mdm/nvixntPurchasePriceAction!deletePriceConditionCountArea.action?id=' + id, '是否删除该数量区间?', countPriceTable);
		}
	};
	/** 页面加载完调用 */
	pageSetUp();

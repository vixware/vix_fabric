	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-cube"></i> 商品管理 <span>>商品列表 </span>
			</h1>
		</div>
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				...
				<button class="btn btn-primary" type="button" onclick="goSaveOrUpdateService('');">
					<i class="glyphicon glyphicon-plus"></i>&nbsp; 新增服务
				</button>
				<div class="btn-group">
					...
					<ul class="dropdown-menu pull-right">
						<li><a href="javascript:void(0);" onclick="uploadItem()">同步商品</a></li>
						<li><a href="javascript:void(0);" onclick="$('#fileToUpload').click()">导入商品</a></li>
						<li><a href="javascript:void(0);" onclick="exportExcel()">导出商品</a></li>
	...
	<section id="" class="">
		<div style="overflow: hidden; position: relative; padding: 0;">
			<article style="width: 260px; float: left; margin-right: 20px">
				<div class="tree-box">
					<h2>商品分类</h2>
					<ul id="tree" class="ztree" style="height: 485px; overflow-x: hidden; overflow-y: auto;"></ul>
...
<script type="text/javascript">
	...
	var itemsTable = initDataTable("itemTable", "${nvix}/nvixnt/mdm/nvixntItemAction!getItemJson.action", itemsColumns, function(page, pageSize, orderField, orderBy) {
	...
	function editSchedul(id) {
		openSaveOrUpdateWindow('${nvix}/nvixnt/mdm/nvixntItemAction!goSaveOrUpdate.action?id=' + id, 'itemsForm', '新增', 720, 420, itemsTable, null, function() {
			itemsTable.ajax.reload();
		});
	}
	...
	function saveOrUpdate(id) {
		...
		if (id) {
			goSaveOrUpdateEntity('${nvix}/nvixnt/mdm/nvixntItemAction!goSaveOrUpdate.action?id=' + id);
		} else {
			if (categoryId) {
	...
	function goSaveOrUpdateService(id) {
		var categoryId = $("#selectId").val();
		goSaveOrUpdateEntity('${nvix}/nvixnt/mdm/nvixntItemAction!goSaveOrUpdateService.action?id=' + id + '&categoryId=' + categoryId);
	}
	...
	function importXlsFile() {
		$.ajaxFileUpload({
		url : '${nvix}/nvixnt/mdm/nvixntItemAction!importFile.action',// 用于文件上传的服务器端请求地址
	...
	function downloadTemplate() {
		var url = "${nvix}/nvixnt/mdm/nvixntItemAction!downloadTemplate.action";
		window.open(url);
	};
	function exportExcel() {
		form = document.getElementById("exportMD");
		form.action = "${nvix}/nvixnt/mdm/nvixntItemAction!exportItemExcel.action";
		form.submit();
	};
	function uploadItem() {
		...
		if (items != '' && items.length == 2) {
			...
			$.ajax({
			url : '${nvix}/nvixnt/mdm/nvixntItemAction!uploadItem.action?itemIds=' + items[0],
			cache : false,
			success : function(result) {
				var r = result.split(':');
				if (r[0] == "0") {
					showMessage(r[1]);
					//loadContent('mdm_item', '${nvix}/nvixnt/mdm/nvixntItemAction!goList.action');
				} else {
					layer.alert(r[1]);
				}
			}
	...
	function uploadItemById(id) {
		if (id) {
			$.ajax({
			url : '${nvix}/nvixnt/mdm/nvixntItemAction!uploadItem.action?id=' + id,
			cache : false,
			success : function(result) {
				var r = result.split(':');
				if (r[0] == "0") {

				...
				<div>
					<div class="widget-body">
						<fieldset style="height: 375px;">
							<input type="hidden" id="saleOrderItemId" name="requireGoodsOrderItem.id" value="${requireGoodsOrderItem.id }" /> <input type="hidden" id="saleOrderItemSalesOrderId" name="id" value="${requireGoodsOrderItem.requireGoodsOrder.id }" />
							...
							<div class="form-group">
								<label class="col-md-4 control-label"><span class="text-danger">*</span>采购价格:</label>
								<div class="col-md-8">
									<div class="input-group">
										<span class="input-group-addon">￥</span> <input id="price" name="requireGoodsOrderItem.price" value="${requireGoodsOrderItem.price}" data-prompt-position="topLeft" class="form-control validate[required,custom[number]]" readonly="readonly" type="text" />
									</div>
								</div>
							</div>
							<div class="form-group">
...
<script type="text/javascript">
	...
	var chooseItemTable = initDataTable("chooseEcProduct", "${nvix}/nvixnt/vixntStoreRequireGoodsAction!goChooseStoreItemList.action", chooseEcProductColumns, function(page, pageSize, orderField, orderBy) {
	...
	/** 初始化分类下拉列表树 */
	var showMenu = initDropListTree("treeMenu", "${nvix}/nvixnt/vixntStoreRequireGoodsAction!findTreeToJson.action", "chooseProductCategoryId", "chooseProductCategoryName", "chooseProductCategoryZtree", "menuContent");
	...
	function saveOrUpdateSaleOrderItem() {
		if ($('#saleOrderItemForm').validationEngine('validate')) {
			...
			type : "post",
			url : "${nvix}/nvixnt/vixntStoreRequireGoodsAction!saveOrUpdateSaleOrderItem.action",
			...
			success : function(result) {
				...
				$.ajax({
				url : '${nvix}/nvixnt/vixntStoreRequireGoodsAction!getStockRecordTotal.action?id=' + $("#saleOrderItemSalesOrderId").val(),
				cache : false,

					...
					<div class="form-group">
						...
						<div class="col-md-3">
							...
							<%-- <input id="clipExpiryDate" name="customerAccountClip.expiryDate" value="${customerAccountClip.expiryDate}" class="form-control validate[required]" type="text" /><span class="input-group-addon"><i class="fa">年</i></span> --%>
								<input id="dateTime" name="" value="" data-prompt-position="topLeft" class="form-control validate[custom[date]]" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})" type="text" /> <span class="input-group-addon" onclick="WdatePicker({dateFmt:'yyyy-MM-dd',el:'birthday'});"><i
	...
	var itemTableColumns = [ {
		...
		function saveOrUpdate(){
			if ($("#memberTagForm").validationEngine('validate')) {
				$("#memberTagForm").ajaxSubmit({
				type : "post",
				url : "${nvix}/nvixnt/nvixCustomerAccountClipAction!goOnCard.action?dateTime="+$("#dateTime").val()+"&customerAccountClipId="+$("#id").val(),
				dataType : "text",

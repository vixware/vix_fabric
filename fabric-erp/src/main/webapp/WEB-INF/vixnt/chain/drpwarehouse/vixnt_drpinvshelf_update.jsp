...
<form id="invShelfForm" class="form-horizontal" style="padding: 40px 40px;" action="${nvix}/nvixnt/vixntDrpWarehouseAction!saveOrUpdateInvShelf.action">
	...
	<fieldset>
		...
		<div class="form-group">
			...
			<div class="col-md-4">
				<div data-toggle="buttons" class="btn-group">
					...
					<label class="btn btn-default <s:if test="invShelf.isDefault == 0">active</s:if>"> <input type="radio" id="" name="invShelf.isDefault" data-prompt-position="topLeft" class="validate[required]" <s:if test="invShelf.isDefault == 0"> checked="checked"</s:if> value="0" />是
					</label>
				</div>
			</div>
		</div>
		...
		<div class="form-group">
			...
			<div class="col-md-4">
				<input id="shelfLength" name="invShelf.shelfLength" value="${invShelf.shelfLength}" class="form-control" type="text" />
			</div>
			<label class="col-md-2 control-label">宽:</label>
			<div class="col-md-4">
				<input id="shelfWidth" name="invShelf.shelfWidth" value="${invShelf.shelfWidth}" class="form-control" type="text" />
			</div>
		</div>
		<div class="form-group">
			<label class="col-md-2 control-label">高:</label>
			<div class="col-md-4">
				<input id="shelfHeight" name="invShelf.shelfHeight" value="${invShelf.shelfHeight}" class="form-control" type="text" />
			</div>
		</div>
	</fieldset>
</form>
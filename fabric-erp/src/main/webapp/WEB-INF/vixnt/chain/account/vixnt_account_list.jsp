			...
			<article style="width: 260px; float: left; margin-right: 20px">
				<div class="tree-box">
					...
					<script type="text/javascript">
						...
						async : {
						enable : true,
						url : "${nvix}/nvixnt/vixntAccountManagementAction!findOrgAndUnitTreeToJson.action",
...
<script type="text/javascript">
	...
	var userAccountTable = initDataTable("userAccountTableId", "${nvix}/nvixnt/vixntAccountManagementAction!goSingleList.action", userAccountColumns, function(page, pageSize, orderField, orderBy) {
	...
	function deleteShelf(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/vixntAccountManagementAction!deleteById.action?id=' + id, '是否删除账号?', userAccountTable);
	};
	$("#userAccountForm").validationEngine();
	function goSaveOrUpdate(id, title) {
		if (id != null && id != '') {
			openSaveOrUpdateWindow('${nvix}/nvixnt/vixntAccountManagementAction!goSaveOrUpdate.action?id=' + id, "userAccountForm", title, 900, 475, userAccountTable);
		} else {
			if ($("#selectId").val() != null && $("#selectId").val() != '' && $("#selectTreeType").val() == 'E') {
				openSaveOrUpdateWindow('${nvix}/nvixnt/vixntAccountManagementAction!goSaveOrUpdate.action?parentId=' + $('#selectId').val() + "&id=" + id, "userAccountForm", title, 900, 475, userAccountTable);
			} else {
				layer.alert("请选择员工!");
			}
		}

			...
			<div class="row">
				<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
					<h1 class="page-title txt-color-blueDark">
						<i class="fa fa-lg fa-fw fa-bar-chart-o"></i> 销售智能分析 <span>&gt; 销售人员业绩排行</span>
					</h1>
					...
					<div class="col-sm-9">
										...
										<div class="row">
											...
											<div class="col-sm-4 col-md-4">
												<div id="wid-id-4" class="jarviswidget jarviswidget-sortable" data-widget-editbutton="false" role="widget">
													<header>
														<span class="widget-icon"> <i class="fa fa-bar-chart-o"></i></span>   
														<h2>销售人员业绩:销售订单数量Top10</h2>
													</header>
													<div>
														<div class="widget-body no-padding">
															<div id="dataViewDnum" style="height: 350px"></div>
														</div>
													</div>
												</div>
											</div>
							...
							<div class="jarviswidget" data-widget-editbutton="false" role="widget"> 
								<header>
									...
									<h2>销售人员业绩:销售订单金额Top10  相关销售人员信息</h2>
									<a href="javascript:void(0);" class="btn bg-color-blueLight mytxt-color-wathet  txt-color-white btn-sm" id=""  onclick="exportExcel('groupArrOrderMoney');" style="float:right;padding:2px 10px 2px;margin:4px 10px 3px 5px;" >导出</a>
								</header>
								<div>
									<div class="widget-body no-padding">
							...
							<div class="jarviswidget" data-widget-editbutton="false" role="widget"> 
								<header>
									<span class="widget-icon"> <i class="fa fa-table"></i></span>
									<h2>销售人员业绩:销售产品数量Top10  相关销售人员信息</h2>
									<a href="javascript:void(0);" class="btn bg-color-blueLight mytxt-color-wathet  txt-color-white btn-sm" id=""  onclick="exportExcel('groupArrProductNum');" style="float:right;padding:2px 10px 2px;margin:4px 10px 3px 5px;" >导出</a>
								</header>
								<div>
									<div class="widget-body no-padding">
							...
							<div class="jarviswidget" data-widget-editbutton="false" role="widget"> 
								<header>
									<span class="widget-icon"> <i class="fa fa-table"></i></span>
									<h2>销售人员业绩:销售订单数量Top10  相关销售人员信息</h2>
									<a href="javascript:void(0);" class="btn bg-color-blueLight mytxt-color-wathet  txt-color-white btn-sm" id=""  onclick="exportExcel('groupArrOrderNum');" style="float:right;padding:2px 10px 2px;margin:4px 10px 3px 5px;" >导出</a>
								</header>
								<div>
									<div class="widget-body no-padding">
										<table id="ordersTableId" class="table table-striped table-bordered table-hover" width="100%"></table>
									</div>
								</div>
							</div>
...
<script type="text/javascript">
/** groupArrOrderMoney  groupArrProductNum... 查询 相关销售人员信息 table用的对象 控制3个table参数 **/
var groupArrOrderMoney = "";
var groupArrProductNum = "";
var groupArrOrderNum = "";
/** 把页面查询条件封装成js对象 **/
...
function myNumArrToMillionYuan(objArr) {
	...
	function queryTopMoney() {
		$.ajax({    
			url: '${nvix}/nvixnt/nvixntSalesAnalysisAction!querySalesmanSellTopView.action', 
			type: "POST",
			data: {queryTime:$('#queryTime').val(),controlSQL:'SalesmanSellTopmoney',regionalID:$('#queryRegional').val(),customerAccountID:$('#queryCustomer').val(),topNum:'10'},
		    dataType: "json",
		    ...
		    $(window).resize(myChart.resize);
		    	 groupArrOrderMoney = json.groupArr;
				 if(groupArrOrderMoney.length > 0){
...
function queryTopNum() {
		$.ajax({    
			url: '${nvix}/nvixnt/nvixntSalesAnalysisAction!querySalesmanSellTopView.action', 
			type: "POST",
			data: {queryTime:$('#queryTime').val(),controlSQL:'SalesmanSellTopnum',regionalID:$('#queryRegional').val(),customerAccountID:$('#queryCustomer').val(),topNum:'10'},
		    dataType: "json",
			success:function(json){
				...
				myChart.setOption(option);
			    $(window).resize(myChart.resize);
			     groupArrProductNum = json.groupArr;
				 if(groupArrProductNum.length > 0){
...
<script type="text/javascript">
function queryTopNumOrders() {
		$.ajax({    
			url: '${nvix}/nvixnt/nvixntSalesAnalysisAction!querySalesmanSellTopView.action', 
			type: "POST",
			data: {queryTime:$('#queryTime').val(),controlSQL:'SalesmanSellTopOrdersnum',regionalID:$('#queryRegional').val(),customerAccountID:$('#queryCustomer').val(),topNum:'10'},
		    dataType: "json",
			success:function(json){
			var myColor = '#ED5350';//控制bar柱条颜色和提示框小圆圈颜色  
			var myChart = echarts.init(document.getElementById('dataViewDnum'));
			var	option = {
	   	    title: {
	   	    	left: 'left',
	   	        text: '',
			...
			myChart.setOption(option);
		    $(window).resize(myChart.resize);
		    groupArrOrderNum = json.groupArr;
			 if(groupArrOrderNum.length > 0){
...
"width" : "20%",
"data" : function(data) {
	return (data.telephone != null ? data.telephone : "" );
}
},{
"title" : "工号",
"width" : "20%",
"data" : function(data) {     
return (data.staffjobnumber != null ? data.staffjobnumber : "" );
}
}];         
	var customerAccountTable = initDataTable("customerAccountTableId", "${nvix}/nvixnt/nvixntSalesAnalysisAction!queryStaffTable.action", customerAccountColumns, function(page, pageSize, orderField, orderBy) {
		var varGroupArrOrderMoney = Url.encode(JSON.stringify(groupArrOrderMoney));
		return {
		"page" : page,
		"groupArrOrderMoney" : varGroupArrOrderMoney,
		"pageSize" : pageSize
		};
	}, 10, '0');
</script>
...
return (data.staffjobnumber != null ? data.staffjobnumber : "" );
	...
	var goodsTable = initDataTable("goodsTableId", "${nvix}/nvixnt/nvixntSalesAnalysisAction!queryStaffTable.action", goodsColumns, function(page, pageSize, orderField, orderBy) {
		 var varGroupArrProductNum = Url.encode(JSON.stringify(groupArrProductNum));
			return {
			"page" : page,
			"groupArrProductNum" : varGroupArrProductNum,
			"pageSize" : pageSize
			};
	}, 10, '0');
</script>
...
return (data.staffjobnumber != null ? data.staffjobnumber : "" );
	...
	var ordersTable = initDataTable("ordersTableId", "${nvix}/nvixnt/nvixntSalesAnalysisAction!queryStaffTable.action", ordersColumns, function(page, pageSize, orderField, orderBy) {
		var varGroupArrOrderNum = Url.encode(JSON.stringify(groupArrOrderNum));
		return {
		"page" : page,
		"groupArrOrderNum" : varGroupArrOrderNum,
		"pageSize" : pageSize
		};
}, 10, '0');
</script>
...
<script type="text/javascript">
/** 导出列表 ***/
function exportExcel(what) {
	form = document.getElementById("exportMD");
	var queryWhat = "";
	if(what=='groupArrOrderMoney'){
		queryWhat = Url.encode(JSON.stringify(groupArrOrderMoney));
	}else if(what=='groupArrProductNum'){
		queryWhat = Url.encode(JSON.stringify(groupArrProductNum));
	}else if(what=='groupArrOrderNum'){
		queryWhat = Url.encode(JSON.stringify(groupArrOrderNum));
	}
	form.action = "${nvix}/nvixnt/nvixntSalesAnalysisAction!outExcelToStaffTable.action?queryWhat="+queryWhat;
	form.submit();
}
</script>
...
<fieldset>
	<div class="col-sm-8" style="text-align: left; padding: 5px 5px;">
		<div id="saleReturnFormItemDiv" class="jarviswidget jarviswidget-color-white">
			<header>
	...
	<div class="col-sm-4" style="text-align: left; padding: 0px 0px;">
		<form id="saleReturnFormItemForm" class="form-horizontal" style="padding:5px 5px;" action="">
			<div class="jarviswidget">
				...
				<div>
					<div class="widget-body">
						<fieldset>
							<input type="hidden" id="oiId" name="saleReturnFormItem.id" value="${saleReturnFormItem.id}"/>
							<input type="hidden" id="id" name="saleReturnFormItem.saleReturnForm.id" value="${saleReturnFormItem.saleReturnForm.id}"/>
							<input type="hidden" id="itemId" name="saleReturnFormItem.item.id" value="${saleReturnFormItem.item.id}"/>
							<input type="hidden" id="tax" name="saleReturnFormItem.tax" value="${saleReturnFormItem.tax}"/>
							<div class="form-group">
								<label class="col-md-4 control-label"><span class="text-danger">*</span>编码:</label>
								<div class="col-md-8">
									<input id="itemCode" name="saleReturnFormItem.item.code" value="${saleReturnFormItem.item.code}" data-prompt-position="topLeft" class="form-control validate[required]" type="text"/>
								</div>
							</div>
							<div class="form-group">
								<label class="col-md-4 control-label">名称:</label>
								<div class="col-md-8">
									<input id="itemName" name="saleReturnFormItem.item.name" value="${saleReturnFormItem.item.name}" class="form-control" type="text"/>
								</div>
							</div>
							<div class="form-group">
								<label class="col-md-4 control-label"><span class="text-danger">*</span>数量:</label>
								<div class="col-md-8">
									<s:if test="saleReturnFormItem != null && saleReturnFormItem.count != null">
										<input id="soiAmount" name="saleReturnFormItem.count" value="${saleReturnFormItem.count}" data-prompt-position="topLeft" class="form-control validate[required]" type="text"/>
									</s:if>
									<s:else>
										<input id="soiAmount" name="saleReturnFormItem.count" value="1" data-prompt-position="topLeft" class="form-control validate[required]" type="text"/>
									</s:else>
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-md-8">
									<input id="specification" name="saleReturnFormItem.specification" value="${saleReturnFormItem.specification}" class="form-control" readonly="readonly" type="text"/>
								</div>
							</div>
							<div class="form-group">
								...
								<div class="col-md-8">
									<div class="input-group">
										...
										<input id="price" name="saleReturnFormItem.price" value="${saleReturnFormItem.price}"  data-prompt-position="topLeft" class="form-control validate[required,custom[number]]" type="text"/>
									</div>
								</div>
							</div>
							<div style="text-align: right;">
								<button type="button" class="btn btn-success" onclick="mergeSaleOrderItem();">
									<s:if test="saleReturnFormItem.id == ''||saleReturnFormItem.id == null">
...
<script type="text/javascript" >
	...
	function mergeSaleOrderItem(){
		if($('#saleReturnFormItemForm').validationEngine('validate')){
			...
			//入库单主体ID
			$("#id").val($("#saleReturnFormId").val());
			$("#saleReturnFormItemForm").ajaxSubmit({
				type: "post",
				url:"${nvix}/nvixnt/nvixntSaleReturnFormAction!saveOrUpdateSaleReturnFormItem.action",

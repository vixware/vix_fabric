	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa fa-lg fa-fw fa-bar-chart-o"></i> 销售管理<span>&gt; 销售退货单</span>
			</h1>
		</div>
			...
			<div class="widget-body no-padding">
				<div>
					<form role="search" class="navbar-form navbar-left">
						...
						<button onclick="saleReturnFormTable.ajax.reload();" type="button" class="btn btn-info">
						...
						<button onclick="clearSearchCondition('mytitle,code,customerId,customerName,createTime',saleReturnFormTable);" type="button" class="btn btn-default">
						...
						<form id="advanceSearchForm" class="form-horizontal">
							<fieldset>
								...
								<div class="form-group" style="text-align:center;">
									...
									<button onclick="clearSearchCondition('mytitle,code,customerId,customerName,createTime',saleReturnFormTable);"
				...
				<table id="saleReturnFormTableId" class="table table-striped table-bordered table-hover" width="100%"></table>
			</div>
		</div>
	</div>
</div>
<script type="text/javascript">
	var saleReturnFormColumns = [ {
	"title" : "编号",
	"width" : "10%",
	"orderable" : false,
	"data" : function(data) {
	...
	var saleReturnFormTable = initDataTable("saleReturnFormTableId", "${nvix}/nvixnt/nvixntSaleReturnFormAction!goSingleList.action", saleReturnFormColumns, function(page, pageSize, orderField, orderBy) {
	...
	function goSaveOrUpdate(id) {
		$.ajax({
		url : '${nvix}/nvixnt/nvixntSaleReturnFormAction!goSaveOrUpdate.action?id=' + id,
		cache : false,
	...
	function show(id) {
		$.ajax({
		url : '${nvix}/nvixnt/nvixntSaleReturnFormAction!show.action?id=' + id,
		cache : false,
		success : function(html) {
			$("#mainContent").html(html);
		}
	...
	function deleteById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/nvixntSaleReturnFormAction!deleteById.action?id=' + id, '是否删除销售出库单?', saleReturnFormTable);
	};
	
	function chooseSalesOrder(){
		chooseListData('${nvix}/nvixnt/nvixntSaleReturnFormAction!goChooseSalesOrder.action', 'single', '选择销售订单', 'saleOrder',function(){
			$.ajax({
				url : '${nvix}/nvixnt/nvixntSaleReturnFormAction!createSalesOutBoundBySalesOrder.action?id=' + $("#saleOrderId").val(),
				cache : false,
				success : function(result) {
					var r = result.split(":");
					if(r[0] == "1"){
						showMessage(r[1]);
						loadContent('sale_outBound','${nvix}/nvixnt/nvixntSaleReturnFormAction!goList.action');
					}else{
						showMessage(r[1]);
						$.ajax({
							url : '${nvix}/nvixnt/nvixntSaleReturnFormAction!goSaveOrUpdate.action?id=' + r[2],
							cache : false,
							success : function(html) {
								$("#mainContent").html(html);
							}
	...
	function advanceSearch(){
		saleReturnFormTable.ajax.reload();
		layer.closeAll('page');
	}
</script>
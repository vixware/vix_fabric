	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-shopping-cart"></i> 销售管理<span>> 单据类型 </span>
			</h1>
		</div>
			...
			<div class="widget-body no-padding">
				<div>
					<form role="search" class="navbar-form navbar-left">
						...
						<button onclick="$('#searchName').val('');orderTypeTable.ajax.reload();" type="button" class="btn btn-default">
							<i class="glyphicon glyphicon-repeat"></i> 清空
						</button>
					</form>
				</div>
				<table id="orderTypeTableId" class="table table-striped table-bordered table-hover" width="100%"></table>
			</div>
		</div>
	</div>
</div>
<script type="text/javascript">
	...
	var orderTypeTable = initDataTable("orderTypeTableId", "${nvix}/nvixnt/nvixntSaleOrderTypeAction!goSingleList.action", orderTypeColumns, function(page, pageSize, orderField, orderBy) {
	...
	function saveOrUpdate(id, title) {
		openSaveOrUpdateWindow('${nvix}/nvixnt/nvixntSaleOrderTypeAction!goSaveOrUpdate.action?id=' + id, "orderTypeForm", title, 850, 350, orderTypeTable);
	};

	function deleteById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/nvixntSaleOrderTypeAction!deleteById.action?id=' + id, '是否删除单据类型?', orderTypeTable);
	};
</script>
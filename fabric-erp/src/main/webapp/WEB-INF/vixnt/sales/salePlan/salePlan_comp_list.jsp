...
<div id="content">
	...
	<section id="" class="">
		<div style="overflow: hidden; position: relative; padding: 0;">
			<article style="width: 130px; float: left; margin-right: 20px">
				<div class="tree-box">
					<h2>组织架构</h2>
				...
				<div class="jarviswidget">
					<header>
						<ul class="nav nav-tabs pull-left in" id="">
							<li class="active"><a data-toggle="tab" href="#hr1"><span class="hidden-mobile hidden-tablet">待汇总/金额</span></a></li>
							<li><a data-toggle="tab" href="#hr2"><span class="hidden-mobile hidden-tablet">已汇总/数量</span></a></li>
						</ul>
						...
						<div class="col-md-3" style="float:right;padding-right:0">
							<div class="input-group">
								<input id="planDate" value="" class="form-control" onClick="WdatePicker({dateFmt:'yyyy'})" type="text"/>
...
<script type="text/javascript">
	...
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.jan* data.item.price);
	}
	}, {
	"title":"02月",
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.feb* data.item.price);
	}
	}, {
	"title":"03月",
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.mar* data.item.price);
	}
	}, {
		"title":"04月",
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.apr* data.item.price);
	}
	}, {
	"title":"05月",
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.may* data.item.price);
	}
	}, {
	"title":"06月",
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.jun* data.item.price);
	}
	}, {
	"title":"07月",
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.jul* data.item.price);
	}
	}, {
	"title":"08月",
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.aug* data.item.price);
	}
	}, {
	"title":"09月",
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.sep * data.item.price);
	}
	}, {
	"title":"10月",
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.oct* data.item.price);
	}
	}, {
	"title":"11月",
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.nov * data.item.price);
	}
	}, {
	"title":"12月",
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.december* data.item.price);
	}
	}, {
	"title":"总计",
	"width" : "4%",
	"data" : function(data) {
		return "￥"+(data.totalAmount* data.item.price);
	}
	}, {
	"orderable" : false,
	"title":"操作",
	...
	var salePlanTable = initDataTable("salePlanTableId", "${nvix}/nvixnt/nvixntSalePlanAction!goSingleListCollect.action", salePlanColumns, function(page, pageSize, orderField, orderBy) {
		var treeId = $("#selectId").val();
		treeId = Url.encode(treeId);
		var treeType = $("#selectTreeType").val();
		treeType = Url.encode(treeType);

...
<script type="text/javascript">
	...
	var choosePurchasePlanColumns = [ {
		"data" : function(data) {
			if (chooseType == 'multi') {
				return "<input id='entityId' name='entityId' value='" + data.id + "' type='checkBox' onchange=\"checkBoxChange('${entityName}',this,'" + data.id + "','" + data.quoteSubject + "');\"/>";
			}
			if (chooseType == 'single') {
				return "<input id='entityId' name='entityId' value='" + data.id + "' type='radio' onchange=\"radioChange('${entityName}','" + data.id + "','" + data.quoteSubject + "');\"/>";
			}
			return "";
		}
	}, {
	...
	var choosePurchasePlanTable = initDataTable("choosePurchasePlanTableId", "${nvix}/nvixnt/nvixSalesQuotationAction!goListContentJson.action", choosePurchasePlanColumns, function(page, pageSize, orderField, orderBy) {

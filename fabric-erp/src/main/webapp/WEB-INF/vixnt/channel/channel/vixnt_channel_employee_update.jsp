	...
	<div class="row">
		<div class="col-xs-12 col-sm-7 col-md-7 col-lg-4">
			<h1 class="page-title txt-color-blueDark">
				<i class="fa-fw fa fa-chain"></i> 渠道管理 <span>> 新增管理员 </span>
			</h1>
		</div>
		<div class="col-xs-12 col-sm-6 col-md-6 col-lg-8 text-align-right">
			<div class="page-title">
				...
				<button class="btn btn-default" type="button" onclick="loadContent('drp_storeperson','${nvix}/nvixnt/channel/vixntChannelAction!goList.action');">
		...
		<div class="widget-body">
			<form class="form-horizontal" id="employeeForm">
				...
				<fieldset>
					...
					<div class="form-group">
						...
						<div class="col-md-3">
							<select id="userType" name="emp.userType" class="form-control">
								<option value="0" <c:if test='${emp.userType eq 0}'>selected="selected"</c:if>>店长</option>
								<option value="1" <c:if test='${emp.userType eq 1}'>selected="selected"</c:if>>店员</option>
							</select>
						</div> --%>
					...
					<legend>账号信息:</legend>
					<input type="hidden" id="id" name="entityForm.id" value="${entity.id}" /> <input type="hidden" id="roleId" name="addUserAccountRoleIds" value="${addUserAccountRoleIds }" /> <input type="hidden" id="parentId" name="parentId" value="${parentId}" /> <input type="hidden" id="deleteUserAccountRoleIds" name="deleteUserAccountRoleIds" value="" />
					<div class="form-group">
...
<script type="text/javascript">
	...
	function saveOrUpdate() {
		if ($("#employeeForm").validationEngine('validate')) {
			...
			type : "post",
			url : "${nvix}/nvixnt/channel/vixntChannelAction!saveOrUpdateEmployee.action",
			dataType : "text",
			success : function(id) {
				loadContent('drp_storeperson', '${nvix}/nvixnt/channel/vixntChannelAction!goList.action');
			}
			});
		} else {
			return false;

			...
			<form class="form-horizontal" id="salesOrderForm">
				<fieldset>
					...
					<div class="form-group">
						...
						<div class="col-md-3">
							<div class="input-group">
								<input type="hidden" id="channelDistributorId" name="requireGoodsOrder.channelDistributor.id" value="${requireGoodsOrder.channelDistributor.id}" /> 
								<input id="channelDistributorName" name="requireGoodsOrder.channelDistributor.name" value="${requireGoodsOrder.channelDistributor.name}" onfocus="chooseChannelDistributor();" data-prompt-position="topLeft" class="form-control validate[required]" type="text" readonly="readonly" /> 
					...
					<div class="form-group">
						...
						<div class="col-md-3">
							<input class="form-control" id="creator" name="requireGoodsOrder.creator" value="${requireGoodsOrder.creator }" type="text" readonly="readonly">
						</div>
						...
						<div class="col-md-3">
							<div class="row">
								<div class="col-sm-12">
									<div class="input-group">
										<input class="form-control validate[custom[number]]" id="requireGoodsOrderAmount" name="requireGoodsOrder.amount" value="${requireGoodsOrder.amount }" type="text" readonly="readonly"> <span class="input-group-addon">(¥)</span>
									</div>
								</div>
							</div>
						</div>
...
<script type="text/javascript">
	...
	var saleOrderItemTable = initDataTable("saleOrderItemTableId", "${nvix}/nvixnt/channel/vixntChannelOrderAction!goSaleOrderItemList.action", stockRecordLinesColumns, function(page, pageSize, orderField, orderBy) {
	...
	function goChooseChannelDistributor() {
		chooseListData('${nvix}/nvixnt/channel/vixntChannelOrderAction!goChooseChannelDistributor.action', 'single', '选择门店', 'channelDistributor');
	};
	...
	function saveOrUpdate() {
		if ($("#salesOrderForm").validationEngine('validate')) {
			...
			if(!tag){
				layer.alert("结束配送时间不能小于开始配送时间!");
				return tag;
			}else{
				$("#salesOrderForm").ajaxSubmit({
					type : "post",
					url : "${nvix}/nvixnt/channel/vixntChannelOrderAction!saveOrUpdate.action",
					dataType : "text",
					success : function(id) {
	...
	function goSaveOrUpdateSaleOrderItem(id, title) {
		...
		if (id != null && id != "") {
			openWindowForShow('${nvix}/nvixnt/channel/vixntChannelOrderAction!goSaveOrUpdateSaleOrderItem.action?id=' + id + "&channelId=" + channelId, title, 850, 675);
		} else {
			if ($("#salesOrderForm").validationEngine('validate')) {
				var startTime = $("#startSendTime").val();
				var endTime = $("#endSendTime").val();
	...
	function deleteById(id) {
		deleteEntityByConfirm('${nvix}/nvixnt/channel/vixntChannelOrderAction!deleteSaleOrderItemById.action?id=' + id, '是否删除订单明细?', saleOrderItemTable,"删除订单明细",function(){
			$.ajax({
				url : '${nvix}/nvixnt/channel/vixntChannelOrderAction!getStockRecordTotal.action?id=' + $("#requireGoodsOrderId").val(),
				cache : false,

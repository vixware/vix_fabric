...
<script type="text/javascript">
	function goSaveOrUpdate(id) {
		window.location = "${vix}/wechat/weChatTaskPlanAction!goMyTask.action?id=" + id;
	};
	function deleteById() {
		var ids = '';
	...
	function searchByName() {
		loadContent('${vix}/wechat/weChatTaskPlanAction!goMyTaskList.action?searchname=' + $('#searchname').val());
	};
</script>
<style type="text/css">
#searchId {
		...
		<div class="subJob">
			<s:if test="vixTaskList.size > 0">
				<s:iterator value="vixTaskList" var="entity" status="s">
					<dl>
						<dt>
							<span name="chkId" value="${entity.id}"></span><a onclick="goSaveOrUpdate('${entity.id}');" style="cursor: hand"> <strong> ${fn:substring(entity.questName, 0,20)}<b> ${fn:substring(entity.taskDescription, 0,20)}</b>
							</strong></a>
						</dt>
						<dd>

	...
	<form theme="simple" enctype="multipart/form-data" id="vixTaskForm">
		...
		<section class="newMeet newCla new">
			<div class="newMeet_list2">
				<dl>
					<dt>
						<span>类型</span> <strong> <select id="type" name="vixTask.type">
								<option value="1" <c:if test='${vixTask.type eq 1}'>selected="selected"</c:if>>临时任务</option>
								<option value="2" <c:if test='${vixTask.type eq 2}'>selected="selected"</c:if>>紧急任务</option>
								<option value="3" <c:if test='${vixTask.type eq 3}'>selected="selected"</c:if>>日清任务</option>
								<option value="4" <c:if test='${vixTask.type eq 4}'>selected="selected"</c:if>>周期任务</option>
								<option value="5" <c:if test='${vixTask.type eq 5}'>selected="selected"</c:if>>里程碑</option>
								<option value="6" <c:if test='${vixTask.type eq 6}'>selected="selected"</c:if>>虚拟任务</option>
						</select>
						</strong>
					</dt>
				</dl>

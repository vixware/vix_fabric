...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/marketingCampaignAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteEntitys() {
	...
	$.ajax({
		url : '${vix}/drp/marketingCampaignAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/drp/marketingCampaignAction!goSingleList.action?1=1", 'marketingCampaign');
			});
		}
	});
}
...
function searchForContent(){
	loadName();
	pager("start","${vix}/drp/marketingCampaignAction!goSingleList.action?name="+name,'marketingCampaign');
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/drp/marketingCampaignAction!goSingleList.action?name=",'marketingCampaign');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/drp/marketingCampaignAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'marketingCampaign');
}
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/marketingCampaignAction!goSingleList.action?name="+name,'marketingCampaign');
}
function saveOrUpdateMarketingCampaign(id){
	$.ajax({
		  url:'${vix}/drp/marketingCampaignAction!goMarketingCampaign.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
					...
					callback : function(action){
						if(action == 'ok'){
							...
							$.post('${vix}/drp/marketingCampaignAction!saveOrUpdateMarketingCampaign.action',
									 {
									  'marketingCampaign.id':$("#marketingCampaignId").val(),
...
function listbystatus(status) {
	pager("start", "${vix}/drp/marketingCampaignAction!goSingleList.action?status=" + status, 'marketingCampaign');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/drp/marketingCampaignAction!goSingleList.action?days=" + days, 'marketingCampaign');
}
</script>
<div class="sub_menu">
	<h2>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div>
			<label>搜索内容:<input type="text" class="int" id="nameS" placeholder="主题"></label><label><input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent(0);" /><input type="button" value="<s:text name='cmn_reset'/>" class="btn" name="" onclick="resetForContent(0);" /></label>
		</div>
	</div>
	<div id="number">
		<span class="num_left_bg"></span> <span class="num_right_bg"></span>
		<ul id="numBox" class="numBox">
			<s:iterator value="marketingCampaignList" var="c">

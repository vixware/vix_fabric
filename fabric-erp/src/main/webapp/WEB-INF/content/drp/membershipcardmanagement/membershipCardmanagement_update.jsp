...
<script type="text/javascript">
	...
	function saveOrUpdateMemberShipCard() {
		if ($('#memberShipCardForm').validationEngine('validate')) {
			$.post('${vix}/drp/membershipCardmanagementAction!saveOrUpdate.action', {
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id="customerAccountName" name="customerAccountName" value="${memberShipCard.customerAccount.name }" size="30" type="text" onchange="fieldChanged(this);" class="validate[required] text-input" /><input type="hidden" id="customerAccountId" name="customerAccountId" value="${memberShipCard.customerAccount.id}" /> <input
												class="btn" type="button" value="选择" onclick="chooseCustomerAccount();" /><span style="color: red;">*</span></td>
											<td align="right">所属门店：</td>
											<td><input id="channelDistributorName" name="channelDistributorName" value="${memberShipCard.channelDistributor.name }" type="text" size="30" class="validate[required] text-input" onchange="fieldChanged(this);" /><input type="hidden" id="channelDistributorId" name="channelDistributorId"
												value="${memberShipCard.channelDistributor.id}" /> <input class="btn" type="button" value="选择" onclick="chooseParentOrganization();" /><span style="color: red;">*</span></td>
										</tr>
										<tr>
											<td align="right">会员卡号：</td>
											<td><input id="vipcardid" name="vipcardid" value="${memberShipCard.vipcardid }" type="text" size="30" class="validate[required] text-input" onchange="fieldChanged(this);" /><span style="color: red;">*</span></td>
											<td align="right">发卡日期：</td>
											<td><input id="opencarddate" name="opencarddate" value="${memberShipCard.opencarddate}" type="text" size="30" readonly="readonly" onchange="fieldChanged(this);" /><img onclick="showTime('opencarddate','yyyy-MM-dd HH:mm:ss')" src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"></td>
										</tr>
										<tr>
											<td align="right">会员卡类型：</td>
											<td><s:select list="cardTypeList" listKey="vipTypeId" listValue="vipTypeName" theme="simple" id="viptypeid" name="viptypeid" onchange="fieldChanged(this);"></s:select></td>
											<td align="right">余额：</td>
											<td><input id="balance_amount" name="balance_amount" value="${memberShipCard.balance_amount }" type="text" size="30" class="validate[required] text-input" onchange="fieldChanged(this);" /><span style="color: red;">*</span></td>
										</tr>
										<tr>
											...
											<td><input id="start_date" name="start_date" value="${memberShipCard.start_date}" type="text" size="30" readonly="readonly" onchange="fieldChanged(this);" /><img onclick="showTime('start_date','yyyy-MM-dd HH:mm:ss')" src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"></td>
										</tr>
										<tr>
											<td align="right">密码：</td>
											<td><input id="password" name="password" value="${memberShipCard.password }" type="password" size="30" onchange="fieldChanged(this);" /></td>
										...
										<tr>
											...
											<td colspan="3"><textarea id="remark" name="remark" onchange="fieldChanged(this);">${memberShipCard.remark }</textarea> <script type="text/javascript" src="${vix}/plugin/KindEditor/kindeditor-min.js"></script> <script type="text/javascript">

...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/consignmentOrderAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/consignmentOrderAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/drp/consignmentOrderAction!goSingleList.action?name="+name,'consignmentOrder');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/consignmentOrderAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'consignmentOrder');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/consignmentOrderAction!goSingleList.action?name="+name,'consignmentOrder');
}
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div class="search_advanced">
			<label><s:text name="drp_client_name1" /><input type="text" class="int" name=""> </label><label><s:text name="drp_time1" /><input type="text" class="int" name=""> </label>
		</div>
	</div>
	<div id="number">
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="consignmentOrderInfo"></b> <s:text name='cmn_to' /> <b class="consignmentOrderTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a> </span> <span><a class="end" onclick="currentPager('end');"></a> </span>
					</div>
				</div>
				<div id="consignmentOrder" class="table"></div>
				<div class="pagelist drop">
					<ul>

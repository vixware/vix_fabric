...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/activityBudgetAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteEntitys() {
	...
	$.ajax({
		url : '${vix}/drp/activityBudgetAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/drp/activityBudgetAction!goSingleList.action?1=1", 'activityBudget');
			});
		}
	});
}
...
function searchForContent(){
	loadName();
	pager("start","${vix}/drp/activityBudgetAction!goSingleList.action?name="+name,'activityBudget');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/activityBudgetAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'activityBudget');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/activityBudgetAction!goSingleList.action?name="+name,'activityBudget');
}
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="activityBudgetInfo"></b> <s:text name='cmn_to' /> <b class="activityBudgetTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a> </span> <span><a class="end" onclick="currentPager('end');"></a> </span>
					</div>
				</div>
				<div id="activityBudget" class="table"></div>
				<div class="pagelist drop">
					<ul>

...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/discountInformationAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/discountInformationAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/drp/discountInformationAction!goSingleList.action?name="+name,'channelDistributor');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/discountInformationAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'channelDistributor');
}
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/discountInformationAction!goSingleList.action?name="+name,'channelDistributor');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/drp/discountInformationAction!goSingleList.action?status=" + status, 'channelDistributor');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/drp/discountInformationAction!goSingleList.action?days=" + days, 'channelDistributor');
}
bindSearch();
</script>
<div class="sub_menu">

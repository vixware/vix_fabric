...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/productAcquisitionAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/productAcquisitionAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/drp/productAcquisitionAction!goSingleList.action?name="+name,'channelDistributor');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/productAcquisitionAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'channelDistributor');
}
bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/productAcquisitionAction!goSingleList.action?name="+name,'channelDistributor');
}
/** 状态 */
function saleOrderStatus(status) {
	pager("start", "${vix}/drp/productAcquisitionAction!goSingleList.action?status=" + status, 'channelDistributor');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/drp/productAcquisitionAction!goSingleList.action?days=" + days, 'channelDistributor');
}
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/drp/productAcquisitionAction!findOrgAndUnitTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/drp/productAcquisitionAction!goSingleList.action?id="+treeNode.id+"&treeType="+treeNode.treeType,"channelDistributor");
				}

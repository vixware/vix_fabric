...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/memberStorereturnAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteEntitys() {
	...
	$.ajax({
		url : '${vix}/drp/memberStorereturnAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/drp/memberStorereturnAction!goSingleList.action?1=1", 'brand');
			});
		}
	});
}
...
function searchForContent(){
	loadName();
	pager("start","${vix}/drp/memberStorereturnAction!goSingleList.action?name="+name,'brand');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/memberStorereturnAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/memberStorereturnAction!goSingleList.action?name="+name,'brand');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="drp_member_storereturn" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div class="search_advanced">
			<label><s:text name="drp_order_on1" /><input type="text" class="int" name=""> </label><label><s:text name="drp_order_date1" /><input type="text" class="int" name=""> </label> <label><s:text name="drp_customer_name1" /><input type="text" class="int" name=""> </label> <label><s:text name="drp_salesman1" /><input

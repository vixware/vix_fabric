...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/membershipPointsdetailsAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/drp/membershipPointsdetailsAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/membershipPointsdetailsAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/membershipPointsdetailsAction!goSingleList.action?name="+name,'brand');
}
function goSearch() {
	$.ajax({
	url : '${vix}/drp/membershipPointsdetailsAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				loadcode();
				loadselectname();
				pager("start", "${vix}/drp/membershipPointsdetailsAction!goSingleList.action?code=" + code + "&name=" + selectname, 'salesOrder');
			}
		},
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="drp_membershi_points_details" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

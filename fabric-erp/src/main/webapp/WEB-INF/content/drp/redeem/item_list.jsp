...
<script type="text/javascript">
	$.returnValue = "";
	function chkChange(chk, code, name, id, redeemPoints, unit, price) {
		if (chk.checked) {
			$.returnValue = code + "," + name + "," + id + "," + redeemPoints + "," + unit + "," + price;
		}
	}
	pager("start", "${vix}/drp/reDeemAction!goInventoryCurrentStockList.action?temp=1", "inventoryCurrentStock");
	//排序 
	function subOrderBy(orderField) {
		...
		pager("start", "${vix}/drp/reDeemAction!goInventoryCurrentStockList.action?orderField=" + orderField + "&orderBy=" + orderBy, "inventoryCurrentStock");
	}
	function currentPager(tag) {
		pager(tag, "${vix}/drp/reDeemAction!goInventoryCurrentStockList.action", 'inventoryCurrentStock');
	}
	...
	function searchForContent(i) {
		loadCode();
		pager("start", "${vix}/drp/reDeemAction!goInventoryCurrentStockList.action?searchContent=" + code, 'inventoryCurrentStock');
	}
	/*重置搜索*/
	...
	function srmStatus(status) {
		pager("start", "${vix}/drp/reDeemAction!goInventoryCurrentStockList.action?status=" + status, 'inventoryCurrentStock');
	}
	//最近使用
	function srmRecent(rencentDate) {
		pager("start", "${vix}/drp/reDeemAction!goInventoryCurrentStockList.action?updateTime=" + rencentDate, 'inventoryCurrentStock');
	}
</script>

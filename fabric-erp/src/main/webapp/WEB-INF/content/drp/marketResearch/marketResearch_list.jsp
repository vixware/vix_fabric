...
function saveOrUpdate(id,pageNo){
 	$.ajax({
		  url:'${vix}/drp/marketResearchAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/marketResearchAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/drp/marketResearchAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function searchForContent(i){
	if(i==0){
		...
		loadNameOr();
		loadPhone();
		pager("start","${vix}/drp/marketResearchAction!goSingleList.action?name="+name+"&telephone="+phone,'brand');
		}
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/marketResearchAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/marketResearchAction!goSingleList.action?name="+name,'brand');
}
//状态
function srmState(state){
	pager("state","${vix}/drp/marketResearchAction!goSingleList.action?state="+state,'brand');
}
//最近使用
function srmRecent(rencentDate){
	pager("start","${vix}/drp/marketResearchAction!goSingleList.action?updateTime="+rencentDate,'brand');
}
function goSearch() {
	$.ajax({

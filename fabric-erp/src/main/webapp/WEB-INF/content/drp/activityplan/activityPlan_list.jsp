...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/drp/activityPlanAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteEntitys() {
	...
	$.ajax({
		url : '${vix}/drp/activityPlanAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/drp/activityPlanAction!goSingleList.action?1=1", 'activityPlan');
			});
		}
	});
}
...
function searchForContent(){
	loadName();
	pager("start","${vix}/drp/activityPlanAction!goSingleList.action?name="+name,'activityPlan');
}
...
function orderBy(orderField){
	var orderBy = $("#activityPlanOrderBy").val();
	if(orderBy == 'desc'){
		orderBy = "asc";
	}else{
		orderBy = 'desc';
	}
	pager("start","${vix}/drp/activityPlanAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'activityPlan');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/activityPlanAction!goSingleList.action?name="+name,'activityPlan');
}
//高级搜索
function goSearch() {
	$.ajax({
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="drp_activity_plan" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="activityPlanList" var="c">
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="activityPlanInfo"></b> <s:text name='cmn_to' /> <b class="activityPlanTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a> </span> <span><a class="end" onclick="currentPager('end');"></a> </span>
					</div>
				</div>
				<div id="activityPlan" class="table"></div>
				<div class="pagelist drop">
					<ul>

...
<script type="text/javascript">
	...
	function saveOrUpdate(id){
		$.ajax({
			  url:'${vix}/drp/storeEnquiryrequestAction!goSaveOrUpdate.action?id='+id+"&parentId="+$('#selectId').val(),
			  cache: false,
	...
	function deleteEntitys() {
		...
		$.ajax({
			url : '${vix}/drp/storeEnquiryrequestAction!deleteByIds.action?ids=' + ids,
			cache : false,
			success : function(html) {
				asyncbox.success(html, "提示信息", function(action) {
					pager("start", "${vix}/drp/storeEnquiryrequestAction!goSingleList.action?1=1", 'salesOrder');
				});
			}
		});
	}
	...
	function searchForContent(tag){
		loadName();
		pager("start","${vix}/drp/storeEnquiryrequestAction!goSingleList.action?name="+name,'salesOrder');
	}
	...
	function orderBy(orderField){
		...
		pager("start","${vix}/drp/storeEnquiryrequestAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'salesOrder');
	}
	
	function currentPager(tag){
		loadName();
		pager(tag,"${vix}/drp/storeEnquiryrequestAction!goSingleList.action?name="+name,'salesOrder');
	}
	/** 状态 */
	function listbystatus(status) {
		pager("start", "${vix}/drp/storeEnquiryrequestAction!goSingleList.action?status=" + status, 'salesOrder');
	}
	/* 最近使用 */
	function leastRecentlyUsed(days) {
		pager("start", "${vix}/drp/storeEnquiryrequestAction!goSingleList.action?days=" + days, 'salesOrder');
	}
	function goSearch() {
		$.ajax({
		url : '${vix}/drp/storeEnquiryrequestAction!goSearch.action',
		cache : false,
		success : function(html) {
			asyncbox.open({
			modal : true,
			...
			callback : function(action) {
				if (action == 'ok') {
					loadcode();
					loadselectname();
					pager("start", "${vix}/drp/storeEnquiryrequestAction!goSingleList.action?code=" + code + "&name=" +selectname, 'salesOrder');
				}
			},
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="drp_store_enquiry_request" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/drp/storeEnquiryrequestAction!goSingleList.action?parentId="+treeNode.id+"&treeType="+treeNode.treeType,"salesOrder");
				}

...
<script type="text/javascript">
	...
	function deleteEntity(id) {
		asyncbox.confirm('确定要删除该数据么?', '提示信息', function(action) {
			if (action == 'ok') {
				$.ajax({
				url : '${vix}/drp/storeEnquiryrequestAction!deleteById.action?id=' + id,
				cache : false,
				success : function(html) {
					showMessage(html);
					setTimeout("", 1000);
					pager("start", "${vix}/drp/storeEnquiryrequestAction!goSingleList.action", 'salesOrder');
				}
				});
			}
		});
...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<th width="10%"><span style="cursor: pointer;" onclick="orderBy('code');">编码&nbsp;<img id="salesOrderImg_code" src="${vix}/common/img/arrow.gif"></span></th>
			<th width="35%"><span style="cursor: pointer;" onclick="orderBy('name');">主题&nbsp;<img id="salesOrderImg_name" src="${vix}/common/img/arrow.gif"></span></th>
			<th><span style="cursor: pointer;" onclick="orderBy('creator');"><s:text name="drp_the_goods_will" />&nbsp;<img id="salesOrderImg_creator" src="${vix}/common/img/arrow.gif"></span></th>
			<th><span style="cursor: pointer;" onclick="orderBy('createTime');">要货时间&nbsp;<img id="salesOrderImg_createTime" src="${vix}/common/img/arrow.gif"></span></th>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td><a href="#" style="color: gray;"><fmt:formatDate value="${entity.deliveryTime }" pattern="yyyy-MM-dd" /></a></td>
				<td>

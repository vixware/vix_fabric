...
<script type="text/javascript">
	function saveOrUpdateChannelDistributor() {
		if ($('#channelDistributorForm').validationEngine('validate')) {
			$.post('${vix}/drp/productInformationAction!saveOrUpdate.action', {
			...
			'channelDistributor.accessory' : $("#accessory").val()
				...
				showMessage(result);
				setTimeout("", 1000);
				loadContent('${vix}/drp/productInformationAction!goList.action');
			});
		} else {
			return false;
		}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/drp/productInformationAction!goList.action');">产品投放信息采集 </a></li>
			</ul>
		</div>
	</h2>
</div>
			...
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateChannelDistributor()" href="#"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <a href="#"
						onclick="loadContent('${vix}/drp/productInformationAction!goList.action?pageNo=${pageNo}');"><img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a></span> <strong><b>新增</b></strong>
				</dt>
				<dd class="clearfix">
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					<div class="right_menu">
						<ul>
							<li class="current"><a onclick="javascript:tab(4,1,'a',event)"><img src="${vix}/common/img/mail.png" alt="" />产品信息</a></li>
						</ul>
					</div>
					<div id="a1" class="right_content" style="position: relative; z-index: 1; background: #FFF;">
						<script type="text/javascript">
							function updateProductInformation(url) {
								...
								success : function(html) {
									...
									callback : function(action, returnValue) {
										if (action == 'ok') {
											if ($('#productInformationForm').validationEngine('validate')) {
												$.post('${vix}/drp/productInformationAction!saveOrUpdateProductInformation.action', {
												'id' : $("#id").val(),
												'productInformation.id' : $("#productInformationId").val(),
												'productInformation.name' : $("#productInformationName").val(),
												'productInformation.specification' : $("#specification").val(),
												'productInformation.region' : $("#productInformationRegion").val(),
												'productInformation.price' : $("#price").val(),
												'productInformation.produceDate' : $("#produceDate").val(),
												'productInformation.qualityPeriod' : $("#qualityPeriod").val(),
												'productInformation.putOnDate' : $("#putOnDate").val(),
												'productInformation.masterUnit' : $("#masterUnit").val(),
												'productInformation.productEnterprise' : $("#productEnterprise").val(),
												'productInformation.brand' : $("#brand").val(),
												'productInformation.origin' : $("#origin").val()
												}, function(result) {
													showMessage(result);
													setTimeout("", 1000);
													$('#productInformation').datagrid("reload");
												});
											} else {
												return false;
											}
							...
							$('#productInformation').datagrid({
							url : '${vix}/drp/productInformationAction!getProductInformationJson.action?id=${channelDistributor.id}',
							width : 'auto',
							height : 450,
							pagination : true,
							rownumbers : true,
							...
							handler : function() {
								...
								updateProductInformation('${vix}/drp/productInformationAction!goSaveOrUpdateProductInformation.action');
							}
							}, '-', {
							id : 'btnedit',
							text : '修改',
							iconCls : 'icon-edit',
							handler : function() {
								var row = $('#productInformation').datagrid("getSelected"); //获取你选择的所有行
								if (row) {
									updateProductInformation('${vix}/drp/productInformationAction!goSaveOrUpdateProductInformation.action?id=' + row.id);
								}
							}
							}, '-', {
							text : '删除',
							iconCls : 'icon-remove',
							handler : function() {
								var rows = $('#productInformation').datagrid("getSelections"); //获取你选择的所有行	
								//循环所选的行
								for (var i = 0; i < rows.length; i++) {
									var index = $('#productInformation').datagrid('getRowIndex', rows[i]);//获取某行的行号
									$('#productInformation').datagrid('deleteRow', index); //通过行号移除该行
									$.ajax({
									url : '${vix}/drp/productInformationAction!deleteProductInformationById.action?id=' + rows[i].id,
									cache : false
									});
								}
							}
						...
						<div style="padding: 8px;">
							<table id="productInformation"></table>
						</div>
					</div>
				</div>
			</dl>

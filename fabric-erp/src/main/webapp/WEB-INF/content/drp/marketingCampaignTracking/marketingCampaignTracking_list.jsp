...
function searchForContent(){
	...
	loadFeedbackContent();
	loadSelectContent();
	pager("start","${vix}/drp/marketingCampaignTrackingAction!goSingleList.action?name="+name+"&feedbackContent="+feedbackContent+"&selectContent="+selectContent,'marketingCampaign');
}
...
loadName();
//载入分页列表数据
pager("start","${vix}/drp/marketingCampaignTrackingAction!goSingleList.action?name="+name,'marketingCampaign');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/drp/marketingCampaignTrackingAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'marketingCampaign');
}
bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/marketingCampaignTrackingAction!goSingleList.action?name="+name,'marketingCampaign');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/drp/marketingCampaignTrackingAction!goSingleList.action?status=" + status, 'marketingCampaign');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/drp/marketingCampaignTrackingAction!goSingleList.action?days=" + days, 'marketingCampaign');
}
//批量删除
function deleteEntitys() {
	...
	$.ajax({
		url : '${vix}/drp/marketingCampaignTrackingAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/drp/marketingCampaignTrackingAction!goSingleList.action?1=1", 'marketingCampaign');
			});
		}
	});
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">营销任务追踪</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#"><span>总结 </span> </a>
		</p>
	</div>
</div>
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			var setting = {
				async: {
					enable: true,
					url:"${vix}/drp/marketingCampaignTrackingAction!findMarketingCampaignTreeToJson.action",
					autoParam:["id"]
				},
				callback: {
					onClick: onClick
			...
			function onClick(event, treeId, treeNode, clickFlag) {
				...
				pager("start","${vix}/drp/marketingCampaignTrackingAction!goSingleList.action?parentId="+treeNode.id,"marketingCampaign");
			}
			zTree = $.fn.zTree.init($("#tree_root"), setting);
			</script>
		</div>
		<input type="hidden" id="selectId" name="selectId" value="${parentId}" /> <input type="hidden" id="selectName" name="selectName" value="${selectName}" /> <input type="hidden" id="selectIdTreeId" name="selectIdTreeId" />
		<div id="right">

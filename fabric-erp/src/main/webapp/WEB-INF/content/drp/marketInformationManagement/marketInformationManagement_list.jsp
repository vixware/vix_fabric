...
function deleteEntitys() {
	...
	$.ajax({
		url : '${vix}/drp/marketInformationManagementAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/drp/marketInformationManagementAction!goSingleList.action?1=1", 'brand');
			});
		}
	});
}
...
function searchForContent(i){
	if(i==0){
		loadName();
		pager("start","${vix}/drp/marketInformationManagementAction!goSingleList.action?name="+name,'brand');
	}else{
		loadNameOr();
		loadPhone();
		pager("start","${vix}/drp/marketInformationManagementAction!goSingleList.action?name="+name+"&telephone="+phone,'brand');
		}
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/marketInformationManagementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/marketInformationManagementAction!goSingleList.action?name="+name,'brand');
}
...
function srmState(state){
	pager("state","${vix}/drp/marketInformationManagementAction!goSingleList.action?state="+state,'brand');
}
//最近使用
function srmRecent(rencentDate){
	pager("start","${vix}/drp/marketInformationManagementAction!goSingleList.action?updateTime="+rencentDate,'brand');
}
</script>
<div class="sub_menu">
	<h2>
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div>
			<!-- 搜索内容 -->
			<label> <s:text name="cmn_content" /> <input type="text" class="int" id="nameS">
			</label> <label> <input id="simpleSearch" type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent(0);"> <input id="simpleReset" type="button" value="<s:text name='cmn_reset'/>" class="btn" onclick="resetForContent(0);">
			</label>
			<!-- 高级搜索 -->
		...
		<div class="search_advanced">
			<!-- 名称 -->
			<label> <s:text name="drp_name1" /> <input id="nameOr" type="text" class="int" name="">
			</label>
			<!-- 电话 -->
			<label> <s:text name="drp_telephone1" /> <input id="phone" type="text" class="int" name="">
			</label> <label> <!-- 搜索 --> <input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent(1);"> <!-- 重置 --> <input type="button" value="<s:text name='cmn_reset'/>" class="btn" name="" onclick="resetForContent(1);">
			</label>
		</div>
	</div>
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="channelInfosList" var="c">

...
function saveOrUpdate(id,parentId,type,treeType){
	$.ajax({
		  url:'${vix}/drp/distributionSystemRelationShipAction!goSaveOrUpdate.action?id='+id+"&parentId="+parentId+"&type="+type+"&treeType="+treeType,
		  cache: false,
...
function deleteEntitys() {
	...
	$.ajax({
		url : '${vix}/drp/distributionSystemRelationShipAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/drp/distributionSystemRelationShipAction!goSingleList.action?1=1", 'channelDistributor');
			});
		}
	});
}
...
function searchForContent(){
	...
	//loadCode();
	//loadSelectName();
	pager("start","${vix}/drp/distributionSystemRelationShipAction!goSingleList.action?name="+name,'channelDistributor');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/distributionSystemRelationShipAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'channelDistributor');
}
function currentPager(tag){
	pager(tag,"${vix}/drp/distributionSystemRelationShipAction!goSingleList.action?1=1",'channelDistributor');
}
bindSearch();
/** 状态 */
function saleOrderStatus(status) {
	pager("start", "${vix}/drp/distributionSystemRelationShipAction!goSingleList.action?status=" + status, 'channelDistributor');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/drp/distributionSystemRelationShipAction!goSingleList.action?days=" + days, 'channelDistributor');
}
//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/drp/distributionSystemRelationShipAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/drp/distributionSystemRelationShipAction!goSingleList.action?warehousecode=" + $('#warehousecode').val() + "&code1=" + $('#code1').val(), 'channelDistributor');
			}
		},
	...
	<div class="drop">
		<ul>
			<li><a href="#"><span>新增</span> </a>
				<ul>
					<li><a href="#" onclick="saveOrUpdate(0,$('#selectId').val(),1,$('#selectIdType').val());">渠道 </a></li>
					<li><a href="#" onclick="saveOrUpdate(0,$('#selectId').val(),2,$('#selectIdType').val());">经销商 </a></li>
					<li><a href="#" onclick="saveOrUpdate(0,$('#selectId').val(),3,$('#selectIdType').val());">代理商 </a></li>
					<li><a href="#" onclick="saveOrUpdate(0,$('#selectId').val(),4,$('#selectIdType').val());">门店 </a></li>
				</ul></li>
		</ul>
	</div>
</div>
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			<s:iterator value="channelDistributorList" var="c">
				<li><a href="#" onclick="saveOrUpdate(${c.id},$('#selectId').val(),${c.type});"><span style="display: none;">${c.chineseCharacter}</span>${c.name}</a></li>
			</s:iterator>
		</ul>
	</div>
	<!-- c_head -->

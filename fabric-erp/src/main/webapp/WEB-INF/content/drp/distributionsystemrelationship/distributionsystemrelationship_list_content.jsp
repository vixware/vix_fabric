...
function deleteEntity(id) {
	asyncbox.confirm('确定要删除该数据么?', '提示信息', function(action) {
		if (action == 'ok') {
			$.ajax({
			url : '${vix}/drp/distributionSystemRelationShipAction!deleteById.action?id=' + id,
			cache : false,
			success : function(html) {
				showMessage(html);
				setTimeout("", 1000);
				pager("start", "${vix}/drp/distributionSystemRelationShipAction!goSingleList.action", 'channelDistributor');
			}
			});
		}
	});
...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<th width="15%"><span style="cursor: pointer;" onclick="orderBy('name');"><s:text name="drp_name" />&nbsp;<img id="channelDistributorImg_name" src="${vix}/common/img/arrow.gif"></span></th>
			<th><span style="cursor: pointer;" onclick="orderBy('artificialPerson');">法人&nbsp;<img id="channelDistributorImg_artificialPerson" src="${vix}/common/img/arrow.gif"></span></th>
			...
			<th><span style="cursor: pointer;" onclick="orderBy('status');"><s:text name="drp_state" />&nbsp;<img id="channelDistributorImg_status" src="${vix}/common/img/arrow.gif"></span></th>
			<th><span style="cursor: pointer;" onclick="orderBy('level');"><s:text name="drp_level" />&nbsp;<img id="channelDistributorImg_level" src="${vix}/common/img/arrow.gif"></span></th>
			<th><span style="cursor: pointer;" onclick="orderBy('createTime');"><s:text name="drp_creation_time" />&nbsp;<img id="channelDistributorImg_createTime" src="${vix}/common/img/arrow.gif"></span></th>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td><a href="#" style="color: gray;"><s:if test="%{#entity.status==0}">营业</s:if> <s:elseif test="%{#entity.status==1}">暂停营业</s:elseif></a></td>

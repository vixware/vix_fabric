...
<script type="text/javascript">
	...
	function deleteEntity(id) {
		asyncbox.confirm('确定要删除该数据么?', '提示信息', function(action) {
			if (action == 'ok') {
				$.ajax({
				url : '${vix}/drp/customerSurveyAction!deleteById.action?id=' + id,
				cache : false,
				success : function(html) {
					showMessage(html);
					setTimeout("", 1000);
					pager("start", "${vix}/drp/customerSurveyAction!goSingleList.action", 'customerSurvey');
				}
				});
			}
		});
...
<table id="purchase_inquire_grid" class="list table" pager='<s:property value="pager.genPagerInfoJsonStr()" escape="false"/>' fillblank="1">
	<tbody>
		<tr class="alt">
			...
			<th><s:text name="drp_respondents" /></th>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			<tr>
				...
				<td><a href="#" style="color: gray;">${entity.surveyObject}</a></td>
				<td><a href="#" style="color: gray;">${entity.surveyPerson}</a></td>
				<td><a href="#" style="color: gray;"><fmt:formatDate value="${entity.surveyDate}" pattern="yyyy-MM-dd" /></a></td>
				<td>

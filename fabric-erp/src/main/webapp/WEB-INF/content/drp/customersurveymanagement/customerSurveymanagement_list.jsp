...
<script type="text/javascript">
	...
	function saveOrUpdate(id,pageNo){
		$.ajax({
			  url:'${vix}/drp/customerSurveyAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
			  cache: false,
	...
	function deleteEntitys() {
		...
		$.ajax({
			url : '${vix}/drp/customerSurveyAction!deleteByIds.action?ids=' + ids,
			cache : false,
			success : function(html) {
				asyncbox.success(html, "提示信息", function(action) {
					pager("start", "${vix}/drp/customerSurveyAction!goSingleList.action?1=1", 'customerSurvey');
				});
			}
		});
	}
	...
	function orderBy(orderField){
		...
		pager("start","${vix}/drp/customerSurveyAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'customerSurvey');
	}
	/** 状态 */
	function listbystatus(status) {
		pager("start", "${vix}/drp/customerSurveyAction!goSingleList.action?status=" + status, 'customerSurvey');
	}
	/* 最近使用 */
	function leastRecentlyUsed(days) {
		pager("start", "${vix}/drp/customerSurveyAction!goSingleList.action?days=" + days, 'customerSurvey');
	}
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"> <s:text name="drp_customer_survey_management" />
				</a></li>
			</ul>
		</div>
	</h2>
...
<div class="content">
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="customerSurveyList" var="c">
		...
		<div id="right">
			<div id="tab_content" class="tabbable addable_tab">
				<ul id="myTab" class="right_menu nav nav-tabs">
					<li pageId="tab_home" class="current"><a href="#tab_home" page="${vix}/drp/customerSurveyAction!goSingleList.action"> <img src="img/mail.png" alt="" /> 客户调查列表
					</a></li>
				</ul>

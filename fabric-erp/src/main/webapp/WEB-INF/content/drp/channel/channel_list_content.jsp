...
<script type="text/javascript">
	...
	function deleteEntity(id) {
		asyncbox.confirm('确定要删除该数据么?', '提示信息', function(action) {
			if (action == 'ok') {
				$.ajax({
				url : '${vix}/drp/channelAction!deleteById.action?id=' + id,
				cache : false,
				success : function(html) {
					showMessage(html);
					setTimeout("", 1000);
					pager("start", "${vix}/drp/channelAction!goSingleList.action", 'channelDistributor');
				}
				});
			}
		});
...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<th><span style="cursor: pointer;" onclick="orderBy('code');"><s:text name="drp_coding" />&nbsp;<img id="channelDistributorImg_code" src="${vix}/common/img/arrow.gif"></span></th>

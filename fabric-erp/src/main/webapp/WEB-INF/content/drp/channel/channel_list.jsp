...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/drp/channelAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteEntitys() {
	...
	$.ajax({
		url : '${vix}/drp/channelAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/drp/channelAction!goSingleList.action?1=1", 'channelDistributor');
			});
		}
	});
}
loadName();
//载入分页列表数据
pager("start","${vix}/drp/channelAction!goSingleList.action?name="+name,'channelDistributor');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/drp/channelAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'channelDistributor');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/channelAction!goSingleList.action?name="+name,'channelDistributor');
}
...
function saleOrderStatus(status) {
	pager("start", "${vix}/drp/channelAction!goSingleList.action?status=" + status, 'channelDistributor');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/drp/channelAction!goSingleList.action?days=" + days, 'channelDistributor');
}
//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/drp/channelAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start","${vix}/drp/channelAction!goSingleList.action?code="+$('#code').val()+"&selectName="+$('#selectName').val(),'channelDistributor');
			}
		},
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="drp_channel_management" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/drp/channelAction!goSingleList.action?id="+treeNode.id+"&treeType="+treeNode.treeType,"channelDistributor");
				}

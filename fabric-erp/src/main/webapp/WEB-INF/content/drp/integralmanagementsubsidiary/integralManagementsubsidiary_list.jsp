...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/integralManagementsubsidiaryAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/integralManagementsubsidiaryAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/drp/integralManagementsubsidiaryAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/integralManagementsubsidiaryAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/integralManagementsubsidiaryAction!goSingleList.action?name="+name,'brand');
}
/** 状态 */
function saleOrderStatus(status) {
	pager("start", "${vix}/drp/integralManagementsubsidiaryAction!goSingleList.action?status=" + status, 'brand');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/drp/integralManagementsubsidiaryAction!goSingleList.action?days=" + days, 'brand');
}
function goSearch() {
		$.ajax({
		url : '${vix}/drp/integralManagementsubsidiaryAction!goSearch.action',
		cache : false,
		success : function(html) {
			asyncbox.open({
			modal : true,
			...
			callback : function(action) {
				if (action == 'ok') {
					loadcode();
					loadselectname();
					pager("start", "${vix}/drp/integralManagementsubsidiaryAction!goSingleList.action?code=" + code + "&name=" + selectname, 'salesOrder');
				}
			},

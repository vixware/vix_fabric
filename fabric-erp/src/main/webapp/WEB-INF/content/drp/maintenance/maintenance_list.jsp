...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/maintenanceAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/maintenanceAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/drp/maintenanceAction!goSingleList.action?name="+name,'channelDistributor');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/drp/maintenanceAction!goSingleList.action?name=",'channelDistributor');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/drp/maintenanceAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'channelDistributor');
}
bindSearch();
bindSwitch();
...
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/maintenanceAction!goSingleList.action?name="+name,'channelDistributor');
}
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/drp/maintenanceAction!findOrgAndUnitTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/drp/maintenanceAction!goSingleList.action?id="+treeNode.id+"&treeType="+treeNode.treeType,"channelDistributor");
				}

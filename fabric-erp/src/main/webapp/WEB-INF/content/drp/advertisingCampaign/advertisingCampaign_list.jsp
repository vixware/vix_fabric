...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/advertisingCampaignAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/advertisingCampaignAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/drp/advertisingCampaignAction!goSingleList.action?name="+name,'channelDistributor');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/drp/advertisingCampaignAction!goSingleList.action?name=",'channelDistributor');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/drp/advertisingCampaignAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'channelDistributor');
}
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/advertisingCampaignAction!goSingleList.action?name="+name,'channelDistributor');
}
</script>
<div class="sub_menu">
	<h2>

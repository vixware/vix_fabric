...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/storeReturnRecordsAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteEntitys() {
	...
	$.ajax({
		url : '${vix}/drp/storeReturnRecordsAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/drp/storeReturnRecordsAction!goSingleList.action?1=1", 'saleReturnForm');
			});
		}
	});
}
...
function searchForContent(){
	loadName();
	pager("start","${vix}/drp/storeReturnRecordsAction!goSingleList.action?name="+name,'saleReturnForm');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/memberStorereturnAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'saleReturnForm');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/storeReturnRecordsAction!goSingleList.action?name="+name,'saleReturnForm');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/drp/storeReturnRecordsAction!goSingleList.action?status=" + status, 'saleReturnForm');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/drp/storeReturnRecordsAction!goSingleList.action?days=" + days, 'saleReturnForm');
}
function goSearch() {
	$.ajax({
	url : '${vix}/drp/storeReturnRecordsAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				loadcode();
				loadselectname();
				pager("start", "${vix}/drp/storeReturnRecordsAction!goSingleList.action?code=" + code + "&name=" + selectname, 'salesOrder');
			}
		},
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"> <s:text name="drp_return_records_tores" />
				</a></li>
			</ul>
		</div>
	</h2>
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="saleReturnFormInfo"></b> <s:text name='cmn_to' /> <b class="saleReturnFormTotalCount"></b>)

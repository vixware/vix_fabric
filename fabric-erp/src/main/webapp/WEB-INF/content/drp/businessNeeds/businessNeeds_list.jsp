...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/businessNeedsAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/businessNeedsAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/drp/businessNeedsAction!goSingleList.action?name="+name,'channelDistributor');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/businessNeedsAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'channelDistributor');
}
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/businessNeedsAction!goSingleList.action?name="+name,'channelDistributor');
}
/** 状态 */
function saleOrderStatus(status) {
	pager("start", "${vix}/drp/businessNeedsAction!goSingleList.action?status=" + status, 'channelDistributor');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/drp/businessNeedsAction!goSingleList.action?days=" + days, 'channelDistributor');
}
function goSearch() {
	$.ajax({
	...
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/drp/businessNeedsAction!goSingleList.action", 'channelDistributor');
			}
		},
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/drp/businessNeedsAction!goSingleList.action?id="+treeNode.id+"&treeType="+treeNode.treeType,"channelDistributor");
				}

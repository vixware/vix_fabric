...
<script type="text/javascript">
	...
	function saveOrUpdateStockRecords() {
		if ($('#stockRecordsForm').validationEngine('validate')) {
			$.post('${vix}/drp/salesStorageAction!saveOrUpdate.action', {
			...
			'stockRecords.createTime' : $("#createTime").val()
				...
				showMessage(result);
				setTimeout("", 1000);
				loadContent('${vix}/drp/salesStorageAction!goList.action');
			});
		}
	}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/drp/salesStorageAction!goList.action');">日配销售入库 </a></li>
			</ul>
		</div>
	</h2>
</div>
			...
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateStockRecords()" href="#"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <a href="#"><img width="22" height="22" title="保存并提交审批"
							src="${vix}/common/img/dt_submit.png" /></a> <a href="#" onclick="loadContent('${vix}/drp/salesStorageAction!goList.action?pageNo=${pageNo}');"><img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a></span> <strong><b>销售入库单</b></strong>
				</dt>
				<dd class="clearfix">
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					...
					<div id="a1" class="right_content" style="position: relative; z-index: 1; background: #FFF;">
						<script type="text/javascript">
									...
									callback : function(action, returnValue) {
										if (action == 'ok') {
											if ($('#stockRecordLinesForm').validationEngine('validate')) {
												$.post('${vix}/drp/salesStorageAction!saveOrUpdateStockRecordLines.action', {
												'id' : $("#id").val(),
							...
							url : '${vix}/drp/salesStorageAction!getStockRecordLinesJson.action?id=${stockRecords.id}',
							width : 'auto',
							height : 450,
							pagination : true,
							rownumbers : true,
							...
							handler : function() {
								...
								saveDeliveryAddress('${vix}/drp/salesStorageAction!goSaveOrUpdateStockRecordLines.action');
							}
							}, '-', {
							id : 'btnedit',
							text : '修改',
							...
							handler : function() {
								...
								if (row) {
									saveDeliveryAddress('${vix}/drp/salesStorageAction!goSaveOrUpdateStockRecordLines.action?id=' + row.id);
								}
							}
							}, '-', {
							text : '删除',
							...
							handler : function() {
								...
								for ( var i = 0; i < rows.length; i++) {
									...
									$.ajax({
									url : '${vix}/drp/salesStorageAction!deleteStockRecordLinesById.action?id=' + rows[i].id,
									cache : false
									});
								}
							}

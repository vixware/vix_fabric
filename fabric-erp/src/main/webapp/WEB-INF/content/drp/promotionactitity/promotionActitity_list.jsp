...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/promotionActitityAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/drp/promotionActitityAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/promotionActitityAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/promotionActitityAction!goSingleList.action?name="+name,'brand');
}
...
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#"><img src="img/drp_channel.png" alt="" /> <s:text name="drp_promotion_actitity" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

...
<script type="text/javascript">
	...
	function deleteEntity(id) {
		asyncbox.confirm('确定要删除该数据么?', '提示信息', function(action) {
			if (action == 'ok') {
				$.ajax({
				url : '${vix}/drp/distributerLevelAction!deleteById.action?id=' + id,
				cache : false,
				success : function(html) {
					showMessage(html);
					setTimeout("", 1000);
					pager("start", "${vix}/drp/distributerLevelAction!goSingleList.action", 'brand');
				}
				});
			}
		});
...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<th width="250"><span style="cursor: pointer;" onclick="orderBy('code');"><s:text name="drp_level_is_adjusted" />&nbsp;<img id="brandImg_code" src="${vix}/common/img/arrow.gif"></span></th>
			<th width="250"><span style="cursor: pointer;" onclick="orderBy('code');"><s:text name="drp_level_application" />&nbsp;<img id="brandImg_code" src="${vix}/common/img/arrow.gif"></span></th>
			<th width="250"><span style="cursor: pointer;" onclick="orderBy('code');"><s:text name="drp_level_examination_approval" />&nbsp;<img id="brandImg_code" src="${vix}/common/img/arrow.gif"></span></th>

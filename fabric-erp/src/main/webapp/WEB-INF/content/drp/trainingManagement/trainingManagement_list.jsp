...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/trainingManagementAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteEntitys() {
	...
	$.ajax({
		url : '${vix}/drp/trainingManagementAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/drp/trainingManagementAction!goSingleList.action?1=1", 'category');
			});
		}
	});
}
loadName();
//载入分页列表数据
pager("start","${vix}/drp/trainingManagementAction!goSingleList.action?name="+name,'category');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/drp/trainingManagementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'category');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/trainingManagementAction!goSingleList.action?name="+name,'category');
}
...
function saleOrderStatus(status) {
	pager("start", "${vix}/drp/trainingManagementAction!goSingleList.action?status=" + status, 'category');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/drp/trainingManagementAction!goSingleList.action?days=" + days, 'category');
}
function goSearch() {
	$.ajax({
	url : '${vix}/drp/trainingManagementAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start","${vix}/drp/trainingManagementAction!goSingleList.action?code="+$('#code').val()+"&name="+$('#name').val(),'category');
			}
		},
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div>
			<label>内容:<input type="text" class="int" id="nameS" placeholder="编码"></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent(0);" /><input type="button" value="<s:text name='cmn_reset'/>" class="btn" name="" onclick="resetForContent();" /></label> <label> <input

...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/basicInformationAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/drp/basicInformationAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/basicInformationAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/basicInformationAction!goSingleList.action?name="+name,'brand');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="drp_basic_information" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div>
			...
			</label> <label> <input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();" /> <input type="button" value="<s:text name='cmn_reset'/>" class="btn" name="" />
			</label> <strong id="lb_search_advanced"><s:text name="cmn_advance_search" /> </strong>
		</div>
		<div class="search_advanced">
			<label> <s:text name="drp_investigators1" /> <input type="text" class="int" name="">
			</label> <label> <s:text name="drp_survey_time1" /> <input type="text" class="int" name="">
			</label>
		</div>
	</div>
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="productsInfoList" var="c">

...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/drp/storeReceivingrecordsAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteEntitys() {
	...
	$.ajax({
		url : '${vix}/drp/storeReceivingrecordsAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/drp/storeReceivingrecordsAction!goSingleList.action?1=1", 'brand');
			});
		}
	});
}
...
function searchForContent(){
	loadName();
	pager("start","${vix}/drp/storeReceivingrecordsAction!goSingleList.action?name="+name,'brand');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/storeReceivingrecordsAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/storeReceivingrecordsAction!goSingleList.action?name="+name,'brand');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/drp/storeReceivingrecordsAction!goSingleList.action?status=" + status, 'brand');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/drp/storeReceivingrecordsAction!goSingleList.action?days=" + days, 'brand');
}

function goSearch() {
	$.ajax({
	url : '${vix}/drp/storeReceivingrecordsAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				loadcode();
				loadselectname();
				pager("start", "${vix}/drp/storeReceivingrecordsAction!goSingleList.action?code=" + code + "&name=" + selectname, 'brand');
			}
		},
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="drp_store_receiving_records" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

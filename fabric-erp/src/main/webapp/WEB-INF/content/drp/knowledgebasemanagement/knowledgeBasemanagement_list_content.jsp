...
<script type="text/javascript">
	...
	function deleteEntity(id) {
		asyncbox.confirm('确定要删除该数据么?', '提示信息', function(action) {
			if (action == 'ok') {
				$.ajax({
				url : '${vix}/drp/knowledgeBasemanagementAction!deleteById.action?id=' + id,
				cache : false,
				success : function(html) {
					showMessage(html);
					setTimeout("", 1000);
					pager("start", "${vix}/drp/knowledgeBasemanagementAction!goSingleList.action", 'knowledgeBase');
				}
				});
			}
		});
...
<table id="purchase_inquire_grid" class="list table" pager='<s:property value="pager.genPagerInfoJsonStr()" escape="false"/>' fillblank="1">
	<tbody>
		<tr class="alt">
			...
			<th>编码</th>
			<th width="35%">主题</th>
			<th><s:text name="drp_key" /></th>
			<th><s:text name="drp_type" /></th>
			<th><s:text name="drp_release_time" /></th>
				...
				<td>
					<div class="untitled" style="position: static; display: inline;">
						...
						<div class="popup" style="display: none; top: -7px;">
							...
							<p>
								<a href="${vix}/drp/knowledgeBasemanagementAction!downloadAttachment.action?id=${entity.id}">下载</a>
							</p>
						</div>
					</div>
				</td>

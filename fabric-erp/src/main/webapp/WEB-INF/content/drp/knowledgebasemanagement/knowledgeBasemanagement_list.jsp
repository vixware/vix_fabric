...
<script type="text/javascript">
	...
	function saveOrUpdate(id,parentId){
		$.ajax({
			  url:'${vix}/drp/knowledgeBasemanagementAction!goSaveOrUpdate.action?id='+id,
			  cache: false,
			  success: function(html){
				  asyncbox.open({
						...
						callback : function(action){
							if(action == 'ok'){
								$("#knowledgeBaseForm").ajaxSubmit({
								     type: "post",
								     url: "${vix}/drp/knowledgeBasemanagementAction!saveOrUpdate.action",
								     dataType: "text",
								     success: function(result){
								    	showMessage(result);
										setTimeout("", 1000);
										loadContent('${vix}/drp/knowledgeBasemanagementAction!goList.action');
								     }
								 });
							}
						},
	...
	function saveOrUpdateKnowledgeBaseCategory(id,parentId){
		$.ajax({
			  url:'${vix}/drp/knowledgeBasemanagementAction!goSaveOrUpdateKnowledgeBaseCategory.action?id='+id,
			  cache: false,
			  success: function(html){
				  asyncbox.open({
					 	modal:true,
						...
						callback : function(action){
							if(action == 'ok'){
								$("#knowledgeBaseCategoryForm").ajaxSubmit({
								     type: "post",
								     url: "${vix}/drp/knowledgeBasemanagementAction!saveOrUpdateKnowledgeBaseCategory.action",
								     dataType: "text",
								     success: function(result){
								    	showMessage(result);
										setTimeout("", 1000);
	...
	function orderBy(orderField){
		...
		pager("start","${vix}/drp/knowledgeBasemanagementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&parentId="+$("#selectId").val(),'knowledgeBase');
	}
	/** 状态 */
	function listbystatus(status) {
		pager("start", "${vix}/drp/knowledgeBasemanagementAction!goSingleList.action?status=" + status, 'knowledgeBase');
	}
	/* 最近使用 */
	function leastRecentlyUsed(days) {
		pager("start", "${vix}/drp/knowledgeBasemanagementAction!goSingleList.action?days=" + days, 'knowledgeBase');
	}
	//批量删除
	function deleteEntitys() {
		...
		$.ajax({
			url : '${vix}/drp/knowledgeBasemanagementAction!deleteByIds.action?ids=' + ids,
			cache : false,
			success : function(html) {
				asyncbox.success(html, "提示信息", function(action) {
					pager("start", "${vix}/drp/knowledgeBasemanagementAction!goSingleList.action?1=1", 'knowledgeBase');
				});
			}
		});
	}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="drp_knowledge_basemanagement" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdateKnowledgeBaseCategory(0);"><span>新增分类 </span> </a> <a href="#" onclick="saveOrUpdate(0);"><span>新增知识点 </span> </a>
		</p>
	</div>
</div>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div class="grid_search search_advanced">
			<label>主题<input name="name" type="text" class="int" />
			</label><label> 关键字<input name="keyword" type="text" class="int" />
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="knowledgeBaseList" var="c">
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/drp/knowledgeBasemanagementAction!findKnowledgeBaseCategoryTree.action",
						autoParam:["id"]
					},
					callback: {
						onClick: onClick
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/drp/knowledgeBasemanagementAction!goSingleList.action?parentId="+treeNode.id,"knowledgeBase");
				}
		...
		<div id="right">
			<div id="tab_content" class="tabbable addable_tab">
				<ul id="myTab" class="right_menu nav nav-tabs">
					<li pageId="tab_home" class="current"><a href="#tab_home" page="${vix}/drp/knowledgeBasemanagementAction!goSingleList.action"> <img src="img/mail.png" alt="" /> 知识库列表
					</a></li>
				</ul>

...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/business/pickingOutAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/business/pickingOutAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/business/pickingOutAction!goWeighingListContent.action?name="+name,'orderBatch');
			});
		  }
	});
}
...
loadName();
//载入分页列表数据
pager("start","${vix}/business/pickingOutAction!goWeighingListContent.action?orderBatchId="+$("#orderBatchId").val(),'orderBatch');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/business/pickingOutAction!goWeighingListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'orderBatch');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/business/pickingOutAction!goWeighingListContent.action?name="+name,'orderBatch');
}
function goWeighing(id){
	$.ajax({
					...
					callback : function(action){
						if(action == 'ok'){
							if ($('#order').validationEngine('validate')) {
								$.post('${vix}/business/outBoundProcessAction!saveOrUpdateWeighing.action', {
								'order.id' : $("#orderId").val(),
								'order.weight' : $("#weight").val()
								}, function(result) {
									showMessage(result);
									setTimeout("", 1000);
									loadContent('${vix}/business/pickingOutAction!goWeighingList.action?orderBatchId='+$("#orderBatchId").val());
								});
							} else {
								return false;
							}

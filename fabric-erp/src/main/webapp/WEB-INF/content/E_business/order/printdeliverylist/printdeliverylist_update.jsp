...
<script type="text/javascript">
	function saveOrUpdateSalesOrder() {
		...
		if ($('#salesOrderForm').validationEngine('validate')) {
			$.post('${vix}/business/printDeliveryListAction!saveOrUpdate.action', {
			'salesOrder.id' : $("#id").val(),
			'sojson' : sojson
			}, function(result) {
				showMessage(result);
				setTimeout("", 1000);
				loadContent('${vix}/business/printDeliveryListAction!goList.action');
			});
		} else {
			return false;
		}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">网店管理 </a></li>
				<li><a href="#">订单管理</a></li>
				<li><a href="#" onclick="loadContent('${vix}/business/printDeliveryListAction!goList.action');">打印发货单</a></li>
			</ul>
		</div>
	</h2>
</div>
				...
				<div class="clearfix" style="background: #FFF; position: relative;" id="selectOptionId1">
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;" class="right_content">
						<script type="text/javascript">
									...
									callback : function(action, returnValue) {
										if (action == 'ok') {
											if ($('#orderForm').validationEngine('validate')) {
												$.post('${vix}/business/printDeliveryListAction!saveOrUpdateSaleOrderItem.action', {
													'id' : $("#id").val()
												}, function(result) {
													showMessage(result);
													setTimeout("", 1000);
							...
							$('#order').datagrid({
							url : '${vix}/business/printDeliveryListAction!getOrderList.action?id=${orderBatch.id}',
							width : "100%",
							height : 450,
							nowrap : true,
							autoRowHeight : false,
							...
							handler : function() {
								...
								saveDeliveryAddress('${vix}/business/printDeliveryListAction!goSaveSaleOrderItem.action');
							}
							}, '-', {
							id : 'btnedit',
							text : '修改',
							...
							handler : function() {
								...
								if (row) {
									saveDeliveryAddress('${vix}/business/printDeliveryListAction!goSaveSaleOrderItem.action?id=' + row.id);
								}
							}
							}, '-', {
							text : '删除',
							...
							handler : function() {
								...
								for (var i = 0; i < rows.length; i++) {
									...
									$.ajax({
									url : '${vix}/business/printDeliveryListAction!deleteSaleOrderItemById.action?id=' + rows[i].id,
									cache : false
									});
								}
							}

...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/business/printDeliveryListAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function goPrint(id,pageNo){
	$.ajax({
		  url:'${vix}/business/outBoundProcessAction!goPrint.action?businussOrderId='+orderid,
		  cache: false,
		  success: function(html){
		  	$("#mainContent").html(html);
		  }
...
function searchForContent(){
	loadName();
	pager("start","${vix}/business/printDeliveryListAction!goSingleList.action?name="+name,'orderBatch');
}
 
loadName();
//载入分页列表数据
pager("start","${vix}/business/printDeliveryListAction!goSingleList.action?orderBatchId="+$("#orderBatchId").val(),'orderBatch');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/business/printDeliveryListAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'orderBatch');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/business/printDeliveryListAction!goSingleList.action?name="+name,'orderBatch');
}
//单条删除
function deleteById(id){
	$.ajax({
		  url:'${vix}/business/printDeliveryListAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
...
function deleteByIds(ids) {
		$.ajax({
		url : '${vix}/business/printDeliveryListAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/business/printDeliveryListAction!goSingleList.action?1=1", 'orderBatch');
			});
		}
	});
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">网店管理 </a></li>
				<li><a href="#">订单管理 </a></li>
				<li><a href="#">打印发货单 </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<a href="#" onclick="goPrint();"><span>打印</span> </a>
	</div>
</div>
<div class="content">

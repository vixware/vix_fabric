...
$('input.btn[type="button"],input.btn[type="submit"]').hover(function(){
 ...
 function saveOrUpdate(id){
	$.ajax({
		  url:"${vix}/oa/vehiclePermitapplicationAction!goSaveOrUpdate.action?id="+id,
		  cache: false,
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#brandForm').validationEngine('validate')){
								$.post('${vix}/oa/vehiclePermitapplicationAction!saveOrUpdate.action',
									...
									function(result){
										asyncbox.open({
											...
											html:result,
											callback : function(action){
												pager("current","${vix}/oa/vehiclePermitapplicationAction!goSingleList.action?name="+name,'brand');
											},
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/vehiclePermitapplicationAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/oa/vehiclePermitapplicationAction!goListContent.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/oa/vehiclePermitapplicationAction!goListContent.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/oa/vehiclePermitapplicationAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/oa/vehiclePermitapplicationAction!goListContent.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="oa_vehicle_message_management" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0,$('#selectId').val());"><span><s:text name="新增车辆信息" /></span></a>
			<%-- <a href="#" onclick="saveOrUpdate1(0,$('#selectId').val());"><span><s:text name="车辆详细信息"/></span></a> --%>
		...
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/oa/vehiclePermitapplicationAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/oa/vehiclePermitapplicationAction!goListContent.action?parentId="+treeNode.id,"category");
				}
			...
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/mail.png" alt="" />
						<s:text name='oa_vehicle_message_management' /></a></li>
				</ul>
			</div>

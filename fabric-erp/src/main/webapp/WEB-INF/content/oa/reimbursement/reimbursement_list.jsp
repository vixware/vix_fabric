...
function saveOrUpdate(id,parentId,type,pageNo){
	$.ajax({
		  url:'${vix}/oa/reimbursementAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo+"&parentId="+parentId+"&type="+type,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/reimbursementAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/oa/reimbursementAction!goSingleList.action?name="+name,'channelDistributor');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/oa/reimbursementAction!goSingleList.action?name=",'channelDistributor');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/oa/reimbursementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'channelDistributor');
}
bindSearch();
bindSwitch();
...
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/oa/reimbursementAction!goSingleList.action?name="+name,'channelDistributor');
}
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<div class="left_content" style="height: 500px;">
				<div style="padding-left: 4px;">
					<img src="${vix}/common/img/file.png" style="padding-right: 5px;" /><a href="#" onclick="loadRoot();">XX集团公司</a>
				</div>
			...
			<script type="text/javascript">	
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/oa/reimbursementAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/oa/reimbursementAction!goSingleList.action?id="+treeNode.id+"&treeType="+treeNode.treeType,"channelDistributor");
				}

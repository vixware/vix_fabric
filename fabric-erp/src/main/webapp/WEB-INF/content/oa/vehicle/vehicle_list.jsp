...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/oa/vehicleAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#transportForm').validationEngine('validate')){
							$.post('${vix}/oa/vehicleAction!saveOrUpdate.action',
									 {
									  'transport.id':$("#transportId").val(),
									  'transport.code':$("#code").val(),
									  'transport.name':$("#name").val()
									},
									function(result){
										showMessage(result);
										setTimeout("", 1000);
										loadContent('${vix}/oa/vehicleAction!goList.action');
									}
								 );
							}else {
								return false;
...
function searchForRightContent(){
	loadName();
	pager("start","${vix}/oa/vehicleAction!goSingleList.action?name="+name,'transport');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/vehicleAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'transport');
}

function categoryPager(tag){
	loadName();
	pager(tag,"${vix}/oa/vehicleAction!goSingleList.action?name="+name,'transport');
}
bindSearch();
</script>
<div class="sub_menu">
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/oa/vehicleAction!goSingleList.action?id="+treeNode.id+"&treeType="+treeNode.treeType,"transport");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'transport')"><img src="img/mail.png" alt="" /> 交通工具 </a></li>
				</ul>
			</div>
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="categoryPager('start');"></a> </span> <span><a class="previous" onclick="categoryPager('previous');"></a> </span> <em>(<b class="transportInfo"></b> <s:text name='cmn_to' /> <b class="transportTotalCount"></b>)
						</em> <span><a class="next" onclick="categoryPager('next');"></a> </span> <span><a class="end" onclick="categoryPager('end');"></a> </span>
					</div>
				</div>
				<div id="transport" class="table"></div>
				<div class="pagelist drop">
					<ul>
						<li class="ed"><a href="#"><s:text name='cmn_choose' /> </a>
							<ul>

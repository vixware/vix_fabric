...
<script type="text/javascript">
	function saveOrUpdateCurrentExpense() {
		if ($('#currentExpenseForm').validationEngine('validate')) {
			$.post('${vix}/oa/currentexpenseAction!saveOrUpdate.action', {
			'currentExpense.id' : $("#currentExpenseId").val(),
			'currentExpense.employee.id' : $("#employeeId").val(),
			...
			'currentExpense.memo' : $("#memo").val()
				...
				showMessage(result);
				setTimeout("", 1000);
				loadContent('${vix}/oa/currentexpenseAction!goList.action');
			});
		} else {
			return false;
		}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">行政办公 </a></li>
				<li><a href="#">报销管理 </a></li>
				<li><a href="#" onclick="loadContent('${vix}/oa/currentexpenseAction!goList.action');">日常费用报销 </a></li>
			</ul>
		</div>
	</h2>
</div>
			...
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateCurrentExpense();"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <a href="#"
						onclick="loadContent('${vix}/oa/currentexpenseAction!goList.action?pageNo=${pageNo}');"><img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a>
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id=name name="name" value="${currentExpense.name }" type="text" size="30"><input class="btn" type="button" value="选择" onclick="chooseEmployees();" /> <s:hidden id="employeeId" name="employeeId" value="%{currentExpense.employee.id}" theme="simple" /></td>
											<td align="right">报销人：</td>
											<td><input id="employeeName" name="employeeName" value="${currentExpense.employee.name }" type="text" size="30"><input class="btn" type="button" value="选择" onclick="chooseEmployees();" /> <s:hidden id="employeeId" name="employeeId" value="%{currentExpense.employee.id}" theme="simple" /></td>
										</tr>
										<tr>
											<td align="right">报销日期：</td>
											<td><input id="createTime" name="createTime" value="${fn:substring(currentExpense.createTime, 0,10)}" type="text" class="validate[required] text-input" readonly="readonly" /><img onclick="showTime('createTime','yyyy-MM-dd HH:mm:ss')" src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"><span
										...
										<tr>
											...
											<td colspan="3"><textarea id="memo" name="memo" style="width: 525px; height: 100px; margin-top: 5px;">${currentExpense.memo }</textarea></td>
										</tr>
									</table>
								</dd>
							</dl>
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id="expensesAmount" name="expensesAmount" value="${currentExpense.expensesAmount }" type="text" size="30" />元</td>
											<td align="right">发票：</td>
											<td><input id="invoiceNumber" name="invoiceNumber" value="${currentExpense.invoiceNumber }" type="text" size="30" />张</td>
										</tr>
									</table>
								</dd>
							</dl>

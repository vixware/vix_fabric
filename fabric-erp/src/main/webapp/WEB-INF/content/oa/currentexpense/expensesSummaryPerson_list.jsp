...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/oa/expensesSummaryPersonAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/expensesSummaryPersonAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/oa/expensesSummaryPersonAction!goSingleList.action?name="+name,'currentExpense');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/oa/expensesSummaryPersonAction!goSingleList.action?name=",'currentExpense');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/oa/expensesSummaryPersonAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'currentExpense');
}
bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/oa/expensesSummaryPersonAction!goSingleList.action?name="+name,'currentExpense');
}
</script>
<div class="sub_menu">
	<h2>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div>
			<label>内容:<input type="text" class="int" id="nameS" placeholder="主题"></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();" /><input type="button" value="<s:text name='cmn_reset'/>" class="btn" name="" onclick="resetForContent();" /></label>
			<%-- <label> <input type="button" value="高级搜索" class="btn" onclick="goSearch();" /></label> --%>
		</div>
		<%-- <div class="search_advanced">
			<label>编码:<input type="text" class="int" name="" id="code" placeholder="编码"></label> <label>主题:<input type="text" class="int" name="" id="selectName" placeholder="主题"></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn" name="" onclick="searchForContent();"><input type="button"

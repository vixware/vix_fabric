...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<th onclick="orderBy('model');" width="15%">型号&nbsp; <s:if test="%{pager.orderField == 'model' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if> <s:elseif test="%{pager.orderField == 'model' && pager.orderBy == 'desc' }">
			...
			<th>车辆类型</th>
			<th onclick="orderBy('plateNumber');" width="15%">车牌号&nbsp; <s:if test="%{pager.orderField == 'plateNumber' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if> <s:elseif test="%{pager.orderField == 'plateNumber' && pager.orderBy == 'desc' }">
					<img src="${vix}/common/img/arrow_up.gif">
				</s:elseif> <s:else>
					<img src="${vix}/common/img/arrow.gif">
				</s:else>
			</th>
			<th onclick="orderBy('engineNumber');" width="10%">发动机号码&nbsp; <s:if test="%{pager.orderField == 'engineNumber' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if> <s:elseif test="%{pager.orderField == 'engineNumber' && pager.orderBy == 'desc' }">
					<img src="${vix}/common/img/arrow_up.gif">
				</s:elseif> <s:else>
					<img src="${vix}/common/img/arrow.gif">
				</s:else>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td>${entity.model}</td>
				<td><a href="#" style="color: gray;">${entity.vehicleType.name}</a></td>
				<td>${entity.plateNumber}</td>
				<td>${entity.engineNumber}</td>
				<td><a href="#" style="color: gray;">${entity.vehicleColor.name}</a></td>

...
/** 保存办公用品借用 */
function saveOrUpdateOTB() {
 if ($('#order').validationEngine('validate')) {
	  $.post('${vix}/oa/officeSuppliesRegisterAction!saveOrUpdateBorrow.action', {
	...
	if($('#order').validationEngine('validate')){
		$.post('${vix}/oa/officeSuppliesRegisterAction!saveOrUpdateBorrow.action',{									  
			...
			function(result){
				showMessage(result);
				setTimeout("",1000);
				loadContent('${vix}/oa/officeSuppliesRegisterAction!goSaveOrUpdateBorrow.action');
			} 
		);
	}
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/oa/officeSuppliesRegisterAction!goList.action?pageNo=${pageNo}');">办公用借用</a></li>
			</ul>
		</div>
	</h2>
</div>
...
<div class="content">
	<form id="order">
		<input type="hidden" id="" name="officeSuppliesRegister.isTemp" value="2" />
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateOTB();"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="javascript:void(0)" onclick="saveOrAdd()"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <a href="#"
					...
					</span> <strong><b><s:text name="办公用品借用" /> </b><i></i> </strong>
				</dt>
				<dd class="clearfix">
								...
								<dd style="display: block;">
									<table style="border: none;">
										...
										<tr style="display: none;">
											<td><s:radio list="#{'2':'借用','1':'归还','0':'归还'}" id="isTemp" name="isTemp" value="%{officeSuppliesRegister.isTemp==2}" theme="simple"></s:radio></td>
										</tr>
									</table>
								</dd>
							</dl>
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;">
						...
						function showBookBorrow(id) {
								$.ajax({
									  url:'${vix}/oa/officeSuppliesRegisterAction!goAddOSBorrow.action?id='+id,
									  cache: false,
									  success: function(html){
										  asyncbox.open({
											 	modal:true,
                            ...
                            $('#dlAddress2').datagrid({
                            url : '${vix}/oa/officeSuppliesRegisterAction!getOSBorrowJson.action?id=${officeSuppliesRegister.id}',
                            width : 'auto',
                            height : 450,
                            pagination : true,
                            rownumbers : true,

...
function searchForContent(i){
	loadName();
	if(i == 0){
		pager("start","${vix}/oa/meetingQueryAction!goSearchList.action?i="+i+"&proposer="+name,'brand');
	}
}
...
function goSearch() {
	$.ajax({
	url : '${vix}/oa/meetingQueryAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/oa/meetingQueryAction!goSearchList.action?proposer=" + $('#proposer').val() 
 ...
 function leastRecentlyUsed(startTime){
 	pager("start","${vix}/oa/meetingQueryAction!goSingleList.action?startTime="+startTime,'brand');
 }
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/meetingQueryAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/oa/meetingQueryAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/meetingQueryAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/oa/meetingQueryAction!goSingleList.action?name="+name,'brand');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/oa/meetingQueryAction!goList.action?pageNo=${pageNo}');">会议室总览</a></li>
			</ul>
		</div>
	</h2>
</div>

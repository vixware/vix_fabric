...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/oa/vehicleApplicationsAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function searchForContent(i){
	loadName();
	if(i == 0){
		pager("start","${vix}/oa/vehicleApplicationsAction!goSearchList.action?i="+i+"&carName="+name,'trends');
	}
}
...
function goSearch() {
	$.ajax({
	url : '${vix}/oa/vehicleApplicationsAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/oa/vehicleApplicationsAction!goSearchList.action?carName=" + $('#carName').val() + "&theme=" + $('#theme').val() + "&reasons=" + $('#reasons').val() + "&pubNames=" + $('#pubNames').val() + "&startTime=" + $('#startTime').val() + "&endTime=" + $('#endTime').val(), 'trends');
			}
		},
...
//状态
 function saleOrderStatus(bookingSituation){
 	pager("start","${vix}/oa/vehicleApplicationsAction!goSingleList.action?bookingSituation="+bookingSituation,'trends');
 }
 //最近使用
 function leastRecentlyUsed(startTime){
 	pager("start","${vix}/oa/vehicleApplicationsAction!goSingleList.action?createTime="+startTime,'trends');
 }
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/vehicleApplicationsAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='车辆申请管理'/>",function(action){
				var treeNode = zTree.getNodeByTId($("#selectIdTreeId").val());
				if(null != treeNode){
					treeNode.isParent = true;
				}
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/oa/vehicleApplicationsAction!goSingleList.action?name="+name,'trends');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/vehicleApplicationsAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'trends');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/oa/vehicleApplicationsAction!goSingleList.action?name="+name,'trends');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/oa/vehicleApplicationsAction!goList.action?pageNo=${pageNo}');">车辆使用申请管理</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0,$('#selectId').val());"><span><s:text name='新增车辆使用申请' /></span></a>
		</p>
	</div>
</div>
<div class="content">

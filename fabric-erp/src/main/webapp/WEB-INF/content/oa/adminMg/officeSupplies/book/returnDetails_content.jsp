...
pager("start","${vix}/oa/bookLibrarianAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'newtab1');
}

bindSearch();
function currentPager(tag){
loadName();
pager(tag,"${vix}/oa/bookLibrarianAction!goSingleList.action?name="+name,'newtab1');
}
	...
	<table id="schedule" class="list">
		<tbody>
			<tr class="alt">
				...
				<th style="cursor: pointer;" onclick="orderBy('code');"><s:text name="图书编码" />&nbsp;<img id="newtab1Img_code" src="${vix}/common/img/arrow.gif"></th>
				<th style="cursor: pointer;" onclick="orderBy('bookName');"><s:text name="图书名称" />&nbsp;<img id="newtab1Img_bookName" src="${vix}/common/img/arrow.gif"></th>
				<th><s:text name="借书证号" /></th>
				<th><s:text name="借书人" /></th>
				<th style="cursor: pointer;" onclick="orderBy('startTime');"><s:text name="借书时间" />&nbsp;<img id="newtab1Img_startTime" src="${vix}/common/img/arrow.gif"></th>
				<th><s:text name="借书量" /></th>
				<th><s:text name="还书量" /></th>
			...
			<s:iterator value="pager.resultList" var="entity" status="s">
				...
				<tr>
					...
					<td><a href="#" style="color: gray;">${entity.bookName}</a></td>
					<td><a href="#" style="color: gray;">${entity.bookRegister.bookNumber}</a></td>
					<td><a href="#" style="color: gray;">${entity.bookRegister.recipientsWho}</a></td>
					<td><a href="#" style="color: gray;"><fmt:formatDate value="${entity.startTime}" type="both" pattern="yyyy-MM-dd" /></a></td>
					<td><a href="#" style="color: gray;">${entity.totalNumber}</a></td>
					<td><a href="#" style="color: gray;">${entity.returnNumber}</a></td>
					<td><a href="#" style="color: gray;">${entity.borrowNumber}</a></td>
					<td><a href="#" style="color: gray;"> <s:if test="%{#entity.totalNumber == #entity.borrowNumber}">未还</s:if> <s:elseif test="%{#entity.totalNumber > #entity.borrowNumber  && #entity.borrowNumber != 0}">部分归还</s:elseif> <s:elseif test="%{#entity.borrowNumber == 0}">全退</s:elseif>
					</a></td>
					<td>
						<div class="untitled">
							<a href="#" onclick="saveOrUpdateReturn('${entity.id}');" title="<s:text name='归还'/>"><img src="${vix}/common/img/icon_edit.png" /></a>
						</div>
					</td>
				</tr>
			</s:iterator>

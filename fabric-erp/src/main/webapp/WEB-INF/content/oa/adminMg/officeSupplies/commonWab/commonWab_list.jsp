...
function searchForContent(i){
	loadName();
	if(i == 0){
		pager("start","${vix}/oa/commonWabAction!goSearchList.action?i="+i+"&mobileno="+name1,'commonWab');
	}
}
...
function goSearch() {
	$.ajax({
	url : '${vix}/oa/commonWabAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/oa/commonWabAction!goSearchList.action?name=" + $('#name').val() 
						+ "&company=" + $('#company').val()
...
function saveOrUpdate(id){
	$.ajax({
		  url:"${vix}/oa/commonWabAction!goSaveOrUpdate.action?id="+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#brandForm').validationEngine('validate')){
								$.post('${vix}/oa/commonWabAction!saveOrUpdate.action',
										{'wab.id':$("#id").val(),
										 'wab.name':$("#name").val(),
									...
									function(result){
										asyncbox.open({
											...
											width : 320,
											height : 50,
											title:"<s:text name='公共通讯簿'/>",
											html:result,
											callback : function(action){
												pager("current","${vix}/oa/commonWabAction!goSingleList.action?name="+name,'commonWab');
											},
											btnsbar : $.btn.OKCANCEL
										});
									}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/commonWabAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/oa/commonWabAction!goSingleList.action?name="+name,'commonWab');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/oa/commonWabAction!goSingleList.action?name=",'commonWab');
}

//排序 
function orderBy(orderField){
	loadName();
	var orderBy = $("#commonWabOrderBy").val();
	if(orderBy == 'desc'){
		orderBy = "asc";
	}else{
		orderBy = 'desc';
	}
	pager("start","${vix}/oa/commonWabAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'commonWab');
}
...
function currentPager(tag){
	loadCode();
	pager(tag,"${vix}/oa/commonWabAction!goSingleList.action?name="+name+'&orderStatus='+orderStatus,'commonWab');
}
...
function showOrder(id){
	$.ajax({
		  url:'${vix}/oa/commonWabAction!showOrder.action?id='+id,
		  cache: false,
		  success: function(html){
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">行政办公</a></li>
				<li><a href="#" onclick="loadContent('${vix}/oa/commonWabAction!goList.action?pageNo=${pageNo}');">公告通讯簿</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		...
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/oa/commonWabAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/oa/commonWabAction!goSingleList.action?parentId="+treeNode.id,"commonWab");
				}
		...
		<div id="right">
			...
			<div class="right_content" id="a1">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="commonWabPager('start','commonWab');"></a></span> <span><a class="previous" onclick="commonWabPager('previous','commonWab');"></a></span> <em>(<b class="commonWabInfo"></b> <s:text name='cmn_to' /> <b class="commonWabTotalCount"></b>)
						</em> <span><a class="next" onclick="commonWabPager('next','commonWab');"></a></span> <span><a class="end" onclick="commonWabPager('end','commonWab');"></a></span>
					</div>
				</div>
				<div id="commonWab" class="table"></div>
				<div class="pagelist drop">
					<%-- <ul>
						<li class="ed">

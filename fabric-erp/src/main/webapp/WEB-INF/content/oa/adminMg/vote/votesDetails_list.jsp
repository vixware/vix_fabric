...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/oa/votesDetailsAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/votesDetailsAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/oa/votesDetailsAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/votesDetailsAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/oa/votesDetailsAction!goSingleList.action?name="+name,'brand');
}
...
function searchForContent(i){
	loadName();
	if(i == 0){
		pager("start","${vix}/oa/votesDetailsAction!goSearchList.action?i="+i+"&searchContent="+name,'brand');
	}
	else{
		pager("start","${vix}/oa/votesDetailsAction!goSearchList.action?i="+i+"&purchasePerson="+purchasePerson+"&name="+purchaseName,'brand');
	}
}
/*重置搜索*/
...
function purStatus(status){
	pager("start","${vix}/oa/votesDetailsAction!goSingleList.action?status="+status,'brand');
}
//最近使用
function purRecent(rencentDate){
	pager("start","${vix}/oa/votesDetailsAction!goSingleList.action?updateTime="+rencentDate,'brand');
}
...
function goSearch() {
	$.ajax({
	url : '${vix}/oa/votesDetailsAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/oa/votesDetailsAction!goSearchList.action?title=" + $('#title').val() + "&uploadPersonName=" + $('#uploadPersonName').val()+ "&pubNames=" + $('#pubNames').val(), 'brand');
			}
		},
...
<div class="sub_menu">
	...
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0,$('#selectId').val());"><span><s:text name='新建子投票' /></span></a>
		</p>
	</div>
</div>
<div class="content">
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="purchaseArrivalList" var="c">

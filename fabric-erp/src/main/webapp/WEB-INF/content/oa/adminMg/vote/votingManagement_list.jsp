...
function searchForContent(i){
	loadName();
	if(i == 0){
		pager("start","${vix}/oa/votingManagementAction!goSearchList.action?i="+i+"&title="+name,'voting');
	}
}

//状态
 function saleOrderStatus(publishType){
 	pager("start","${vix}/oa/votingManagementAction!goSingleList.action?publishType="+publishType,'voting');
 }
 //最近使用
 function leastRecentlyUsed(startTime){
 	pager("start","${vix}/oa/votingManagementAction!goSingleList.action?startTime="+startTime,'voting');
 }


 /*投票结果 */
 function votingOrResult(id){
 	$.ajax({
 		  url:"${vix}/oa/votingManagementAction!goVotingOrResult.action?id="+id,
 		  cache: false,
...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/oa/votingManagementAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
...
/* 控制发布或者终止、待发布 */
function updateIsPublish(id,publishType){
	var param={'id':id};  
	$.ajax({ 
		url:"${vix}/oa/votingManagementAction!updateIsPublish.action",
		data:param,
		dataType:"text",
		success:function(data){
			/* alert('修改成功'); */
			loadName();
			pager("start","${vix}/oa/votingManagementAction!goSingleList.action?name="+name,'voting');
		}
		});
}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/votingManagementAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/votingManagementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'voting');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/oa/votingManagementAction!goSingleList.action?name="+name,'voting');
}
...
function goSearch() {
	$.ajax({
	url : '${vix}/oa/votingManagementAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/oa/votingManagementAction!goSearchList.action?title=" + $('#title').val()
...
<div class="sub_menu">
	...
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0,$('#selectId').val());"><span><s:text name='oa_new_voting' /></span></a>
		</p>
	</div>
</div>
<div class="content">

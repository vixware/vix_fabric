...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/oa/od/odReceiveAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/od/odReceiveAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/oa/od/odReceiveAction!goSingleList.action?code="+code,'order');
			});
		  }
	});
}
...
function showOrder(id){
	$.ajax({
		  url:'${vix}/oa/od/odReceiveAction!showOrder.action?id='+id,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function dengjiOd1(){
	if(confirm('确认登记吗')){
		loadContent('${vix}/oa/od/odReceiveAction!goList.action');
	}
}
function dengjiOd2(){
	if(confirm('确认删除吗')){
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/od/odReceiveAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&code="+code,'order');
}

bindSearch();
function currentPager(tag){
	loadCode();
	pager(tag,"${vix}/oa/od/odReceiveAction!goSingleList.action?code="+code,'order');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/oa/odReceiveAction!goList.action?pageNo=${pageNo}');"><s:text name="收文登记" /> </a></li>
			</ul>
		</div>
	</h2>
	<%-- <div class="drop">

...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/oa/od/odReceiveDistributeAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/od/odReceiveDistributeAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/oa/od/odReceiveDistributeAction!goSingleList.action?code="+code,'order');
			});
		  }
	});
}
...
function showOrder(id){
	$.ajax({
		  url:'${vix}/oa/od/odReceiveDistributeAction!showOrder.action?id='+id,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function dengjiOd(){
	if(confirm('确认分发吗？')){
		loadContent('${vix}/oa/od/odReceiveDistributeAction!goList.action');
	}
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/od/odReceiveDistributeAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&code="+code,'order');
}

bindSearch();
function currentPager(tag){
	loadCode();
	pager(tag,"${vix}/oa/od/odReceiveDistributeAction!goSingleList.action?code="+code,'order');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/oa/odReceiveDistributeAction!goList.action?pageNo=${pageNo}');"><s:text name="收文分发" /> </a></li>
			</ul>
		</div>
	</h2>
	<%-- <div class="drop">

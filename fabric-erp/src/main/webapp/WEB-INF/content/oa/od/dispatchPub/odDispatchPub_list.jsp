...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/oa/od/odDispatchPubAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/od/odDispatchPubAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/oa/od/odDispatchPubAction!goSingleList.action?code="+code,'order');
			});
		  }
	});
}
...
function showOrder(id){
	$.ajax({
		  url:'${vix}/oa/od/odDispatchPubAction!showOrder.action?id='+id,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function pubOd(){
	if(confirm('确认发送吗')){
		loadContent('${vix}/oa/od/odDispatchPubAction!goList.action');
	}
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/od/odDispatchPubAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&code="+code,'order');
}

bindSearch();
function currentPager(tag){
	loadCode();
	pager(tag,"${vix}/oa/od/odDispatchPubAction!goSingleList.action?code="+code,'order');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/oa/odDispatchPubAction!goList.action?pageNo=${pageNo}');"><s:text name="公文发布" /> </a></li>
			</ul>
		</div>
	</h2>
	<%-- <div class="drop">

...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/oa/accomodationsExpensesStandardsAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#areaExpensesStandardsForm').validationEngine('validate')){
							$.post('${vix}/oa/accomodationsExpensesStandardsAction!saveOrUpdate.action',
									 {
									  'areaExpensesStandards.id':$("#areaExpensesStandardsId").val(),
									  'areaExpensesStandards.code':$("#code").val(),
									  'areaExpensesStandards.standardName':$("#standardName").val(),
									  'areaExpensesStandards.orgPosition.id':$("#parentPosId").val(),
									  'areaExpensesStandards.areaLevel.id':$("#areaLevelId").val(),
									  'areaExpensesStandards.reimbursementAmount':$("#reimbursementAmount").val()
									},
									function(result){
										showMessage(result);
										setTimeout("", 1000);
										loadContent('${vix}/oa/accomodationsExpensesStandardsAction!goList.action');
									}
								 );
							}else {
								return false;
...
function searchForRightContent(){
	loadName();
	pager("start","${vix}/oa/accomodationsExpensesStandardsAction!goSingleList.action?name="+name,'areaExpensesStandards');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/accomodationsExpensesStandardsAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'areaExpensesStandards');
}

function categoryPager(tag){
	loadName();
	pager(tag,"${vix}/oa/accomodationsExpensesStandardsAction!goSingleList.action?name="+name,'areaExpensesStandards');
}
bindSearch();
</script>
<div class="sub_menu">
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/oa/accomodationsExpensesStandardsAction!goSingleList.action?id="+treeNode.id+"&treeType="+treeNode.treeType,"areaExpensesStandards");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'areaExpensesStandards')"><img src="img/mail.png" alt="" /> 报销标准 </a></li>
				</ul>
			</div>
			<div class="right_content">
				...
				<div id="areaExpensesStandards" class="table"></div>
				<div class="pagelist drop">
					<ul>
						<li class="ed"><a href="#"><s:text name='cmn_choose' /> </a>
							<ul>

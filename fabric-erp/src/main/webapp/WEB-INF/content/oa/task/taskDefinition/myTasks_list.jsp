...
function searchForContent(i){
	...
	if(i == 0){
		...
		pager("start","${vix}/oa/taskDefineAction!goSearchList.action?i="+i+"&questName="+questName+"&executiveAgency="+executiveAgency,'task');
	}
}
...
function goSearch() {
	...
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/oa/taskDefineAction!goSearchList.action?questName=" + $('#questName').val() + "&executiveAgency=" + $('#executiveAgency').val(), 'task');
			}
		},
...
//状态
 function saleOrderStatus(complete){
 	pager("start","${vix}/oa/myTasksAction!goSingleList.action?complete="+complete,'task');
 }
 //最近使用
 function leastRecentlyUsed(endTime){
 	pager("start","${vix}/oa/myTasksAction!goSingleList.action?endTime="+endTime,'task');
 }
...
/* $(function(){
 ...
 function saveOrUpdate(id,taskId){
	/*  alert('${vix}/oa/task/taskDefinition/feedbackAction!goSaveOrUpdate.action?id='+id+"&taskId="+taskId) */
	$.ajax({
		  url:'${vix}/oa/task/taskDefinition/feedbackAction!goSaveOrUpdate.action?id='+id+"&taskId="+taskId,
		  cache: false,
		  success: function(html){
 ...
 function saveOrUpdate1(id){
	$.ajax({
		  url:'${vix}/oa/task/taskDefinition/myTasksAction!goSaveOrUpdate1.action?id='+id,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
	...
	function updateComplete(id,complete){
		var param={'id':id};  
		$.ajax({ 
			url:"${vix}/oa/task/taskDefinition/myTasksAction!updateComplete.action",
			data:param,
			dataType:"text",
			success:function(data){
			    if(data==0){
					$("#complete"+id).text("完成");
					$("#ip_link_"+id).text("未完成");
			    }else if(data==1){
			    	$("#complete"+id).text("未完成");
					$("#ip_link_"+id).text("完成");
			    }else{
					alert('return:'+data);
			    }
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/task/taskDefinition/myTasksAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/oa/task/taskDefinition/myTasksAction!goSingleList.action?name="+name,'task');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/task/taskDefinition/myTasksAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'task');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/oa/task/taskDefinition/myTasksAction!goSingleList.action?name="+name,'task');
}

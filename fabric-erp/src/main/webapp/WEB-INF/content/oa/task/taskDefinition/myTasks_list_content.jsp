...
function deleteEntity(id){
	asyncbox.confirm('确定要删除该任务么?','<s:text name='任务'/>',function(action){
		if(action == 'ok'){
			deleteById(id);
		}
	});
...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<th width="160" style="cursor: pointer;" onclick="orderBy('questName');">主题&nbsp;<img id="taskImg_questName" src="${vix}/common/img/arrow.gif">
			</th>
			...
			<th><s:text name="执行情况" /></th>
			<th style="cursor: pointer;" onclick="orderBy('updateTime');">更新时间&nbsp;<img id="taskImg_updateTime" src="${vix}/common/img/arrow.gif">
			</th>
			<th style="cursor: pointer;" onclick="orderBy('validity');">有效期&nbsp;<img id="taskImg_validity" src="${vix}/common/img/arrow.gif">
			</th>
			<th style="cursor: pointer;" onclick="orderBy('endTime');">完成时间&nbsp;<img id="taskImg_endTime" src="${vix}/common/img/arrow.gif">
			</th>
			<th><s:text name="完成状态" /></th>
			...
			<s:iterator value="pager.resultList" var="entity" status="s">
				...
				<tr>
					...
					<td><a href="#" style="color: gray;"><fmt:formatDate value="${entity.updateTime}" type="both" pattern="yyyy-MM-dd" /></a></td>
					<td><a href="#" style="color: gray;">${entity.validity}</a></td>
					...
					<td>
						<div class="untitled" style="position: static; display: inline;">
							<s:if test="%{#entity.complete==1 }">
								<a href="#" id="ip_link_${entity.id}" onclick="updateComplete('${entity.id}',${entity.complete})"> <span id="entity_${entity.id}">完成</span>
								</a>
							</s:if>
							<s:elseif test="%{#entity.complete==0 }">
								<a href="#" id="ip_link_${entity.id}" onclick="updateComplete('${entity.id}',${entity.complete})"> <span id="entity_${entity.id}" style="color: green;">生效</span>
								</a>
							</s:elseif>
						</div>
						<div class="untitled" style="position: static; display: inline;">
							<a href="#" onclick="saveOrUpdate1('${entity.id}');">查看</a>
						</div>
						<div class="untitled" style="position: static; display: inline;">
							<a href="#" onclick="saveOrUpdate(0,'${entity.id}');">执行反馈</a>
						</div>
					</td>
				</tr>
			</s:iterator>

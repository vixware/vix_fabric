...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/oa/task/typeSettings/progressSituationAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#progressSituationForm').validationEngine('validate')){
								$.post('${vix}/oa/task/typeSettings/progressSituationAction!saveOrUpdate.action',
									 {'progressSituation.id':$("#id").val(),
									  'progressSituation.code':$("#code").val(),
									  'progressSituation.name':$("#name").val(),
									  /* 'progressSituation.disabled':$('input:radio[name=disabled]').val() */
									  'progressSituation.disabled':$(":radio[name=disabled][checked]").val()
									},
									function(result){
										asyncbox.success(result,"<s:text name='保存进度情况'/>",function(action){
											pager("current","${vix}/oa/task/typeSettings/progressSituationAction!goListContent.action?name="+name,'currencyType');
										});
									}
								 ); 
			  				}else{
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/task/typeSettings/progressSituationAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/oa/task/typeSettings/progressSituationAction!goListContent.action?name="+name,'currencyType');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/task/typeSettings/progressSituationAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'currencyType');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/oa/task/typeSettings/progressSituationAction!goListContent.action?name="+name,'currencyType');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">任务管理</a></li>
				<li><a href="#">初始设置</a></li>
				<li><a href="#" onclick="loadContent('${vix}/oa/task/typeSettings/progressSituationAction!goList.action');">进度情况</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

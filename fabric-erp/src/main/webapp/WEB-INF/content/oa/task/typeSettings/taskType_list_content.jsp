...
function deleteEntity(id){
	asyncbox.confirm('确定要删除该类型么?','<s:text name='任务类型'/>',function(action){
		if(action == 'ok'){
			deleteById(id);
		}
	});
...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<%-- <th onclick="orderBy('notDeleted');" width="20%">
				不能删除&nbsp;
				<s:if test="%{pager.orderField == 'notDeleted' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if>
				<s:elseif test="%{pager.orderField == 'notDeleted' && pager.orderBy == 'desc' }">
					<img src="${vix}/common/img/arrow_up.gif">
				</s:elseif>
				<s:else>
					<img src="${vix}/common/img/arrow.gif">
			...
			<th onclick="orderBy('name');" width="25%">任务类型&nbsp; <s:if test="%{pager.orderField == 'name' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">

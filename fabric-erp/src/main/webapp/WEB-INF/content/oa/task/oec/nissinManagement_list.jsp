...
function searchForContent(i){
	loadName();
	if(i == 0){
		pager("start","${vix}/oa/nissinManagementAction!goSearchList.action?i="+i+"&questName="+name,'task');
	}
	else{
		pager("start","${vix}/oa/nissinManagementAction!goSearchList.action?i="+i+"&questName="+questName+"&executiveAgency="+executiveAgency,'task');
	}
}
...
function goSearch() {
	$.ajax({
	url : '${vix}/oa/nissinManagementAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/oa/nissinManagementAction!goSearchList.action?questName=" + $('#questName').val() + "&executiveAgency=" + $('#executiveAgency').val(), 'task');
			}
		},
...
/* $(function(){
 ...
 function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/oa/oatask/oec/nissinManagementAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/oatask/oec/nissinManagementAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/oa/oatask/oec/nissinManagementAction!goSingleList.action?name="+name,'task');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/oatask/oec/nissinManagementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'task');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/oa/oatask/oec/nissinManagementAction!goSingleList.action?name="+name,'task');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">任务管理</a></li>
				<li><a href="#" onclick="loadContent('${vix}/oa/nissinManagementAction!goList.action?pageNo=${pageNo}');">日清管理</a></li>
			</ul>
		</div>
	</h2>
	<%-- <div class="drop">
		<p>
			...
			<a href="#" onclick="modifyByIds(0);"><span><s:text name='修改'/></span></a>
			<a href="#" onclick="viewByIds(0);"><span><s:text name='查看'/></span></a>
			<a href="#" onclick="deleteByIds();"><span><s:text name='cmn_delete'/></span></a>
			<a href="#" onclick="issuedByIds();"><span><s:text name='下达'/></span></a>
			<a href="#" onclick="executionOrFeedback();"><span><s:text name='执行反馈'/></span></a>
			<a href="#" onclick="evaluationOrAssessment();"><span><s:text name='评价评估'/></span></a>
		</p>
	</div> --%>
</div>
<div class="content">

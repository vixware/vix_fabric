...
function saveOrUpdateOrder(){
	if($('#orderForm').validationEngine('validate')){
		$.post('${vix}/oa/personalAttendanceAction!saveOrUpdate.action',
			{'personalAttendance.id':$("#id").val(),									  
			  'personalAttendance.approver':$("#approver").val(),						  
			  'personalAttendance.outReason' : outReasons.html(),
			  'personalAttendance.applicationTime':$("#applicationTime").val(),	
			  'personalAttendance.outstartDate':$("#outstartDate").val(),	
			  'personalAttendance.minutes':$("#minutes").val(),	
			  'personalAttendance.dates':$("#dates").val(),	
			  'personalAttendance.outendDate':$("#outendDate").val(),
			  'personalAttendance.isPublish':$('input:radio[name=isPublish]:checked').val()
			},
			function(result){
				showMessage(result);
				setTimeout("",1000);
	...
	if($('#orderForm').validationEngine('validate')){
		$.post('${vix}/oa/personalAttendanceAction!saveOrUpdate.action',
			...
			function(result){
				showMessage(result);
				setTimeout("",1000);
				loadContent('${vix}/oa/personalAttendanceAction!goSaveOrUpdate.action');
			} 
		);
	}
}
...
function calculation() {
    ...
    var startTime = new Date(Date.parse(outstartDate.replace(/-/g, "/")));
    var endTime = new Date(Date.parse(outendDate.replace(/-/g, "/"))); 
...
$(function() {
    //加载更新时间(新增)
    if (document.getElementById("applicationTime").value == "") {
	    var myDate = new Date();
	    $("#applicationTime").val(myDate.getFullYear() + "-" + (myDate.getMonth() + 1) + "-" + myDate.getDate() + " " + myDate.getHours()+ ":" + myDate.getMinutes()+ ":" + myDate.getSeconds());
    }
}); 
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/oa/personalAttendanceAction!goList.action?pageNo=${pageNo}');">外出登记 </a></li>
			</ul>
		</div>
	</h2>
</div>
		...
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					...
					</span> <strong><b><s:text name="外出登记" /> </b><i></i> </strong>
				</dt>
				<dd class="clearfix">
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id="applicationTime" name="personalAttendance.applicationTime" value="${personalAttendance.applicationTime}" onchange="salesOrderFieldChanged(this);" type="text" style="width: 200px; height: 22px;" validate[required] text-input"  /></td>
										</tr>
										<tr>
											<th align="right">负责人：</th>
											<td><input id="approver" name="approver" value="${personalAttendance.approver}" type="text" style="width: 200px; height: 22px;" validate[required] text-input" /> <span style="color: red;">*</span> <input type="hidden" id="parentId" name="parentId" value="${hrCategory.parentCategory.id }" /> <input class="btn" type="button"
												value="选择" onclick="chooseEmployees();" /></td>
										</tr>
										<tr>
											<th align="right">外出时间：</th>
											<td><input id="outstartDate" type="text" name="outstartDate" value="${personalAttendance.outstartDate}" style="width: 200px; height: 22px;" validate[required] text-input" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',skin:'blue'})"><span style="color: red;">*</span>至 <input id="outendDate" type="text" name="outendDate"
												value="${personalAttendance.outendDate}" style="width: 200px; height: 22px;" validate[required] text-input" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',skin:'blue'})"><span style="color: red;">*</span> <input class="btn" type="button" value="计算天数" onclick="calculation();" /> <input id="dates" type="text" name="dates"
												style="width: 20px;" value="${personalAttendance.dates}">天 <input id="minutes" type="text" name="minutes" style="width: 20px;" value="${personalAttendance.minutes}">小时</td>
										</tr>
										<tr>
											<th align="right">是否用车：</th>
											<td><s:radio list="#{'0':'是','1':'否'}" id="isPublish" name="isPublish" value="%{personalAttendance.isPublish}" theme="simple"></s:radio></td>
										</tr>
										<tr>
											<th align="right">外出原因：</th>
											<td colspan="3"><textarea id="outReason" name="outReason">${personalAttendance.outReason}</textarea> <script type="text/javascript">
													 var outReasons = KindEditor.create('textarea[name="outReason"]',

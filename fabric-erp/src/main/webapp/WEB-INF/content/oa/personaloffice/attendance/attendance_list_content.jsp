...
function deleteEntity(id,a){
	asyncbox.confirm('确定要删除该外出记录么?','提示信息',function(action){
		if(action == 'ok'){
			deleteById(id,a);
		}
	});
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/personalAttendanceAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'newtab1');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/oa/personalAttendanceAction!goSingleList.action?name="+name,'newtab1');
}
	...
	<table id="table" class="list">
		<tbody>
			<tr class="alt">
				...
				<th style="cursor: pointer;" onclick="orderBy('uploadPersonName');">外出人&nbsp;<img id="newtab1Img_uploadPersonName" src="${vix}/common/img/arrow.gif">
				</th>
				<th width="27%" style="cursor: pointer;" onclick="orderBy('outReason');">外出原因&nbsp;<img id="newtab1Img_outReason" src="${vix}/common/img/arrow.gif">
				</th>
				<th style="cursor: pointer;" onclick="orderBy('outstartDate');">外出时间&nbsp;<img id="newtab1Img_outstartDate" src="${vix}/common/img/arrow.gif">
				</th>
				<th style="cursor: pointer;" onclick="orderBy('outendDate');">结束时间&nbsp;<img id="newtab1Img_outendDate" src="${vix}/common/img/arrow.gif">
				</th>
				<th>外出时长</th>
				<th>是否用车</th>
			...
			<s:iterator value="pager.resultList" var="entity" status="s">
				...
				<tr>
					...
					<td><a href="#" style="color: gray;">${entity.outReason}</a></td>
					<td><a href="#" style="color: gray;"><fmt:formatDate value="${entity.outstartDate}" type="both" pattern="yyyy-MM-dd HH:mm:ss" /></a></td>
					<td><a href="#" style="color: gray;"><fmt:formatDate value="${entity.outendDate}" type="both" pattern="yyyy-MM-dd HH:mm:ss" /></a></td>
					<td><a href="#" style="color: gray;">${entity.dates}天${entity.minutes}小时</a></td>
					<td id="isPublish_${entity.id}"><s:if test="%{#entity.isPublish == 0}">是</s:if> <s:elseif test="%{#entity.isPublish == 1}">否</s:elseif></td>
					<td>
						<div class="untitled" style="position: static; display: inline;">
							<s:if test="tag != 'choose'">
								<a href="#" onclick="saveOrUpdate('${entity.id}',$('#parentId').val());">修改</a>
							</s:if>
							<a href="#" onclick="deleteEntity('${entity.id}',this);">删除</a>
						</div>
					</td>
				</tr>
			</s:iterator>

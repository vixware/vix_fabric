...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/oa/flow/oaFlowWorkAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/flow/oaFlowWorkAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/oa/flow/oaFlowWorkAction!goSingleList.action?code="+code,'order');
			});
		  }
	});
}
...
function showOrder(id){
	$.ajax({
		  url:'${vix}/oa/flow/oaFlowWorkAction!showOrder.action?id='+id,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/flow/oaFlowWorkAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&code="+code,'order');
}

bindSearch();
function currentPager(tag){
	loadCode();
	pager(tag,"${vix}/oa/flow/oaFlowWorkAction!goSingleList.action?code="+code,'order');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="oa_flow_work" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div>
			<label><s:text name="cmn_name" /><input type="text" class="int" id="codeS"></label> <label>工作类型：<s:select list="#{'gw':'公文','xz':'行政','rs':'人事','cw':'财务','sc':'生产','xs':'销售'}" id="gwType" name="entityForm.gwType" theme="simple"></s:select></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn"
				onclick="searchForContent();" /><input type="button" value="<s:text name='cmn_reset'/>" class="btn" name="" /></label> <strong id="lb_search_advanced"><s:text name="cmn_advance_search" /></strong>
		</div>

...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/oa/flow/oaFlowLogSearchAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/oa/flow/oaFlowLogSearchAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/oa/flow/oaFlowLogSearchAction!goSingleList.action?code="+code,'order');
			});
		  }
	});
}
...
function showOrder(id){
	$.ajax({
		  url:'${vix}/oa/flow/oaFlowLogSearchAction!showOrder.action?id='+id,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function orderBy(orderField){
	...
	pager("start","${vix}/oa/flow/oaFlowLogSearchAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&code="+code,'order');
}

bindSearch();
function currentPager(tag){
	loadCode();
	pager(tag,"${vix}/oa/flow/oaFlowLogSearchAction!goSingleList.action?code="+code,'order');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="oa_flow_logsearchwork" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#" onclick="javascript:void(0);"><span>删除</span></a> <a href="#" onclick="javascript:void(0);"><span>导出工作表</span></a>
		</p>
	</div>
</div>
<div class="content">
	<div class="drop" id="c_head">
		...
		<div>
			<label>流程名称：<input type="text" class="int" id="codeS"></label> <label>发生时间：<input type="text" class="time_aoto_input ipt" name="" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',skin:'blue'})">－<input type="text" class="time_aoto_input ipt" name="" onfocus="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',skin:'blue'})"><input
				type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();" /><input type="button" value="<s:text name='cmn_reset'/>" class="btn" name="" /></label> <strong id="lb_search_advanced"><s:text name="cmn_advance_search" /></strong>
		</div>

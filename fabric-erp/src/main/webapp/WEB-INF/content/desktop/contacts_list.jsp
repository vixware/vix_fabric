...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/common/contactsAction!goAddContacts.action?id='+id,
		  cache: false,
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#contactsForm').validationEngine('validate')){
								$.post('${vix}/common/contactsAction!saveOrUpdate.action', {
										'contacts.id' : $("#cId").val(),
										'contacts.code' : $("#newCode").val(),
										'contacts.name' : $("#newName").val(),
										'contacts.contactsCategory.id' : $("#parentId").val(),
										'contacts.contactsCategory.name' : $("#catalog").val(),
										'contacts.age' : $("#newAge").val(),
										'contacts.sex' : $("#newSex").val(),
										'contacts.telephone' : $("#newTelephone").val()
										}, function(result){
											showMessage(result);
											setTimeout("",1000);
											pager("start","${vix}/common/contactsAction!goSingleList.action?code="+code+"&pageNo=${pageNo}",'category');
										});
							}else{
			  					return false;
			  				}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/contactsAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/common/contactsAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
function searchForContent(i){
	...
	loadContacts();
	if(i == 0){
		pager("start","${vix}/common/contactsAction!goSearchList.action?i="+i+"&searchContent="+code,'category');
	}
	else{
		pager("start","${vix}/common/contactsAction!goSearchList.action?i="+i+"&contacts="+contacts+"&name="+name,'category');
	}
}
/*判断搜索内容是否为空*/
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/common/contactsAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/common/contactsAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadCode();
	if(entity == 'category'){
		pager(tag,"${vix}/common/contactsAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function srmStatus(status){
	pager("start","${vix}/common/contactsAction!goSingleList.action?status="+status,'category');
}
//最近使用
function srmRecent(rencentDate){
	pager("start","${vix}/common/contactsAction!goSingleList.action?updateTime="+rencentDate,'category');
}
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#"><img src="img/srm_supplier.png" alt="" />工作台</a></li>
				<li><a href="#">通讯录</a></li>
			</ul>
		</div>
	</h2>
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/common/contactsAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/common/contactsAction!goSingleList.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/mail.png" alt="" />联系人</a></li>
				</ul>
			</div>

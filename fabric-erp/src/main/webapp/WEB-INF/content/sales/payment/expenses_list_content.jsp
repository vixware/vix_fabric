...
<input type="hidden" id="expensesTotalCountHidden" value="${pager.totalCount}" />
<input type="hidden" id="expensesPageNoHidden" value="${pager.pageNo}" />
<input type="hidden" id="expensesTotalPageHidden" value="${pager.totalPage}" />
<input type="hidden" id="expensesOrderField" value="${pager.orderField}" />
<input type="hidden" id="expensesOrderBy" value="${pager.orderBy}" />
<input type="hidden" id="expensesInfoHidden" value="${(pager.pageNo-1)*pager.pageSize+1}-${(pager.pageNo-1)*pager.pageSize+pager.pageSize}" />
<table class="list">
	<tbody>
		<tr class="alt">
			<th width="50">
			...
			<th width="15%"><span style="cursor: pointer;" onclick="orderBy('expensesDate');"> 支付日期&nbsp; <s:if test="%{pager.orderField == 'expensesDate' && pager.orderBy == 'asc' }">
						<img src="${vix}/common/img/arrow_down.gif">
					</s:if> <s:elseif test="%{pager.orderField == 'expensesDate' && pager.orderBy == 'desc' }">
						<img src="${vix}/common/img/arrow_up.gif">
					</s:elseif> <s:else>
						<img src="${vix}/common/img/arrow.gif">
					</s:else>
			</span></th>
			<th width="20%"><span style="cursor: pointer;"> 支付单号&nbsp; <s:if test="%{pager.orderField == 'expensesBillNumber' && pager.orderBy == 'asc' }">
						<img src="${vix}/common/img/arrow_down.gif">
					</s:if> <s:elseif test="%{pager.orderField == 'expensesBillNumber' && pager.orderBy == 'desc' }">
						<img src="${vix}/common/img/arrow_up.gif">
					</s:elseif> <s:else>
						<img src="${vix}/common/img/arrow.gif">
					</s:else>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td><s:property value="formatDateToString(#entity.expensesDate)" /></td>
				<td>${entity.expensesBillNumber}</td>

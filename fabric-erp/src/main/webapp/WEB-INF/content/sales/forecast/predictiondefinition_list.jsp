...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:'${vix}/sales/predictionDefinitionAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							$.post('${vix}/sales/predictionDefinitionAction!saveOrUpdate.action',
								 {'category.id':$("#id").val(),
								...
								function(result){
									asyncbox.success(result,"<s:text name='cmn_message'/>",function(action){
										...
										pager("start","${vix}/sales/predictionDefinitionAction!goListContent.action?name="+name+"&id="+$("#selectId").val(),'category');
									});
								}
							 );
						}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/sales/predictionDefinitionAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/sales/predictionDefinitionAction!goListContent.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/sales/predictionDefinitionAction!goListContent.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/sales/predictionDefinitionAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/sales/predictionDefinitionAction!goListContent.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name='sa_salesforecast' /></a></li>
				<li><a href="#">预测定义</a></li>
			</ul>
		</div>
	</h2>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div class="search_advanced">
			<label>名称：<input name="" type="text" class="int" /></label> <label>时间：<input name="" type="text" class="int" /></label>
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/sales/predictionDefinitionAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/sales/predictionDefinitionAction!goListContent.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/mail.png" alt="" />预测定义</a></li>
				</ul>
			</div>

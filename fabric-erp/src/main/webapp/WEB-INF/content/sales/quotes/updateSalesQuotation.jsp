...
function saveOrUpdateSalesQuotation(){
	if($('#salesQuotationForm').validationEngine('validate')){
		$.post('${vix}/sales/quotes/salesQuotationAction!saveOrUpdate.action',
			{'salesQuotation.id':$("#id").val(),
			  'salesQuotation.billDate':$("#billDate").val(),
			  'salesQuotation.customerAccount.id':$("#customerAccountId").val(),
			...
			function(result){
				showMessage(result);
				setTimeout("",1000);
				loadContent('${vix}/sales/quotes/salesQuotationAction!goList.action');
			}
		);
	}
}
...
function relatedSalesOrder(){
	$.ajax({
		  url:"${vix}/sales/order/salesOrderAction!goRelatedSalesOrder.action?id="+$("#id").val()+"&type=salesQuotation",
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
...
<input type="hidden" id="id" name="id" value="${salesQuotation.id}" />
		...
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateSalesQuotation();" href="#"><img width="22" height="22" alt="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" alt="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <s:if test="isAllowAudit == 1">
							<a onclick="saveOrUpdateOrder('approval');" href="###"><img width="22" height="22" alt="保存并提交审批" src="${vix}/common/img/dt_aprroval.png" /></a>
						</s:if>
						<div class="ms">
							<a href="#"><img width="22" height="22" alt="" src="img/icon_add.gif"></a>
							<ul style="display: none;">
								<li><a href="###" onclick="relatedSalesOrder();"><img src="${vix}/common/img/sale/saleOrder.png" alt="">销售订单</a></li>
								<li><a href="###" class=""><img src="img/icon_10.png" alt="">销售发货单</a></li>
							</ul>
						</div> <a href="#" onclick="loadContent('${vix}/sales/quotes/salesQuotationAction!goList.action?pageNo=${pageNo}');"><img width="22" height="22" alt="返回" src="${vix}/common/img/dt_goback.png" /></a>
					</span> <strong> <b> <s:if test="salesQuotation.code != null ">
								...
								<dd style="display: block;">
									<table>
										<tr>
											...
											<td width="35%"><input id="code" name="salesQuotation.code" value="${salesQuotation.code}" type="text" size="30" class="validate[required] text-input" /><span style="color: red;">*</span></td>
											<td align="right" width="10%">报价主题:</td>
											<td width="40%"><input id="quoteSubject" name="salesQuotation.quoteSubject" value="${salesQuotation.quoteSubject}" type="text" size="30" class="validate[required] text-input" /><span style="color: red;">*</span></td>
										</tr>
										<tr>
											<td align="right">客户名称:</td>
											<td><input id="customerName" name="salesQuotation.customerAccount.name" value="${salesQuotation.customerAccount.name}" type="text" size="30" class="validate[required] text-input" /><span style="color: red;">*</span> <input type="hidden" id="customerAccountId" name="customerAccountId" value="${salesQuotation.customerAccount.id}" />
												<span><a class="abtn" href="###" onclick="chooseCustomerAccount();"><span>选择</span></a></span> <span><a class="abtn" href="#" onclick="addCustomerAccount();"><span>新增</span></a></span></td>
											<td align="right">报价单类型:</td>
											<td><s:select id="typeId" headerKey="" headerValue="--请选择--" list="%{billsTypeList}" listValue="typeName" listKey="typeCode" value="%{salesQuotation.type}" multiple="false" theme="simple"></s:select></td>
										</tr>
										<tr>
											...
											<td><input type="hidden" id="saleOrgId" value="${salesQuotation.salesOrg.id}" /> <input id="saleOrgName" name="salesOrder.saleOrg.fullName" value="${salesQuotation.salesOrg.fullName}" type="text" size="30" /> <a class="abtn" href="###" onclick="chooseSaleOrg();"><span>选择</span></a></td>
										</tr>
										<tr>
											<td align="right">业务类型:</td>
											<td><input id="bizType" name="salesQuotation.bizType" value="${salesQuotation.bizType}" type="text" size="30" class="validate[required] text-input" /><span style="color: red;">*</span></td>
											<td align="right">地域:</td>
											<td><s:select id="regionalId" list="%{regionalList}" headerKey="-1" headerValue="--请选择--" listValue="name" listKey="id" value="%{salesQuotation.regional.id}" multiple="false" theme="simple"></s:select></td>
										</tr>
										<tr>
											<td align="right">城市:</td>
											<td><input id="city" name="salesQuotation.city" value="${salesQuotation.city}" type="text" size="30" /></td>
											<td align="right">税率 :</td>
											<td><input id="tax" name="salesQuotation.tax" value="${salesQuotation.tax}" type="text" size="30" />% 范围(1-100)</td>
										</tr>
										<tr>
											<td align="right">交货日期:</td>
											<td><input id="dilveryDate" name="salesQuotation.dilveryDate" value="<s:property value='formatDateToString(salesQuotation.dilveryDate)'/>" onfocus="showTime('dilveryDate','yyyy-MM-dd HH:mm')" type="text" size="30" /> <img onclick="showTime('dilveryDate','yyyy-MM-dd HH:mm')" src="${vix}/common/img/datePicker.gif" width="16"
												height="22" align="absmiddle"></td>
											<td align="right">单据日期:</td>
											<td><input type="hidden" id="bdString" value="<s:property value='formatDateToTimeString(salesQuotation.billDate)'/>" /> <input id="billDate" name="salesQuotation.billDate" value="<s:property value='formatDateToString(salesQuotation.billDate)'/>" onfocus="showTime('billDate','yyyy-MM-dd HH:mm')" type="text" size="30" /> <img
										...
										<tr>
											...
											<td><input id="validQuotationFrom" name="salesQuotation.validQuotationFrom" value="<s:property value='formatDateToString(salesQuotation.validQuotationFrom)'/>" onfocus="showTime('validQuotationFrom','yyyy-MM-dd HH:mm')" type="text" size="30" class="validate[required] text-input" /> <img
											...
											<td><input id="validQuotationTo" name="salesQuotation.validQuotationTo" value="<s:property value='formatDateToString(salesQuotation.validQuotationTo)'/>" onfocus="showTime('validQuotationTo','yyyy-MM-dd HH:mm')" type="text" size="30" class="validate[required] text-input" /> <img
					...
					<div class="order_table">
						<table>
							<tr>
								...
								<td width="35%"><input type="hidden" id="salesManId" value="${salesQuotation.salesMan.id}" /> <input id="salesManName" name="salesQuotation.salesMan.name" value="${salesQuotation.salesMan.name}" type="text" size="30" /> <a class="abtn" href="###" onclick="chooseEmployee();"><span>选择</span></a></td>
								<td width="10%" align="right">金额:</td>
								<td width="40%"><input id="amount" name="amount" value="${salesQuotation.amount}" type="text" size="30" /></td>
							</tr>
						</table>
					</div>
				</dd>

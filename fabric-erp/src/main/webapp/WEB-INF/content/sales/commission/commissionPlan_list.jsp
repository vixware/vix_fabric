...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/sales/commission/commissionPlanAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/sales/commission/commissionPlanAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				pager("start","${vix}/sales/commission/commissionPlanAction!goListContent.action?name="+name,'CommissionPlan');
			});
		  }
	});
}
...
function searchForContent(i){
	...
	loadPurchasePerson();
	if(i == 0){
		pager("start","${vix}/sales/commission/commissionPlanAction!goSearchList.action?i="+i+"&searchContent="+name,'CommissionPlan');
	}
	else{
		pager("start","${vix}/sales/commission/commissionPlanAction!goSearchList.action?i="+i+"&purchasePerson="+purchasePerson+"&name="+purchaseName,'CommissionPlan');
	}
}
/*重置搜索*/
...
function purStatus(status){
	pager("start","${vix}/sales/commission/commissionPlanAction!goListContent.action?status="+status,'CommissionPlan');
}
//最近使用
function purRecent(rencentDate){
	pager("start","${vix}/sales/commission/commissionPlanAction!goListContent.action?updateTime="+rencentDate,'CommissionPlan');
}
...
function orderBy(orderField){
	loadName();
	var orderBy = $("#CommissionPlanOrderBy").val();
	if(orderBy == 'desc'){
		orderBy = "asc";
	}else{
		orderBy = 'desc';
	}
	pager("start","${vix}/sales/commission/commissionPlanAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'CommissionPlan');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/sales/commission/commissionPlanAction!goListContent.action?name="+name,'CommissionPlan');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/sales/commission/commissionPlanAction!goList.action');">佣金计划</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a></span> <span><a class="previous" onclick="currentPager('previous');"></a></span> <em>(<b class="CommissionPlanInfo"></b> <s:text name='cmn_to' /> <b class="CommissionPlanTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a></span> <span><a class="end" onclick="currentPager('end');"></a></span>
					</div>
				</div>
				<div id="CommissionPlan" class="table" style="overflow-x: auto; width: 100%;"></div>
				<div class="pagelist drop">
					<ul>

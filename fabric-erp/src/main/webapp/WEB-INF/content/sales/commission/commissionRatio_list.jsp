...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/sales/commission/commissionRatioAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/sales/commission/commissionRatioAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				pager("start","${vix}/sales/commission/commissionRatioAction!goListContent.action?name="+name,'commissionRatio');
			});
		  }
	});
}
...
function orderBy(orderField){
	loadName();
	var orderBy = $("#commissionRatioOrderBy").val();
	if(orderBy == 'desc'){
		orderBy = "asc";
	}else{
		orderBy = 'desc';
	}
	pager("start","${vix}/sales/commission/commissionRatioAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'commissionRatio');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/sales/commission/commissionRatioAction!goListContent.action?name="+name,'commissionRatio');
}
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a></span> <span><a class="previous" onclick="currentPager('previous');"></a></span> <em>(<b class="commissionRatioInfo"></b> <s:text name='cmn_to' /> <b class="commissionRatioTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a></span> <span><a class="end" onclick="currentPager('end');"></a></span>
					</div>
				</div>
				<div id="commissionRatio" class="table" style="overflow-x: auto; width: 100%;"></div>
				<div class="pagelist drop">
					<ul>

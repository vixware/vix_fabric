...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/sales/commission/commissionPlanAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
	...
	$.ajax({
		  ...
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				--%>
				pager("start","${vix}/sales/commission/commissionPlanAction!goPlanList.action?name="+name,'brand');
			});
		  }
	});
}
...
function searchForContent(i){
	...
	loadPurchasePerson();
	if(i == 0){
		pager("start","${vix}/sales/commission/commissionPlanAction!goSearchList.action?i="+i+"&searchContent="+name,'brand');
	}
	else{
		pager("start","${vix}/sales/commission/commissionPlanAction!goSearchList.action?i="+i+"&purchasePerson="+purchasePerson+"&name="+purchaseName,'brand');
	}
}
/*重置搜索*/
...
function purStatus(status){
	pager("start","${vix}/sales/commission/commissionPlanAction!goPlanList.action?status="+status,'brand');
}
//最近使用
function purRecent(rencentDate){
	pager("start","${vix}/sales/commission/commissionPlanAction!goPlanList.action?updateTime="+rencentDate,'brand');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/sales/commission/commissionPlanAction!goPlanList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/sales/commission/commissionPlanAction!goPlanList.action?name="+name,'brand');
}

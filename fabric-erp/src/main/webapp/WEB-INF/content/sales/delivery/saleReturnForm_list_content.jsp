...
function deleteEntity(id){
	asyncbox.confirm('确定要删除该销售退货单么?','提示信息',function(action){
		if(action == 'ok'){
			deleteById(id);
		}
	});
...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<th width="20%">客户名称</th>
			<th width="15%">单据类型</th>
			<th width="15%"><s:text name="sa_modeofdispatch" /></th>
			<th width="15%">退货日期</th>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td><span style="color: gray;">${entity.returnOrderCode }</span></td>
				<td><span style="color: gray;">${entity.customerAccount.name }</span></td>
				<td><span style="color: gray;">${entity.returnGoodsType.name }</span></td>
				<td><span style="color: gray;">${entity.transferStyle }</span></td>
				<td width="90"><span style="color: gray;"><fmt:formatDate value="${entity.returnTime }" type="both" pattern="yyyy-MM-dd" /></span></td>
				<td><a href="#" title="<s:text name='cmn_show'/>" onclick="showSaleReturnForm('${entity.id}');"> <img src="${vix}/common/img/icon_19.gif" />
				</a> <a href="#" onclick="saveOrUpdate('${entity.id}');" title="<s:text name='cmn_update'/>"> <img src="${vix}/common/img/icon_edit.png" />
				</a> <a href="#" onclick="deleteById('${entity.id}');" title="<s:text name='cmn_delete'/>"> <img src="${vix}/common/img/icon_12.png" />
				</a></td>
			</tr>
		</s:iterator>

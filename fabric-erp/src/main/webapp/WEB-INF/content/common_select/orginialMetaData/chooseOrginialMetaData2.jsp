...
function searchForSelectEpmContent(){
	loadMetaDataName();
	pager("start","${vix}/common/select/commonSelectOrginialMetaDataAction!goChooseMetaDataList2.action?tag=${tag}&chkStyle=${chkStyle}&checkedObjIds=${checkedObjIds}&metaDataName="+metaDataName,'metaDataChk');
}

loadMetaDataName();
//载入分页列表数据
pager("start","${vix}/common/select/commonSelectOrginialMetaDataAction!goChooseMetaDataList2.action?tag=${tag}&chkStyle=${chkStyle}&checkedObjIds=${checkedObjIds}","metaDataChk");
...
function metaDataChkPager(tag,entity){
	...
	if(entity == 'metaDataChk'){
		pager(tag,"${vix}/common/select/commonSelectOrginialMetaDataAction!goChooseMetaDataList2.action?tag=${tag}&chkStyle=${chkStyle}&checkedObjIds=${checkedObjIds}&metaDataName="+metaDataName+'&id='+$('#selectId').val(),entity);
	}
	/* if(entity == 'brand'){
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/select/commonSelectOrginialMetaDataAction!goChooseMetaDataList2.action?tag=${tag}&chkStyle=${chkStyle}&checkedObjIds=${checkedObjIds}&orderField="+orderField+"&orderBy="+orderBy+"&metaDataName="+metaDataName+"&id="+$("#selectId").val(),'metaDataChk');
}
...
<div class="content" style="background: #DCE7F1">
	<div id="c_head" class="drop">
		...
		<div>
			...
			<label><input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForSelectEpmContent();" /> </label>
		</div>
		...
		<div id="left">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/common/select/commonSelectOrginialMetaDataAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/common/select/commonSelectOrginialMetaDataAction!goChooseMetaDataList2.action?chkStyle=${chkStyle}&checkedObjIds=${checkedObjIds}&id="+treeNode.id+"&metaDataName="+metaDataName,'metaDataChk');
				}

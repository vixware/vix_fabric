...
function deleteById(id){
	$.ajax({
		  url:'${vix}/qm/inThelibrarytestAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/qm/inThelibrarytestAction!goListContent.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/qm/inThelibrarytestAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/qm/inThelibrarytestAction!goListContent.action?name="+name,'brand');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/qm/inThelibrarytestAction!goList.action?pageNo=${pageNo}');"><s:text name="在库检验" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div class="search_advanced">
			<label>单据编号:<input id="trends_title" name="" type="text" class="int" /></label> <label>抽检规格:<input id="trends_uploadPersonName" name="" type="text" class="int" /></label> <label> <input name="" type="button" class="btn" value="<s:text name='cmn_search'/>" onclick="searchForContent(1);" /> <input name="" type="button" class="btn"

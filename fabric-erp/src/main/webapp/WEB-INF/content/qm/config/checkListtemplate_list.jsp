...
function saveOrUpdate(id){
	$.ajax({
		  url:"${vix}/qm/config/checkListtemplateAction!goSaveOrUpdate.action?id="+id,
		  cache: false,
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#brandForm').validationEngine('validate')){
								$.post('${vix}/qm/config/checkListtemplateAction!saveOrUpdate.action',
									 {'checkListtemplate.id':$("#id").val(),									  
									  'checkListtemplate.code':$("#code").val(),						  
									  'checkListtemplate.name':$("#name").val(),						  
									  'checkListtemplate.memo':$("#memo").val(),						  
									  'checkListtemplate.createTime':$("#createTime").val(),	
									  'checkListtemplate.enable':$('input:radio[name=enable]:checked').val()					  
									},
									 function (result){
										showMessage (result);
										setTimeout ("" , 1000);
										loadContent ('${vix}/qm/config/checkListtemplateAction!goList.action');
									}
								 ); 
			  				}else{
			  					return false;
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/qm/config/checkListtemplateAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='检验单模板'/>",function(action){
				var treeNode = zTree.getNodeByTId($("#selectIdTreeId").val());
				if(null != treeNode){
					treeNode.isParent = true;
				}
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/qm/config/checkListtemplateAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/qm/config/checkListtemplateAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/qm/config/checkListtemplateAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/qm/config/checkListtemplateAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div class="search_advanced">
			<label><s:text name='编码' />:<input name="" type="text" class="int" /></label> <label><s:text name='模板名称' />:<input name="" type="text" class="int" /></label> <label><input name="" type="button" class="btn" value="<s:text name='cmn_search'/>" /><input name="" type="button" class="btn" value="<s:text name='cmn_reset'/>" /></label>
		</div>
	</div>
	<div id="number">
		...
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/qm/config/checkListtemplateAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/qm/config/checkListtemplateAction!goSingleList.action?parentId="+treeNode.id,"category");
				}
			...
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/common_listx16.png" alt="" />
						<s:text name='qm_check_list_template' /></a></li>
				</ul>
			</div>

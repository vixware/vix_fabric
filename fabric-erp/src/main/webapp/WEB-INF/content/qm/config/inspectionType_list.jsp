...
function saveOrUpdate(id){
	$.ajax({
		  url:"${vix}/qm/config/inspectionTypeAction!goSaveOrUpdate.action?id="+id,
		  cache: false,
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#brandForm').validationEngine('validate')){
								$.post('${vix}/qm/config/inspectionTypeAction!saveOrUpdate.action',
									 {'inspectionType.id':$("#id").val(),									  
									  'inspectionType.code':$("#code").val(),						  
									  'inspectionType.name':$("#name").val(),						  
									  'inspectionType.memo':$("#memo").val(),						  
									  'inspectionType.createTime':$("#createTime").val(),	
									  'inspectionType.enable':$('input:radio[name=enable]:checked').val()
									},
									 function (result){
										showMessage (result);
										setTimeout ("" , 1000);
										loadContent ('${vix}/qm/config/inspectionTypeAction!goList.action');
									}
								 ); 
			  				}else{
			  					return false;
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/qm/config/inspectionTypeAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='检验类型'/>",function(action){
				var treeNode = zTree.getNodeByTId($("#selectIdTreeId").val());
				if(null != treeNode){
					treeNode.isParent = true;
				}
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/qm/config/inspectionTypeAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/qm/config/inspectionTypeAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/qm/config/inspectionTypeAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/qm/config/inspectionTypeAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<ul>
			<li><a href="#" id="numBtn"> <img src="img/find.png" alt="" /> <s:text name="cmn_index" />
			</a></li>
			<li class="fly"><a href="#"><img alt="" src="img/icon_10.png"> <s:text name="cmn_mode" /> </a>
				<ul>
					<li><a href="#" onclick="saleOrderStatus('0')"><img alt="" src="img/uncommitted.png">
						<s:text name="cmn_uncommitted" /></a></li>
		...
		<div class="search_advanced">
			<label><s:text name='编码' />:<input name="" type="text" class="int" /></label> <label><s:text name='状态' />:<input name="" type="text" class="int" /></label> <label><input name="" type="button" class="btn" value="<s:text name='cmn_search'/>" /><input name="" type="button" class="btn" value="<s:text name='cmn_reset'/>" /></label>
		</div>
	</div>
	<div id="number">
		...
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/qm/config/inspectionTypeAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/qm/config/inspectionTypeAction!goSingleList.action?parentId="+treeNode.id,"category");
				}

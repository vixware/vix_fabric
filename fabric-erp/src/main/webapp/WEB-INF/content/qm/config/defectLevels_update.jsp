...
<div class="content">
	...
	<div class="box cardTable">
		<form id="brandForm">
			<input type="hidden" id="id" name="id" value="${defectLevels.id}" />
			<table width="100%">
				<tr>
					<th class="tr" width="150">编码：</th>
					<td><input id="code" name="defectLevels.code" value="${defectLevels.code}" type="text" style="width: 200px;" class="ipt" /></td>
				</tr>
				<tr>
					<th class="tr" width="150">内容：</th>
					<td colspan="3"><textarea id="name" class="example" rows="1" style="width: 200px; height: 54px;">${defectLevels.name}</textarea></td>
				</tr>
				<tr>
					<th class="tr" width="150">备注：</th>
					<td colspan="3"><textarea id="memo" class="memo" rows="1" style="width: 200px; height: 54px;">${defectLevels.memo}</textarea></td>
				</tr>
				<tr>
					<td align="right">是否启用:&nbsp;</td>
					<td><s:radio list="#{0:\"否\",1:\"是\"}" name="enable" value="%{defectLevels.enable}" theme="simple"></s:radio></td>
				</tr>
				<tr>
					<th class="tr" width="150">时间：</th>
					<td><input id="createTime" name="defectLevels.createTime" value="${defectLevels.createTime}" onchange="salesOrderFieldChanged(this);" type="text" class="validate[required] text-input" /> <img onclick="showTime('createTime','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"></td>
				</tr>
			</table>
		</form>
	</div>

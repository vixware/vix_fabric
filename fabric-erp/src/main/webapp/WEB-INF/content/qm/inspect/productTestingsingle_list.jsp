...
function searchForContent(i){
	...
	loadItemName();
	if(i == 0){
		pager("start","${vix}/qm/productTestingsingleAction!goSearchList.action?i="+i+"&itemCode="+name,'brand');
	}
	else{
		pager("start","${vix}/qm/productTestingsingleAction!goSearchList.action?i="+i+"&itemCode="+itemCode+"&itemName="+itemName,'brand');
	}
}
...
/*改变搜索按钮的显示*/
 ...
 function leastRecentlyUsed(inspectionDate){
 	pager("start","${vix}/qm/productTestingsingleAction!goSingleList.action?inspectionDate="+inspectionDate,'brand');
 }
...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/qm/productTestingsingleAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/qm/productTestingsingleAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/qm/productTestingsingleAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/qm/productTestingsingleAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/qm/productTestingsingleAction!goSingleList.action?name="+name,'brand');
}

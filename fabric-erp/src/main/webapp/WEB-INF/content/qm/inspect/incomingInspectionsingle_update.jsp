...
<script type="text/javascript">
	...
	function saveOrUpdateOrder(){
		...
		if($('#orderForm').validationEngine('validate')){
			$.post('${vix}/qm/incomingInspectionsingleAction!saveOrUpdate.action',
				{
				'incomingInspectionsingle.id' : $("#id").val(),
				'incomingInspectionsingle.receiptNumber':$("#receiptNumber").val(),
				'incomingInspectionsingle.receiptName':$("#receiptName").val(),
				'incomingInspectionsingle.productModel':$("#productModel").val(),
				'incomingInspectionsingle.inspectionDate':$("#inspectionDate").val(),
				'incomingInspectionsingle.itemId':$("#itemId").val(),
				'incomingInspectionsingle.itemName':$("#itemName").val(),
				'incomingInspectionsingle.itemCode':$("#itemCode").val(),
				'incomingInspectionsingle.participant':$("#participant").val(),
				'incomingInspectionsingle.departmentName':$("#departmentName").val(),
				'incomingInspectionsingle.testMethods.id':$("#testMethodsId").val(),
				'incomingInspectionsingle.inspectionCharacteristics.id':$("#inspectionCharacteristicsId").val(),
				'dlJson' : dlJson,	
				'spJson' : spJson
				},
				function(result){
					showMessage(result);
					setTimeout("",1000);
					loadContent('${vix}/qm/incomingInspectionsingleAction!goList.action?menuId=menuContract');
				}
			);
		}
	}
...
<div class="sub_menu">
	<h2>
		<i> <a href="#"><img alt="" src="img/icon_14.gif"> <s:text name="print" /> </a> <a href="#"><img alt="" src="img/icon_15.gif">
			<s:text name="help" /> </a>
		</i>
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#" onclick="loadContent('${vix}/qm/incomingInspectionsingleAction!goList.action?pageNo=${pageNo}');"><img src="img/qm_quality_management.png" alt="" /> <s:text name="supplyChain" /></a></li>
				<li><a href="#" onclick="loadContent('${vix}/qm/incomingInspectionsingleAction!goList.action?pageNo=${pageNo}');"><s:text name="qm_quality_management" /></a></li>
				<li><a href="#" onclick="loadContent('${vix}/qm/incomingInspectionsingleAction!goList.action?pageNo=${pageNo}');"><s:text name="qm_incoming_inspection" /></a></li>
				<li><a href="#" onclick="loadContent('${vix}/qm/incomingInspectionsingleAction!goList.action?pageNo=${pageNo}');"><s:text name="qm_incoming_inspection_single" /></a></li>
				<li><a href="#" onclick="loadContent('${vix}/qm/incomingInspectionsingleAction!goList.action?pageNo=${pageNo}');"><s:text name="add" /></a></li>
			</ul>
		</div>
	</h2>
</div>
<input type="hidden" id="id" name="id" value="${incomingInspectionsingle.id}" />
<div class="content">
	<form id="orderForm">
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateOrder();"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <%--<a href="#"><img width="22" height="22" title="保存并提交审批" src="${vix}/common/img/dt_submit.png" /></a>  
						--%> <a href="#" onclick="loadContent('${vix}/qm/incomingInspectionsingleAction!goList.action?pageNo=${pageNo}');"> <img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a>
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr height="40">
											...
											<td><input id="itemCode" name="incomingInspectionsingle.itemCode" value="${incomingInspectionsingle.itemCode}" type="text" class="validate[required] text-input" /> <input id="itemId" type="hidden" /> <span style="color: red;">*</span> <input class="btn" type="button" value="选择" onclick="chooseItem();" /></td>
											<th align="right">${vv:varView("vix_mdm_item")}名称:&nbsp;</th>
											<td><input id="itemName" name="incomingInspectionsingle.itemName" value="${incomingInspectionsingle.itemName}" type="text" class="validate[required] text-input" /> <span style="color: red;">*</span></td>
										</tr>
										<tr>
											<td align="right">单据编号：</td>
											<td><input id="receiptNumber" name="receiptNumber" value="${incomingInspectionsingle.receiptNumber}" type="text" size="30" class="validate[required] text-input" /></td>
											<td align="right">单据名称：</td>
											<td><input id="receiptName" name="receiptName" value="${incomingInspectionsingle.receiptName}" type="text" size="30" class="validate[required] text-input" /></td>
										</tr>
										<tr>
											<td align="right">型号：</td>
											<td><input id="productModel" name="productModel" value="${incomingInspectionsingle.productModel}" type="text" size="30" class="validate[required] text-input" /></td>
											<td align="right">抽样标准：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="criterion" listKey="id" value="%{incomingInspectionsingle.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
										<tr>
											<td align="right">报检人：</td>
											<td><input id="participant" name="parentId" value="${incomingInspectionsingle.participant}" type="text" class="validate[required] text-input" /> <input type="hidden" id="parentId" name="parentId" value="${hrCategory.parentCategory.id }" /> <input class="btn" type="button" value="选择" onclick="chooseEmployees();" /></td>
											<td align="right">检验部门：</td>
											<td colspan="3"><input type="hidden" id="pubIds" name="pubIds" value="${pubIds}" /> <input type="hidden" id="treeType" name="treeType" value="${treeType}" /> <input type="text" id="departmentName" name="departmentName" readonly="readonly" value="${incomingInspectionsingle.departmentName}" /> <input class="btn" type="button"
												value="选择" onclick="chooseParentOrganization();" /></td>
										</tr>
										<tr>
											<td align="right">检验方法：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="name" listKey="id" value="%{productInspectionform.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
											<td align="right">样品等级：</td>
											<td><s:select id="inspectionCharacteristicsId" headerKey="-1" headerValue="--请选择--" list="%{inspectionCharacteristicsList}" listValue="name" listKey="id" value="%{incomingInspectionsingle.inspectionCharacteristics.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
										<tr>
											<td align="right">报检日期：</td>
											<td><input id="inspectionDate" name="incomingInspectionsingle.inspectionDate" value="${incomingInspectionsingle.inspectionDate}" onclick="showTime('inspectionDate','yyyy-MM-dd')" onchange="salesOrderFieldChanged(this);" type="text" size="30" class="validate[required] text-input" /> <img
												onclick="showTime('inspectionDate','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"></td>
										</tr>
									</table>
								</dd>
							</dl>
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;">
						<div style="padding: 8px;">
							<table id="dlLineItem" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlLineItemTb',url: '${vix}/qm/incomingInspectionsingleAction!getIncoming.action?id=${incomingInspectionsingle.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
							...
							function removeDlLineItem(){
								...
								$.ajax({url:'${vix}/qm/incomingInspectionsingleAction!deleteIncomingLineById.action?id='+row.id,cache: false});
					...
					<div id="a2" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<script type="text/javascript">
						$('#soAttach').datagrid({
							url: '${vix}/qm/incomingInspectionsingleAction!getIncomingTemplate.action?id=${incomingInspectionsingle.id}',
							title: '报检单附件',
							width: 'auto',
							height: '450',
							fitColumns: true,
							...
							toolbar:[{
								...
								handler:function(){
									...
									$.ajax({
										  url:'${vix}/qm/incomingInspectionsingleAction!addAttachFile.action',
										  cache: false,
										  success: function(html){
											  asyncbox.open({
												 	modal:true,
													...
													callback : function(action,returnValue){
														if(action == 'ok'){
															uploadFile('${vix}/qm/incomingInspectionsingleAction!uploadAttachment.action?id=${incomingInspectionsingle.id}','fileToUpload');
					...
					<div id="a3" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<div style="padding: 8px;">
							<table id="dlInvoice" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlInvoiceTb',url: '${vix}/qm/incomingInspectionsingleAction!getIncoming1.action?id=${incomingInspectionsingle.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
										<th data-options="field:'id',align:'center',width:120">序号</th>
							...
							function removeDlInvoice(){
								...
								$.ajax({url:'${vix}/qm/incomingInspectionsingleAction!deleteApplicationFormById.action?id='+row.id,cache: false});

...
function searchForContent(i){
	...
	loadItemCode();
	if(i == 0){
		pager("start","${vix}/qm/defectiveProductsAction!goSearchList.action?i="+i+"&receiptNumber="+name,'brand');
	}
	else{
		pager("start","${vix}/qm/defectiveProductsAction!goSearchList.action?i="+i+"&receiptNumber="+receiptNumber+"&itemName="+itemName+"&itemCode="+itemCode,'brand');
	}
}
...
/*改变搜索按钮的显示*/
 ...
 function leastRecentlyUsed(inspectionDate){
 	pager("start","${vix}/qm/defectiveProductsAction!goSingleList.action?inspectionDate="+inspectionDate,'brand');
 }
...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/qm/defectiveProductsAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/qm/defectiveProductsAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/qm/defectiveProductsAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/qm/defectiveProductsAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/qm/defectiveProductsAction!goSingleList.action?name="+name,'brand');
}

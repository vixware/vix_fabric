...
/** input 获取焦点input效果绑定  */
	...
	function saveOrUpdateOrder(){
		...
		if($('#orderForm').validationEngine('validate')){
			$.post('${vix}/qm/inStorehouseinspectionformAction!saveOrUpdate.action',
				{
				'inStorehouseinspectionform.id' : $("#id").val(),
				'inStorehouseinspectionform.receiptsNumber':$("#receiptsNumber").val(),
				'inStorehouseinspectionform.produceDate':$("#produceDate").val(),
				'inStorehouseinspectionform.departmentDate':$("#departmentDate").val(),
				'inStorehouseinspectionform.failureDate':$("#failureDate").val(),
				'inStorehouseinspectionform.checkQuantity':$("#checkQuantity").val(),
				'inStorehouseinspectionform.departmentName':$("#departmentName").val(),
				'inStorehouseinspectionform.testMethods.id':$("#testMethodsId").val(),
				'inStorehouseinspectionform.inspectionCharacteristics.id':$("#inspectionCharacteristicsId").val(),
				'inStorehouseinspectionform.inspectionStatus.id':$("#inspectionStatusId").val(),
				'inStorehouseinspectionform.inspectionType.id':$("#inspectionTypeId").val(),
				'dlJson' : dlJson,	
				'spJson' : spJson
				},
				function(result){
					showMessage(result);
					setTimeout("",1000);
					loadContent('${vix}/qm/inStorehouseinspectionformAction!goList.action?menuId=menuContract');
				}
			);
		}
	}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="qm_in_storehouse_inspection" /></a></li>
				<li><a href="#" onclick="loadContent('${vix}/qm/inStorehouseinspectionformAction!goList.action?pageNo=${pageNo}');"><s:text name="qm_in_storehouse_inspection_form" /></a></li>
				<li><a href="#"><s:text name="add" /></a></li>
			</ul>
		</div>
	</h2>
</div>
<input type="hidden" id="id" name="id" value="${inStorehouseinspectionform.id}" />
<div class="content">
	<form id="orderForm">
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateOrder();"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <%-- <a href="#"><img width="22" height="22" title="保存并提交审批" src="${vix}/common/img/dt_submit.png" /></a> 
						 --%> <a href="#" onclick="loadContent('${vix}/qm/inStorehouseinspectionformAction!goList.action?pageNo=${pageNo}');"> <img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a>
					</span> <strong><b><s:text name="新增在库报检单" /> </b><i></i> </strong>
				</dt>
				<dd class="clearfix">
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id="receiptsNumber" name="receiptsNumber" value="${inStorehouseinspectionform.receiptsNumber}" type="text" size="30" class="validate[required] text-input" /></td>
											<td align="right">生产日期：</td>
											<td><input id="produceDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="produceDate" value="<fmt:formatDate value='${inStorehouseinspectionform.produceDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('produceDate','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif"
												width="16" height="22" align="absmiddle"></td>
										</tr>
										<tr>
											<td align="right">抽检规格：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="criterion" listKey="id" value="%{inStorehouseinspectionform.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
											<td align="right">检验类型：</td>
											<td><s:select id="inspectionTypeId" headerKey="-1" headerValue="--请选择--" list="%{inspectionTypeList}" listValue="name" listKey="id" value="%{inStorehouseinspectionform.inspectionType.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
										<tr>
											<td align="right">检验方法：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="name" listKey="id" value="%{inStorehouseinspectionform.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
											<td align="right">检验时间：</td>
											<td><input id="departmentDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="departmentDate" value="<fmt:formatDate value='${inStorehouseinspectionform.departmentDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('departmentDate','yyyy-MM-dd')"
												src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"></td>
										</tr>
										<tr>
											<td align="right">失效时间：</td>
											<td><input id="failureDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="failureDate" value="<fmt:formatDate value='${inStorehouseinspectionform.failureDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('failureDate','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif"
												width="16" height="22" align="absmiddle"></td>
										</tr>
										<tr>
											<td align="right">抽检量：</td>
											<td><input id="checkQuantity" name="checkQuantity" value="${inStorehouseinspectionform.checkQuantity}" type="text" size="30" /></td>
											<td align="right">检验部门：</td>
											<td><input type="hidden" id="pubIds" name="pubIds" value="${pubIds}" /> <input type="hidden" id="treeType" name="treeType" value="${treeType}" /> <input type="text" id="departmentName" name="departmentName" readonly="readonly" value="${inStorehouseinspectionform.departmentName}" /> <input class="btn" type="button" value="选择"
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;">
						<div style="padding: 8px;">
							<table id="dlLineItem" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlLineItemTb',url: '${vix}/qm/inStorehouseinspectionformAction!getIncoming.action?id=${inStorehouseinspectionform.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
							...
							function removeDlLineItem(){
								...
								$.ajax({url:'${vix}/qm/inStorehouseinspectionformAction!deleteIncomingLineById.action?id='+row.id,cache: false});
					...
					<div id="a2" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<script type="text/javascript">
						$('#soAttach').datagrid({
							url: '${vix}/qm/inStorehouseinspectionformAction!getIncomingTemplate.action?id=${inStorehouseinspectionform.id}',
							title: '报检单附件',
							width: 'auto',
							height: '450',
							fitColumns: true,
							...
							toolbar:[{
								...
								handler:function(){
									...
									$.ajax({
										  url:'${vix}/qm/inStorehouseinspectionformAction!addAttachFile.action',
										  cache: false,
										  success: function(html){
											  asyncbox.open({
												 	modal:true,
													...
													callback : function(action,returnValue){
														if(action == 'ok'){
															uploadFile('${vix}/qm/inStorehouseinspectionformAction!uploadAttachment.action?id=${inStorehouseinspectionform.id}','fileToUpload');
					...
					<div id="a3" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<div style="padding: 8px;">
							<table id="dlInvoice" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlInvoiceTb',url: '${vix}/qm/inStorehouseinspectionformAction!getIncoming1.action?id=${inStorehouseinspectionform.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
										<th data-options="field:'id',align:'center',width:120">序号</th>
							...
							function removeDlInvoice(){
								...
								$.ajax({url:'${vix}/qm/inStorehouseinspectionformAction!deleteApplicationFormById.action?id='+row.id,cache: false});

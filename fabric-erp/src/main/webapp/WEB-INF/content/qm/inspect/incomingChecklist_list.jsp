...
function searchForContent(i){
	...
	loadItemCode();
	if(i == 0){
		pager("start","${vix}/qm/incomingChecklistAction!goSearchList.action?i="+i+"&receiptNumber="+name,'incoming');
	}
	else{
		pager("start","${vix}/qm/incomingChecklistAction!goSearchList.action?i="+i+"&receiptNumber="+receiptNumber+"&itemName="+itemName+"&itemCode="+itemCode,'incoming');
	}
}
...
/*改变搜索按钮的显示*/
 ...
 function leastRecentlyUsed(inspectionDate){
 	pager("start","${vix}/qm/incomingChecklistAction!goSingleList.action?inspectionDate="+inspectionDate,'incoming');
 }
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/qm/incomingChecklistAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/qm/incomingChecklistAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/qm/incomingChecklistAction!goSingleList.action?name="+name,'incoming');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/qm/incomingChecklistAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'incoming');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/qm/incomingChecklistAction!goSingleList.action?name="+name,'incoming');
}
//面包屑
if($('.sub_menu').length){
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">质量管理</a></li>
				<li><a href="#">来料检验</a></li>
				<li><a href="#" onclick="loadContent('${vix}/qm/incomingChecklistAction!goList.action?pageNo=${pageNo}');">来料检验单</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<ul>
			<li class="fly"><a href="#"><img alt="" src="img/leastRecentlyUsed.png"> <s:text name="最近检验时间" /> </a>
				<ul>
	...
	<div class="box">
		...
		<div id="right">
			<div class="right_content">
				...
				<div id="incoming" class="table" style="overflow-x: auto; width: 100%;"></div>
				<div class="pagelist drop">
					<%-- <ul>
						<li class="ed">

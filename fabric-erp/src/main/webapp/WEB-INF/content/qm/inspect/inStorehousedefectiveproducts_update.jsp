...
/** input 获取焦点input效果绑定  */
	...
	function saveOrUpdateOrder(){
		...
		if($('#orderForm').validationEngine('validate')){
			$.post('${vix}/qm/inStorehousedefectiveproductsAction!saveOrUpdate.action',
				{
				'inStorehousedefectiveproducts.id' : $("#id").val(),
				'inStorehousedefectiveproducts.receiptsNumber':$("#receiptsNumber").val(),
				'inStorehousedefectiveproducts.productName':$("#productName").val(),
				'inStorehousedefectiveproducts.operator':$("#operator").val(),
				'inStorehousedefectiveproducts.scrappedHandle':$("#scrappedHandle").val(),
				'inStorehousedefectiveproducts.returnProcessing':$("#returnProcessing").val(),
				'inStorehousedefectiveproducts.principal':$("#principal").val(),
				'inStorehousedefectiveproducts.produceDate':$("#produceDate").val(),
				'inStorehousedefectiveproducts.departmentName':$("#departmentName").val(),
				'inStorehousedefectiveproducts.testMethods.id':$("#testMethodsId").val(),
				'inStorehousedefectiveproducts.inspectionCharacteristics.id':$("#inspectionCharacteristicsId").val(),
				'inStorehousedefectiveproducts.inspectionStatus.id':$("#inspectionStatusId").val(),
				'inStorehousedefectiveproducts.inspectionType.id':$("#inspectionTypeId").val(),
				'dlJson' : dlJson,	
				'spJson' : spJson
				},
				function(result){
					showMessage(result);
					setTimeout("",1000);
					loadContent('${vix}/qm/inStorehousedefectiveproductsAction!goList.action?menuId=menuContract');
				}
			);
		}
	}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/qm/inStorehousedefectiveproductsAction!goList.action?pageNo=${pageNo}');"><s:text name="qm_in_library_products_handle_single" /></a></li>
				<li><a href="#"><s:text name="add" /></a></li>
			</ul>
		</div>
	</h2>
</div>
<input type="hidden" id="id" name="id" value="${inStorehousedefectiveproducts.id}" />
<div class="content">
	<form id="orderForm">
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateOrder();"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <%-- <a href="#"><img width="22" height="22" title="保存并提交审批" src="${vix}/common/img/dt_submit.png" /></a> 
							 --%> <a href="#" onclick="loadContent('${vix}/qm/inStorehousedefectiveproductsAction!goList.action?pageNo=${pageNo}');"> <img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a>
					</span> <strong><b><s:text name="qm_in_library_products_handle_single" /> </b><i></i> </strong>
				</dt>
				<dd class="clearfix">
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id="receiptsNumber" name="receiptsNumber" value="${inStorehousedefectiveproducts.receiptsNumber}" type="text" size="30" /></td>
											<td align="right">产品名称：</td>
											<td><input id="productName" name="productName" value="${inStorehousedefectiveproducts.productName}" type="text" size="30" /></td>
										</tr>
										<tr>
											<td align="right">报废处理：</td>
											<td><input id="scrappedHandle" name="scrappedHandle" value="${inStorehousedefectiveproducts.scrappedHandle}" type="text" size="30" /></td>
											<td align="right">退货处理：</td>
											<td><input id="returnProcessing" name="returnProcessing" value="${inStorehousedefectiveproducts.returnProcessing}" type="text" size="30" /></td>
										</tr>
										<tr>
											<td align="right">生产日期：</td>
											<td><input id="produceDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="produceDate" value="<fmt:formatDate value='${inStorehousedefectiveproducts.produceDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('produceDate','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif"
												width="16" height="22" align="absmiddle"></td>
											<td align="right">抽检规格：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="criterion" listKey="id" value="%{inStorehousedefectiveproducts.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
										<tr>
											<td align="right">负责人：</td>
											<td><input id="principal" name="parentId" value="${inStorehousedefectiveproducts.principal}" type="text" /> <input type="hidden" id="parentId" name="parentId" value="${hrCategory.parentCategory.id }" /> <input class="btn" type="button" value="选择" onclick="chooseEmployees1();" /></td>
											<td align="right">操作人：</td>
											<td><input id="operator" name="parentId" value="${inStorehousedefectiveproducts.operator}" type="text" /> <input type="hidden" id="parentId" name="parentId" value="${hrCategory.parentCategory.id }" /> <input class="btn" type="button" value="选择" onclick="chooseEmployees2();" /></td>
										</tr>
										<tr>
											<td align="right">状态：</td>
											<td><s:select id="inspectionStatusId" headerKey="-1" headerValue="--请选择--" list="%{inspectionStatusList}" listValue="name" listKey="id" value="%{inStorehousedefectiveproducts.inspectionStatus.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
									</table>
								</dd>
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;">
						<div style="padding: 8px;">
							<table id="dlLineItem" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlLineItemTb',url: '${vix}/qm/inStorehousedefectiveproductsAction!getIncoming.action?id=${inStorehousedefectiveproducts.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
							...
							function removeDlLineItem(){
								...
								$.ajax({url:'${vix}/qm/inStorehousedefectiveproductsAction!deleteIncomingLineById.action?id='+row.id,cache: false});
					...
					<div id="a2" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<script type="text/javascript">
						$('#soAttach').datagrid({
							url: '${vix}/qm/inStorehousedefectiveproductsAction!getIncomingTemplate.action?id=${inStorehousedefectiveproducts.id}',
							title: '报检单附件',
							width: 'auto',
							height: '450',
							fitColumns: true,
							...
							toolbar:[{
								...
								handler:function(){
									...
									$.ajax({
										  url:'${vix}/qm/inStorehousedefectiveproductsAction!addAttachFile.action',
										  cache: false,
										  success: function(html){
											  asyncbox.open({
												 	modal:true,
													...
													callback : function(action,returnValue){
														if(action == 'ok'){
															uploadFile('${vix}/qm/inStorehousedefectiveproductsAction!uploadAttachment.action?id=${inStorehousedefectiveproducts.id}','fileToUpload');
					...
					<div id="a3" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<div style="padding: 8px;">
							<table id="dlInvoice" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlInvoiceTb',url: '${vix}/qm/inStorehousedefectiveproductsAction!getIncoming1.action?id=${inStorehousedefectiveproducts.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
										<th data-options="field:'id',align:'center',width:120">序号</th>
							...
							function removeDlInvoice(){
								...
								$.ajax({url:'${vix}/qm/inStorehousedefectiveproductsAction!deleteApplicationFormById.action?id='+row.id,cache: false});

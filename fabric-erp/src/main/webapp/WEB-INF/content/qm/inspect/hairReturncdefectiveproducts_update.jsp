...
/** input 获取焦点input效果绑定  */
	...
	function saveOrUpdateOrder(){
		...
		if($('#orderForm').validationEngine('validate')){
			$.post('${vix}/qm/hairReturncdefectiveproductsAction!saveOrUpdate.action',
				{
				'hairReturncdefectiveproducts.id' : $("#id").val(),
				'hairReturncdefectiveproducts.receiptsNumber':$("#receiptsNumber").val(),
				'hairReturncdefectiveproducts.productName':$("#productName").val(),
				'hairReturncdefectiveproducts.departmentDate':$("#departmentDate").val(),
				'hairReturncdefectiveproducts.sendReturnDepartment':$("#sendReturnDepartment").val(),
				'hairReturncdefectiveproducts.unqualifiedProcessingMode':$("#unqualifiedProcessingMode").val(),
				'hairReturncdefectiveproducts.scrappedHandle':$("#scrappedHandle").val(),
				'hairReturncdefectiveproducts.returnProcessing':$("#returnProcessing").val(),
				'hairReturncdefectiveproducts.departmentName':$("#departmentName").val(),
				'hairReturncdefectiveproducts.testMethods.id':$("#testMethodsId").val(),
				'hairReturncdefectiveproducts.inspectionCharacteristics.id':$("#inspectionCharacteristicsId").val(),
				'hairReturncdefectiveproducts.inspectionStatus.id':$("#inspectionStatusId").val(),
				'hairReturncdefectiveproducts.inspectionType.id':$("#inspectionTypeId").val(),
				'dlJson' : dlJson,	
				'spJson' : spJson
				},
				function(result){
					showMessage(result);
					setTimeout("",1000);
					loadContent('${vix}/qm/hairReturncdefectiveproductsAction!goList.action?menuId=menuContract');
				}
			);
		}
	}
						...
						callback : function(action,returnValue){
							if(action == 'ok'){
								if(returnValue != undefined){
									...
									selectNames = $("#chooseParentOrganization").val()+selectNames;
									
									$("#pubIds").val(selectIds);
									selectNames = selectNames.substring(1,selectNames.length);
									$("#chooseParentOrganization").val(selectNames);
								}
							}
						},
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="qm_hair_return_check" /></a></li>
				<li><a href="#" onclick="loadContent('${vix}/qm/hairReturncdefectiveproductsAction!goList.action?pageNo=${pageNo}');"><s:text name="qm_hair_return_defective_products_single" /></a></li>
				<li><a href="#"><s:text name="add" /></a></li>
			</ul>
		</div>
	</h2>
</div>
<input type="hidden" id="id" name="id" value="${hairReturncdefectiveproducts.id}" />
<div class="content">
	<form id="orderForm">
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateOrder();"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <%-- <a href="#"><img width="22" height="22" title="保存并提交审批" src="${vix}/common/img/dt_submit.png" /></a> 
						 --%> <a href="#" onclick="loadContent('${vix}/qm/hairReturncdefectiveproductsAction!goList.action?pageNo=${pageNo}');"> <img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a>
					</span> <strong><b><s:text name="新增发退货不良品处理单" /> </b><i></i> </strong>
				</dt>
				<dd class="clearfix">
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id="receiptsNumber" name="receiptsNumber" value="${hairReturncdefectiveproducts.receiptsNumber}" type="text" size="30" /></td>
											<td align="right">产品名称：</td>
											<td><input id="productName" name="productName" value="${hairReturncdefectiveproducts.productName}" type="text" size="30" /></td>
										</tr>
										<tr>
											<td align="right">检验日期：</td>
											<td><input id="departmentDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="departmentDate" value="<fmt:formatDate value='${hairReturncdefectiveproducts.departmentDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('departmentDate','yyyy-MM-dd')"
												src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"></td>
											<td align="right">发/退货部门：</td>
											<td><input type="hidden" id="pubIds" name="pubIds" value="${pubIds}" /> <input type="hidden" id="treeType" name="treeType" value="${treeType}" /> <input type="text" id="sendReturnDepartment" name="departmentName" readonly="readonly" value="${hairReturncdefectiveproducts.sendReturnDepartment}" /> <input class="btn" type="button"
												value="选择" onclick="chooseParentOrganization();" /></td>
										</tr>
										<tr>
											<td align="right">抽检规格：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="criterion" listKey="id" value="%{hairReturncdefectiveproducts.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
											<td align="right">检验方法：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="name" listKey="id" value="%{hairReturncdefectiveproducts.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
										<tr>
											<td align="right">不合格处理方式：</td>
											<td><input id="unqualifiedProcessingMode" name="unqualifiedProcessingMode" value="${hairReturncdefectiveproducts.unqualifiedProcessingMode}" type="text" size="30" /></td>
											<td align="right">报废处理：</td>
											<td><input id="scrappedHandle" name="scrappedHandle" value="${hairReturncdefectiveproducts.scrappedHandle}" type="text" size="30" /></td>
										</tr>
										<tr>
											<td align="right">退货处理：</td>
											<td><input id="returnProcessing" name="returnProcessing" value="${hairReturncdefectiveproducts.returnProcessing}" type="text" size="30" /></td>
										<tr>
											<td align="right">检验结果：</td>
											<td><s:select id="inspectionStatusId" headerKey="-1" headerValue="--请选择--" list="%{inspectionStatusList}" listValue="name" listKey="id" value="%{hairReturncdefectiveproducts.inspectionStatus.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
									</table>
								</dd>
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;">
						<div style="padding: 8px;">
							<table id="dlLineItem" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlLineItemTb',url: '${vix}/qm/hairReturncdefectiveproductsAction!getIncoming.action?id=${hairReturncdefectiveproducts.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
							...
							function removeDlLineItem(){
								...
								$.ajax({url:'${vix}/qm/hairReturncdefectiveproductsAction!deleteIncomingLineById.action?id='+row.id,cache: false});
					...
					<div id="a2" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<script type="text/javascript">
						$('#soAttach').datagrid({
							url: '${vix}/qm/hairReturncdefectiveproductsAction!getIncomingTemplate.action?id=${hairReturncdefectiveproducts.id}',
							title: '报检单附件',
							width: 'auto',
							height: '450',
							fitColumns: true,
							...
							toolbar:[{
								...
								handler:function(){
									...
									$.ajax({
										  url:'${vix}/qm/hairReturncdefectiveproductsAction!addAttachFile.action',
										  cache: false,
										  success: function(html){
											  asyncbox.open({
												 	modal:true,
													...
													callback : function(action,returnValue){
														if(action == 'ok'){
															uploadFile('${vix}/qm/hairReturncdefectiveproductsAction!uploadAttachment.action?id=${hairReturncdefectiveproducts.id}','fileToUpload');
					...
					<div id="a3" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<div style="padding: 8px;">
							<table id="dlInvoice" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlInvoiceTb',url: '${vix}/qm/hairReturncdefectiveproductsAction!getIncoming1.action?id=${hairReturncdefectiveproducts.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
										<th data-options="field:'id',align:'center',width:120">序号</th>
							...
							function removeDlInvoice(){
								...
								$.ajax({url:'${vix}/qm/hairReturncdefectiveproductsAction!deleteApplicationFormById.action?id='+row.id,cache: false});

...
function searchForContent(i){
	...
	loadProductName();
	if(i == 0){
		pager("start","${vix}/qm/hairReturnchecklistAction!goSearchList.action?i="+i+"&receiptsNumber="+name,'brand');
	}
	else{
		pager("start","${vix}/qm/hairReturnchecklistAction!goSearchList.action?i="+i+"&receiptsNumber="+receiptsNumber+"&productName="+productName,'brand');
	}
}
...
/*改变搜索按钮的显示*/
 ...
 function leastRecentlyUsed(inspectionDate){
 	pager("start","${vix}/qm/hairReturnchecklistAction!goSingleList.action?inspectionDate="+inspectionDate,'brand');
 }
...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/qm/hairReturnchecklistAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/qm/hairReturnchecklistAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/qm/hairReturnchecklistAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/qm/hairReturnchecklistAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/qm/hairReturnchecklistAction!goSingleList.action?name="+name,'brand');
}

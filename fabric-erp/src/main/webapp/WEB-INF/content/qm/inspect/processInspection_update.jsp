...
/** input 获取焦点input效果绑定  */
	...
	function saveOrUpdateOrder(){
		...
		if($('#orderForm').validationEngine('validate')){
			$.post('${vix}/qm/processInspectionAction!saveOrUpdate.action',
				{
				'processInspection.id' : $("#id").val(),
				'processInspection.participant':$("#participant").val(),
				'processInspection.inspectionDate':$("#inspectionDate").val(),
				'processInspection.inspectionSheetNumber':$("#inspectionSheetNumber").val(),
				'processInspection.departmentPM':$("#departmentPM").val(),
				'processInspection.checkoutDate':$("#checkoutDate").val(),
				'processInspection.qcEngineer':$("#qcEngineer").val(),
				'processInspection.qcOperator':$("#qcOperator").val(),
				'processInspection.processCode':$("#processCode").val(),
				'processInspection.processName':$("#processName").val(),
				'processInspection.testMethods.id':$("#testMethodsId").val(),
				'processInspection.inspectionCharacteristics.id':$("#inspectionCharacteristicsId").val(),
				'processInspection.inspectionStatus.id':$("#inspectionStatusId").val(),
				'dlJson' : dlJson,	
				'spJson' : spJson
				},
				function(result){
					showMessage(result);
					setTimeout("",1000);
					loadContent('${vix}/qm/processInspectionAction!goList.action?menuId=menuContract');
				}
			);
		}
	}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/qm/processInspectionAction!goList.action?pageNo=${pageNo}');"><s:text name="qm_product_defective_products_handle_single" /></a></li>
				<li><a href="#"><s:text name="add" /></a></li>
			</ul>
		</div>
	</h2>
</div>
<input type="hidden" id="id" name="id" value="${processInspection.id}" />
<div class="content">
	<form id="orderForm">
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateOrder();"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <%-- <a href="#"><img width="22" height="22" title="保存并提交审批" src="${vix}/common/img/dt_submit.png" /></a> 
						 --%> <a href="#" onclick="loadContent('${vix}/qm/processInspectionAction!goList.action?pageNo=${pageNo}');"> <img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a>
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id="participant" name="parentId" value="${processInspection.participant}" type="text" class="validate[required] text-input" /> <input type="hidden" id="parentId" name="parentId" value="${hrCategory.parentCategory.id }" /> <span style="color: red;">*</span> <input class="btn" type="button" value="选择"
												onclick="chooseEmployees();" /></td>
											<td align="right">报检日期：</td>
											<td><input id="inspectionDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="inspectionDate" value="<fmt:formatDate value='${processInspection.inspectionDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('inspectionDate','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif"
												width="16" height="22" align="absmiddle"></td>
										</tr>
										<tr>
											<td align="right">检验单编号：</td>
											<td><input id="inspectionSheetNumber" name="inspectionSheetNumber" value="${processInspection.inspectionSheetNumber}" type="text" size="30" /></td>
											<td align="right">检验日期：</td>
											<td><input id="checkoutDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="checkoutDate" value="<fmt:formatDate value='${processInspection.checkoutDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('checkoutDate','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif"
												width="16" height="22" align="absmiddle"></td>
										</tr>
										<tr>
											<td align="right">质检工程师：</td>
											<td><input id="qcEngineer" name="parentId" value="${processInspection.qcEngineer}" type="text" /> <input type="hidden" id="parentId" name="parentId" value="${hrCategory.parentCategory.id }" /> <input class="btn" type="button" value="选择" onclick="chooseEmployees1();" /></td>
											<td align="right">质检员：</td>
											<td><input id="qcOperator" name="parentId" value="${processInspection.qcOperator}" type="text" /> <input type="hidden" id="parentId" name="parentId" value="${hrCategory.parentCategory.id }" /> <input class="btn" type="button" value="选择" onclick="chooseEmployees2();" /></td>
										</tr>
										<tr>
											<td align="right">工序编码：</td>
											<td><input id="processCode" name="processCode" value="${processInspection.processCode}" type="text" size="30" /></td>
											<td align="right">工序名称：</td>
											<td><input id="processName" name="processName" value="${processInspection.processName}" type="text" size="30" /></td>
										</tr>
										<tr>
											<td align="right">检验方法：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="name" listKey="id" value="%{processInspection.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
											<td align="right">样品等级：</td>
											<td><s:select id="inspectionCharacteristicsId" headerKey="-1" headerValue="--请选择--" list="%{inspectionCharacteristicsList}" listValue="name" listKey="id" value="%{processInspection.inspectionCharacteristics.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
										<tr>
											<td align="right">检验项目：</td>
											<td><input id="departmentPM" name="departmentPM" value="${processInspection.departmentPM}" type="text" size="30" /></td>
											<%-- <td align="right">检验结果：</td>
										<td>
											<s:select id="inspectionStatusId" headerKey="-1" headerValue="--请选择--" 
											list="%{inspectionStatusList}" listValue="name" listKey="id" 
											value="%{processInspection.inspectionStatus.id}" multiple="false" theme="simple">
											</s:select>
										</td> --%>
										</tr>
									</table>
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					<div class="right_menu">
						<ul>
							...
							<li><a onclick="javascript:$('#a2').attr('style','');tab(3,2,'a',event);"><img src="${vix}/common/img/mail.png" alt="" />附件</a></li>
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;">
						<div style="padding: 8px;">
							<table id="dlLineItem" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlLineItemTb',url: '${vix}/qm/processInspectionAction!getIncoming.action?id=${processInspection.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
							...
							function removeDlLineItem(){
								...
								$.ajax({url:'${vix}/qm/processInspectionAction!deleteIncomingLineById.action?id='+row.id,cache: false});
					...
					<div id="a2" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<script type="text/javascript">
						$('#soAttach').datagrid({
							url: '${vix}/qm/processInspectionAction!getIncomingTemplate.action?id=${processInspection.id}',
							title: '报检单附件',
							width: 'auto',
							height: '450',
							fitColumns: true,
							...
							toolbar:[{
								...
								handler:function(){
									...
									$.ajax({
										  url:'${vix}/qm/processInspectionAction!addAttachFile.action',
										  cache: false,
										  success: function(html){
											  asyncbox.open({
												 	modal:true,
													...
													callback : function(action,returnValue){
														if(action == 'ok'){
															uploadFile('${vix}/qm/processInspectionAction!uploadAttachment.action?id=${processInspection.id}','fileToUpload');
					...
					<div id="a3" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<div style="padding: 8px;">
							<table id="dlInvoice" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlInvoiceTb',url: '${vix}/qm/processInspectionAction!getIncoming1.action?id=${processInspection.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
										<th data-options="field:'id',align:'center',width:120">序号</th>
							...
							function removeDlInvoice(){
								...
								$.ajax({url:'${vix}/qm/processInspectionAction!deleteApplicationFormById.action?id='+row.id,cache: false});

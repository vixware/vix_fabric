...
/** input 获取焦点input效果绑定  */
	...
	function saveOrUpdateOrder(){
		...
		if($('#orderForm').validationEngine('validate')){
			$.post('${vix}/qm/productTestingsingleAction!saveOrUpdate.action',
				{
				'productTestingsingle.id' : $("#id").val(),
				'productTestingsingle.productNumber':$("#productNumber").val(),
				'productTestingsingle.productName':$("#productName").val(),
				'productTestingsingle.productModel':$("#productModel").val(),
				'productTestingsingle.inspectionDate':$("#inspectionDate").val(),
				'productTestingsingle.itemId':$("#itemId").val(),
				'productTestingsingle.itemName':$("#itemName").val(),
				'productTestingsingle.itemCode':$("#itemCode").val(),
				'productTestingsingle.participant':$("#participant").val(),
				'productTestingsingle.departmentName':$("#departmentName").val(),
				'productTestingsingle.testMethods.id':$("#testMethodsId").val(),
				'productTestingsingle.inspectionCharacteristics.id':$("#inspectionCharacteristicsId").val(),
				'productTestingsingle.inspectionStatus.id':$("#inspectionStatusId").val(),
				'dlJson' : dlJson,	
				'spJson' : spJson
				},
				function(result){
					showMessage(result);
					setTimeout("",1000);
					loadContent('${vix}/qm/productTestingsingleAction!goList.action?menuId=menuContract');
				}
			);
		}
	}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="qm_incoming_inspection" /></a></li>
				<li><a href="#" onclick="loadContent('${vix}/qm/productTestingsingleAction!goList.action?pageNo=${pageNo}');"><s:text name="qm_product_testing_single" /></a></li>
				<li><a href="#"><s:text name="add" /></a></li>
			</ul>
		</div>
	</h2>
</div>
<input type="hidden" id="id" name="id" value="${productTestingsingle.id}" />
<div class="content">
	<form id="orderForm">
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateOrder();"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <%-- <a href="#"><img width="22" height="22" title="保存并提交审批" src="${vix}/common/img/dt_submit.png" /></a> 
							 --%> <a href="#" onclick="loadContent('${vix}/qm/productTestingsingleAction!goList.action?pageNo=${pageNo}');"> <img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a>
					</span> <strong><b><s:text name="qm_product_testing_single" /> </b><i></i> </strong>
				</dt>
				<dd class="clearfix">
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr height="40">
											...
											<td><input id="itemCode" name="productTestingsingle.itemCode" value="${productTestingsingle.itemCode}" type="text" class="validate[required] text-input" /> <input id="itemId" type="hidden" /> <span style="color: red;">*</span> <input class="btn" type="button" value="选择" onclick="chooseItem();" /></td>
											<th align="right">${vv:varView("vix_mdm_item")}名称:&nbsp;</th>
											<td><input id="itemName" name="productTestingsingle.itemName" value="${productTestingsingle.itemName}" type="text" class="validate[required] text-input" /> <span style="color: red;">*</span></td>
										</tr>
										<%-- <tr>
										<td align="right">产品编号：</td>
										<td><input id="productNumber" name="productNumber" value="${productTestingsingle.productNumber}" type="text" size="30" class="validate[required] text-input" /></td>
										<td align="right">产品名称：</td>
										<td><input id="productName" name="productName" value="${productTestingsingle.productName}" type="text" size="30" class="validate[required] text-input" /></td>
									</tr> --%>
										<tr>
											<td align="right">产品型号：</td>
											<td><input id="productModel" name="productModel" value="${productTestingsingle.productModel}" type="text" size="30" /></td>
											<td align="right">抽样标准：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="criterion" listKey="id" value="%{productTestingsingle.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
										<tr>
											<td align="right">报检人：</td>
											<td><input id="participant" name="parentId" value="${productTestingsingle.participant}" type="text" class="validate[required] text-input" /> <input type="hidden" id="parentId" name="parentId" value="${hrCategory.parentCategory.id }" /> <span style="color: red;">*</span> <input class="btn" type="button" value="选择"
												onclick="chooseEmployees();" /></td>
											<td align="right">检验部门：</td>
											<td><input type="hidden" id="pubIds" name="pubIds" value="${pubIds}" /> <input type="hidden" id="treeType" name="treeType" value="${treeType}" /> <input type="text" id="departmentName" name="departmentName" readonly="readonly" value="${productTestingsingle.departmentName}" /> <input class="btn" type="button" value="选择"
												onclick="chooseParentOrganization();" /></td>
										</tr>
										<tr>
											<td align="right">检验方法：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="name" listKey="id" value="%{productTestingsingle.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
											<td align="right">样品等级：</td>
											<td><s:select id="inspectionCharacteristicsId" headerKey="-1" headerValue="--请选择--" list="%{inspectionCharacteristicsList}" listValue="name" listKey="id" value="%{productTestingsingle.inspectionCharacteristics.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
										<tr>
											...
											<td><input id="inspectionDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="inspectionDate" value="<fmt:formatDate value='${productTestingsingle.inspectionDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('inspectionDate','yyyy-MM-dd')"
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;">
						<div style="padding: 8px;">
							<table id="dlLineItem" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlLineItemTb',url: '${vix}/qm/productTestingsingleAction!getIncoming.action?id=${productTestingsingle.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
							...
							function removeDlLineItem(){
								...
								$.ajax({url:'${vix}/qm/productTestingsingleAction!deleteIncomingLineById.action?id='+row.id,cache: false});
					...
					<div id="a2" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<script type="text/javascript">
						$('#soAttach').datagrid({
							url: '${vix}/qm/productTestingsingleAction!getIncomingTemplate.action?id=${productTestingsingle.id}',
							title: '报检单附件',
							width: 'auto',
							height: '450',
							fitColumns: true,
							...
							toolbar:[{
								...
								handler:function(){
									...
									$.ajax({
										  url:'${vix}/qm/productTestingsingleAction!addAttachFile.action',
										  cache: false,
										  success: function(html){
											  asyncbox.open({
												 	modal:true,
													...
													callback : function(action,returnValue){
														if(action == 'ok'){
															uploadFile('${vix}/qm/productTestingsingleAction!uploadAttachment.action?id=${productTestingsingle.id}','fileToUpload');
					...
					<div id="a3" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<div style="padding: 8px;">
							<table id="dlInvoice" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlInvoiceTb',url: '${vix}/qm/productTestingsingleAction!getIncoming1.action?id=${productTestingsingle.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
										<th data-options="field:'id',align:'center',width:120">序号</th>
							...
							function removeDlInvoice(){
								...
								$.ajax({url:'${vix}/qm/productTestingsingleAction!deleteApplicationFormById.action?id='+row.id,cache: false});

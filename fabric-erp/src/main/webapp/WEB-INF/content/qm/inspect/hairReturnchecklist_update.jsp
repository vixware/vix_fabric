...
/** input 获取焦点input效果绑定  */
	...
	function saveOrUpdateOrder(){
		...
		if($('#orderForm').validationEngine('validate')){
			$.post('${vix}/qm/hairReturnchecklistAction!saveOrUpdate.action',
				{
				'hairReturnchecklist.id' : $("#id").val(),
				'hairReturnchecklist.receiptsNumber':$("#receiptsNumber").val(),
				'hairReturnchecklist.productName':$("#productName").val(),
				'hairReturnchecklist.departmentDate':$("#departmentDate").val(),
				'hairReturnchecklist.sendReturnDepartment':$("#sendReturnDepartment").val(),
				'hairReturnchecklist.unqualifiedProcessingMode':$("#unqualifiedProcessingMode").val(),
				'hairReturnchecklist.customerName':$("#customerName").val(),
				'hairReturnchecklist.qcInspectionScheme':$("#qcInspectionScheme").val(),
				'hairReturnchecklist.departmentName':$("#departmentName").val(),
				'hairReturnchecklist.testMethods.id':$("#testMethodsId").val(),
				'hairReturnchecklist.inspectionCharacteristics.id':$("#inspectionCharacteristicsId").val(),
				'hairReturnchecklist.inspectionStatus.id':$("#inspectionStatusId").val(),
				'hairReturnchecklist.inspectionType.id':$("#inspectionTypeId").val(),
				'dlJson' : dlJson,	
				'spJson' : spJson
				},
				function(result){
					showMessage(result);
					setTimeout("",1000);
					loadContent('${vix}/qm/hairReturnchecklistAction!goList.action?menuId=menuContract');
				}
			);
		}
	}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/qm/hairReturnchecklistAction!goList.action?pageNo=${pageNo}');"><s:text name="qm_hair_return_check_list" /></a></li>
				<li><a href="#"><s:text name="add" /></a></li>
			</ul>
		</div>
	</h2>
</div>
<input type="hidden" id="id" name="id" value="${hairReturnchecklist.id}" />
<div class="content">
	<form id="orderForm">
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateOrder();"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <%-- <a href="#"><img width="22" height="22" title="保存并提交审批" src="${vix}/common/img/dt_submit.png" /></a> 
						 --%> <a href="#" onclick="loadContent('${vix}/qm/hairReturnchecklistAction!goList.action?pageNo=${pageNo}');"> <img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a>
					</span> <strong><b><s:text name="新增发退货检验单" /> </b><i></i> </strong>
				</dt>
				<dd class="clearfix">
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id="receiptsNumber" name="receiptsNumber" value="${hairReturnchecklist.receiptsNumber}" type="text" size="30" /></td>
											<td align="right">产品名称：</td>
											<td><input id="productName" name="productName" value="${hairReturnchecklist.productName}" type="text" size="30" /></td>
										</tr>
										<tr>
											<td align="right">检验日期：</td>
											<td><input id="departmentDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="departmentDate" value="<fmt:formatDate value='${hairReturnchecklist.departmentDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('departmentDate','yyyy-MM-dd')"
												src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"></td>
											<td align="right">发/退货部门：</td>
											<td><input id="sendReturnDepartment" name="sendReturnDepartment" value="${hairReturnchecklist.sendReturnDepartment}" type="text" size="30" /></td>
										</tr>
										<tr>
											<td align="right">抽检规格：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="criterion" listKey="id" value="%{hairReturnchecklist.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
											<td align="right">检验方法：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="name" listKey="id" value="%{hairReturnchecklist.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
										<tr>
											<td align="right">不合格处理方式：</td>
											<td><input id="unqualifiedProcessingMode" name="unqualifiedProcessingMode" value="${hairReturnchecklist.unqualifiedProcessingMode}" type="text" size="30" class="validate[required] text-input" /></td>
											<td align="right">客户名称：</td>
											<td><input id="customerName" name="customerName" value="${hairReturnchecklist.customerName}" type="text" size="30" /></td>
										</tr>
										<tr>
											<td align="right">质量检验方案：</td>
											<td><input id="qcInspectionScheme" name="qcInspectionScheme" value="${hairReturnchecklist.qcInspectionScheme}" type="text" size="30" /></td>
											<td align="right">检验部门：</td>
											<td><input type="hidden" id="pubIds" name="pubIds" value="${pubIds}" /> <input type="hidden" id="treeType" name="treeType" value="${treeType}" /> <input type="text" id="departmentName" name="departmentName" readonly="readonly" value="${hairReturnchecklist.departmentName}" /> <input class="btn" type="button" value="选择"
												onclick="chooseParentOrganization();" /></td>
										</tr>
										<tr>
										<tr>
											<td align="right">检验结果：</td>
											<td><s:select id="inspectionStatusId" headerKey="-1" headerValue="--请选择--" list="%{inspectionStatusList}" listValue="name" listKey="id" value="%{hairReturnchecklist.inspectionStatus.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
									</table>
								</dd>
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;">
						<div style="padding: 8px;">
							<table id="dlLineItem" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlLineItemTb',url: '${vix}/qm/hairReturnchecklistAction!getIncoming.action?id=${hairReturnchecklist.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
										...
										<!-- <th
											data-options="field:'inspectionDate',width:120,align:'center',editor:'datebox'">检验日期</th> -->
									</tr>
								</thead>
							</table>
							...
							function removeDlLineItem(){
								...
								$.ajax({url:'${vix}/qm/hairReturnchecklistAction!deleteIncomingLineById.action?id='+row.id,cache: false});
					...
					<div id="a2" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<script type="text/javascript">
						$('#soAttach').datagrid({
							url: '${vix}/qm/hairReturnchecklistAction!getIncomingTemplate.action?id=${hairReturnchecklist.id}',
							title: '报检单附件',
							width: 'auto',
							height: '450',
							fitColumns: true,
							...
							toolbar:[{
								...
								handler:function(){
									...
									$.ajax({
										  url:'${vix}/qm/hairReturnchecklistAction!addAttachFile.action',
										  cache: false,
										  success: function(html){
											  asyncbox.open({
												 	modal:true,
													...
													callback : function(action,returnValue){
														if(action == 'ok'){
															uploadFile('${vix}/qm/hairReturnchecklistAction!uploadAttachment.action?id=${hairReturnchecklist.id}','fileToUpload');
					...
					<div id="a3" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<div style="padding: 8px;">
							<table id="dlInvoice" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlInvoiceTb',url: '${vix}/qm/hairReturnchecklistAction!getIncoming1.action?id=${hairReturnchecklist.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
										<th data-options="field:'id',align:'center',width:120">序号</th>
										...
										<!-- <th
											data-options="field:'approvalDate',width:150,align:'center',editor:'datebox'">审批时间</th> -->
							...
							function removeDlInvoice(){
								...
								$.ajax({url:'${vix}/qm/hairReturnchecklistAction!deleteApplicationFormById.action?id='+row.id,cache: false});

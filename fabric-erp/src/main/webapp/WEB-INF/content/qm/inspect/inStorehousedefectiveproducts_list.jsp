...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/qm/inStorehousedefectiveproductsAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/qm/inStorehousedefectiveproductsAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/qm/inStorehousedefectiveproductsAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/qm/inStorehousedefectiveproductsAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/qm/inStorehousedefectiveproductsAction!goSingleList.action?name="+name,'brand');
}
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div class="search_advanced">
			<label>单据编号:<input id="trends_title" name="" type="text" class="int" /></label> <label>产品名称:<input id="trends_uploadPersonName" name="" type="text" class="int" /></label> <label> <input name="" type="button" class="btn" value="<s:text name='cmn_search'/>" onclick="searchForContent(1);" /> <input name="" type="button" class="btn"

...
<script type="text/javascript">
	...
	function saveOrUpdateOrder(){
		...
		if($('#orderForm').validationEngine('validate')){
			$.post('${vix}/qm/incomingChecklistAction!saveOrUpdate.action',
				{
				'incomingChecklist.id' : $("#id").val(),
				'incomingChecklist.receiptNumber':$("#receiptNumber").val(),
				'incomingChecklist.inspectionDate':$("#inspectionDate").val(),
				'incomingChecklist.itemId':$("#itemId").val(),
				'incomingChecklist.itemName':$("#itemName").val(),
				'incomingChecklist.itemCode':$("#itemCode").val(),
				'incomingChecklist.participant':$("#participant").val(),
				'incomingChecklist.departmentName':$("#departmentName").val(),
				'incomingChecklist.testMethods.id':$("#testMethodsId").val(),
				'incomingChecklist.inspectionCharacteristics.id':$("#inspectionCharacteristicsId").val(),
				'incomingChecklist.inspectionStatus.id':$("#inspectionStatusId").val(),
				'dlJson' : dlJson,	
				'spJson' : spJson
				},
				function(result){
					showMessage(result);
					setTimeout("",1000);
					loadContent('${vix}/qm/incomingChecklistAction!goList.action?menuId=menuContract');
				}
			);
		}
	}
...
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#" onclick="loadContent('${vix}/qm/incomingChecklistAction!goList.action?pageNo=${pageNo}');"><img src="img/qm_quality_management.png" alt="" /> <s:text name="supplyChain" /></a></li>
				<li><a href="#" onclick="loadContent('${vix}/qm/incomingChecklistAction!goList.action?pageNo=${pageNo}');"><s:text name="qm_quality_management" /></a></li>
				<li><a href="#" onclick="loadContent('${vix}/qm/incomingChecklistAction!goList.action?pageNo=${pageNo}');"><s:text name="qm_incoming_inspection" /></a></li>
				<li><a href="#" onclick="loadContent('${vix}/qm/incomingChecklistAction!goList.action?pageNo=${pageNo}');"><s:text name="来料检验单" /></a></li>
				<li><a href="#" onclick="loadContent('${vix}/qm/incomingChecklistAction!goList.action?pageNo=${pageNo}');"><s:text name="add" /></a></li>
			</ul>
		</div>
	</h2>
</div>
<input type="hidden" id="id" name="id" value="${incomingChecklist.id}" />
<div class="content">
	<form id="orderForm">
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateOrder();"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <%-- <a href="#"><img width="22" height="22" title="保存并提交审批" src="${vix}/common/img/dt_submit.png" /></a> 
							  --%> <a href="#" onclick="loadContent('${vix}/qm/incomingChecklistAction!goList.action?pageNo=${pageNo}');"> <img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a>
					</span> <strong><b><s:text name="新增来料检验单" /> </b><i></i> </strong>
				</dt>
				<dd class="clearfix">
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr height="40">
											...
											<td><input id="itemCode" name="incomingChecklist.itemCode" value="${incomingChecklist.itemCode}" type="text" class="validate[required] text-input" /> <input id="itemId" type="hidden" /> <span style="color: red;">*</span> <input class="btn" type="button" value="选择" onclick="chooseItem();" /></td>
											<th align="right">${vv:varView("vix_mdm_item")}名称:&nbsp;</th>
											<td><input id="itemName" name="incomingChecklist.itemName" value="${incomingChecklist.itemName}" type="text" class="validate[required] text-input" /> <span style="color: red;">*</span></td>
										</tr>
										<tr>
											<td align="right">单据编号：</td>
											<td><input id="receiptNumber" name="receiptNumber" value="${incomingChecklist.receiptNumber}" type="text" size="30" /></td>
											<span style="color: red;">*</span>
											<td align="right">抽样标准：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="criterion" listKey="id" value="%{incomingChecklist.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
										<tr>
											<td align="right">报检人：</td>
											<td><input id="participant" name="parentId" value="${incomingChecklist.participant}" type="text" class="validate[required] text-input" /> <input type="hidden" id="parentId" name="parentId" value="${hrCategory.parentCategory.id }" /> <span style="color: red;">*</span> <input class="btn" type="button" value="选择"
												onclick="chooseEmployees();" /></td>
											<td align="right">检验状态：</td>
											<td><s:select id="inspectionStatusId" headerKey="-1" headerValue="--请选择--" list="%{inspectionStatusList}" listValue="name" listKey="id" value="%{incomingChecklist.inspectionStatus.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
										<tr>
											<td align="right">检验部门：</td>
											<td><input type="hidden" id="pubIds" name="pubIds" value="${pubIds}" /> <input type="hidden" id="treeType" name="treeType" value="${treeType}" /> <input type="text" id="departmentName" name="departmentName" readonly="readonly" value="${incomingChecklist.departmentName}" /> <input class="btn" type="button" value="选择"
												onclick="chooseParentOrganization();" /></td>
											<td align="right">检验方法：</td>
											<td><s:select id="testMethodsId" headerKey="-1" headerValue="--请选择--" list="%{testMethodsList}" listValue="name" listKey="id" value="%{incomingChecklist.testMethods.id}" multiple="false" theme="simple">
												</s:select></td>
										</tr>
										<tr>
											<td align="right">样品等级：</td>
											<td><s:select id="inspectionCharacteristicsId" headerKey="-1" headerValue="--请选择--" list="%{inspectionCharacteristicsList}" listValue="name" listKey="id" value="%{incomingChecklist.inspectionCharacteristics.id}" multiple="false" theme="simple">
												</s:select></td>
											<td align="right">检验日期：</td>
											<td><input id="inspectionDate" name="incomingChecklist.inspectionDate" value="${incomingChecklist.inspectionDate}" onclick="showTime('inspectionDate','yyyy-MM-dd')" onchange="salesOrderFieldChanged(this);" type="text" size="30" class="validate[required] text-input" /> <span style="color: red;">*</span> <img
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;">
						<div style="padding: 8px;">
							<table id="dlLineItem" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlLineItemTb',url: '${vix}/qm/incomingChecklistAction!getIncoming.action?id=${incomingChecklist.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
							...
							function removeDlLineItem(){
								...
								$.ajax({url:'${vix}/qm/incomingChecklistAction!deleteIncomingLineById.action?id='+row.id,cache: false});
					...
					<div id="a2" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<script type="text/javascript">
						$('#soAttach').datagrid({
							url: '${vix}/qm/incomingChecklistAction!getIncomingTemplate.action?id=${incomingChecklist.id}',
							title: '报检单附件',
							width: 'auto',
							height: '450',
							fitColumns: true,
							...
							toolbar:[{
								...
								handler:function(){
									...
									$.ajax({
										  url:'${vix}/qm/incomingChecklistAction!addAttachFile.action',
										  cache: false,
										  success: function(html){
											  asyncbox.open({
												 	modal:true,
													...
													callback : function(action,returnValue){
														if(action == 'ok'){
															uploadFile('${vix}/qm/incomingChecklistAction!uploadAttachment.action?id=${incomingChecklist.id}','fileToUpload');
					...
					<div id="a3" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<div style="padding: 8px;">
							<table id="dlInvoice" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlInvoiceTb',url: '${vix}/qm/incomingChecklistAction!getIncoming1.action?id=${incomingChecklist.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
										<th data-options="field:'id',align:'center',width:120">序号</th>
							...
							function removeDlInvoice(){
								...
								$.ajax({url:'${vix}/qm/incomingChecklistAction!deleteApplicationFormById.action?id='+row.id,cache: false});

...
function saveOrUpdate(id){
	/* $.ajax({
		  url:'${vix}/common/model/orginialBaseMetaDataCategoryAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  ...
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							$("#baseMetaDataCategoryForm").validationEngine('validate');
							var queryString = $('#baseMetaDataCategoryForm').formSerialize(); 
							$.post('${vix}/common/model/orginialBaseMetaDataCategoryAction!saveOrUpdate.action',
								queryString,
								function(result){
									showMessage(result);
									setTimeout("", 1000);
									pager("start","${vix}/common/model/orginialBaseMetaDataCategoryAction!goSingleList.action?categoryName="+name,'metaDataCategory');
								}
							 );
						}
					},
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/model/orginialBaseMetaDataCategoryAction!deleteById.action?id='+id,
		  cache: false,
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/model/orginialBaseMetaDataCategoryAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&categoryName="+name,'metaDataCategory');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/model/orginialBaseMetaDataCategoryAction!goSingleList.action?categoryName="+name,'metaDataCategory');
}
...
function importXlsFile(){
	$.ajaxFileUpload({
		url : '${vix}/common/model/orginialBaseMetaDataCategoryAction!importFile.action',//用于文件上传的服务器端请求地址
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="system_control_basedata_metadatacategory" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/common/model/orginialVarAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	asyncbox.confirm('确定要删除该常量么?','<s:text name='vix_message'/>',function(action){
		if(action == 'ok'){
			$.ajax({
				  url:'${vix}/common/model/orginialVarAction!deleteById.action?id='+id,
				  cache: false,
				  success: function(result){
					  showMessage(result);
					  setTimeout("", 1000);
						pager("start","${vix}/common/model/orginialVarAction!goSingleList.action?varCode="+name,'orginialVar');
				  }
			});
		}
	});
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/model/orginialVarAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&varCode="+name,'orginialVar');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/model/orginialVarAction!goSingleList.action?varCode="+name,'orginialVar');
}
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/common/model/orginialVarAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#orginialVarForm').validationEngine('validate')){
								var queryString = $('#orginialVarForm').formSerialize(); 
								$.post('${vix}/common/model/orginialVarAction!saveOrUpdate.action',
									queryString,
									function(result){
										showMessage(result);
										setTimeout("", 1000);
										//loadContent('${vix}/common/model/orginialVarAction!goList.action');
										pager("start","${vix}/common/model/orginialVarAction!goSingleList.action?varCode="+name,'orginialVar');
									}
								 );
							}else {
...
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb orginialVar">
			<ul>
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="orginialVarInfo"></b> <s:text name='cmn_to' /> <b class="orginialVarTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a> </span> <span><a class="end" onclick="currentPager('end');"></a> </span>
					</div>
				</div>
				<div id="orginialVar" class="table"></div>
				<div class="pagelist drop">
					<ul>

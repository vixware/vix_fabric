...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/common/currencyTypeAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#currencyTypeForm').validationEngine('validate')){
								$.post('${vix}/common/currencyTypeAction!saveOrUpdate.action',
									 {'currencyType.id':$("#id").val(),
									  'currencyType.code':$("#code").val(),
									  'currencyType.name':$("#name").val(),
									  'companyId':$("#selectId").val(),
									  'currencyType.isBaseCurrency':$(":radio[name=isBaseCurrency][checked]").val()
									},
									function(result){
										asyncbox.success(result,"<s:text name='vix_message'/>",function(action){
											pager("current","${vix}/common/currencyTypeAction!goListContent.action?name="+name,'currencyType');
										});
									}
								 ); 
			  				}else{
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/currencyTypeAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/common/currencyTypeAction!goListContent.action?name="+name,'currencyType');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/currencyTypeAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'currencyType');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/currencyTypeAction!goListContent.action?name="+name,'currencyType');
}
					...
					callback : function(action,returnValue){
						if(action == 'ok'){
							if(returnValue != undefined){
								...
								loadName();
								pager('start',"${vix}/common/currencyTypeAction!goListContent.action?name="+name+'&companyCode='+result[0],'currencyType');
							}
						}
					},
					btnsbar : $.btn.OKCANCEL
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">基础设置</a></li>
				<li><a href="#">字典管理</a></li>
				<li><a href="#" onclick="loadContent('${vix}/common/currencyTypeAction!goList.action');">币种管理</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">
                ...
                function onClick(event,treeId,treeNode,clickFlag) {
                	...
                	pager("start","${vix}/common/currencyTypeAction!goListContent.action?companyCode="+treeNode.id,'currencyType');
                }

...
function initAuthorityTree(){
	...
	var treeNodeTmp = [];
	$.ajax({
		url:"${vix}/common/security/role/roleAction!findAuthorityTreeData.action",
		data:{bizType : bizTypeTmp, roleId : '${id}'},
		cache: false,
		async:false,
		dataType : "text",
			...
			btn.bind("click", function(){
				asyncbox.confirm('确定要授权\''+treeNode.name+'\'及所有子权限么?','<s:text name="cmn_message"/>',function(action){
					if(action == 'ok'){
						...
						$.post('${vix}/common/security/role/roleAction!saveForAuthorityByAuthId.action',
								{
									roleId :'${id}',
									bizType : bizTypeTmp,
									authId : treeNode.id									
			...
			btn.bind("click", function(){
				asyncbox.confirm('确定要取消授权\''+treeNode.name+'\'及所有子权限么?','<s:text name="cmn_message"/>',function(action){
					if(action == 'ok'){
						...
						$.post('${vix}/common/security/role/roleAction!deleteForAuthorityByAuthId.action',
								{
									roleId :'${id}',
									bizType : bizTypeTmp,
									authId : treeNode.id									
...
function loadAuthorityFunGrid(selectId){
	...
	$('#dgAuthFun').datagrid({
		...
		striped: true,
		collapsible:true,
		url:'${vix}/common/security/role/roleAction!findFunGrid.action',
		queryParams : {
			roleId : '${id}',
			authId : selectId, 
		...
		frozenColumns:[[
			...
			{field:'memo',title:'说明',width:400,editor:'text'}
		]],
		/* columns:[[
...
function saveForAuthority(){
	...
	$.post('${vix}/common/security/role/roleAction!saveForAuthority.action',
		{
			roleId:'${id}',
			bizType : bizTypeTmp,

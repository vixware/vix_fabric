...
function saveOrUpdate(id,parentId){
	if(parentId!=null && parentId!=""){
		$.ajax({
			  url:'${vix}/common/security/authority/authorityAction!goSaveOrUpdate.action?id='+id+"&parentTreeId="+parentId,
			  cache: false,
							...
							if(action == 'ok'){
								if($("#authorityForm").validationEngine('validate')){
									if(confirm("确定要保存权限吗?")){
										...
										$.post('${vix}/common/security/authority/authorityAction!saveOrUpdate.action',
											queryString,
											function(result){
													...
													pager("start","${vix}/common/security/authority/authorityAction!goSingleList.action?authorityName="+name+"&treeId="+$("#selectId").val()+"&bizType="+$("#selectBizType").val(),'category');
											}
										 );
									}
								}else{
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/security/authority/authorityAction!deleteById.action?id='+id,
		  cache: false,
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/security/authority/authorityAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&authorityName="+name+"&treeId="+$("#selectId").val()+"&bizType="+$("#selectBizType").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/common/security/authority/authorityAction!goSingleList.action?authorityName="+name+"&treeId="+$("#selectId").val()+"&bizType="+$("#selectBizType").val(),entity);
	}
	/* if(entity == 'brand'){
...
function importXlsFile(){
	$.ajaxFileUpload({
		url : '${vix}/common/security/authorityAction!importFile.action',//用于文件上传的服务器端请求地址
		...
		success : function(data, status){ //服务器成功响应处理函数
			...
			if(data.success=='1'){
				...
				setTimeout("", 1000);
				//pager("start","${vix}/common/security/authority/authorityAction!goSingleList.action?authorityName="+name+"&id="+$("#selectId").val(),'category');
				//debugger;
				//var treeObj = $.fn.zTree.getZTreeObj("tree_root");
				//treeObj.refresh();
				//debugger;
				//zTree.refresh();
				loadContent('${vix}/common/security/authorityAction!goList.action');
			}else{
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/common/security/authority/authorityAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/common/security/authority/authorityAction!goSingleList.action?treeId="+treeNode.treeId+"&bizType="+treeNode.bizType,"category");
				}

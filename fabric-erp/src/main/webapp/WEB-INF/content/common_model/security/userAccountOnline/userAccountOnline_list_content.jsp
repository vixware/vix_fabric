...
<input type="hidden" id="userAccountInfoHidden" value="${(pager.pageNo-1)*pager.pageSize+1}-${(pager.pageNo-1)*pager.pageSize+pager.pageSize}"/>
 --%>
<input type="hidden" id="userAccountTotalCountHidden" value="${param.userAccountOnlineMap.keySet.size}" />
...
<table class="list">
	<tbody>
		...
		<% int count = 0; %>
		<s:iterator value="%{#request.userAccountOnlineMap.keySet}" var="entity" status="s">
			<% count++; %>
			<tr>
				<td><input id="chkId" name="chkId" value="${entity.id}" type="checkbox" onchange="selectCount()" /></td>
				<%-- <td>${s.count + (pager.pageSize * (pager.pageNo -1))}</td> --%>
				<td>${entity.account}</td>
				<td><span style="color: blue;"> ${entity.tenantId} </span></td>
				<td><s:date name="%{#entity.lastRequestDate}" /></td>
				<td><s:if test="tag != 'choose'">
						<a href="#" onclick="removeUserAccount('${entity.sessionId}');" title="<s:text name='update'/>"> 剔除 </a>

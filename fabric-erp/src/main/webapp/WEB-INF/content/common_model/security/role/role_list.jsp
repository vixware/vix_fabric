...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/common/security/roleAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
								...
								if(confirm("确定要保存角色吗?")){
									...
									$.post('${vix}/common/security/roleAction!saveOrUpdate.action',
										...
										function(result){
											showMessage(result);
										  	setTimeout("", 1000);
											pager("start","${vix}/common/security/roleAction!goSingleList.action?roleName="+name,'role');
										}
									 );
								}
							}else{
...
function toAuthority(id){
	$.ajax({
		  url:'${vix}/common/security/roleAction!goRoleAuthority.action?id='+id+"&roleName="+name,
		  cache: false,
		  success: function(html){
	...
	/*$.ajax({
		  url:'${vix}/common/security/roleAction!goRoleAuthority.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
					...
					callback : function(action){
						if(action == 'ok'){
							$("#roleForm").validationEngine('validate');
							var queryString = $('#roleForm').formSerialize(); 
							$.post('${vix}/common/security/roleAction!saveOrUpdate.action',
								queryString,
								function(result){
...
function toRoleColConfig(id){
	$.ajax({
		  url:'${vix}/common/security/roleAction!goColConfig.action?roleId='+id,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function toRolePdc(id){//roleIndexPdc_list ${vix}/common/security/roleAction!goIndexPdcList.action?roleId=${roleId}
	$.ajax({
		  url:'${vix}/common/security/roleAction!goIndexPdc.action?roleId='+id,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/security/roleAction!deleteById.action?id='+id,
		  cache: false,
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/security/roleAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&roleName="+name,'role');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/security/roleAction!goSingleList.action?roleName="+name,'role');
}
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div>
			<label>角色名：<input type="text" class="int" id="roleName" value="${roleName}"></label> <label> <input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();" />
			</label>
		</div>

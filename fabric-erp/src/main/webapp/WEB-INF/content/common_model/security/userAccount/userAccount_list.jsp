...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/common/security/userAccountAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
					...
					callback : function(action){
						if(action == 'ok'){
							if($("#userAccountForm").validationEngine('validate')){
								...
								var msgTmp = "";
								$.ajax({
									url:"${vix}/common/security/userAccountAction!checkUserAccount.action",
									data:{id : $("#id").val(), account : $("#account").val()},
									cache: false,
									async:false,
									dataType : "text",
								...
								if(!isSuccess){
									showErrorMessage(msgTmp);
									setTimeout("", 1000);
									return false;
								}
								if(confirm("确定要保存账号吗?")){
									var queryString = $('#userAccountForm').formSerialize(); 
									$.post('${vix}/common/security/userAccountAction!saveOrUpdate.action',
										queryString,
										function(result){
											showMessage(result);
											setTimeout("", 1000);
											pager("start","${vix}/common/security/userAccountAction!goSingleList.action?account="+name,'userAccount');
										}
									 );
								}else{
									return false;
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/security/userAccountAction!deleteById.action?id='+id,
		  cache: false,
...
function resetPwd(id){
	$.ajax({
		  url:'${vix}/common/security/userAccountAction!goResetPwd.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
					...
					callback : function(action){
						if(action == 'ok'){
								...
								$.ajax({
									url:"${vix}/common/security/userAccountAction!resetPwd.action",
									data:{
										id : id, 
										password : md5Vixnt.hex_md5($("#password").val()),
										passwordConfirm : md5Vixnt.hex_md5($("#passwordConfirm").val())
									},
									cache: false,
									async:false,
									dataType : "text",
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/security/userAccountAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&account="+name,'userAccount');
}


function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/security/userAccountAction!goSingleList.action?account="+name,'userAccount');
}
/* 
bindSearch();
...
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#"><img src="${vix}/common/img/system/sec_useraccount.png" alt="" />
				...
				<li><a href="#"><s:text name="system_control_org_sec_useraccount" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div>
			<label><s:text name="cmn_name" /><input type="text" class="int" id="userAccountName" value=""></label> <label> <input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();" />
			</label>
		</div>

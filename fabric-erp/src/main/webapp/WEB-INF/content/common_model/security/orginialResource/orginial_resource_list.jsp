...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/common/security/orginialResourceAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
						...
						if(action == 'ok'){
							if($("#resourceForm").validationEngine('validate')){
								if(confirm("确定要保存资源吗?")){
									...
									$.post('${vix}/common/security/orginialResourceAction!saveOrUpdate.action',
										queryString,
										function(result){
											showMessage(result);
											setTimeout("", 1000);
											pager("start","${vix}/common/security/orginialResourceAction!goSingleList.action?searchResourceName="+name,'resource');
										}
									 );
								}
							}else{
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/security/orginialResourceAction!deleteById.action?id='+id,
		  cache: false,
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/security/orginialResourceAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&searchResourceName="+name,'resource');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/security/orginialResourceAction!goSingleList.action?searchResourceName="+name,'resource');
}

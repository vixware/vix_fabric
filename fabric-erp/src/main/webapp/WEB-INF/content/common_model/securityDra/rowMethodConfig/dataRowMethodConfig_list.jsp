...
function saveOrUpdate(id){
	...
	$.ajax({
		  url:'${vix}/common/security/dataResRowMethodConfigAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							$("#dataRowMethodConfigForm").validationEngine();
							
							if($("#dataRowMethodConfigForm").validationEngine('validate')){
								var queryString = $('#dataRowMethodConfigForm').formSerialize(); 
								$.post('${vix}/common/security/dataResRowMethodConfigAction!saveOrUpdate.action',
									queryString,
									function(result){
										showMessage(result);
										setTimeout("", 1000);
										pager("start","${vix}/common/security/dataResRowMethodConfigAction!goSingleList.action?methodName="+name,'dataRowMethodConfig');
									}
								 );
							}else{
								showErrorMessage("配置方法或者元数据未填写！");
								setTimeout("", 1000);
								return false;
							}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/security/dataResRowMethodConfigAction!deleteById.action?id='+id,
		  cache: false,
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/security/dataResRowMethodConfigAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&methodName="+name,'dataRowMethodConfig');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/security/dataResRowMethodConfigAction!goSingleList.action?methodName="+name,'dataRowMethodConfig');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="system_control_org_sec_dataRowMethodConfig" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div>
			<label><s:text name="cmn_name" /><input type="text" class="int" id="methodName"></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();" /> </label>
		</div>
	...
	<div class="box">
		...
		<div id="right">
			<div class="right_content">
				...
				<div id="dataRowMethodConfig" class="table"></div>
				<div class="pagelist drop">
					<ul>

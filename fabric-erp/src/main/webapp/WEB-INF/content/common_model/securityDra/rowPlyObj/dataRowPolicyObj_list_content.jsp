...
function deleteEntity(id){
	asyncbox.confirm('确定要删除该策略配置么?','<s:text name='cmn_message'/>',function(action){
		if(action == 'ok'){
			deleteById(id);
		}
	});
...
<input type="hidden" id="dataRowPolicyTotalCountHidden" value="${pager.totalCount}" />
<input type="hidden" id="dataRowPolicyPageNoHidden" value="${pager.pageNo}" />
<input type="hidden" id="dataRowPolicyTotalPageHidden" value="${pager.totalPage}" />
<input type="hidden" id="dataRowPolicyOrderField" value="${pager.orderField}" />
<input type="hidden" id="dataRowPolicyOrderBy" value="${pager.orderBy}" />
<input type="hidden" id="dataRowPolicyInfoHidden" value="${(pager.pageNo-1)*pager.pageSize+1}-${(pager.pageNo-1)*pager.pageSize+pager.pageSize}" />
<table class="list">
	<tbody>
		<tr class="alt">
			<th width="5%">
			...
			<th width="25%">权限标识名</th>
			<th width="5%">优先级</th>
			<th width="40%">业务对象全称</th>
			<th width="15%">元数据名称</th>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td>${entity.baseMetaData.metaDataDisplayName}</td>
				<td><s:if test="tag != 'choose'">
						...
						<a href="#" onclick="setPolicyObj('${entity.id}');" title="<s:text name='update'/>"> 配置 </a>

...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/common/security/dataResRowMethodAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						$("#dataRowMethodForm").validationEngine();
						
						if(action == 'ok'){
							
							if($("#dataRowMethodForm").validationEngine('validate')){
								var queryString = $('#dataRowMethodForm').formSerialize(); 
								$.post('${vix}/common/security/dataResRowMethodAction!saveOrUpdate.action',
									queryString,
									function(result){
										showMessage(result);
										setTimeout("", 1000);
										pager("start","${vix}/common/security/dataResRowMethodAction!goSingleList.action?hqlProvider="+name,'dataRowMethod');
									}
								 );
							}else{
								return false;
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/security/dataResRowMethodAction!deleteById.action?id='+id,
		  cache: false,
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/security/dataResRowMethodAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&hqlProvider="+name,'dataRowMethod');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/security/dataResRowMethodAction!goSingleList.action?hqlProvider="+name,'dataRowMethod');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="system_control_org_sec_dataRowMethod" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div>
			<label><s:text name="cmn_name" /><input type="text" class="int" id="hqlProvider"></label> <label><s:text name="cmn_my_item" /><input type="checkbox" value="" name=""></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();" /><input type="button"
	...
	<div class="box">
		...
		<div id="right">
			<div class="right_content">
				...
				<div id="dataRowMethod" class="table"></div>
				<div class="pagelist drop">
					<ul>

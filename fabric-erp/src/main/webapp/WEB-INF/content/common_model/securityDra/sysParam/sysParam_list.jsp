...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/common/security/dataResRowSysParamAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/security/dataResRowSysParamAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(result){
			  showMessage(result);
			  setTimeout("", 1000);
				pager("start","${vix}/common/security/dataResRowSysParamAction!goSingleList.action?keyPropertyName="+name,'userAccount');
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/security/dataResRowSysParamAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&keyPropertyName="+name,'userAccount');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/security/dataResRowSysParamAction!goSingleList.action?keyPropertyName="+name,'userAccount');
}
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div>
			<label><s:text name="cmn_name" /><input type="text" class="int" id="keyPropertyName" value=""></label> <label><s:text name="cmn_my_item" /><input type="checkbox" value="" name=""></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();" /><input type="button"

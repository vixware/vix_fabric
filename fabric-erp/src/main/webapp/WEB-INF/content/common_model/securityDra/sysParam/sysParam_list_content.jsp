...
function deleteEntity(id){
	asyncbox.confirm('确定要删除该参数配置么?','<s:text name='cmn_message'/>',function(action){
		if(action == 'ok'){
			deleteById(id);
		}
	});
	...
	<tbody>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td>${entity.keyProperty}</td>
				<td>${entity.keyPropertyName}</td>
				<td><s:if test="#entity.paramType == \"S\"">
						<span style="color: red;">系统常量</span>
					</s:if> <s:elseif test="#entity.paramType == \"P\"">
						<span style="color: blue;">预处理常量</span>
					</s:elseif> <s:else>其他</s:else></td>
				<td><s:if test="#entity.isActive == \"0\"">
						<span style="color: red;">禁用</span>
					</s:if> <s:elseif test="#entity.isActive == \"1\"">
						<span style="color: blue;">激活</span>
					</s:elseif> <s:else>其他</s:else></td>
				<%-- <td>
					<s:if test="#entity.readFromPreData == \"0\"">
						<span style="color: red;">否</span>
					</s:if>
					<s:elseif test="#entity.readFromPreData == \"1\"">
						<span style="color: blue;">是</span>
					</s:elseif>
					<s:else>其他${entity.readFromPreData }</s:else>
				</td> --%>

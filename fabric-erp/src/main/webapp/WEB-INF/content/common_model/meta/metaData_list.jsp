...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/common/model/baseMetaDataAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/model/baseMetaDataAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(result){
			  showMessage(result);
			  setTimeout("", 1000);
			  pager("start","${vix}/common/model/baseMetaDataAction!goSingleList.action?metaDataName="+name,'metaDataCategory');
		  }
	});
}
...
function initHbmMetadata(){
	$.ajax({
		  url:'${vix}/common/model/baseMetaDataAction!initSystemMetadata.action',
		  cache: false,
		  success: function(result){
			  showMessage(result);
			  setTimeout("", 1000);
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/model/baseMetaDataAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&metaDataName="+name,'metaDataCategory');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/model/baseMetaDataAction!goSingleList.action?metaDataName="+name,'metaDataCategory');
}
...
function importXlsFile(){
	$.ajaxFileUpload({
		url : '${vix}/common/model/baseMetaDataAction!importFile.action',//用于文件上传的服务器端请求地址
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/common/model/baseMetaDataAction!findTreeToJson.action",
						autoParam:["id"]
					},
					callback: {
						onClick: onClick

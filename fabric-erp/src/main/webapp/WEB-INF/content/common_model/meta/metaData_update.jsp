...
<!--
	...
	function goSaveUpdateMetaDataDefine(metaDataDefineId){
		...
		$.ajax({
			  url:'${vix}/common/model/baseMetaDataDefineAction!goSaveOrUpdate.action',
				  ...
				  asyncbox.open({
						...
						callback : function(action,returnValue){
							if(action == 'ok'){
								...
								$.post('${vix}/common/model/baseMetaDataDefineAction!saveOrUpdate.action',
									queryString,
									function(result){
										showMessage(result);
										setTimeout("", 1000);
	...
	function loadMetaDataGrid(){
		...
		$('#dgdA1').datagrid({
			...
			striped: true,
			collapsible:true,
			url:'${vix}/common/model/baseMetaDataDefineAction!goSingleList.action?metaDataId='+metaDataId,
			remoteSort: false,
			idField:'id',
			frozenColumns:[[
			...
			toolbar:[{
				...
				handler:function(){
					...
					if(idVal!=''){
						...
						$.post('${vix}/common/model/baseMetaDataDefineAction!deleteById.action',
							queryString,
							function(result){
								showMessage(result);
								setTimeout("", 1000);
	...
	function loadMetaDataGridExt(){
		...
		$('#dgdA2').datagrid({
			...
			striped: true,
			collapsible:true,
			url:'${vix}/common/model/baseMetaDataExtAction!goSingleList.action?metaDataId='+metaDataId,
			remoteSort: false,
			idField:'id',
			frozenColumns:[[
				{field:'ck',checkbox:true},
				...
				handler:function(){
					...
					asyncbox.confirm('确定要删除么?','提示信息',function(action){
						if(action == 'ok'){
							...
							if(idVal!=''){
								var queryString = "id="+idVal;
								$.post('${vix}/common/model/baseMetaDataExtAction!deleteById.action',
									queryString,
									function(result){
										showMessage(result);
										setTimeout("", 1000);
	...
	function goSaveUpdateMetaDataExt(metaDataExtId){
		var metaDataId = $("#metaDataId").val();//metaDataId
		if(metaDataId=="" || metaDataId=="0"){
			asyncbox.alert("请先保存元数据信息！","提示");
			return;
		}

		$.ajax({
			  url:'${vix}/common/model/baseMetaDataExtAction!goSaveOrUpdate.action',
				  ...
				  asyncbox.open({
						...
						callback : function(action,returnValue){
							if(action == 'ok'){
								...
								$.post('${vix}/common/model/baseMetaDataExtAction!saveOrUpdate.action',
									queryString,
									function(result){
										showMessage(result);
										setTimeout("", 1000);
	...
	<dl>
		<dt>
			<span class="no_line"> <a href="#" onclick="saveOrUpdateMetaData()"><img width="22" height="22" alt="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" alt="保存并新增" src="${vix}/common/img/dt_savenew.png" /></a> <a href="#"><img width="22" height="22" alt="取消"
				...
				</div> <a href="#"><img width="22" height="22" alt="导出" src="${vix}/common/img/dt_export.png"></a> <a href="#" onclick="loadContent('${vix}/common/model/baseMetaDataAction!goList.action?pageNo=${pageNo}');"><img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a>

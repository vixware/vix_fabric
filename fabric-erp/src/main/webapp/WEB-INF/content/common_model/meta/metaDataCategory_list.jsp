...
function saveOrUpdate(id){
	/* $.ajax({
		  url:'${vix}/common/model/baseMetaDataCategoryAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
			  ...
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							...
							$.post('${vix}/common/model/baseMetaDataCategoryAction!saveOrUpdate.action',
								queryString,
								function(result){
									showMessage(result);
									setTimeout("", 1000);
									pager("start","${vix}/common/model/baseMetaDataCategoryAction!goSingleList.action?categoryName="+name,'metaDataCategory');
								}
							 );
						}
					},
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/model/baseMetaDataCategoryAction!deleteById.action?id='+id,
		  cache: false,
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/model/baseMetaDataCategoryAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&categoryName="+name,'metaDataCategory');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/model/baseMetaDataCategoryAction!goSingleList.action?categoryName="+name,'metaDataCategory');
}
...
function importXlsFile(){
	$.ajaxFileUpload({
		url : '${vix}/common/model/baseMetaDataCategoryAction!importFile.action',//用于文件上传的服务器端请求地址

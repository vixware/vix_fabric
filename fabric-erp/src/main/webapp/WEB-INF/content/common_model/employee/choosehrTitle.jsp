...
$.returnValue = "";
function hrTitlechkChange(chk,titlesysCode,titlesysName){
	if(chk.checked){
		$.returnValue = titlesysCode+","+titlesysName;
	}
} 
pager("start","${vix}/common/org/employeeOrgAction!goSubRadioSingleLists.action?temp=1","radio");
//排序 
function subOrderBy(orderField){
	...
	pager("start","${vix}/template/simpleGridAction!goSubRadioSingleLists.action?orderField="+orderField+"&orderBy="+orderBy,"radio");
}

function currentPager(tag){
	pager(tag,"${vix}/common/org/employeeOrgAction!goSubRadioSingleLists.action?code="+code+"&pageNo=${pageNo}",'radio');
}
...
function searchForContent(i){
	loadCode();
	pager("start","${vix}/common/org/employeeOrgAction!goSubRadioSingleLists.action?searchContent="+code,'radio');
}
/*重置搜索*/
...
function srmStatus(status){
	pager("start","${vix}/common/org/employeeOrgAction!goSubRadioSingleLists.action?status="+status,'radio');
}
//最近使用
function srmRecent(rencentDate){
	pager("start","${vix}/common/org/employeeOrgAction!goSubRadioSingleLists.action?updateTime="+rencentDate,'radio');
}
</script>
<div class="content" style="background: #DCE7F1">
	<div id="c_head" class="drop">
		...
		<div>
			<label>职称编号:<input id="srm_code" name="" type="text" class="int" /></label> <label> <input id="simpleSearch" name="" type="button" class="btn" value="<s:text name='cmn_search'/>" onclick="searchForContent();" /> <input id="simpleReset" name="" type="button" class="btn" value="<s:text name='cmn_reset'/>" onclick="resetForContent();" />
			</label>
		</div>
	</div>
	<!-- c_head -->

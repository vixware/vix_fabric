...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/common/security/dataColSecAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							if($("#dataColForm").validationEngine('validate')){
								var queryString = $('#dataColForm').formSerialize(); 
								$.post('${vix}/common/security/dataColSecAction!saveOrUpdate.action',
									queryString,
									function(result){
										showMessage(result);
										setTimeout("", 1000);
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/security/dataColSecAction!deleteById.action?id='+id,
		  cache: false,
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/security/dataColSecAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'dataCol');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/security/dataColSecAction!goSingleList.action?name="+name,'dataCol');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="system_control_org_sec_dataCol" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

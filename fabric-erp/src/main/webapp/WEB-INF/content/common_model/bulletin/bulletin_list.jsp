...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/common/model/bulletinAction!goSaveOrUpdate.action?id='+id+"&title="+name,
		  cache: false,
...
function saveOrUpdateBulletin(){
	asyncbox.confirm('确定要保存么?','提示信息',function(action){
		if(action == 'ok'){
			
			$("#bulletinContentHidden").val(bulletinEditor.html());
			
			var queryString = $('#bulletinForm').formSerialize(); 

			$.post('${vix}/common/model/bulletinAction!saveOrUpdate.action',
				queryString,
				function(result){
					asyncbox.success(result,"<s:text name='message'/>",function(action){
						if(action == 'ok'){
							loadContent('${vix}/common/model/bulletinAction!goList.action?title=${title}');
						}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/common/model/bulletinAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/common/model/bulletinAction!goSingleList.action?title="+name,'role');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/common/model/bulletinAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&title="+name,'role');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/common/model/bulletinAction!goSingleList.action?title="+name,'role');
}
...
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#"><s:text name="oa" /></a></li>
				<li><a href="#"><s:text name="oa_pes" /></a></li>
				<li><a href="#"><s:text name="oa_pes_bulletin" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div>
			<label><s:text name="cmn_name" /><input type="text" class="int" id="bulletinTitle" value="${bulletinTitle}"></label> <label><s:text name="cmn_my_item" /><input type="checkbox" value="" name=""></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();" /><input

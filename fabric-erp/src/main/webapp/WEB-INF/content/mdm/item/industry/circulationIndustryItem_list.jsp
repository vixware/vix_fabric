...
loadName();
//载入分页列表数据
pager("start","${vix}/mdm/item/fastAddItemAction!goListContent.action?name="+name+"&pageNo=${pageNo}",'item');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/mdm/item/fastAddItemAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&categoryId="+$("#selectId").val(),'item');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/mdm/item/fastAddItemAction!goListContent.action?name="+name+"&categoryId="+$("#selectId").val()+'&companyCode='+$("#companyCode").val(),'item');
}
	...
	asyncbox.confirm('确定要删除选中的${vv:varView('vix_mdm_item')}么?','提示信息',function(action){
		if(action == 'ok'){
			$.ajax({
				  ...
				  success: function(html){
					asyncbox.success(html,"提示信息",function(action){
						pager("current","${vix}/mdm/item/fastAddItemAction!goListContent.action?name=",'item');
					});
				  }
			});
		}
...
function reset(){
	$('#nameS').val('');$('#codeAS').val('');$('#nameAS').val('');
}
//选择公司
					...
					callback : function(action,returnValue){
						if(action == 'ok'){
							if(returnValue != undefined){
								...
								loadName();
								pager('start',"${vix}/mdm/item/fastAddItemAction!goListContent.action?name="+name+"&categoryId="+$("#selectId").val()+'&companyCode='+result[0],'item');
							}
						}
					},
...
<div class="sub_menu">
	...
	<div class="drop">
		<p>
			<a href="#" onclick="fastAdd(0);"><span><s:text name="cmn_add" /></span></a> <a href="#" onclick="importItemFile();"><span>导入${vv:varView('vix_mdm_item')}</span></a>
		</p>
	</div>
</div>
<div class="content">
	<div class="drop" id="c_head">
		...
		<div class="search_advanced">
			<label>编码<input type="text" class="int" id="codeAS"></label> <label><s:text name="cmn_name" /><input type="text" class="int" id="nameAS"></label> <label> <input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();"> <input type="button" value="<s:text name='cmn_reset'/>"
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="itemList" var="c">
				<li><a href="###" onclick="fastAdd(${c.id});"><span style="display: none;">${c.chinaShortName}</span>${c.name}</a></li>
			</s:iterator>
		</ul>
	</div>
	<!-- c_head -->

...
function chkChange(chk,id,name,price,purchasePrice){
	 if(chk.checked){
		 var p = purchasePrice =='' ? price : purchasePrice;
		 $.returnValue = id + "," + name+ ',' + p;
	 }
}
</script>
...
<table id="itemTable" class="list">
	<tbody>
		<tr class="alt">
			<th>选择</th>
			<th style="cursor: pointer;" onclick="orderBy('code');">编码&nbsp;<img id="itemPriceManageImg_code" src="${vix}/common/img/arrow.gif">
			</th>
			<th style="cursor: pointer;" onclick="orderBy('name');">名称&nbsp;<img id="itemPriceManageImg_name" src="${vix}/common/img/arrow.gif">
			</th>
			<th style="cursor: pointer;" onclick="orderBy('specification');">规格型号&nbsp;<img id="itemPriceManageImg_specification" src="${vix}/common/img/arrow.gif">
			</th>
			<th style="cursor: pointer;" onclick="orderBy('measureUnit.id');">计量单位&nbsp;<img id="itemPriceManageImg_measureUnit" src="${vix}/common/img/arrow.gif">
			</th>
			<th style="cursor: pointer;" onclick="orderBy('purchasePrice');">采购价&nbsp;<img id="itemPriceManageImg_purchasePrice" src="${vix}/common/img/arrow.gif">
			</th>
			<th>规格</th>
		</tr>
		<% int count = 0; %>
		<s:iterator value="pager.resultList" var="entity">
			<% count++; %>
			<tr>
				<td style="height: 20px;"><input id="chkId" name="chkId" value="${entity.id}" type="radio" onchange="chkChange(this,'${entity.id}','${entity.name}','${entity.price}','${entity.purchasePrice}');" /></td>
				<td>${entity.code}</td>
				<td>${entity.name}</td>

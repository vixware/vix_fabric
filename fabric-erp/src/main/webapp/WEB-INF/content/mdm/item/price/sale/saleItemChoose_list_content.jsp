...
function chkChange(chk,id,name,price){
	 if(chk.checked){
		 $.returnValue = id + ","  + name+ ',' + price;
	 }
}
</script>
...
<table id="itemTable" class="list">
	<tbody>
		<tr class="alt">
			...
			<th style="cursor: pointer;" onclick="orderBy('price');">零售价&nbsp;<img id="itemPriceManageImg_price" src="${vix}/common/img/arrow.gif">
			</th>
			<th>规格</th>
		</tr>
		<% int count = 0; %>
		<s:iterator value="pager.resultList" var="entity">
			<% count++; %>
			<tr>
				<td style="height: 20px;"><input id="chkId" name="chkId" value="${entity.id}" type="radio" onchange="chkChange(this,'${entity.id}','${entity.name}','${entity.price}');" /></td>
				<td>${entity.code}</td>
				<td>${entity.name}</td>

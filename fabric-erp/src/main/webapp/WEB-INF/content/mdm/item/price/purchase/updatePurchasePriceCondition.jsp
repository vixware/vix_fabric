...
function chooseItem(part){
	$.ajax({
		  url:'${vix}/mdm/item/purchaseItemPriceAction!goList.action',
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
...
function loadPriceConditionDetail(priceType){
	var url = '';
	if(priceType == 'price'){
		url = '${vix}/mdm/item/purchasePriceConditionPriceAreaAction!goListContent.action?id='+$("#id").val();
	}else{
		url = '${vix}/mdm/item/purchasePriceConditionCountAreaAction!goListContent.action?id='+$("#id").val();
	}
	$.ajax({
		  url:url,
		  cache: false,
...
function savePriceCondition(tag){
	if($('#priceConditionForm').validationEngine('validate')){
		$.post('${vix}/mdm/item/purchasePriceConditionAction!saveOrUpdate.action',
			...
			function(result){
				asyncbox.success(result,"<s:text name='vix_message'/>",function(action){
					if(tag == '0'){
						loadContent('${vix}/mdm/item/purchasePriceConditionAction!goList.action');
					}else{
						$.ajax({
							  url:'${vix}/mdm/item/purchasePriceConditionAction!goSaveOrUpdate.action?id='+id,
							  cache: false,
							  success: function(html){
...
function saveOrUpdatePriceConditionDetail(id,priceType){
	...
	var title = '';
	if(priceType == 'price'){
		url = '${vix}/mdm/item/purchasePriceConditionPriceAreaAction!goSaveOrUpdate.action?id='+id;
		title = '定价价格区域';
	}else{
		url = '${vix}/mdm/item/purchasePriceConditionCountAreaAction!goSaveOrUpdate.action?id='+id;
		title = '定价数量区域';
	}
	$.ajax({
		  url:url,
						...
						if(action == 'ok'){
							if($('#priceConditionAreaForm').validationEngine('validate')){
								...
								if(ct == 'purchaseFrameworkAgreement'){
									...
									if($("#itemGroupId").val() == "-1"){
										asyncbox.success("请选择${vv:varView('vix_mdm_item')}组!","<s:text name='vix_message'/>");
										return false;
									}
									variableData = 'regionalId:' + $("#regionalId").val() +  ',itemGroupId:'+$("#itemGroupId").val();
								}else if(ct =='customerAccountGroup'){
									if($("#customerAccountGroupId").val() == "-1"){
								...
								if(priceType == 'price'){
									$.post('${vix}/mdm/item/purchasePriceConditionPriceAreaAction!saveOrUpdate.action',
									...
									$.post('${vix}/mdm/item/purchasePriceConditionCountAreaAction!saveOrUpdate.action',
...
function changePriceConditionType(){
	...
	if(type == 'purchaseFrameworkAgreement'){
		$("td[name=regionalTd]").attr('style','display:none;');
		$("tr[id=purchaseFrameworkAgreementTr]").attr('style','display:;');
		$("tr[id=saleFrameworkAgreementTr]").attr('style','display:none;');
		$("tr[id=customerAccountCategoryTr]").attr('style','display:none;');
		$("tr[id=itemCategoryTr]").attr('style','display:none;');
		$("tr[id=itemGroupTr]").attr('style','display:none;');
		$("tr[id=customerAccountGroupTr]").attr('style','display:none;');
		$("tr[id=channelDistributorTr]").attr('style','display:none;');
		$("tr[id=customerAccountTr]").attr('style','display:none;');
		$("tr[id=itemTr]").attr('style','display:none;');
		$("tr[id=customerAccountAndItemTr]").attr('style','display:none;');
		$("tr[id=supplierTr]").attr('style','display:none;');
		$("tr[id=priceTypeTr]").attr('style','display:none;');
		$("#refund").val("");
	}else if(type == 'saleFrameworkAgreement'){
		$("td[name=regionalTd]").attr('style','display:none;');
		$("tr[id=purchaseFrameworkAgreementTr]").attr('style','display:none;');
		$("tr[id=saleFrameworkAgreementTr]").attr('style','display:;');
		$("tr[id=customerAccountCategoryTr]").attr('style','display:none;');
		$("tr[id=itemCategoryTr]").attr('style','display:none;');
		$("tr[id=itemGroupTr]").attr('style','display:none;');
		$("tr[id=customerAccountGroupTr]").attr('style','display:none;');
		...
		$("td[name=regionalTd]").attr('style','display:;');
		$("tr[id=purchaseFrameworkAgreementTr]").attr('style','display:none;');
		$("tr[id=saleFrameworkAgreementTr]").attr('style','display:none;');
		$("tr[id=customerAccountCategoryTr]").attr('style','display:;');
		$("tr[id=itemCategoryTr]").attr('style','display:none;');
		$("tr[id=itemGroupTr]").attr('style','display:none;');
		$("tr[id=customerAccountGroupTr]").attr('style','display:none;');
		$("tr[id=channelDistributorTr]").attr('style','display:none;');
		...
		$("tr[id=saleFrameworkAgreementTr]").attr('style','display:none;');
		$("tr[id=customerAccountCategoryTr]").attr('style','display:none;');
		$("tr[id=itemCategoryTr]").attr('style','display:;');
		$("tr[id=itemGroupTr]").attr('style','display:none;');
		$("tr[id=customerAccountGroupTr]").attr('style','display:none;');
		$("tr[id=channelDistributorTr]").attr('style','display:none;');
		$("tr[id=customerAccountTr]").attr('style','display:none;');
		...
		$("tr[id=customerAccountCategoryTr]").attr('style','display:none;');
		$("tr[id=itemCategoryTr]").attr('style','display:none;');
		$("tr[id=itemGroupTr]").attr('style','display:;');
		$("tr[id=customerAccountGroupTr]").attr('style','display:none;');
		$("tr[id=channelDistributorTr]").attr('style','display:none;');
		$("tr[id=customerAccountTr]").attr('style','display:none;');
		$("tr[id=itemTr]").attr('style','display:none;');
		...
		$("tr[id=itemGroupTr]").attr('style','display:none;');
		$("tr[id=customerAccountGroupTr]").attr('style','display:none;');
		$("tr[id=channelDistributorTr]").attr('style','display:;');
		$("tr[id=customerAccountTr]").attr('style','display:none;');
		$("tr[id=itemTr]").attr('style','display:none;');
		$("tr[id=customerAccountAndItemTr]").attr('style','display:none;');
		$("tr[id=supplierTr]").attr('style','display:none;');
		...
		$("tr[id=customerAccountGroupTr]").attr('style','display:none;');
		$("tr[id=channelDistributorTr]").attr('style','display:none;');
		$("tr[id=customerAccountTr]").attr('style','display:;');
		$("tr[id=itemTr]").attr('style','display:none;');
		$("tr[id=customerAccountAndItemTr]").attr('style','display:none;');
		$("tr[id=supplierTr]").attr('style','display:none;');
		$("tr[id=priceTypeTr]").attr('style','display:none;');
		...
		$("tr[id=channelDistributorTr]").attr('style','display:none;');
		$("tr[id=customerAccountTr]").attr('style','display:none;');
		$("tr[id=itemTr]").attr('style','display:;');
		$("tr[id=customerAccountAndItemTr]").attr('style','display:none;');
		$("tr[id=supplierTr]").attr('style','display:none;');
		$("tr[id=priceTypeTr]").attr('style','display:;');
		...
		$("tr[id=customerAccountTr]").attr('style','display:none;');
		$("tr[id=itemTr]").attr('style','display:none;');
		$("tr[id=customerAccountAndItemTr]").attr('style','display:;');
		$("tr[id=supplierTr]").attr('style','display:none;');
		$("tr[id=priceTypeTr]").attr('style','display:;');
		...
		$("tr[id=itemTr]").attr('style','display:none;');
		$("tr[id=customerAccountAndItemTr]").attr('style','display:none;');
		$("tr[id=supplierTr]").attr('style','display:;');
		$("tr[id=priceTypeTr]").attr('style','display:none;');
		$("#refund").val("");
		...
		$("tr[id=itemCategoryTr]").attr('style','display:none;');
		$("tr[id=itemGroupTr]").attr('style','display:none;');
		$("tr[id=customerAccountGroupTr]").attr('style','display:;');
		$("tr[id=channelDistributorTr]").attr('style','display:none;');
		$("tr[id=customerAccountTr]").attr('style','display:none;');
		$("tr[id=itemTr]").attr('style','display:none;');
		$("tr[id=customerAccountAndItemTr]").attr('style','display:none;');
...
function deletePriceConditionEntity(id,priceType){
	var url = '';
	if(priceType == 'price'){
		url = '${vix}/mdm/item/purchasePriceConditionPriceAreaAction!deleteById.action?id='+id;
	}else{
		url = '${vix}/mdm/item/purchasePriceConditionCountAreaAction!deleteById.action?id='+id;
	}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/mdm/item/purchasePriceConditionAction!goList.action');">${vv:varView('vix_mdm_item')}采购定价</a></li>
			</ul>
		</div>
	</h2>
</div>
		...
		<dl>
			<dt id="orderTitleFixd">
				<span class="no_line"> <a onclick="savePriceCondition('0');" href="#"><img width="22" height="22" alt="保存" src="${vix}/common/img/dt_save.png"></a> <a onclick="savePriceCondition('1');" href="#"><img width="22" height="22" alt="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <a href="#"
					onclick="loadContent('${vix}/mdm/item/purchasePriceConditionAction!goList.action');"><img width="22" height="22" alt="返回" src="${vix}/common/img/dt_goback.png" /></a>
				</span> <strong> <b> <s:if test="priceCondition.name != null">${priceCondition.name}</s:if> <s:else>新增${vv:varView('vix_mdm_item')}采购定价</s:else>
				</b> <i></i>
				</strong>
			</dt>
			<dd class="clearfix">
				<div class="order_table">
					<div class="voucher newvoucher">
						<dl>
							<dt>
								<b></b> <strong>${vv:varView('vix_mdm_item')}采购定价</strong>
							</dt>
									...
									<div class="box order_table" style="line-height: normal;">
										<table>
											...
											<tr height="30">
												...
												<td><input id="defaultTax" name="priceCondition.defaultTax" value="${priceCondition.defaultTax}" class="validate[required] text-input" />% 范围(1-100)<span style="color: red;">*</span></td>
												<td align="right">币种:&nbsp;</td>
												<td><s:select id="currencyTypeId" headerKey="-1" headerValue="--请选择--" list="%{currencyTypeList}" listValue="name" listKey="id" value="%{priceCondition.currencyType.id}" multiple="false" theme="simple"></s:select></td>
											</tr>
											<tr height="30">

...
function deleteById(id){
	$.ajax({
		  url:'${vix}/mdm/item/purchasePriceConditionAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='vix_message'/>",function(action){
				pager("current","${vix}/mdm/item/purchasePriceConditionAction!goListContent.action?subject="+subject+'&priceType='+$("#ptForSearch").val(),'priceCondition');
			});
		  }
	});
}
function searchForRightContent(tag){
	...
	pager("start","${vix}/mdm/item/purchasePriceConditionAction!goListContent.action?"+params,'priceCondition');
}
loadSubject();
//载入分页列表数据
pager("start","${vix}/mdm/item/purchasePriceConditionAction!goListContent.action?subject="+subject,'priceCondition');
function loadRoot(){
	$('#subjectS').val("");
	$('#selectId').val("");
	pager("start",'${vix}/mdm/item/purchasePriceConditionAction!goListContent.action?subject='+'&companyCode='+$("#companyCode").val()+'&priceType='+$("#ptForSearch").val(),'priceCondition');
}
//排序 
function orderBy(orderField){
	...
	pager("current","${vix}/mdm/item/purchasePriceConditionAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&subject="+subject+"&id="+$("#selectId").val()+'&companyCode='+$("#companyCode").val()+'&priceType='+$("#ptForSearch").val(),'priceCondition');
}
...
function currentPager(tag){
	loadSubject();
	pager(tag,"${vix}/mdm/item/purchasePriceConditionAction!goListContent.action?subject="+subject+'&companyCode='+$("#companyCode").val()+'&priceType='+$("#ptForSearch").val(),'priceCondition');
}

function currentPagerClick(input,event){
	loadSubject();
	pagerClick(input,event,"${vix}/mdm/item/purchasePriceConditionAction!goListContent.action?subject="+subject+'&companyCode='+$("#companyCode").val()+'&priceType='+$("#ptForSearch").val(),'priceCondition');
}
//选择公司
					...
					callback : function(action,returnValue){
						if(action == 'ok'){
							if(returnValue != undefined){
								...
								loadSubject();
								pager('start',"${vix}/mdm/item/purchasePriceConditionAction!goListContent.action?subject="+subject+'&companyCode='+result[0],'priceCondition');
							}
						}
					},

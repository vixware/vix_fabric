...
function loadAge(){
	...
	age=(thisYear-brith);
	$("#age").text(age);
	cAid = $("#customerAccountId").val();
}
loadAge();

function addMbtags(id) {
	$.ajax({
		url : '${vix}/crm/member/memberTagManagementAction!goSaveOrUpdate.action?id=' + id,
		cache : false,
				...
				callback : function(action) {
					if (action == 'ok') {
						if ($('#memberTagForm').validationEngine('validate')) {
							$.post('${vix}/crm/member/memberTagManagementAction!saveOrUpdate.action', {
							'memberTag.id' : $("#memberTagId").val(),
							'memberTag.name' : $("#memberTagName").val(),
...
function editMbtags(id,tags){
	$.ajax({
		url : '${vix}/crm/member/memberTagManagementAction!goSaveOrUpdate.action?id=' + id + "&tags=" + tags,
		cache : false,
		success : function(html) {
			asyncbox.open({
				modal : true,
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<s:if test="source =='mdm' ">
					...
					<li><a href="#" onclick="loadContent('${vix}/mdm/crm/customerAccountAction!goList.action?source=${source}');">会员</a></li>
				</s:if>
				<s:else>
					<li><a href="#"><img src="${vix}/common/img/crm/customer.png" alt="" />供应链</a></li>
					<li><a href="#">会员关系管理</a></li>
					<li><a href="#" onclick="loadContent('${vix}/mdm/crm/customerAccountAction!goList.action?source=${source}');">会员</a></li>
				</s:else>
			</ul>
		</div>
	...
	<div class="npcontent clearfix">
		<div class="np_right">
			<div class="np_right_box">
				...
				<div class="ld">
					<table border="0" width="100%;">
						...
						<tr>
							<td>积分：</td>
							<s:if test="null != customerAccount.pointValue">
								<td align="right">${customerAccount.pointValue }分</td>
							</s:if>
							<s:else>
								<td align="right">0分</td>
							</s:else>
						</tr>
						<tr>
							<td>会员等级：</td>
							<td align="right">${customerAccount.memberLevel.name }</td>
						</tr>
						<tr>
							<td>储值余额：</td>
			...
			<div class="np_right_box">
				...
				<input id="customerAccountId" name="customerAccount.id" value="${customerAccount.id }" type="hidden" />
				<div class="ld">
					<table border="0" width="100%;">
						<s:if test="null != customerAccount.mbtags.name">
							<tr>
								<td><a href="###">${customerAccount.memberTag }</a></td>
							</tr>
							<tr>
								<td style="border-bottom: 0px solid #ddd;"><a href="###" onclick="editMbtags('${customerAccount.id}','edit');" class="abtn"><span>修改会员标签</span></a></td>
								<td style="border-bottom: 0px solid #ddd;"><a href="###" onclick="addMbtags('${customerAccount.id}');" class="abtn"><span>新增会员标签</span></a></td>
							</tr>
						</s:if>
						<s:else>
							<tr>
								<%-- 								<td style="border-bottom: 0px solid #ddd;">
									<a href="###" onclick="editMbtags('${customerAccount.id}','edit');" class="abtn"><span>设置会员标签</span></a>
								</td>
 --%>
								<td style="border-bottom: 0px solid #ddd;"><a href="###" onclick="addMbtags('${customerAccount.id}');" class="abtn"><span>新增会员标签</span></a></td>
							</tr>
			...
			<div class="np_right_box">
				...
				<s:iterator value="customerAccount.contactPersons" var="cp">
					<p align="center">
						<img border="0" src="img/n2.png"><br> <a href="javascript:void(0);" onclick="javascript:showBCard();">${cp.name }</a><br /> <b>${cp.mobile }</b><img border="0" align="Baseline" src="img/sphone.png"><br />
					</p>
					<input type="hidden" id="birthday" name="cp.birthday" value="${cp.birthday }" />
				</s:iterator>
			</div>
		</div>
		<div class="np_left clearfix" id="common_table">
			...
			<div class="task_list">
				<table width="100%">
					<tr>
						<th colspan="4">会员名称：<b>${customerAccount.name}</b></th>
					</tr>
					<tr>
						<td width="15%" align="right">性别：</td>
						<s:if test="customerAccount.sex == 1">
							<td width="35%">女</td>
						</s:if>
						<s:else>
							<td width="35%">男</td>
						...
						<td id="age" width="40%"></td>
					</tr>
					<tr>
						<td align="right">QQ：</td>
						<td>${customerAccount.qqAccount }</td>
					...
					<tr>
						...
						<td>${customerAccount.birthday}</td>
					</tr>
					<tr>
									...
									<script type="text/javascript">
		                    			function saveOrUpdateCustomerService(id){
		                    				$.ajax({
		                    					  ...
		                    					  success: function(html){
		                    						  asyncbox.open({
		                    							 	modal:true,width : 920,height : 440,title:"会员服务",html:html,
		                    								callback : function(action){
		                    									if(action == 'ok'){
									...
									<script type="text/javascript">
		                    			function saveOrUpdateCustomerCare(id){
		                    				$.ajax({
		                    					  ...
		                    					  success: function(html){
		                    						  asyncbox.open({
		                    							 	modal:true,width : 920,height : 340,title:"会员关怀",html:html,
		                    								callback : function(action){
		                    									if(action == 'ok'){
									...
									<script type="text/javascript">
		                    			function saveOrUpdateCustomerComplaint(id){
		                    				$.ajax({
		                    					  ...
		                    					  success: function(html){
		                    						  asyncbox.open({
		                    							 	modal:true,width : 920,height : 440,title:"会员投诉",html:html,
		                    								callback : function(action){
		                    									if(action == 'ok'){
									...
									<script type="text/javascript">
		                    			function saveOrUpdateCustomerServiceNotepad(id){
		                    				$.ajax({
		                    					  ...
		                    					  success: function(html){
		                    						  asyncbox.open({
		                    							 	modal:true,width :820,height : 240,title:"会员记事",html:html,
		                    								callback : function(action){
		                    									if(action == 'ok'){

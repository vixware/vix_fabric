...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/mdm/crm/customerAccountGroupAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/mdm/crm/customerAccountGroupAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/mdm/crm/customerAccountGroupAction!goListContent.action?name="+name,'customerAccountGroup');
			});
		  }
	});
}
...
function orderBy(orderField){
	loadName();
	var orderBy = $("#customerAccountGroupOrderBy").val();
	if(orderBy == 'desc'){
		orderBy = "asc";
	}else{
		orderBy = 'desc';
	}
	pager("start","${vix}/mdm/crm/customerAccountGroupAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'customerAccountGroup');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/mdm/crm/customerAccountGroupAction!goListContent.action?name="+name,'customerAccountGroup');
}
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a></span> <span><a class="previous" onclick="currentPager('previous');"></a></span> <em>(<b class="customerAccountGroupInfo"></b> <s:text name='cmn_to' /> <b class="customerAccountGroupTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a></span> <span><a class="end" onclick="currentPager('end');"></a></span>
					</div>
				</div>
				<div id="customerAccountGroup" class="table"></div>
				<div class="pagelist drop">
					<ul>

...
/* 新增类型 */
function saveOrUpdateBillsType(id,billsCategoryId){
	$.ajax({
		  url:'${vix}/system/processDefinitionAction!goSaveOrUpdate.action?id='+id+"&billsCategoryId="+billsCategoryId,
		  cache: false,
			  ...
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							...
							$.post('${vix}/system/processDefinitionAction!saveOrUpdate.action',
								 {
								  'billsType.id':$("#billsTypeId").val(),
								  'billsCategoryId':$("#billsCategoryId").val(),
								...
								function(result){
									asyncbox.success(result,"<s:text name='message'/>",function(action){
										...
										pager("start","${vix}/system/processDefinitionAction!goSingleList.action?name="+name+"&id="+$("#selectId").val(),'category');
									});
								}
							 );
							}else {
...
function searchForRightContent(){
	loadName();
	pager("start","${vix}/system/processDefinitionAction!goSingleList.action?name="+name,'channelDistributor');
}
loadName();
//载入分页列表数据
pager("start","${vix}/system/processDefinitionAction!goSingleList.action?name="+name,'channelDistributor');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/system/processDefinitionAction!goSingleList.action?name=",'channelDistributor');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/system/processDefinitionAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'channelDistributor');
}
bindSearch();
bindSwitch();
...
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/system/processDefinitionAction!goSingleList.action?name="+name,'channelDistributor');
}
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">流程管理 </a></li>
				<li><a href="#">流程关联定义 </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdateBillsType(0,$('#selectId').val());"><span><s:text name='cmn_add' /> </span> </a>
		</p>
	</div>
</div>
<div class="content">
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/system/processDefinitionAction!goSingleList.action?billTypeId="+treeNode.id+"&treeType="+treeNode.treeType,"encodingRulesTableInTheMiddle");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><img src="img/mail.png" alt="" />业务流程</li>
				</ul>
			</div>

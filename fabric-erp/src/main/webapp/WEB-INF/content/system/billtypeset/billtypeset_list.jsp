...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/system/billTypeSetAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/system/billTypeSetAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/system/billTypeSetAction!goSingleList.action?name="+name,'billsCategoryDictionary');
			});
		  }
	});
}
...
function orderBy(orderField){
	var orderBy = $("#billsCategoryDictionaryOrderBy").val();
	if(orderBy == 'desc'){
		orderBy = "asc";
	}else{
		orderBy = 'desc';
	}
	pager("start","${vix}/system/billTypeSetAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'billsCategoryDictionary');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/system/billTypeSetAction!goSingleList.action?name="+name,'billsCategoryDictionary');
}
...
function searchForContent(tag){
	...
	if(tag==0){
     	pager("start","${vix}/system/billTypeSetAction!goSearchTransferList.action?content="+name+"&tag="+tag,'billsCategoryDictionary');
	}else {
	    pager("start","${vix}/system/billTypeSetAction!goSearchTransferList.action?outwarehousecodes="+outwarehousecodes+"&inwarehousecodes="+inwarehousecodes+"&code1="+code1+"&tag="+tag,'billsCategoryDictionary');
	}
}
/* 重置 */
...
<div class="content">
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="billsCategoryDictionaryList" var="c">
				<li><a href="#" onclick="saveOrUpdate(${c.id});">${c.categoryCode}</a></li>
			</s:iterator>
		</ul>
	</div>
	<!-- c_head -->
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="billsCategoryDictionaryInfo"></b> <s:text name='cmn_to' /> <b class="billsCategoryDictionaryTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a> </span> <span><a class="end" onclick="currentPager('end');"></a> </span>
					</div>
				</div>
				<div id="billsCategoryDictionary" class="table"></div>
				<div class="pagelist drop">
					<ul>

...
function saveOrUpdateBillsCategory(id,parentId,treeType){
	if(id!=0){
		saveOrUpdateBillsCategory_1(id,parentId,treeType);
	}
	else if(treeType=='C'){
		saveOrUpdateBillsCategory_1(id,parentId,treeType);
	}else {
		asyncbox.success('只能在公司节点下创建单据类型!','提示信息');
	}
};
/* 新增分类 */
function saveOrUpdateBillsCategory_1(id,parentId,treeType){
	$.ajax({
		  url:'${vix}/system/billTypeManagementAction!goSaveOrUpdateBillsCategory.action?id='+id+"&parentId="+parentId,
		  cache: false,
			  ...
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							...
							$.post('${vix}/system/billTypeManagementAction!saveOrUpdateBillsCategory.action',
									 {
									...
									function(result){
									    showMessage(result);
									    setTimeout("", 1000);
									    loadContent('${vix}/system/billTypeManagementAction!goList.action');
									}
								 );
							}else {
								return false;
...
function saveOrUpdateBillsType_1(id,billsPropertyId,treeType){
	$.ajax({
		  url:'${vix}/system/billTypeManagementAction!goSaveOrUpdate.action?id='+id+"&billsPropertyId="+billsPropertyId,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
					...
					callback : function(action){
						if(action == 'ok'){
							...
							$.post('${vix}/system/billTypeManagementAction!saveOrUpdate.action',
								 {
...
function searchForRightContent(){
	loadName();
	pager("start","${vix}/system/billTypeManagementAction!goSingleList.action?name="+name,'channelDistributor');
}
loadName();
//载入分页列表数据
pager("start","${vix}/system/billTypeManagementAction!goSingleList.action?name="+name,'channelDistributor');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/system/billTypeManagementAction!goSingleList.action?name=",'channelDistributor');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/system/billTypeManagementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'channelDistributor');
}
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/system/billTypeManagementAction!goSingleList.action?name="+name,'channelDistributor');
}
function goSearch() {
	$.ajax({
	...
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/system/billTypeManagementAction!goSingleList.action",'channelDistributor');
			}
		},
		btnsbar : $.btn.OKCANCEL
		});
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/system/billTypeManagementAction!goSingleList.action?id="+treeNode.id+"&treeType="+treeNode.treeType,"channelDistributor");
				}

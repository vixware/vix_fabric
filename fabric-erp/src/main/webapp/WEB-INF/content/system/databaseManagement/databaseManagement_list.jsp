...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/system/databaseManagementAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/system/databaseManagementAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/system/databaseManagementAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/system/databaseManagementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/system/databaseManagementAction!goSingleList.action?name="+name,'brand');
}
...
<div class="sub_menu">
	...
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0);"><span>执行 </span> </a> <a href="#"><span>运行监控</span> </a> <a href="#"><span>备份 </span> </a>
		</p>
	</div>
</div>
<div class="content">

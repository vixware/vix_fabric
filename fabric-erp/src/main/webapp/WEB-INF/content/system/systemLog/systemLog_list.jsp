...
//载入分页列表数据
pager("start","${vix}/system/systemLogManagementAction!goSingleList.action?1=1",'customer');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/system/systemLogManagementAction!goLoginList.action?orderField="+orderField+"&orderBy="+orderBy,'customer');
}

function currentPager(tag){
	pager(tag,"${vix}/system/systemLogManagementAction!goLoginList.action?1=1",'customer');
}
...
function categoryPager(tag,entity){
	if(entity == 'customer'){
		pager(tag,"${vix}/system/systemLogManagementAction!goSingleList.action?1=1",entity);
	}
	if(entity == 'customer1'){
		pager(tag,"${vix}/system/systemLogManagementAction!goErrorList.action?1=1",entity);
	}
	if(entity == 'latestOperateEntity'){
		pager(tag,"${vix}/system/systemLogManagementAction!goLoginList.action?1=1",entity);
	}
}
...
function searchForContent(){
	loadBuyerNick();
	loadReceiverMobile();
    pager("start","${vix}/system/systemLogManagementAction!goErrorList.action?buyerNick="+buyerNick+"&receiverMobile="+$("#receiverMobile").val(),'customer1');
}
...
function goSyncCustomerAccount(){
	$.ajax({
	  url:'${vix}/system/systemLogManagementAction!goSyncCustomerAccount.action',
	  cache: false,
	  success: function(result){
		  showMessage(result);
		  setTimeout("", 1000);
		  loadContent('${vix}/system/systemLogManagementAction!goList.action');
	  }
});
};
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#"><img src="${vix}/common/img/crm/customer.png" alt="" /> 系统管理 </a></li>
				<li><a href="#">运行管理</a></li>
				<li><a href="#">系统日志 </a></li>
			</ul>
		</div>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div>
			<label>日志内容:<input type="text" class="int" id="buyerNick"></label> <label><input id="simpleSearch" type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();"> <input id="simpleReset" type="button" value="<s:text name='cmn_reset'/>" class="btn" onclick="resetForContent()"> </label>
		</div>
	</div>
	<div id="number">
	...
	<div class="box">
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(5,1,'a',event,'customer')"><img src="img/holidaysam.png" alt="" />操作日志</a></li>
					<li><a onclick="categoryTab(5,2,'a',event,'customer1')"><img src="img/adjust_view_day.png" alt="" />登陆日志</a></li>
					<li><a onclick="categoryTab(5,3,'a',event,'latestOperateEntity')"><img src="img/adjust_view_day.png" alt="" />错误日志</a></li>
				</ul>
			</div>

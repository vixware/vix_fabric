...
$.returnValue = "";
function chkChange(chk,id,processCode,processDescription){
	 if(chk.checked){
		 $.returnValue = id+","+processCode+","+processDescription;
	 }
} 
pager("start","${vix}/mm/craftsAction!goSubRadioSingleListss.action?temp=1","radio");
//排序 
...
function currentPager(tag){
	pager(tag,"${vix}/mm/craftsAction!goSubRadioSingleListss.action?code="+code+"&pageNo=${pageNo}",'radio');
}
...
function searchForContent(i){
	loadCode();
	pager("start","${vix}/mm/craftsAction!goSubRadioSingleListss.action?searchContent="+code,'radio');
}
/*重置搜索*/
...
function srmStatus(status){
	pager("start","${vix}/mm/craftsAction!goSubRadioSingleListss.action?status="+status,'radio');
}
//最近使用
function srmRecent(rencentDate){
	pager("start","${vix}/mm/craftsAction!goSubRadioSingleListss.action?updateTime="+rencentDate,'radio');
}
</script>
<div class="content" style="background: #DCE7F1">
	<div id="c_head" class="drop">
		...
		<div>
			<label>工艺路线编码:<input id="srm_code" name="" type="text" class="int" /></label> <label> <input id="simpleSearch" name="" type="button" class="btn" value="<s:text name='cmn_search'/>" onclick="searchForContent();" /> <input id="simpleReset" name="" type="button" class="btn" value="<s:text name='cmn_reset'/>" onclick="resetForContent();" />
			</label>
		</div>
	</div>
	<!-- c_head -->

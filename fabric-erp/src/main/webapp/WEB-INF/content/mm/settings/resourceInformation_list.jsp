...
/* loadMenuContent('${vix}/mm/hrMainAction!goMenuContent.action'); */
var code = "";
var name = "";
var contacts ="";
function loadCode(){ 
...
function saveOrUpdate(id,pageNo){
	/* $.ajax({
		  url:'${vix}/mm/resourceInformationAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#supplierCategoryForm').validationEngine('validate')){
								$.post('${vix}/mm/resourceInformationAction!saveOrUpdate.action?parentId='+$("#newScId").val(), {
										...
										'supplierCategory.name' : $("#newName").val()
											...
											setTimeout("",1000);
											//载入分页列表数据
											//pager("start","${vix}/mm/resourceInformationAction!goSingleList.action?code="+code+"&pageNo=${pageNo}",'category');
											loadContent('${vix}/mm/resourceInformationAction!goList.action');
										});
							}else{
			  					return false;
			  				}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/mm/resourceInformationAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/mm/resourceInformationAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
function searchForContent(i){
	...
	loadContacts();
	if(i == 0){
		pager("start","${vix}/mm/resourceInformationAction!goSearchList.action?i="+i+"&searchContent="+code+"&orderField=id&orderBy=desc",'category');
	}
	else{
		pager("start","${vix}/mm/resourceInformationAction!goSearchList.action?i="+i+"&contacts="+contacts+"&pName="+name+"&orderField=id&orderBy=desc",'category');
	}
}
/*判断搜索内容是否为空*/
...
loadCode();
//载入分页列表数据
pager("start","${vix}/mm/resourceInformationAction!goSingleList.action?name="+name+"&orderField=id&orderBy=desc",'category');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/mm/resourceInformationAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/mm/resourceInformationAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadCode();
	if(entity == 'category'){
		pager(tag,"${vix}/mm/resourceInformationAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function srmStatus(status){
	pager("start","${vix}/mm/resourceInformationAction!goSingleList.action?status="+status,'category');
}
//最近使用
function srmRecent(rencentDate){
	pager("start","${vix}/mm/resourceInformationAction!goSingleList.action?updateTime="+rencentDate,'category');
}
/* 加载顶部工具栏 */
/* loadTopDynamicMenuContent('${vix}/mm/resourceInformationAction!goTopDynamicMenuContent.action'); */
</script>
<div class="sub_menu">
	<h2>
	...
	<div id="c_head" class=" drop">
		...
		<ul>
			...
			<li class="fly"><a href="#"><img alt="" src="img/icon_10.png">
				...
				<ul>
					<li><a href="#" onclick="srmStatus('S1')"><img alt="" src="${vix}/common/img/icon_10.png">未审批</a></li>
					<li><a href="#" onclick="srmStatus('S2')"><img alt="" src="${vix}/common/img/icon_10.png">审批中</a></li>
					<li><a href="#" onclick="srmStatus('S3')"><img alt="" src="${vix}/common/img/icon_10.png">已审批</a></li>
				</ul></li>
			<li class="fly"><a href="#"><img alt="" src="img/icon_10.png">
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<div class="left_content" style="height: 500px;">
				<div style="padding-left: 4px;">
					<img src="${vix}/common/img/file.png" style="padding-right: 5px;" /><a href="#" onclick="loadRoot();">生产部门</a>
				</div>
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/mm/resourceInformationAction!findTreeToJson2.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/mm/resourceInformationAction!goSingleList.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/mail.png" alt="" />资源资料</a></li>
				</ul>
			</div>

...
/* loadMenuContent('${vix}/mm/processAction!goMenuContent.action'); */
var code = "";
var name = "";
var contacts ="";
function loadName(){
...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/mm/processAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/mm/processAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/mm/processAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
function searchForContent(i){
	...
	loadContacts();
	if(i == 0){
		pager("start","${vix}/mm/processAction!goSearchList.action?i="+i+"&searchContent="+code+"&orderField=id&orderBy=desc",'category');
	}
	else{
		pager("start","${vix}/mm/processAction!goSearchList.action?i="+i+"&contacts="+contacts+"&pName="+name+"&orderField=id&orderBy=desc",'category');
	}
}
/*判断搜索内容是否为空*/
...
loadName();
//载入分页列表数据
pager("start","${vix}/mm/processAction!goSingleList.action?name="+name+"&orderField=id&orderBy=desc",'category');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/mm/processAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/mm/processAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/mm/processAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function hrStatus(status){
	pager("start","${vix}/mm/processAction!goSingleList.action?status="+status,'category');
}
//最近使用
function srmRecent(rencentDate){
	pager("start","${vix}/mm/processAction!goSingleList.action?updateTime="+rencentDate,'category');
}
/* 加载顶部工具栏 */
/* loadTopDynamicMenuContent('${vix}/mm/processAction!goTopDynamicMenuContent.action'); */
</script>
<div class="sub_menu">
	<h2>
	...
	<div class="box">
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/mm/processAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/mm/processAction!goSingleList.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/mail.png" alt="" />工序管理</a></li>
				</ul>
			</div>

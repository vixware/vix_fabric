...
function saveOrUpdate(id,pageNo,biztype){
	  if(biztype==1){
		  makeToOrder(id,pageNo);
      }else if(biztype==0){
    	  makeToForecast(id,pageNo);
       }else if (biztype==2){
    	   makeToInsert(id,pageNo);
    }
 }
function makeToOrder(id,pageNo){
		$.ajax({
			  url:'${vix}/mm/planSourceAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
			  cache: false,
...
function makeToForecast(id,pageNo){
	$.ajax({
		  url:'${vix}/mm/planSourceAction!goMakeToForecast.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
		  	$("#mainContent").html(html);
		  }
...
function makeToInsert(id,pageNo){
	   $.ajax({
			  url:'${vix}/mm/planSourceAction!goMakeToInsert.action?id='+id+"&pageNo="+pageNo,
			  cache: false,
			  success: function(html){
			  	$("#mainContent").html(html);
			  }
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/mm/planSourceAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/mm/planSourceAction!goListContent.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/mm/planSourceAction!goListContent.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/mm/planSourceAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/mm/planSourceAction!goListContent.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#"><img src="img/mm_masterproductionschedule.png" alt="" />生产管理</a></li>
				<li><a href="#">主生产计划</a></li>
				<li><a href="#">MPS计划来源</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<ul>
			<li><a href="#"><span>计划来源</span></a>
				<ul>
					<li><a href="#" onclick="makeToOrder(0);">订单</a></li>
					<li><a href="#" onclick="makeToForecast(0);">预测 </a></li>
					<li><a href="#" onclick="makeToInsert(0);">紧急插单</a></li>
				</ul></li>
		</ul>
	</div>
</div>
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<div class="left_content" style="height: 500px;">
				<%-- <div style="padding-left: 4px;">
					<img src="${vix}/common/img/file.png" style="padding-right: 5px;" /><a href="#"
						onclick="loadRoot();"><s:text name='cmn_rootDirectory' /></a>
				</div> --%>
			...
			<script type="text/javascript">				
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/mm/planSourceAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/mm/planSourceAction!goListContent.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><img src="img/mail.png" alt="" />MPS计划来源</li>
				</ul>
			</div>

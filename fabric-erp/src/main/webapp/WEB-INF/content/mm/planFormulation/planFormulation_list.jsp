...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/mm/productForecastPlanAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/mm/productForecastPlanAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/mm/productForecastPlanAction!goListContent.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/mm/productForecastPlanAction!goListContent.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/mm/productForecastPlanAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/mm/productForecastPlanAction!goListContent.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function searchForContent(i){
	...
	loadPurchasePerson();
	if(i == 0){
		pager("start","${vix}/mm/productForecastPlanAction!goSearchList.action?i="+i+"&searchContent="+name,'category');
	}
	else{
		pager("start","${vix}/mm/productForecastPlanAction!goSearchList.action?i="+i+"&purchasePerson="+purchasePerson+"&purchasePlanName="+purchaseName,'category');
	}
}
/*重置搜索*/
...
function purStatus(status){
	pager("start","${vix}/mm/productForecastPlanAction!goListContent.action?status="+status,'category');
}
//最近使用
function purRecent(rencentDate){
	pager("start","${vix}/mm/productForecastPlanAction!goListContent.action?updateTime="+rencentDate,'category');
}
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/mm/productForecastPlanAction!goList.action');">MPS计划编制</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
	...
	<div class="box">
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/mm/productForecastPlanAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/mm/productForecastPlanAction!goListContent.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/mail.png" alt="" />生产预测计划</a></li>
				</ul>
			</div>

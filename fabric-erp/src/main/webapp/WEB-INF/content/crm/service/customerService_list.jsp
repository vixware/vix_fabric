...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/crm/service/customerServiceAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
	...
	$.ajax({
		  ...
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/crm/service/customerServiceAction!goListContent.action?name="+name,'customerService');
			});
		  }
	});
}
...
function orderBy(orderField){
	loadName();
	var orderBy = $("#customerServiceOrderBy").val();
	if(orderBy == 'desc'){
		orderBy = "asc";
	}else{
		orderBy = 'desc';
	}
	pager("start","${vix}/crm/service/customerServiceAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'customerService');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/crm/service/customerServiceAction!goListContent.action?name="+name,'customerService');
}
...
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#"><img src="${vix}/common/img/crm/customerService.png" alt="" />供应链</a></li>
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a></span> <span><a class="previous" onclick="currentPager('previous');"></a></span> <em>(<b class="customerServiceInfo"></b> <s:text name='cmn_to' /> <b class="customerServiceTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a></span> <span><a class="end" onclick="currentPager('end');"></a></span>
					</div>
				</div>
				<div id="customerService" class="table"></div>
				<div class="pagelist drop">
					<ul>

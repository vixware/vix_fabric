...
function goView(id){
	$.ajax({
		  url:'${vix}/crm/project/crmProjectAction!goView.action?id='+id,
		  cache: false,
...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/crm/project/crmProjectAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			$("#mainContent").html(html);
		  }
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/crm/project/crmProjectAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/crm/project/crmProjectAction!goListContent.action?name="+name+'&companyCode='+$("#companyCode").val(),'crmProject');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/crm/project/crmProjectAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+'&companyCode='+$("#companyCode").val(),'crmProject');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/crm/project/crmProjectAction!goListContent.action?name="+name+'&companyCode='+$("#companyCode").val(),'crmProject');
}
					...
					callback : function(action,returnValue){
						if(action == 'ok'){
							if(returnValue != undefined){
								...
								loadTitle();
								pager('start',"${vix}/crm/project/crmProjectAction!goListContent.action?title="+title+'&companyCode='+result[0],'crmProject');
							}
						}
					},
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a></span> <span><a class="previous" onclick="currentPager('previous');"></a></span> <em>(<b class="crmProjectInfo"></b> <s:text name='cmn_to' /> <b class="crmProjectTotalCount"></b>)

...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/crm/project/projectCostAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/crm/project/projectCostAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/crm/project/projectCostAction!goListContent.action?name="+name,'projectCost');
			});
		  }
	});
}
...
function orderBy(orderField){
	loadName();
	var orderBy = $("#projectCostOrderBy").val();
	if(orderBy == 'desc'){
		orderBy = "asc";
	}else{
		orderBy = 'desc';
	}
	pager("start","${vix}/crm/project/projectCostAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'projectCost');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/crm/project/projectCostAction!goListContent.action?name="+name,'projectCost');
}
...
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#"><img src="${vix}/common/img/crm/projectCost.png" alt="" />供应链</a></li>
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a></span> <span><a class="previous" onclick="currentPager('previous');"></a></span> <em>(<b class="projectCostInfo"></b> <s:text name='cmn_to' /> <b class="projectCostTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a></span> <span><a class="end" onclick="currentPager('end');"></a></span>
					</div>
				</div>
				<div id="projectCost" class="table"></div>
				<div class="pagelist drop">
					<ul>

...
function saveOrUpdate(id,pageNo){
	$.ajax({
		url:'${vix}/crm/business/couponAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
					...
					callback : function(action){
						if(action == 'ok'){
							if ($('#couponForm').validationEngine('validate')) {
								$.post('${vix}/crm/business/couponAction!saveOrUpdate.action', {
									'coupon.id' : $("#couponId").val(),
									'coupon.channelDistributor.id' : $("#channelDistributorId").val(),
									'coupon.name' : $("#name").val(),
									'coupon.userule' : $("#userule").val(),
									'coupon.type' : $("#type").val(),
									'coupon.amount' : $("#amount").val(),
									'coupon.expenditure' : $("#expenditure").val(),
									'coupon.quantity' : $("#quantity").val(),
									'coupon.effectiveDate' : $("#effectiveDate").val(),
									'coupon.cutOffDate' : $("#cutOffDate").val(),
									'updateField' : updateField,
									'coupon.memo' : $("#memo").val()
								}, function(result) {
									showMessage(result);
									setTimeout("", 1000);
									loadContent('${vix}/crm/business/couponAction!goList.action');
								});
							} else {
								return false;
							}
...
loadName();
//载入分页列表数据
pager("start","${vix}/crm/business/couponAction!goSingleList.action?1=1",'coupon');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/crm/business/couponAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'coupon');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/crm/business/couponAction!goSingleList.action?name="+name,'coupon');
}
//bindSearch();
//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/crm/business/couponAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/crm/business/couponAction!goSingleList.action?name=" + $('#name').val() , 'coupon');
			}
		},
		btnsbar : $.btn.OKCANCEL
		});
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/crm/business/couponAction!goSingleList.action?parentId="+treeNode.id+"&treeType="+treeNode.treeType,"coupon");
				}

...
<script type="text/javascript">
	...
	//载入分页列表数据
	pager("start", "${vix}/crm/business/orderExpeditingAction!goSingleList.action?1=1", 'order');

	function currentPager(tag) {
		pager(tag, "${vix}/crm/business/orderExpeditingAction!goSingleList.action?1=1", 'order');
	}

	function saveOrUpdate() {
		if ($('#expeditingSetting').validationEngine('validate')) {
			$.post('${vix}/crm/business/orderExpeditingAction!saveOrUpdate.action', {
			...
			'expeditingSetting.orderTime' : $("#orderTime").val(),
			...
			'expeditingSetting.goodsName' : $(":checkbox[name=goodsName][checked]").val()
				...
				showMessage(result);
				setTimeout("", 1000);
				loadContent('${vix}/crm/business/orderExpeditingAction!goList.action');
			});
		} else {
			return false;
		}
	...
	function listbystatus(status) {
		pager("start", "${vix}/crm/business/orderExpeditingAction!goSingleList.action?status=" + status, 'order');
	}
	/* 最近使用 */
	function leastRecentlyUsed(days) {
		pager("start", "${vix}/crm/business/orderExpeditingAction!goSingleList.action?days=" + days, 'order');
	}
	//bindSearch();
	function searchForContent() {
		loadName();
		pager("start", "${vix}/crm/business/orderExpeditingAction!goSingleList.action?name=" + name, 'order');
	}		
	function goSearch() {
		$.ajax({
		url : '${vix}/crm/business/orderExpeditingAction!goSearch.action',
		cache : false,
		success : function(html) {
			...
			callback : function(action) {
				if (action == 'ok') {
					pager("start", "${vix}/crm/business/orderExpeditingAction!goSingleList.action?code=" + $('#code').val() + "&selectName=" + $('#selectName').val(), 'order');
				}
			},
		...
		<div class="order_box">
			<div class="order_left">
				<form id="expeditingSetting">
					<input type="hidden" id="expeditingSettingId" name="expeditingSettingId" value="${expeditingSetting.id}" />
					<h2>
						订单催付<span class="gray">店铺选择：<s:select id="channelDistributorId" headerKey="" headerValue="--请选择--" list="%{channelDistributorList}" listValue="name" listKey="id" value="" multiple="false" theme="simple">
							</s:select></span>
					</h2>
					<div class="order_table">
						<table>
							<tr>
								<th>催付类型:</th>
								<td><input name="isExpediting" type="radio" value="0" /> 不催付&nbsp;&nbsp;&nbsp;&nbsp;<input name="isExpediting" type="radio" value="1" /> 次日催付</td>
							</tr>
							<tr>
								<th>启动类型:</th>
							...
							<tr>
								<th>催付时间:</th>
								<td>下单&nbsp;&nbsp;<input class="sweet-tooltip" id="orderTime" name="orderTime" type="text" value="${expeditingSetting.startDate}" />&nbsp;&nbsp;分钟,后开始催付 </br>
								</td>
							</tr>
							<tr>
								<th></th>
								<td>超出发送时间&nbsp;&nbsp;&nbsp;&nbsp;<input name="timeOutIsExpediting" type="radio" value="0" /> 催付&nbsp;&nbsp;<input name="timeOutIsExpediting" type="radio" value="1" /> 不催付
								</td>
							</tr>
							<tr>
								<th>会员等级:</th>

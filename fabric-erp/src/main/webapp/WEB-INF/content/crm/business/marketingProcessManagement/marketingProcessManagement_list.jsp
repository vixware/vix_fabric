...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/crm/business/marketingProcessManagementAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function goStart(id){
	$.ajax({
		  url:'${vix}/crm/business/marketingProcessManagementAction!goStartFlowTask.action?id='+id,
		  cache: false,
...
function goStop(id){
	$.ajax({
		  url:'${vix}/crm/business/marketingProcessManagementAction!goStopFlowTask.action?id='+id,
		  cache: false,
		  success: function(result){
				showMessage(result);
				setTimeout("", 1000);
...
//载入分页列表数据
pager("start","${vix}/crm/business/marketingProcessManagementAction!goListContent.action?1=1",'marketingAutomationProcess');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/crm/business/marketingProcessManagementAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy,'marketingAutomationProcess');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/crm/business/marketingProcessManagementAction!goListContent.action?name="+name,'marketingAutomationProcess');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/crm/business/marketingProcessManagementAction!goListContent.action?status=" + status, 'marketingAutomationProcess');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/crm/business/marketingProcessManagementAction!goListContent.action?days=" + days, 'marketingAutomationProcess');
}
function searchForContent() {
	loadName();
	pager("start","${vix}/crm/business/marketingProcessManagementAction!goListContent.action?name="+name,'marketingAutomationProcess');
}
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="marketingAutomationProcessInfo"></b> <s:text name='cmn_to' /> <b class="marketingAutomationProcessTotalCount"></b>)

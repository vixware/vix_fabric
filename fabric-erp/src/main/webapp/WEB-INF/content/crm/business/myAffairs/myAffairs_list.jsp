...
function saveOrUpdate(id) {
	$.ajax({
	url : '${vix}/crm/business/followPaymentAction!goSaveOrUpdateFollowing.action?id=' + id,
	cache : false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/crm/business/myAffairsAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/crm/business/myAffairsAction!goSingleList.action?name="+name,'myAffairs');
			});
		  }
	});
}
...
function orderBy(orderField){
	var orderBy = $("#myAffairsOrderBy").val();
	if(orderBy == 'desc'){
		orderBy = "asc";
	}else{
		orderBy = 'desc';
	}
	pager("start","${vix}/crm/business/myAffairsAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'myAffairs');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/crm/business/myAffairsAction!goSingleList.action?name="+name,'myAffairs');
}
...
function searchForContent(tag){
	loadName();
	if(tag==0){
     	pager("start","${vix}/crm/business/myAffairsAction!goSingleList.action?content="+name+"&tag="+tag,'myAffairs');
	}else {
	    pager("start","${vix}/crm/business/myAffairsAction!goSingleList.action?outwarehousecodes="+outwarehousecodes+"&inwarehousecodes="+inwarehousecodes+"&code1="+code1+"&tag="+tag,'myAffairs');
	}
}
/* 重置 */
...
function listbystatus(status) {
	pager("start", "${vix}/crm/business/myAffairsAction!goSingleList.action?status=" + status, 'myAffairs');
}myAffairs
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/crm/business/myAffairsAction!goSingleList.action?days=" + days, 'myAffairs');
}
bindSearch();
</script>
<div class="sub_menu">
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="myAffairsInfo"></b> <s:text name='cmn_to' /> <b class="myAffairsTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a> </span> <span><a class="end" onclick="currentPager('end');"></a> </span>
					</div>
				</div>
				<div id="myAffairs" class="table"></div>
				<div class="pagelist drop">
					<ul>

...
<script type="text/javascript">
	function chosedatabydate() {
		loadContent('${vix}/crm/business/membershipStructureAnalysisAction!goList.action?date=' + $('#date').val() + "&dateType=" + $(":radio[name=datetype][checked]").val() + "&channelDistributorId=" + $('#channelDistributorId').val() + "&weekday=" + $('#weekday').val() + "&monthdate=" + $('#monthdate').val() + "&startdate=" + $('#startdate').val() + "&enddate=" + $('#enddate').val());
	}
	...
	function goSearch() {
		$.ajax({
		url : '${vix}/crm/business/membershipStructureAnalysisAction!goSearch.action',
		cache : false,
		success : function(html) {
			asyncbox.open({
			modal : true,
...
<div class="content">
	...
	<div class="box">
		...
		<script>
			...
			"h" : 100,
			"th" : 40,
			"url" : "/vixnt/crm/business/membershipStructureAnalysisAction!goShowData.action"
			}, {
			"x" : 505,
			"y" : 314,
			"title" : "老客户",
			...
			"h" : 100,
			"th" : 40,
			"url" : "/vixnt/crm/business/membershipStructureAnalysisAction!goOldCustomerData.action"
			}, {
			"x" : 505,
			"y" : 107,
			"title" : "新客户",
			...
			"h" : 100,
			"th" : 40,
			"url" : "/vixnt/crm/business/membershipStructureAnalysisAction!goNewCustomerData.action"
			}, {
			"x" : 866,
			"y" : 33,
			"title" : "购买多次",
			...
			"h" : 100,
			"th" : 40,
			"url" : "/vixnt/crm/business/membershipStructureAnalysisAction!goBuyMore.action"
			}, {
			"x" : 866,
			"y" : 178,
			"title" : "购买一次",
			...
			"h" : 100,
			"th" : 40,
			"url" : "/vixnt/crm/business/membershipStructureAnalysisAction!goBuyOne.action"
			}, {
			"x" : 1200,
			"y" : 494,
			"title" : "季度激活客户",
			...
			<table class="list">
				...
				<s:iterator value="combinedSalesGoodsList" var="entity" status="s">
					<tr>
						...
						<td>${entity.apeoplenumber }<br /> <input type="button" class="btn" value="立即营销" /><br />
						</td>
						<td>${entity.bpeoplenumber }</td>
					</tr>
				</s:iterator>

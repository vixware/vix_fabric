...
function saveOrUpdate(id) {
	$.ajax({
	url : '${vix}/crm/business/sentLogAction!goSaveOrUpdateFollowing.action?id=' + id,
	cache : false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/crm/business/sentLogAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/crm/business/sentLogAction!goSingleList.action?name="+name,'myAffairs');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/crm/business/sentLogAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'myAffairs');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/crm/business/sentLogAction!goSingleList.action?name="+name,'myAffairs');
}
...
function searchForContent(tag){
	loadName();
	if(tag==0){
     	pager("start","${vix}/crm/business/sentLogAction!goSearchTransferList.action?content="+name+"&tag="+tag,'myAffairs');
	}else {
	    pager("start","${vix}/crm/business/sentLogAction!goSearchTransferList.action?outwarehousecodes="+outwarehousecodes+"&inwarehousecodes="+inwarehousecodes+"&code1="+code1+"&tag="+tag,'myAffairs');
	}
}
/* 重置 */
...
function listbystatus(status) {
	pager("start", "${vix}/crm/business/sentLogAction!goSingleList.action?status=" + status, 'myAffairs');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/crm/business/sentLogAction!goSingleList.action?days=" + days, 'myAffairs');
}
bindSearch();
</script>
<div class="sub_menu">

...
$("#emailTemplateForm").validationEngine();
function saveOrUpdate(id,pageNo) {
	$.ajax({
	url : '${vix}/crm/business/businessSetAction!goEmailTemplate.action?id='+id+"&pageNo="+pageNo,
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				if ($('#emailTemplateForm').validationEngine('validate')) {
					$.post('${vix}/crm/business/businessSetAction!saveOrUpdateEmailTemplate.action', {
						'emailTemplate.id' : $("#emailTemplateId").val(),
						'emailTemplate.name' : $("#emailTemplateName").val(),
						'emailTemplate.isOrderCode' : $(":checkbox[name=isOrderCode][checked]").val(),
						'emailTemplate.isBuyerNick' : $(":checkbox[name=isBuyerNick][checked]").val(),
						'emailTemplate.isGoodsName' : $(":checkbox[name=isGoodsName][checked]").val(),
						'updateField' : updateField,
						'emailTemplate.emailContent' :  $("#emailContent").val()
					}, function(result) {
						showMessage(result);
						setTimeout("", 1000);
						loadContent('${vix}/crm/business/businessSetAction!goEmailList.action');
					});
				} else {
					return false;
				}
...
loadName();
//载入分页列表数据
pager("start","${vix}/crm/business/businessSetAction!goEmailListContent.action?1=1",'emailTemplate');
//排序 
function orderBy(orderField){
	var orderBy = $("#emailTemplateOrderBy").val();
	if(orderBy == 'desc'){
		orderBy = "asc";
	}else{
		orderBy = 'desc';
	}
	pager("start","${vix}/crm/business/businessSetAction!goEmailListContent.action?orderField="+orderField+"&orderBy="+orderBy,'emailTemplate');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/crm/business/businessSetAction!goEmailListContent.action?name="+name,'emailTemplate');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/crm/business/businessSetAction!goSingleList.action?status=" + status, 'emailTemplate');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/crm/business/businessSetAction!goSingleList.action?days=" + days, 'emailTemplate');
}
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/crm/business/businessSetAction!goEmailListContent.action?parentId="+treeNode.id+"&treeType="+treeNode.treeType,"salesOrder");
				}
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="emailTemplateInfo"></b> <s:text name='cmn_to' /> <b class="emailTemplateTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a> </span> <span><a class="end" onclick="currentPager('end');"></a> </span>
					</div>
				</div>
				<div id="emailTemplate" class="table"></div>
				<div class="pagelist drop">
					<ul>

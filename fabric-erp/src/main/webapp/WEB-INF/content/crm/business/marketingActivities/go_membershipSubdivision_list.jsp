...
<script type="text/javascript">
	$.returnValue = "";
	pager("start", "${vix}/crm/business/marketingActivitiesAction!goMemberShipSubdivisionListContent.action?1=1", "radio");
	//排序 
	function subOrderBy(orderField) {
		...
		pager("start", "${vix}/crm/business/marketingActivitiesAction!goMemberShipSubdivisionListContent.action?orderField=" + orderField + "&orderBy=" + orderBy, "radio");
	}
	//选择采购订单翻页
	function currentOrderPager(tag) {
		pager(tag, "${vix}/crm/business/marketingActivitiesAction!goMemberShipSubdivisionListContent.action?1=1", 'radio');
	}
	var code = "";
	function loadCode() {
		code = $('#srm_code').val();
	...
	function searchForContent(i) {
		loadCode();
		pager("start", "${vix}/crm/business/marketingActivitiesAction!goMemberShipSubdivisionListContent.action?searchContent=" + code, 'radio');
	}
	/*重置搜索*/
	...
	function srmStatus(status) {
		pager("start", "${vix}/crm/business/marketingActivitiesAction!goMemberShipSubdivisionListContent.action?status=" + status, 'radio');
	}
	//最近使用
	function srmRecent(rencentDate) {
		pager("start", "${vix}/crm/business/marketingActivitiesAction!goMemberShipSubdivisionListContent.action?updateTime=" + rencentDate, 'radio');
	}

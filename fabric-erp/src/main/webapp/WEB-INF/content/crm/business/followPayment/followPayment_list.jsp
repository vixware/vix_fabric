...
<script type="text/javascript">
	//载入分页列表数据
	pager("start", "${vix}/crm/business/followPaymentAction!goSingleList.action?1=1", 'salesOrder');

	function currentPager(tag) {
		pager(tag, "${vix}/crm/business/followPaymentAction!goSingleList.action?1=1", 'salesOrder');
	}

	function saveOrUpdateExpediting(salesOrderId) {
		$.ajax({
		url : '${vix}/crm/business/followPaymentAction!goSaveOrUpdate.action?salesOrderId=' + salesOrderId,
		cache : false,
			...
			callback : function(action) {
				if (action == 'ok') {
					if ($('#messageLogForm').validationEngine('validate')) {
						$.post('${vix}/crm/business/followPaymentAction!saveOrUpdateMessage.action', {
							...
							showMessage(result);
							setTimeout("", 1000);
							loadContent('${vix}/crm/business/followPaymentAction!goList.action');
						});
					} else {
						return false;
					}
	...
	function saveOrUpdate(salesOrderId) {
		$.ajax({
		url : '${vix}/crm/business/followPaymentAction!goSaveOrUpdateFollowing.action?salesOrderId=' + salesOrderId,
		cache : false,
		success : function(html) {
			asyncbox.open({
			modal : true,
			...
			callback : function(action) {
				if (action == 'ok') {
					if ($('#myAffairsForm').validationEngine('validate')) {
						$.post('${vix}/crm/business/followPaymentAction!saveOrUpdateTransactions.action', {

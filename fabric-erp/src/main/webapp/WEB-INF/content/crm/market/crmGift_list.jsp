...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/crm/market/crmGiftAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/crm/market/crmGiftAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/crm/market/crmGiftAction!goListContent.action?name="+name,'crmGift');
			});
		  }
	});
}
...
function orderBy(orderField){
	loadName();
	var orderBy = $("#crmGiftOrderBy").val();
	if(orderBy == 'desc'){
		orderBy = "asc";
	}else{
		orderBy = 'desc';
	}
	pager("start","${vix}/crm/market/crmGiftAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'crmGift');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/crm/market/crmGiftAction!goListContent.action?name="+name,'crmGift');
}
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="indexList" var="index">
				<li><a href="###" onclick="saveOrUpdate(${index.id});"><span style="display: none;">${index.chineseFirstLetter}</span>${index.name}</a></li>
			</s:iterator>
		</ul>
	</div>
	<!-- c_head -->
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a></span> <span><a class="previous" onclick="currentPager('previous');"></a></span> <em>(<b class="crmGiftInfo"></b> <s:text name='cmn_to' /> <b class="crmGiftTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a></span> <span><a class="end" onclick="currentPager('end');"></a></span>
					</div>
				</div>
				<div id="crmGift" class="table"></div>
				<div class="pagelist drop">
					<ul>

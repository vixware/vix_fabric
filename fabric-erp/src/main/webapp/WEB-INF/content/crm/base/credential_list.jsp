...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/crm/base/credentialAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							$.post('${vix}/crm/base/credentialAction!saveOrUpdate.action',
								 {'credential.id':$("#id").val(),
								  'credential.credentialCode':$("#credentialCode").val(),
								  'credential.credentialType.id':$("#credentialTypeId").val(),
								  'credential.memo':$("#memo").val()
								},
								function(result){
									asyncbox.success(result,"<s:text name='message'/>",function(action){
										pager("start","${vix}/crm/base/credentialAction!goListContent.action?name="+name,'credential');
									});
								}
							 );
						}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/crm/base/credentialAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
...
function orderBy(orderField){
	loadName();
	var orderBy = $("#credentialOrderBy").val();
	if(orderBy == 'desc'){
		orderBy = "asc";
	}else{
		orderBy = 'desc';
	}
	pager("start","${vix}/crm/base/credentialAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'credential');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/crm/base/credentialAction!goListContent.action?name="+name,'credential');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">设置</a></li>
				<li><a href="#" onclick="loadContent('${vix}/crm/base/credentialAction!goList.action');">证件</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a></span> <span><a class="previous" onclick="currentPager('previous');"></a></span> <em>(<b class="credentialInfo"></b> <s:text name='cmn_to' /> <b class="credentialTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a></span> <span><a class="end" onclick="currentPager('end');"></a></span>
					</div>
				</div>
				<div id="credential" class="table"></div>
				<div class="pagelist drop">
					<ul>

...
function saveOrUpdate(id){
	if(id==0){
		   return  loadContent('${vix}/dtbcenter/yardManagementAction!goSaveOrUpdate.action');
	}else
		return null;
};
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/dtbcenter/yardManagementAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/dtbcenter/yardManagementAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/dtbcenter/yardManagementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/dtbcenter/yardManagementAction!goSingleList.action?name="+name,'brand');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/dtbcenter/yardManagementAction!goSingleList.action?status=" + status, 'brand');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/dtbcenter/yardManagementAction!goSingleList.action?days=" + days, 'brand');
}
function goSearch() {
	$.ajax({
	url : '${vix}/dtbcenter/yardManagementAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/dtbcenter/yardManagementAction!goSingleList.action?code=" + $('#code').val() + "&name=" + $('#name').val(), 'brand');
			}
		},

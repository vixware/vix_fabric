...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/dtbcenter/automatedQuotationAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/dtbcenter/automatedQuotationAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
...
function orderBy(orderField){
	...
	pager("start","${vix}/dtbcenter/automatedQuotationAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/dtbcenter/automatedQuotationAction!goSingleList.action?name="+name,'brand');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/dtbcenter/automatedQuotationAction!goSingleList.action?status=" + status, 'brand');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/dtbcenter/automatedQuotationAction!goSingleList.action?days=" + days, 'brand');
}
</script>
<div class="sub_menu">
	<h2>

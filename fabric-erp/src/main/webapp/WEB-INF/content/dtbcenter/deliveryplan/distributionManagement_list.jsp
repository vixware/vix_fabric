...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/dtbcenter/distributionManagementAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/dtbcenter/distributionManagementAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/dtbcenter/distributionManagementAction!goSingleList.action?name="+name,'deliveryPlan');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/dtbcenter/distributionManagementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'deliveryPlan');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/dtbcenter/distributionManagementAction!goSingleList.action?status=" + status, 'deliveryPlan');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/dtbcenter/distributionManagementAction!goSingleList.action?days=" + days, 'deliveryPlan');
}
bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/dtbcenter/distributionManagementAction!goSingleList.action?name="+name,'deliveryPlan');
}
function goSearch() {
	$.ajax({
	url : '${vix}/dtbcenter/distributionManagementAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/dtbcenter/distributionManagementAction!goSingleList.action?code=" + $('#code').val() + "&name=" + $('#name').val(), 'deliveryPlan');
			}
		},
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="ldm_distribution" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="deliveryPlanList" var="c">
	...
	<div class="box">
		...
		<div id="right">
			<div class="right_content">
				...
				<div id="deliveryPlan" class="table"></div>
				<div class="pagelist drop">
					<ul>

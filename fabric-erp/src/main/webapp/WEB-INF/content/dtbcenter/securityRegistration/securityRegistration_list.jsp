...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/dtbcenter/securityRegistrationAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/dtbcenter/securityRegistrationAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/dtbcenter/securityRegistrationAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/dtbcenter/securityRegistrationAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/dtbcenter/securityRegistrationAction!goSingleList.action?name="+name,'brand');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/dtbcenter/securityRegistrationAction!goSingleList.action?status=" + status, 'brand');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/dtbcenter/securityRegistrationAction!goSingleList.action?days=" + days, 'brand');
}
</script>
<div class="sub_menu">
	<h2>

...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/dtbcenter/expressSingleAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/dtbcenter/expressSingleAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/dtbcenter/expressSingleAction!goSingleList.action?name="+name,'expressSingle');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/dtbcenter/expressSingleAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'expressSingle');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/dtbcenter/expressSingleAction!goSingleList.action?name="+name,'expressSingle');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/dtbcenter/expressSingleAction!goSingleList.action?status=" + status, 'expressSingle');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/dtbcenter/expressSingleAction!goSingleList.action?days=" + days, 'expressSingle');
}
function goSearch() {
	$.ajax({
	url : '${vix}/dtbcenter/expressSingleAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/dtbcenter/expressSingleAction!goSingleList.action?code=" + $('#code').val() + "&name=" + $('#name').val(), 'expressSingle');
			}
		},
...
<div class="content">
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="expressSingleList" var="c">
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="expressSingleInfo"></b> <s:text name='cmn_to' /> <b class="expressSingleTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a> </span> <span><a class="end" onclick="currentPager('end');"></a> </span>
					</div>
				</div>
				<div id="expressSingle" class="table"></div>
				<div class="pagelist drop">
					<ul>

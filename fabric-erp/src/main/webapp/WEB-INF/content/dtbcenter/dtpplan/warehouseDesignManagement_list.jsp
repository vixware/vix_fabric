...
function deleteById(id){
	$.ajax({
		  url:'${vix}/dtbcenter/warehouseDesignManagementAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/dtbcenter/warehouseDesignManagementAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/dtbcenter/warehouseDesignManagementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/dtbcenter/warehouseDesignManagementAction!goSingleList.action?name="+name,'brand');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="ldm_dtbcenter_plan" /> </a></li>
				<li><a href="#"><s:text name="ldm_warehouse_plan_management" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0);"><span><s:text name='ldm_warehouse_control' /> </span> </a><a href="#" onclick="saveOrUpdate(0);"><span><s:text name='ldm_lyplanning_control' /> </span> </a><a href="#" onclick="saveOrUpdate(0);"><span><s:text name='ldm_hr_planning' /> </span> </a><a href="#" onclick="deleteByIds();"><span><s:text
						name='cmn_delete' /> </span> </a><a href="#" onclick="saveOrUpdate(0);"><span><s:text name='ldm_export' /> </span> </a><a href="#" onclick="saveOrUpdate(0);"><span><s:text name='ldm_enabled' /> </span> </a>
		</p>
	</div>
</div>
<div class="content">
	<div class="drop" id="c_head">
		...
		<div class="search_advanced">
			<label><s:text name="ldm_warehouse_code1" /><input type="text" class="int" name=""> </label> <label><s:text name="ldm_warehouse_name1" /><input type="text" class="int" name=""> </label>
		</div>
	</div>
	<div id="number">

...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/dtbcenter/vehiclePlanAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/dtbcenter/vehiclePlanAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/dtbcenter/vehiclePlanAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/dtbcenter/vehiclePlanAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/dtbcenter/vehiclePlanAction!goSingleList.action?name="+name,'brand');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/dtbcenter/vehiclePlanAction!goSingleList.action?status=" + status, 'brand');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/dtbcenter/vehiclePlanAction!goSingleList.action?days=" + days, 'brand');
}
function goSearch() {
	$.ajax({
	...
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/dtbcenter/vehiclePlanAction!goSingleList.action?code=" + $('#code').val() + "&name=" + $('#name').val(), 'brand');
			}
		},
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div>
			<label>内容:<input type="text" class="int" id="nameS" placeholder="派车单号"></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent(0);" /><input type="button" value="<s:text name='cmn_reset'/>" class="btn" name="" onclick="resetForContent(0);" /></label> <label> <input
				type="button" value="高级搜索" class="btn" onclick="goSearch();" /></label>
		</div>
		<%-- <div class="search_advanced">
			<label>派车单号:<input type="text" class="int" name="code1" id="code1">
			</label> <label> 车辆: <input type="text" class="int" name="warehousecode" id="warehousecode1">

...
<script type="text/javascript">
	...
	pager("start", "${vix}/dtbcenter/sentCarSingleManagementAction!goDispatchRouteList.action?temp=1", "radio");
	//排序 
	function subOrderBy(orderField) {
		...
		pager("start", "${vix}/dtbcenter/sentCarSingleManagementAction.action?orderField=" + orderField + "&orderBy=" + orderBy, "radio");
	}
	function currentPager(tag) {
		pager(tag, "${vix}/dtbcenter/sentCarSingleManagementAction.action", 'radio');
	}
	var code = "";
	function loadCode() {
		code = $('#srm_code').val();
	...
	function searchForContent(i) {
		loadCode();
		pager("start", "${vix}/dtbcenter/sentCarSingleManagementAction!goDispatchRouteList.action?searchContent=" + code, 'radio');
	}
	/*重置搜索*/
	...
	function srmStatus(status) {
		pager("start", "${vix}/dtbcenter/sentCarSingleManagementAction!goDispatchRouteList.action?status=" + status, 'radio');
	}
	//最近使用
	function srmRecent(rencentDate) {
		pager("start", "${vix}/dtbcenter/sentCarSingleManagementAction!goDispatchRouteList.action?updateTime=" + rencentDate, 'radio');
	}
</script>

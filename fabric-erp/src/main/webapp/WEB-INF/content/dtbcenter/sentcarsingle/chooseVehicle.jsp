...
<script type="text/javascript">
	$.returnValue = "";
	function chkChange(chk, id, name, carryingcapacity, capacity, driver, driverTelephone) {
		if (chk.checked) {
			$.returnValue = id + "," + name + "," + carryingcapacity + "," + capacity + "," + driver + "," + driverTelephone;
		}
	}
	pager("start", "${vix}/dtbcenter/sentCarSingleManagementAction!goVehicleList.action?temp=1", "radio");
	//排序 
	function subOrderBy(orderField) {
		...
		pager("start", "${vix}/template/simpleGridAction!goVehicleList.action?orderField=" + orderField + "&orderBy=" + orderBy, "radio");
	}
	function currentPager(tag) {
		pager(tag, "${vix}/template/simpleGridAction!goVehicleList.action", 'radio');
	}
	var code = "";
	function loadCode() {
		code = $('#srm_code').val();
	...
	function searchForContent(i) {
		loadCode();
		pager("start", "${vix}/dtbcenter/sentCarSingleManagementAction!goVehicleList.action?searchContent=" + code, 'radio');
	}
	/*重置搜索*/
	...
	function srmStatus(status) {
		pager("start", "${vix}/dtbcenter/sentCarSingleManagementAction!goVehicleList.action?status=" + status, 'radio');
	}
	//最近使用
	function srmRecent(rencentDate) {
		pager("start", "${vix}/dtbcenter/sentCarSingleManagementAction!goVehicleList.action?updateTime=" + rencentDate, 'radio');
	}
</script>

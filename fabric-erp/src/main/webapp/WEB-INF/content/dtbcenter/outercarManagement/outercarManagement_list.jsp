...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/dtbcenter/outercarManagementAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/dtbcenter/outercarManagementAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/dtbcenter/outercarManagementAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/dtbcenter/outercarManagementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/dtbcenter/outercarManagementAction!goSingleList.action?name="+name,'brand');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/dtbcenter/outercarManagementAction!goSingleList.action?status=" + status, 'brand');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/dtbcenter/outercarManagementAction!goSingleList.action?days=" + days, 'brand');
}
function goSearch() {
	$.ajax({
	url : '${vix}/dtbcenter/outercarManagementAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/dtbcenter/outercarManagementAction!goSingleList.action?code=" + $('#code').val() + "&name=" + $('#name').val(), 'brand');
			}
		},
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="ldm_outer_car_management" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

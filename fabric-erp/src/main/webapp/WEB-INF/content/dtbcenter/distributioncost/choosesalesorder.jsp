...
<script type="text/javascript">
	$.returnValue = "";
	function chkChange (chk , code , name){
		if (chk.checked){
			$.returnValue = code + "," + name;
		}
	}
	pager ("start" , "${vix}/dtbcenter/distributionCostAction!goSalesOrderList.action?temp=1" , "radio");
	//排序 
	function subOrderBy (orderField){
		...
		pager ("start" , "${vix}/dtbcenter/distributionCostAction.action?orderField=" + orderField + "&orderBy=" + orderBy , "radio");
	}
	function currentPager (tag){
		pager (tag , "${vix}/dtbcenter/distributionCostAction.action" , 'radio');
	}
	...
	function searchForContent (i){
		loadCode ();
		pager ("start" , "${vix}/dtbcenter/distributionCostAction!goSalesOrderList.action?searchContent=" + code , 'radio');
	}
	/*重置搜索*/
	...
	function srmStatus (status){
		pager ("start" , "${vix}/dtbcenter/distributionCostAction!goSalesOrderList.action?status=" + status , 'radio');
	}
	//最近使用
	function srmRecent (rencentDate){
		pager ("start" , "${vix}/dtbcenter/distributionCostAction!goSalesOrderList.action?updateTime=" + rencentDate , 'radio');
	}
</script>

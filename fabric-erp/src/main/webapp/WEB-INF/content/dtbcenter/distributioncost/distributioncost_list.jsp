...
function saveOrUpdate(id,pageNo){	
	$.ajax({
	  url:'${vix}/dtbcenter/distributionCostAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
	  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/dtbcenter/distributionCostAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/dtbcenter/distributionCostAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/dtbcenter/distributionCostAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/dtbcenter/distributionCostAction!goSingleList.action?name="+name,'brand');
}
/** 状态 */
function listbystatus(status) {
	pager("start", "${vix}/dtbcenter/distributionCostAction!goSingleList.action?status=" + status, 'brand');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/dtbcenter/distributionCostAction!goSingleList.action?days=" + days, 'brand');
}
</script>
<div class="sub_menu">
	<h2>

...
<script type="text/javascript">
	...
	function saveOrUpdateOrder() {
		if ($('#order').validationEngine('validate')) {
			$.post('${vix}/dtbcenter/settlementManagementAction!saveOrUpdate.action', {
			...
			'deliveryReceipt.carrier' : $("#carrier").val()
				...
				showMessage(result);
				setTimeout("", 1000);
				loadContent('${vix}/dtbcenter/settlementManagementAction!goList.action');
			});
		} else {
			return false;
		}
		...
		success : function(html) {
			...
			callback : function(action, returnValue) {
				if (action == 'ok') {
					if (returnValue != '') {
						$.ajax({
						url : '${vix}/dtbcenter/settlementManagementAction!converterSalesOrderToTruckingOrder.action?salesOrderid=' + returnValue + "&id=${deliveryReceipt.id}",
						cache : false,
		...
		success : function(html) {
			...
			callback : function(action, returnValue) {
				if (action == 'ok') {
					if (returnValue != '') {
						$.ajax({
						url : '${vix}/dtbcenter/settlementManagementAction!converterDispatchRouteToDeliveryReceipt.action?dispatchRouteid=' + returnValue + "&id=${deliveryReceipt.id}",
						cache : false,
						success : function(result) {
							showMessage(result);
							setTimeout("", 1000);
	...
	function choosePurchaseOrder() {
		$.ajax({
		url : '${vix}/dtbcenter/settlementManagementAction!goChooseVehicle.action',
		cache : false,
		success : function(html) {
			asyncbox.open({
			modal : true,
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/dtbcenter/settlementManagementAction!goList.action?pageNo=${pageNo}');">结算管理 </a></li>
			</ul>
		</div>
	</h2>
</div>
			...
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateOrder()" href="#"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"> </a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /> </a> <a href="#"><img width="22" height="22" title="保存并提交审批"
							src="${vix}/common/img/dt_submit.png" /> </a> <a href="#"><img width="22" height="22" title="取消" src="${vix}/common/img/dt_cancelback.png" /> </a> <a href="#" onclick="loadContent('${vix}/dtbcenter/settlementManagementAction!goList.action?pageNo=${pageNo}');"><img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png">
					</a>
					</span> <strong><b>结算单 </b> </strong>
				</dt>
				<dd class="clearfix">
								...
								<dd style="display: block;">
									<table style="border: none;">
										...
										<tr>
											...
											<th>车牌号：</th>
											<td><input id="vehicleNO" name="vehicleNO" value="${deliveryReceipt.vehicleNO }" type="text" size="20" class="validate[required] text-input"> <input class="btn" type="button" value="选车" onclick="choosePurchaseOrder();" /><span style="color: red;">*</span></td>
										</tr>
										<tr>
											<th>载重 ：</th>
										...
										<tr>
											<th>驾驶员：</th>
											<td><select id="driver" name="driver" class="validate[required] text-input">
													<option value="">请选择</option>
										...
										<tr>
											<th>计划发车时间 ：</th>
											<td><input id="sendInPlan" name="sendInPlan" value="${deliveryReceipt.sendInPlan}" type="text" class="validate[required] text-input" readonly="readonly" /> <img onclick="showTime('sendInPlan','yyyy-MM-dd HH:mm:ss')" src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"></td>
											<th>实际发车时间：</th>
											<td><input id="dispatchTime" name="dispatchTime" value="${deliveryReceipt.dispatchTime}" type="text" class="validate[required] text-input" readonly="readonly" /> <img onclick="showTime('dispatchTime','yyyy-MM-dd HH:mm:ss')" src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"></td>
										</tr>
										<tr>
											<th>出发里程数 ：</th>
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;">
						<script type="text/javascript">
							...
							url : '${vix}/dtbcenter/settlementManagementAction!getSalesOrderJson.action?id=${deliveryReceipt.id}',
							width : "100%",
							height : 450,
							nowrap : true,
							autoRowHeight : false,
							...
							handler : function() {
								...
								saveDeliveryAddress('${vix}/dtbcenter/settlementManagementAction!goChooseSalesOrder.action');
							}
							}, '-', {
							text : '删除',
							iconCls : 'icon-remove',
							handler : function() {
								...
								for ( var i = 0; i < rows.length; i++) {
									...
									$.ajax({
									url : '${vix}/dtbcenter/settlementManagementAction!deleteWimStockrecordlinesById.action?id=' + rows[i].id,
									cache : false
									});
								}
							}
					...
					<div id="a2" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<script type="text/javascript">
							...
							url : '${vix}/dtbcenter/settlementManagementAction!getDispatchRouteJson.action?id=${deliveryReceipt.id}',
							width : "100%",
							height : 450,
							nowrap : true,
							autoRowHeight : false,
							...
							handler : function() {
								$('#btnsave').linkbutton('enable');
								saveDispatchRoute('${vix}/dtbcenter/settlementManagementAction!goChooseDispatchRoute.action');
							}
							}, '-', {
							text : '删除',
							iconCls : 'icon-remove',

...
function saveOrUpdate(id,pageNo){	
	$.ajax({
	  url:'${vix}/dtbcenter/pickupBusinessAnalysisAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
	  cache: false,
...
function goSearch() {
	$.ajax({
	url : '${vix}/dtbcenter/pickupManagementAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/dtbcenter/pickupManagementAction!goSingleList.action?code=" + $('#code').val() + "&name=" + $('#name').val(), 'brand');
			}
		},

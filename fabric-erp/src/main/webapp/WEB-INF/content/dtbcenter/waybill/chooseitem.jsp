...
<script type="text/javascript">
	$.returnValue = "";
	pager ("start" , "${vix}/dtbcenter/orderProcessingAction!goItemList.action?temp=1" , "radio");
	//排序 
	function subOrderBy (orderField){
		...
		pager ("start" , "${vix}/template/simpleGridAction!goItemList.action?orderField=" + orderField + "&orderBy=" + orderBy , "radio");
	}
	function currentPager (tag){
		pager (tag , "${vix}/template/simpleGridAction!goItemList.action" , 'radio');
	}
	...
	function searchForContent (i){
		loadCode ();
		pager ("start" , "${vix}/dtbcenter/orderProcessingAction!goItemList.action?searchContent=" + code , 'radio');
	}
	/*重置搜索*/
	...
	function srmStatus (status){
		pager ("start" , "${vix}/dtbcenter/orderProcessingAction!goItemList.action?status=" + status , 'radio');
	}
	//最近使用
	function srmRecent (rencentDate){
		pager ("start" , "${vix}/dtbcenter/orderProcessingAction!goItemList.action?updateTime=" + rencentDate , 'radio');
	}

...
function saveOrUpdateTransfer(id,pageNo){
	$.ajax({
		  url:'${vix}/inventory/orderReservationAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function saveOrUpdateAllocateandTransfer(id,pageNo){
	$.ajax({
		  url:'${vix}/inventory/orderReservationAction!goSaveOrUpdateAllocateandTransfer.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
		  	$("#mainContent").html(html);
		  }
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/inventory/orderReservationAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/inventory/orderReservationAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/inventory/orderReservationAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/inventory/orderReservationAction!goSingleList.action?name="+name,'brand');
}
...
function searchForContent(tag){
	...
	if(tag==0){
     	pager("start","${vix}/inventory/orderReservationAction!goSearchTransferList.action?content="+name+"&tag="+tag,'brand');
	}else {
	    pager("start","${vix}/inventory/orderReservationAction!goSearchTransferList.action?outwarehousecodes="+outwarehousecodes+"&inwarehousecodes="+inwarehousecodes+"&code1="+code1+"&tag="+tag,'brand');
	}
}
/* 重置 */

...
<script type="text/javascript">
	...
	loadName();
	//载入分页列表数据
	pager("start", "${vix}/inventory/outBoundAction!goListItemList.action?name=" + name, 'inventory');
	//排序 
	function orderBy(orderField) {
		loadName();
		var orderBy = $("#inventoryOrderBy").val();
		if (orderBy == 'desc') {
			orderBy = "asc";
		} else {
			orderBy = 'desc';
		}
		pager("start", "${vix}/inventory/outBoundAction!goListItemList.action?orderField=" + orderField + "&orderBy=" + orderBy + "&name=" + name, 'inventory');
	}

	function currentPager(tag) {
		loadName();
		pager(tag, "${vix}/inventory/outBoundAction!goListItemList.action?name=" + name, 'inventory');
	}
	...
	<div class="box">
		<div id="left" style="width: 550px;">
			<div class="left_content">
				<div id="inventory" class="table"></div>
				<div class="pagelist drop">
					<div>
						<span><a class="start" onclick="currentPager('start');"></a></span> <span><a class="previous" onclick="currentPager('previous');"></a></span> <em>(<b class="inventoryInfo"></b> <s:text name='cmn_to' /> <b class="inventoryTotalCount"></b>)

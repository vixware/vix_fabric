...
<script type="text/javascript">
	$.returnValue = "";
	pager("start", "${vix}/inventory/outBoundAction!goSalesOrderList.action?temp=1", "radio");
	//排序 
	function subOrderBy(orderField) {
		...
		pager("start", "${vix}/template/simpleGridAction!goSalesOrderList.action?orderField=" + orderField + "&orderBy=" + orderBy, "radio");
	}
	function currentPager(tag) {
		pager(tag, "${vix}/template/simpleGridAction!goSalesOrderList.action", 'radio');
	}
	var code = "";
	function loadCode() {
		code = $('#srm_code').val();
	...
	function searchForContent(i) {
		loadCode();
		pager("start", "${vix}/inventory/outBoundAction!goSalesOrderList.action?searchContent=" + code, 'radio');
	}
	/*重置搜索*/
	...
	function srmStatus(status) {
		pager("start", "${vix}/inventory/outBoundAction!goSalesOrderList.action?status=" + status, 'radio');
	}
	//最近使用
	function srmRecent(rencentDate) {
		pager("start", "${vix}/inventory/outBoundAction!goSalesOrderList.action?updateTime=" + rencentDate, 'radio');
	}
	function chkChange(chk, id) {
		if (chk.checked) {
			$.ajax({
			url : '${vix}/inventory/outBoundAction!getSalesOrderItemList.action?purchaseOrderid=' + id,
			cache : false,

...
<script type="text/javascript">
	...
	function saveOrUpdateInventoryCurrentStock() {
		if ($("#objectExpandId").val() != null && $("#objectExpandId").val() != '') {
			...
			if ($('#inventoryForm').validationEngine('validate')) {
				$.post('${vix}/inventory/beginDefectiveItemAction!saveOrUpdate.action', {
				...
				'inventoryCurrentStock.qualityperiod' : $("#qualityperiod").val(),
				...
				'skudata' : ids
					...
					showMessage(result);
					setTimeout("", 1000);
					loadContent('${vix}/inventory/beginDefectiveItemAction!goList.action');
				});
			}
		} else {
	...
	function loadSpecification(id) {
		$.ajax({
		url : '${vix}/inventory/beginDefectiveItemAction!loadSpecification.action?id=' + id,
		cache : false,
		success : function(html) {
	...
	function updateSpecification() {
		...
		$.ajax({
		url : '${vix}/inventory/beginDefectiveItemAction!goUpdateSpecification.action?specificationDetailids=' + specificationDetailids + "&objectExpandId=" + $("#objectExpandId").val(),
		cache : false,
		success : function(html) {
	...
	function chooseItem() {
		$.ajax({
		url : '${vix}/inventory/beginDefectiveItemAction!goItem.action',
		cache : false,
		success : function(html) {
			asyncbox.open({
			modal : true,
			...
			callback : function(action, returnValue) {
				if (action == 'ok') {
					if (returnValue != '') {
						$.ajax({
						url : '${vix}/inventory/beginDefectiveItemAction!getItemListJson.action?itemId=' + returnValue,
						cache : false,
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div class="untitled">
			<b><a onclick="saveOrUpdateInventoryCurrentStock()" href="#"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"> </a> <a href="#" onclick="saveOrUpdateInventoryCurrentStock()"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /> </a> <s:if test="source =='business' ">
					...
					<div class="nt">
						...
						<table>
							...
							<tr height="30">
								...
								<td><input type="hidden" id="warehouseId" name="warehouseId" onchange="fieldChanged(this);" /><input type="text" name="warehouseName" id="warehouseName" value="${inventoryCurrentStock.invWarehouse.name }" size="30" class="validate[required] text-input" /><input class="btn" type="button" value="选择" onclick="choosewarehouse();" /><span

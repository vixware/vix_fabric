...
function saveOrUpdate(id,pageNo,type){
	$.ajax({
		  url:'${vix}/inventory/productAssemblyAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
loadName();
//载入分页列表数据
pager("start","${vix}/inventory/productAssemblyAction!goSingleList.action?name="+name,'inventoryCurrentStock');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/inventory/productAssemblyAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'inventoryCurrentStock');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/inventory/productAssemblyAction!goSingleList.action?name="+name,'inventoryCurrentStock');
}
...
function searchForContent(tag){
	...
	if(tag==0){
     	...
     	pager("start","${vix}/inventory/productAssemblyAction!goSingleList.action?selectCode="+selectCode+"&selectName="+selectName,'inventoryCurrentStock');
	}
}
/* 重置 */
...
function goSearch() {
	$.ajax({
	url : '${vix}/inventory/productAssemblyAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/inventory/productAssemblyAction!goSingleList.action?selectCode=" + $('#selectCode').val() + "&selectName=" + $('#selectName').val(), 'inventoryCurrentStock');
			}
		},
...
function deleteById(id) {
	$.ajax({
	url : '${vix}/inventory/productAssemblyAction!deleteById.action?id=' + id,
	cache : false,
	success : function(html) {
		asyncbox.success(html, "提示信息", function(action) {
			pager("start", "${vix}/inventory/productAssemblyAction!goSingleList.action?1=1", 'inventoryCurrentStock');
		});
	}
});
}
...
function deleteByIds(ids) {
		$.ajax({
		url : '${vix}/inventory/productAssemblyAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/inventory/productAssemblyAction!goSingleList.action?1=1", 'inventoryCurrentStock');

...
loadMenuContent('${vix}/inventory/inventoryMainAction!goMenuContent.action');
    ...
    function standingbookPager(tag) {
	    pager(tag, "${vix}/inventory/itemBatchAction!goSingleList.action?name=" + name, 'standingbook');
    }
    function searchForRightContent() {
	    loadName();
	    pager("start", "${vix}/inventory/itemBatchAction!goSingleList.action?name=" + name, 'standingbook');
    }
    loadName();
    //载入分页列表数据
    pager("start", "${vix}/inventory/itemBatchAction!goSingleList.action?name=" + name, 'standingbook');
    function loadRoot() {
	    $('#name').val("");
	    $('#selectId').val("");
	    pager("start", "${vix}/inventory/itemBatchAction!goSingleList.action?name=", 'standingbook');
    }
    //排序 
    function orderBy(orderField) {
	    ...
	    pager("start", "${vix}/inventory/itemBatchAction!goSingleList.action?orderField=" + orderField + "&orderBy=" + orderBy + "&name=" + name + "&parentId=" + $("#selectId")
	            .val(), 'standingbook');
    }
    bindSearch();
    bindSwitch();
    ...
    function saveOrUpdate(id) {
	    $.ajax({
	    url : '${vix}/inventory/itemBatchAction!goSaveOrUpdate.action?id=' + id,
	    cache : false,
		    ...
		    callback : function(action) {
			    if (action == 'ok') {
				    if ($('#itemform').validationEngine('validate')) {
					    $.post('${vix}/inventory/itemBatchAction!saveOrUpdate.action', {
					    'item.id' : $("#id").val(),
					    'item.batchCode' : $("#batchCode").val()
					    }, function(result) {
						    showMessage(result);
						    setTimeout("", 1000);
						    loadContent('${vix}/inventory/itemBatchAction!goList.action');
					    });
				    } else {
					    return false;
				    }
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">
                ...
                async : {
                enable : true,
                url : "${vix}/inventory/initInventoryAction!findTreeToJson.action",

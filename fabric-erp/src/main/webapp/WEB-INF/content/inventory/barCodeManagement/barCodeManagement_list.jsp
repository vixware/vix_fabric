...
<script type="text/javascript">
	...
	function saveOrUpdate() {
		if ($('#baseCoderForm').validationEngine('validate')) {
			$.post('${vix}/inventory/barCodeManagementAction!saveOrUpdate.action', {
			...
			'encodingRulesTableInTheMiddle.islevel12' : $(":checkbox[name=islevel12][checked]").val()
				...
				setTimeout("", 1000);
				$.ajax({
				url : '${vix}/inventory/barCodeManagementAction!goSaveOrUpdate.action?billTypeCode=' + $("#billTypeCode").val(),
				cache : false,
	...
	$(document).ready(function() {
		$.ajax({
		url : '${vix}/inventory/barCodeManagementAction!goSaveOrUpdate.action',
		cache : false,
		success : function(html) {
			$("#right").html(html);
		}
		...
		<div class="box">
			<div id="left" class="ui-resizable" style="height: 500px;">
				...
				<script type="text/javascript">
					...
					async : {
					enable : true,
					url : "${vix}/inventory/barCodeManagementAction!findItemCatalogAndItemTreeToJson.action",
					...
					function onClick(event, treeId, treeNode, clickFlag) {
						$.ajax({
						url : '${vix}/inventory/barCodeManagementAction!goSaveOrUpdate.action?billTypeCode=' + treeNode.code,
						cache : false,
						success : function(html) {
							$("#right").html(html);
							checkedradio();

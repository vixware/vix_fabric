...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/inventory/reverseFlushingMaterialAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/inventory/reverseFlushingMaterialAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/inventory/reverseFlushingMaterialAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/inventory/reverseFlushingMaterialAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/inventory/reverseFlushingMaterialAction!goSingleList.action?name="+name,'brand');
}
/** 状态 */
function saleOrderStatus(status) {
	pager("start", "${vix}/inventory/reverseFlushingMaterialAction!goSingleList.action?status=" + status, 'brand');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/inventory/reverseFlushingMaterialAction!goSingleList.action?days=" + days, 'brand');
}
bindSearch();
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<a href="#"><s:text name="wim_reverse_flushing_material" /> </a>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div class="search_advanced">
			<label>单号:<input type="text" class="int" name="stocktakingcode" id="stocktakingcode" placeholder="单号"></label> <label>仓库:<input type="text" class="int" name="warehousename" id="warehousename" placeholder="仓库"></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn" name=""

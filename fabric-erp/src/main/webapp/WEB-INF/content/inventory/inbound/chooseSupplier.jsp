...
<script type="text/javascript">
	...
	pager("start", "${vix}/inventory/inboundWarehouseAction!goSubRadioSingleList.action?temp=1", "radio");
	//排序 
	...
	function searchForContent(i) {
		loadCode();
		pager("start", "${vix}/inventory/inboundWarehouseAction!goSubRadioSingleList.action?searchContent=" + code, 'radio');
	}
	/*重置搜索*/
	...
	function srmStatus(status) {
		pager("start", "${vix}/inventory/inboundWarehouseAction!goSubRadioSingleList.action?status=" + status, 'radio');
	}
	//最近使用
	function srmRecent(rencentDate) {
		pager("start", "${vix}/inventory/inboundWarehouseAction!goSubRadioSingleList.action?updateTime=" + rencentDate, 'radio');
	}
</script>

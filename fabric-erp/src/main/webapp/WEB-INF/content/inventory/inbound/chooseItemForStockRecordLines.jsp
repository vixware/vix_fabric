			...
			<div class="right_content">
				<form id="saleOrderItemForm">
					...
					<div class="box order_table" style="line-height: normal;">
						<table>
							<tr height="40">
								<th><span style="color: red;">*</span> 商品编码:</th>
								<td><input id="itemCode" name="itemCode" value="${stockRecordLines.itemcode}" type="text" class="validate[required] text-input" /></td>
							...
							<tr height="40">
								...
								<td><input id="specification" name="specification" value="${stockRecordLines.specification}" type="text" class="validate[required] text-input" /></td>
								<th><span style="color: red;">*</span> 价格:</th>
								<td><input id="unitcost" name="unitcost" value="${stockRecordLines.unitcost}" type="text" class="validate[required,custom[number]] text-input" /></td>
							</tr>
							<tr height="40">
								<th><span style="color: red;">*</span> 数量:</th>
								<td><input id="quantity" name="quantity" value="${stockRecordLines.quantity}" type="text" class="validate[required,custom[number]] text-input" /></td>
								<th><span style="color: red;">*</span> 货位:</th>
								<td><input type="hidden" id="invShelfId" name="invShelfId" value="${stockRecordLines.invShelf.id }" /><input type="text" name="invShelfName" id="invShelfName" value="${stockRecordLines.invShelf.name }" size="20" class="validate[required] text-input" /><input class="btn" type="button" value="选择" onclick="chooseShelf();" /></td>
							</tr>
							<tr height="40">
								<th><span style="color: red;">*</span> 单位:</th>
								<td><input id="unit" name="unit" value="${stockRecordLines.unit}" type="text" /></td>
								<th><span style="color: red;">*</span> 仓库:</th>
								<td><input type="hidden" id="invWarehouseId" name="invWarehouseId" value="${stockRecordLines.invWarehouse.id }" /><input type="text" name="invWarehouseName" id="invWarehouseName" value="${stockRecordLines.invWarehouse.name }" size="20" class="validate[required] text-input" /><input class="btn" type="button" value="选择"
									onclick="chooseInvWareHouse();" /></td>
							</tr>
							<tr height="40">
								<th><span style="color: red;">*</span> SKU编码:</th>
								<td><input id="skuCode" name="skuCode" value="${stockRecordLines.skuCode}" type="text" class="validate[required] text-input" /></td>
								<th>供应商:</th>
								<td><input id="suppliercode" name="suppliercode" value="${stockRecordLines.suppliercode }" type="text" /><input class="btn" type="button" value="选择" onclick="chooseSupplier();" /></td>
							</tr>
							<tr height="40">
								<s:if test='inventoryParameters.isBatch=="1"'>
									<th><span style="color: red;">*</span> 批次号:</th>
									<td><input id="batchcode" name="batchcode" value="${stockRecordLines.batchcode}" type="text" class="validate[required] text-input" /></td>
								</s:if>
								<s:if test='inventoryParameters.isShelfLife=="1"'>
									<th><span style="color: red;">*</span> 到期日:</th>
									<td><input id="massunitEndTime" name="massunitEndTime" value="<s:date name="%{stockRecordLines.massunitEndTime}" format="yyyy-MM-dd"/>" type="text" class="validate[required] text-input" /><img onclick="showTime('massunitEndTime','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"></td>
								</s:if>
							</tr>

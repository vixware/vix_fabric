...
<script type="text/javascript">
	...
	function chooseItemForPrice(id, code, name, price, skuCode, amount) {
		$("#itemCode").val(code);
		$("#skuCode").val(skuCode);
		$("#itemName").val(name);
		$("#quantity").val(amount);
	...
	//载入分页列表数据
	pager("start", "${vix}/inventory/inboundWarehouseAction!goListStoreEnquiryRequestContent.action", 'saleOrderItem');
	pager("start", "${vix}/inventory/inboundWarehouseAction!goListSalesOrderContent.action", 'salesOrder');
	//排序 
	function orderBy(orderField) {
		...
		pager("start", "${vix}/inventory/inboundWarehouseAction!goListStoreEnquiryRequestContent.action?orderField=" + orderField + "&orderBy=" + orderBy + "&name=" + name, 'itemPriceManage');
	}

	function currentPager(tag) {
		loadName();
		pager(tag, "${vix}/inventory/inboundWarehouseAction!goListStoreEnquiryRequestContent.action?name=" + name + '&companyCode=' + $("#companyCode").val(), 'itemPriceManage');
	}
	...
	function salesOrderFieldChanged(salesOrderId) {
		pager("start", "${vix}/inventory/inboundWarehouseAction!goListStoreEnquiryRequestContent.action?salesOrderId=" + salesOrderId, 'saleOrderItem');
	}
	function reset() {
		$("#itemCategoryId").val("");
		$("#itemCategory").val("");
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div>
			<label>主题:<input type="text" class="int" id="nameS" onchange="searchForContent();"></label> <label> <input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();" /> <input type="button" value="<s:text name='cmn_reset'/>" class="btn" onclick="reset();" />
			</label>
		</div>
	</div>
	<!-- c_head -->
	<div class="box">
		<div id="left" style="width: 300px;">
			<div class="left_content">
				<div id="salesOrder" class="table" style="overflow-y: auto; height: 255px;"></div>
				<div class="pagelist drop">
					<div>
						<span><a class="start" onclick="currentPager('start','account');"></a></span> <span><a class="previous" onclick="currentPager('previous','account');"></a></span> <em>(<b class="salesOrderInfo"></b> <s:text name='cmn_to' /> <b class="salesOrderTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next','account');"></a></span> <span><a class="end" onclick="currentPager('end','account');"></a></span>
					</div>
				</div>
				<div id="saleOrderItem" class="table" style="overflow-y: auto; height: 255px;"></div>
				<div class="pagelist drop">
					<div>
						<span><a class="start" onclick="currentPager('start','account');"></a></span> <span><a class="previous" onclick="currentPager('previous','account');"></a></span> <em>(<b class="saleOrderItemInfo"></b> <s:text name='cmn_to' /> <b class="saleOrderItemTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next','account');"></a></span> <span><a class="end" onclick="currentPager('end','account');"></a></span>
					</div>
				</div>
			</div>

...
function loadTempleteId(){
	templeteId = $('#ht_templeteId').val();
	templeteId = Url.encode(templeteId);
	templeteId = Url.encode(templeteId);
}
function loadContractApproveCode(){
	contractApproveCode = $('#ht_contractApproveCode').val();
	contractApproveCode = Url.encode(contractApproveCode);
	contractApproveCode = Url.encode(contractApproveCode);
}
function loadTheme(){
	theme = $('#ht_theme').val();
...
function loadUploadPersonName(){
	uploadPersonName = $('#ht_uploadPersonName').val();
...
function resetForContent(i){
	if(i == 0){
		...
		$("#ht_contractCode").val("");
		$("#ht_templeteId").val("");
		$("#ht_contractApproveCode").val("");
		$("#ht_theme").val("");
		$("#ht_uploadPersonName").val("");
	}
}
...
function searchForContent(i){
	loadName();
	pager("start","${vix}/contract/contractAssociateTemplateAction!goSearchList.action?i="+i+"&theme="+name,'brand');
}

//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/contract/contractAssociateTemplateAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/contract/contractAssociateTemplateAction!!goSearchList.action?contractCode=" + $('#contractCode').val() 
						+ "&templeteId=" + $('#templeteId').val()
						+ "&contractApproveCode=" + $('#contractApproveCode').val()
						+ "&theme=" + $('#theme').val()
						+ "&uploadPersonName=" + $('#uploadPersonName').val(), 'brand');
			}
		},
...
function leastRecentlyUsed(startTime){
	pager("start","${vix}/contract/contractInquiryAction!goSingleList.action?startTime="+startTime,'brand');
}
...
$(function(){
 ...
 function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/contract/contractAssociateTemplateAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/contract/contractAssociateTemplateAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/contract/contractAssociateTemplateAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/contract/contractAssociateTemplateAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/contract/contractAssociateTemplateAction!goSingleList.action?name="+name,'brand');
}
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<ul>
			...
			<li class="fly"><a href="#"><img alt="" src="img/leastRecentlyUsed.png"> <s:text name="最近模板" /> </a>
				<ul>

...
function searchForContent(i){
	loadName();
	pager("start","${vix}/contract/contractProgressAction!goSearchList.action?i="+i+"&contractName="+name,'category');
}

//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/contract/contractProgressAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/contract/contractProgressAction!!goSearchList.action?masterContractCoding=" + $('#masterContractCoding').val() 
...
function saleOrderStatus(contractType){
	pager("start","${vix}/contract/contractProgressAction!goSingleList.action?contractType="+contractType,'category');
}
//最近使用
function leastRecentlyUsed(contractStartTime){
	pager("start","${vix}/contract/contractProgressAction!goSingleList.action?contractStartTime="+contractStartTime,'category');
}
...
function saveOrUpdate1(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractProgressAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
...
function saveOrUpdate2(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractProgressAction!goPurchaseFA.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function saveOrUpdate3(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractProgressAction!goSalesContract.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function saveOrUpdate4(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractProgressAction!goContractMarket.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function saveOrUpdate5(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractProgressAction!goPmContract.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function searchForRightContent(){
	loadName();
	pager("start","${vix}/contract/contractProgressAction!goSingleList.action?name="+name,'category');
}

loadName();
//载入分页列表数据
pager("start","${vix}/contract/contractProgressAction!goSingleList.action?name="+name+"&orderField=id&orderBy=desc",'category');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/contract/contractProgressAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/contract/contractProgressAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/contract/contractProgressAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/contract/contractProgressAction!goList.action?pageNo=${pageNo}');"><s:text name="ctm_agreement" /> </a></li>
				<li><a href="#" onclick="loadContent('${vix}/contract/contractProgressAction!goList.action?pageNo=${pageNo}');"><s:text name="合同进度" /> </a></li>
			</ul>
		</div>
	</h2>
</div>
	...
	<div class="box">
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/contract/contractProgressAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/contract/contractProgressAction!goSingleList.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/common_listx16.png" alt="" />合同进度</a></li>
				</ul>
			</div>

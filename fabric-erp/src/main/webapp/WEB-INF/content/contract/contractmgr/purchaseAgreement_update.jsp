 ...
 */
 ...
 function createSupplier(){
 	$.ajax({
 		  url:'${vix}/contract/contractDraftingAction!goAddQuicklySupplier.action?',
 		  cache: false,
 		  success: function(html){
 		 	  asyncbox.open({
 				 	modal:true,
 					...
 					callback : function(action){
 						if(action == 'ok'){
 							if($('#supplierForm').validationEngine('validate')){
 								$.post('${vix}/contract/contractDraftingAction!saveOrUpdateSupplier.action', {
...
function saveOrUpdateOrder(){
	...
	if($('#orderForm').validationEngine('validate')){
		$.post('${vix}/contract/contractDraftingAction!saveOrUpdate.action',
			...
			/* 'contract.viewDepartment' : $("#viewDepartment").val(),
			'contract.contractNature' : $("#contractNature").val(),
			'contract.viewIndustry' : $("#viewIndustry").val(),
			
			'contract.nameEachOther' : $("#nameEachOther").val(),
			'contract.otherContacts' : $("#otherContacts").val(),
			'contract.otherPhone' : $("#otherPhone").val(), */
		...
		/* 	'contract.contractSubject' : $("#contractSubject").val(), */
			'contract.type' :$("#type").val(),
			...
			function(result){
				showMessage(result);
				setTimeout("",1000);
				loadContent('${vix}/contract/contractDraftingAction!goList.action?menuId=menuContract');
			}
		);
	}
}
...
/* $ (function (){
	$ ("#moneyTypeId").val ('${contract.moneyType.id}'); 
}); */
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="新增框架协议" /> </a></li>
			</ul>
		</div>
	</h2>
</div>
		...
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateOrder(0);"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="#"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <a href="#"><img width="22" height="22" title="保存并提交审批"
							src="${vix}/common/img/dt_submit.png" /></a> <%--   --%> <a href="#" onclick="loadContent('${vix}/contract/contractDraftingAction!goList.action?pageNo=${pageNo}');"><img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a>
					</span> <strong><b><s:text name="新增采购框架协议" /> </b><i></i> </strong>
				</dt>
				<dd class="clearfix">
					<div class="order_table">
								...
								<dd style="display: block;">
									<table style="border: none;">
										...
										<tr>
											...
											<td><input id="signDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="signDate" value="<fmt:formatDate value='${contract.signDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('signDate','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif" width="16" height="22"
												align="absmiddle"></td>
										</tr>
										<tr>
											<th align="right">合同开始日期：</th>
											<td><input id="contractStartTime" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="contractStartTime" value="<fmt:formatDate value='${contract.contractStartTime}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('contractStartTime','yyyy-MM-dd')"
												src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"></td>
											<th align="right">合同终止日期：</th>
											<td><input id="contractEndTime" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="contractEndTime" value="<fmt:formatDate value='${contract.contractEndTime}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('contractEndTime','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif"
										...
										<tr>
											...
											<td><input id="approvalDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="approvalDate" value="<fmt:formatDate value='${contract.approvalDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('approvalDate','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif" width="16"
						...
						<div class="voucher newvoucher">
							<dl>
								<dt>
									<span style=""><input class="btn" type="button" value="新增丙方" onclick="NewContract();" /> </span> <strong>合同方</strong>
									<!-- <div  align="right">
										<ul> 
											<li>
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id="firstParty" name="firstParty" value="${contract.firstParty}" type="text" size="8" class="validate[required] text-input" /> <span style="color: red;">*</span> <input type="hidden" id="firstParty" name="firstParty" value="${contract.id}" /> <input class="btn" type="button" value="选择" onclick="chooseRadioSupplier();" />
												<input class="btn" type="button" value="新增" onclick="createSupplier();" /></td>
											<td align="right">乙方：</td>
											<td><input id="secondParty" name="secondParty" value="${contract.secondParty}" type="text" size="30" /></td>
										</tr>
										<tr>
										...
										<tr>
											...
											<td><input id="firstPartyPhone" name="firstPartyPhone" value="${contract.firstPartyPhone}" type="text" size="30" class="validate[required,custom[integer]] text-input" /></td>
											<th align="right">乙方电话：</th>
											<td><input id="bPhone" name="bPhone" value="${contract.bphone}" type="text" size="30" class="validate[required,custom[integer]] text-input" /></td>
										</tr>
										<tr>
											<th align="right">甲方邮件：</th>
											<td><input id="firstPartyEmail" name="firstPartyEmail" value="${contract.firstPartyEmail}" type="text" size="30" class="validate[required,custom[email]] text-input" /></td>
											<th align="right">乙方邮件：</th>
											<td><input id="email" name="email" value="${contract.email}" type="text" size="30" class="validate[required,custom[email]] text-input" /></td>
										</tr>
										<%-- <tr>
											<th align="right">丙方单位地址：</th>
											<td><input id="partyCAddress" name="partyCAddress"
												value="${contract.partyCAddress}" type="text" size="30" />
											</td>
											<th align="right">丙方单位名称：</th>
											<td><input id="partyCUnitName" name="partyCUnitName"
												value="${contract.partyCUnitName}" type="text" size="30" />
											</td>
										</tr>
										<tr>
											<th align="right">丙方联系人：</th>
											<td><input id="partyCContact" name="partyCContact"
												value="${contract.partyCContact}" type="text" size="30" />
											</td>
											<th align="right">丙方电话：</th>
											<td><input id="partyCPhone" name="partyCPhone"
												value="${contract.partyCPhone}" type="text" size="30"
												class="validate[required,custom[integer]] text-input" /></td>
										</tr>
										<tr>
											<th align="right">丙方邮件：</th>											
											<td><input id="partyCEmail" name="partyCEmail"
												 value="${contract.partyCEmail}" type="text" size="30" 
												 class="validate[required,custom[email]] text-input"/></td>
										</tr> --%>
									</table>
								</dd>
							</dl>
								...
								<dd style="display: block;">
									<table style="border: none;">
										...
										<tr>
											...
											<td><input id="retentionEndDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="retentionEndDate" value="<fmt:formatDate value='${contract.retentionEndDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('retentionEndDate','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif"
												width="16" height="22" align="absmiddle"></td>
								...
								<dd style="display: block;">
									<table style="border: none;">
										...
										<tr>
											...
											<td colspan="3"><textarea id="remark" class="example" rows="1" style="width: 950px">${contract.remark}</textarea></td>
										</tr>
									</table>
								</dd>
							</dl>
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					<div class="right_menu">
						<ul>
							<li class="current"><a onclick="javascript:tab(7,1,'a',event)"><img src="${vix}/common/img/mail.png" alt="" />合同项</a></li>
							<li><a onclick="javascript:$('#a2').attr('style','');tab(7,2,'a',event);"><img src="${vix}/common/img/mail.png" alt="" />标的</a></li>
							<li><a onclick="javascript:$('#a3').attr('style','');tab(7,3,'a',event);"><img src="${vix}/common/img/mail.png" alt="" />采购定价条件</a></li>
							<li><a onclick="javascript:$('#a4').attr('style','');tab(7,4,'a',event);"><img src="${vix}/common/img/mail.png" alt="" />附件</a></li>
							<li><a onclick="javascript:$('#a5').attr('style','');tab(7,5,'a',event);"><img src="${vix}/common/img/mail.png" alt="" />预警</a></li>
							<li><a onclick="javascript:$('#a6').attr('style','');tab(7,6,'a',event);"><img src="${vix}/common/img/mail.png" alt="" />审批</a></li>
							<li><a onclick="javascript:$('#a7').attr('style','');tab(7,7,'a',event);"><img src="${vix}/common/img/mail.png" alt="" />价格条件</a></li>
						</ul>
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;">
						<div style="padding: 8px;">
							...
							function removeDlLineItem(){
								...
								$.ajax({url:'${vix}/contract/contractDraftingAction!deleteContractLineById.action?id='+row.id,cache: false});
					...
					<div id="a2" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						...
						$('#dlAddress2').datagrid({
							...
							frozenColumns:[[
											...
											{field:'subjectCode',title:'标的编码',width:80,align:'center'},
											{field:'subjectName',title:'标的名称',width:80,align:'center'},
							...
							columns:[[
								...
								{field:'startTime',title:'开始时间',width:120,align:'center',editor:'datebox',formatter:formatDatebox},
								{field:'endTime',title:'结束时间',width:120,align:'center',editor:'datebox',formatter:formatDatebox},
							]],
							toolbar:[{
								id:'da2Btnadd',
								text:'新增',
								...
								handler:function(){
									...
									for(var i =0;i<rows.length;i++){
										...
										$.ajax({url:'${vix}/contract/contractDraftingAction!deleteContractSubjectById.action?id='+rows[i].id,cache: false});
					...
					<div id="a3" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<script type="text/javascript">
						function saveDeliveryAddress1(url){
							$.ajax({
								  url:url,
								  cache: false,
								  success: function(html){
									  asyncbox.open({
											...
											callback : function(action,returnValue){
												if(action == 'ok'){
													if($('#daForm1').validationEngine('validate')){
														$.post('${vix}/contract/contractDraftingAction!saveOrUpdateContractSubjectDetails1.action',
																{ 'id':$("#id").val(),
																  'contractPricingConditions.id':$("#daId").val(),
																  'contractPricingConditions.item.id':$("#itemId").val(),
																  'contractPricingConditions.itemName':$("#itemName").val(),
																  'contractPricingConditions.description':$("#description").val(), 
																  'contractPricingConditions.supplierId':$("#suppliername").val(), 
																  'contractPricingConditions.from':$("#minCount").val(),
																  'contractPricingConditions.to':$("#maxCount").val(),
																  'contractPricingConditions.price':$("#price").val(),
																  'contractPricingConditions.discount':$("#discount").val(),
																  'contractPricingConditions.startTime':$("#startDate").val(),
																  'contractPricingConditions.endTime':$("#endDate").val()
																},
																function(result){
																	showMessage(result);
																	setTimeout("",1000);
																	$('#dlAddress3').datagrid("reload");
																}
														);
													}else {
														return false;
						...
						$('#dlAddress3').datagrid({
							url: '${vix}/contract/contractDraftingAction!getContractSubjectJson1.action?id=${contract.id}',
							title: '采购订单定价条件',
							width: 'auto',
							height: '450',
							iconCls:'icon-save',
							...
							frozenColumns:[[
							    			{field:'ck',checkbox:true},
							    			{field:'id',title:'编号',width:80,align:'center'},
											{field:'supplierId',title:'供应商',width:80,align:'center'},
											{field:'item.name',title:'${vv:varView("vix_mdm_item")}名称',width:80,align:'center'},
											{field:'item.code',title:'${vv:varView("vix_mdm_item")}编码',width:80,align:'center'},
											{field:'purchasingOrganization',title:'采购组织',width:100,align:'center'},
							    		]],
							columns:[[
								{field:'description',title:'描述',width:98,align:'center'},
								{field:'from',title:'采购数量起',width:98,align:'center'},
								{field:'to',title:'采购数量止',width:98,align:'center'},
								{field:'inventoriesSpecification',title:'定价类型',width:98,align:'center'},
								{field:'nnTOCurrencyPrice',title:'定价等级',width:98,align:'center'},
								{field:'ttIOriginalCurrencyPrice',title:'计算类型',width:98,align:'center'},
								{field:'nnTaxAmountOriginalCurrency',title:'定价类别',width:100,align:'center'},
								{field:'price',title:'价格（折扣）',width:100,align:'center'},
								{field:'startTime',title:'开始时间',width:120,align:'center',editor:'datebox',formatter:formatDatebox},
								{field:'endTime',title:'结束时间',width:120,align:'center',editor:'datebox',formatter:formatDatebox},
							]],
							toolbar:[{
								...
								handler:function(){
									$('#btnsave').linkbutton('enable');
									saveDeliveryAddress1('${vix}/contract/contractDraftingAction!goAddProcurementPricing.action?id=0');
								}
							},'-',{
								id:'btnedit',
								text:'修改',
								iconCls: 'icon-edit',
								handler: function(){
									var row = $('#dlAddress3').datagrid("getSelected");	//获取你选择的所有行
									if(row){
										saveDeliveryAddress1('${vix}/contract/contractDraftingAction!goAddProcurementPricing.action?id='+row.id);
									}
								}
							},'-',{
								text:'删除',
								iconCls:'icon-remove',
								handler:function(){
									var rows = $('#dlAddress3').datagrid("getSelections");	//获取你选择的所有行	
									//循环所选的行
									for(var i =0;i<rows.length;i++){
										var index = $('#dlAddress3').datagrid('getRowIndex',rows[i]);//获取某行的行号										
										$.ajax({url:'${vix}/contract/contractDraftingAction!deleteContractSubjectById.action?id='+rows[i].id,cache: false});
										$('#dlAddress3').datagrid('deleteRow',index);	//通过行号移除该行
									}
								}
							}]
						});
					...
					<div id="a5" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<div style="padding: 8px;">
							...
							<div id="dlDeliveryPlanTb" style="height: auto">
								<a href="javascript:void(0)" class="easyui-linkbutton l-btn l-btn-plain" data-options="iconCls:'icon-add',plain:true" onclick="appendDlDeliveryPlan()"><span class="l-btn-left"><span class="l-btn-text icon-add l-btn-icon-left"><s:text name="cmn_add" /></span></span></a> <a href="javascript:void(0)" class="easyui-linkbutton l-btn l-btn-plain"
									data-options="iconCls:'icon-remove',plain:true" onclick="removeDlDeliveryPlan()"><span class="l-btn-left"><span class="l-btn-text icon-remove l-btn-icon-left"><s:text name="cmn_delete" /></span></span></a> <a href="javascript:void(0)" class="easyui-linkbutton l-btn l-btn-plain" data-options="iconCls:'icon-save',plain:true"
									onclick="saveDlDeliveryPlan()"><span class="l-btn-left"><span class="l-btn-text icon-save l-btn-icon-left"><s:text name="cmn_save" /></span></span></a>
							</div>
							<script type="text/javascript">
							...
							function removeDlDeliveryPlan(){
								...
								$.ajax({url:'${vix}/contract/contractDraftingAction!deleteContractWarningById.action?id='+row.id,cache: false});
					...
					<div id="a6" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<div style="padding: 8px;">
							...
							function removeDlInvoice(){
								...
								$.ajax({url:'${vix}/contract/contractDraftingAction!deleteApplicationFormById.action?id='+row.id,cache: false});
					...
					<div id="a7" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<div style="padding: 8px;">
							<table id="dlInvoice1" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlInvoiceTb1',url: '${vix}/contract/contractDraftingAction!getContract4.action?id=${contract.id}',onClickRow: onDlClickRow">
								<thead>
									<tr>
										...
										<th data-options="field:'purchasepriceCondition',width:400,align:'center',editor:'text'">采购价格条件</th>
									</tr>
								</thead>
							</table>
							...
							function endDlEditing(){
								if (editIndexDlInvoice1 == undefined){return true;}
							...
							function appendDlInvoice1(){
								if (endDlEditing()){
									$('#dlInvoice1').datagrid('appendRow',{status:'P'});
							...
							function removeDlInvoice1(){
								if (editIndexDlInvoice1 == undefined){return;}
								var row = $('#dlInvoice1').datagrid("getSelected");
								$.ajax({url:'${vix}/contract/contractDraftingAction!deleteApplicationFormById.action?id='+row.id,cache: false});
								$('#dlInvoice1').datagrid('cancelEdit', editIndexDlInvoice1)
										.datagrid('deleteRow', editIndexDlInvoice1);
								editIndexDlInvoice1 = undefined;
							}

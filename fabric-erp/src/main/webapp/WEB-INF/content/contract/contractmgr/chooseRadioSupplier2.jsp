...
pager("start","${vix}/contract/contractDraftingAction!goSubRadioSingleList2.action?temp=1","radio");
//排序 
...
function searchForContent(i){
	loadCode();
	pager("start","${vix}/contract/contractDraftingAction!goSubRadioSingleList.action?searchContent="+code,'radio');
}
/*重置搜索*/
...
function srmStatus(status){
	pager("start","${vix}/contract/contractDraftingAction!goSubRadioSingleList.action?status="+status,'radio');
}
//最近使用
function srmRecent(rencentDate){
	pager("start","${vix}/contract/contractDraftingAction!goSubRadioSingleList.action?updateTime="+rencentDate,'radio');
}
</script>
<div class="content" style="background: #DCE7F1">
	<div id="c_head" class="drop">
		...
		<div>
			<label><s:text name='信息编码' />:<input id="srm_code" name="" type="text" class="int" /></label> <label> <input id="simpleSearch" name="" type="button" class="btn" value="<s:text name='cmn_search'/>" onclick="searchForContent();" /> <input id="simpleReset" name="" type="button" class="btn" value="<s:text name='cmn_reset'/>"

	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="新增项目合同" /> </a></li>
			</ul>
		</div>
	</h2>
</div>
		...
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					...
					</span> <strong><b><s:text name="新增项目合同" /> </b><i></i> </strong>
				</dt>
				<dd class="clearfix">
								...
								<dd style="display: block;">
									<table style="border: none;">
										...
										<tr>
											...
											<%-- <td>
											<s:select id="contractNatureTypeId" headerKey="-1" headerValue="--请选择--" 
											list="%{contractNatureList}" listValue="name" listKey="id" 
											value="%{contract.contractNatureType.id}" multiple="false" theme="simple">
											</s:select>
										</td>	 --%>
										</tr>
										<tr>
										...
										<tr>
											...
											<td><input id="approvalDate" class="sweet-tooltip" data-text-tooltip="时间格式 yyyy-MM-dd" name="contractEndTime" value="<fmt:formatDate value='${contract.approvalDate}' type='both' pattern='yyyy-MM-dd' />" type="text" size="30" /> <img onclick="showTime('approvalDate','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif" width="16"
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id="secondParty" name="secondParty" value="${contract.secondParty}" type="text" size="8" class="validate[required] text-input" /> <span style="color: red;">*</span> <input type="hidden" id="secondParty" name="secondParty" value="${contract.id}" /> <input class="btn" type="button" value="选择信息"
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					<div class="right_menu">
						<ul>
							...
							<li><a onclick="javascript:$('#a3').attr('style','');tab(7,3,'a',event);"><img src="${vix}/common/img/mail.png" alt="" />项目合同</a></li>
					...
					<div id="a3" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
							...
							$.ajax({
								  ...
								  success: function(html){
									  asyncbox.open({
											...
											callback : function(action,returnValue){
												if(action == 'ok'){
													if($('#pmContractForm').validationEngine('validate')){
														$.post('${vix}/contract/contractDraftingAction!saveOrUpdatePmContractDetails.action',
																{'id':$("#id").val(),
																  'pmContract.id':$("#daId").val(),
																  'pmContract.contractingParties':$("#contractingParties").val(),
																  'pmContract.employer':$("#employer").val(),
																  'pmContract.createTime':$("#createTime").val(),
																  'pmContract.contractor':$("#contractor").val(),
																  'pmContract.supervisor':$("#supervisor").val(),
																  'pmContract.engineeringEquipment':$("#engineeringEquipment").val(),
																  'pmContract.unitProject':$("#unitProject").val(),
																  'pmContract.temporaryCovering':$("#temporaryCovering").val(),
																  'pmContract.jiaoGongTime':$("#jiaoGongTime").val(),
																  'pmContract.defectsLiabilityPeriod':$("#defectsLiabilityPeriod").val(),
																  'pmContract.warrantyPeriod':$("#warrantyPeriod").val()
																},
																function(result){
																	showMessage(result);
																	setTimeout("",1000);
						...
						$('#dlAddress3').datagrid({
							url: '${vix}/contract/contractDraftingAction!getContractPmJson.action?id=${contract.id}',
							title: '项目合同',
							width: 'auto',
							height: '450',
							iconCls:'icon-save',
							...
							frozenColumns:[[
											...
											{field:'contractingParties',title:'合同当事人',width:80,align:'center'},
											{field:'startTime',title:'开始时间',width:120,align:'center',editor:'datebox',formatter:formatDatebox},
											{field:'employer',title:'发包人',width:80,align:'center'},
											{field:'contractor',title:'承包人',width:100,align:'center'},
							    		]],
							columns:[[
								{field:'supervisor',title:'监理人',width:98,align:'center'},
								{field:'engineeringEquipment',title:'工程和设备',width:98,align:'center'},
								{field:'unitProject',title:'单位工程',width:98,align:'center'},
								{field:'temporaryCovering',title:'临时占地',width:98,align:'center'},
								{field:'jiaoGongTime',title:'开始时间',width:120,align:'center',editor:'datebox',formatter:formatDatebox},
								{field:'defectsLiabilityPeriod',title:'缺陷责任期',width:98,align:'center'},
								{field:'warrantyPeriod',title:'保修期',width:98,align:'center'},
							]],
							toolbar:[{
								id:'da2Btnadd',
								text:'新增',
								...
								handler:function(){
									$('#btnsave').linkbutton('enable');
									saveDeliveryAddress1('${vix}/contract/contractDraftingAction!goAddPmContract.action?id=0');
								}
							},'-',{
								id:'btnedit',
								text:'修改',
								...
								handler: function(){
									...
									if(row){
										saveDeliveryAddress1('${vix}/contract/contractDraftingAction!goAddPmContract.action?id='+row.id);
									}
								}
							},'-',{
								text:'删除',
								...
								handler:function(){
									...
									for(var i =0;i<rows.length;i++){
										...
										$.ajax({url:'${vix}/contract/contractDraftingAction!deletePmContractById.action?id='+rows[i].id,cache: false});
						...
						<div style="padding: 8px;">
							...
							<%-- <div id="dlDeliveryPlanTb" style="height: auto">
								<a href="javascript:void(0)" class="easyui-linkbutton"
									...
									onclick="appendDlDeliveryPlan()"><s:text name="cmn_add" />
								</a> <a href="javascript:void(0)" class="easyui-linkbutton"
									data-options="iconCls:'icon-remove',plain:true"
									onclick="removeDlDeliveryPlan()"><s:text name="cmn_delete" />
								</a> <a href="javascript:void(0)" class="easyui-linkbutton"
									data-options="iconCls:'icon-save',plain:true"
									onclick="saveDlDeliveryPlan()"><s:text name="cmn_save" />
								</a>
							</div> --%>
							<div id="dlDeliveryPlanTb" style="height: auto">

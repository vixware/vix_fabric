...
function searchForContent(i){
	loadName();
	pager("start","${vix}/contract/contractReceivablesAction!goSearchList.action?i="+i+"&contractName="+name,'brand');
}

//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/contract/contractReceivablesAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/contract/contractReceivablesAction!!goSearchList.action?contractCode=" + $('#contractCode').val() 
...
function leastRecentlyUsed(paymentTime){
	pager("start","${vix}/contract/contractReceivablesAction!goSingleList.action?paymentTime="+paymentTime,'brand');
}
...
function saveOrUpdate(id){
	$.ajax({
		  url:"${vix}/contract/contractReceivablesAction!goSaveOrUpdate.action?id="+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#brandForm').validationEngine('validate')){
								$.post('${vix}/contract/contractReceivablesAction!saveOrUpdate.action',
									...
									function(result){
										asyncbox.open({
											...
											html:result,
											callback : function(action){
												pager("current","${vix}/contract/contractReceivablesAction!goSingleList.action?name="+name,'brand');
											},
											btnsbar : $.btn.OKCANCEL
										});
									}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/contract/contractReceivablesAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='合同付款'/>",function(action){
				...
				pager("start","${vix}/contract/contractReceivablesAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/contract/contractReceivablesAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/contract/contractReceivablesAction!goSingleList.action?name="+name,'brand');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/contract/contractReceivablesAction!goList.action?pageNo=${pageNo}');"><s:text name="合同收款" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<ul>
			...
			<li class="fly"><a href="#"><img alt="" src="img/leastRecentlyUsed.png"> <s:text name="最近收款合同" /> </a>
				<ul>

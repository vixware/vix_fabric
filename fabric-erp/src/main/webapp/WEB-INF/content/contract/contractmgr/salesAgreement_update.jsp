	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="新增销售框架协议" /> </a></li>
			</ul>
		</div>
	</h2>
</div>
		...
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					...
					</span> <strong><b><s:text name="新增销售框架协议" /> </b><i></i> </strong>
				</dt>
				<dd class="clearfix">
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					<div class="right_menu">
						<ul>
							...
							<li><a onclick="javascript:$('#a3').attr('style','');tab(7,3,'a',event);"><img src="${vix}/common/img/mail.png" alt="" />销售定价条件</a></li>
					...
					<div id="a3" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
								  ...
								  success: function(html){
									  asyncbox.open({
											...
											callback : function(action,returnValue){
												if(action == 'ok'){
													if($('#daForm').validationEngine('validate')){
														$.post('${vix}/contract/contractDraftingAction!saveOrUpdateMarket.action',
																{'id':$("#id").val(),
																  'contractMarket.id':$("#daId").val(),
																  'contractMarket.supplier':$("#supplier").val(),
																  'contractMarket.materialCode':$("#materialCode").val(),
																  'contractMarket.purchasingOrganization':$("#purchasingOrganization").val(),
																  'contractMarket.descriptions':$("#descriptions").val(),
																  'contractMarket.purchasequantityRise':$("#purchasequantityRise").val(),
																  'contractMarket.purchasequantityTo':$("#purchasequantityTo").val(),
																  'contractMarket.pricingType':$("#pricingType").val(),
																  'contractMarket.pricingGrade':$("#pricingGrade").val(),
																  'contractMarket.calculationType':$("#calculationType").val(),
																  'contractMarket.pricingCategory':$("#pricingCategory").val(),
																  'contractMarket.price':$("#price").val(),
																  'contractMarket.effectiveTime':$("#effectiveTime").val(),
																  'contractMarket.startTime':$("#startTime").val(),
																  'contractMarket.endTime':$("#endTime").val()
																},
																function(result){
																	showMessage(result);
																	setTimeout("",1000);
						...
						$('#dlAddress3').datagrid({
							url: '${vix}/contract/contractDraftingAction!getContractMarketJson.action?id=${contract.id}',
							title: '销售定价条件',
							width: 'auto',
							height: '450',
							iconCls:'icon-save',
							...
							frozenColumns:[[
											...
											{field:'materialCode',title:'${vv:varView("vix_mdm_item")}编码',width:80,align:'center'},
											{field:'purchasingOrganization',title:'采购组织',width:100,align:'center'},
							    		]],
							columns:[[
								{field:'descriptions',title:'描述',width:98,align:'center'},
								{field:'purchasequantityRise',title:'采购数量起',width:98,align:'center'},
								{field:'purchasequantityTo',title:'采购数量止',width:98,align:'center'},
								{field:'pricingType',title:'定价类型',width:98,align:'center'},
								{field:'pricingGrade',title:'定价等级',width:98,align:'center'},
								{field:'calculationType',title:'计算类型',width:98,align:'center'},
								{field:'pricingCategory',title:'定价类别',width:100,align:'center'},
								{field:'price',title:'价格（折扣）',width:100,align:'center'},
								{field:'effectiveTime',title:'有效时间',width:120,align:'center',editor:'datebox',formatter:formatDatebox},
								{field:'startTime',title:'开始时间',width:120,align:'center',editor:'datebox',formatter:formatDatebox},
								{field:'endTime',title:'结束时间',width:120,align:'center',editor:'datebox',formatter:formatDatebox},
							]],
							toolbar:[{
								...
								handler:function(){
									$('#btnsave').linkbutton('enable');
									saveDeliveryAddress1('${vix}/contract/contractDraftingAction!goAddSalesPricing.action?id=0');
								}
							},'-',{
								id:'btnedit',
								text:'修改',
								...
								handler: function(){
									...
									if(row){
										saveDeliveryAddress1('${vix}/contract/contractDraftingAction!goAddSalesPricing.action?id='+row.id);
									}
								}
							},'-',{
								text:'删除',
								...
								handler:function(){
									...
									for(var i =0;i<rows.length;i++){
										...
										$.ajax({url:'${vix}/contract/contractDraftingAction!deleteContractMarketById.action?id='+rows[i].id,cache: false});

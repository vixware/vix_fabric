	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/contract/contractInquiryAction!goList.action?pageNo=${pageNo}');"><s:text name="新增采购框架协议" /> </a></li>
			</ul>
		</div>
	</h2>
</div>
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					<div class="right_menu">
						<ul>
							...
							<li><a onclick="javascript:$('#a6').attr('style','');tab(7,6,'a',event)"><img src="${vix}/common/img/mail.png" alt="" />采购定价条件</a></li>
					...
					<div id="a6" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<script type="text/javascript">
						function saveContractPricingConditions(url){
							$.ajax({
								  url:url,
								  cache: false,
								  success: function(html){
									  asyncbox.open({
											...
											callback : function(action,returnValue){
												if(action == 'ok'){
													if($('#contractPricingConditionsForm').validationEngine('validate')){
														$.post('${vix}/contract/contractInquiryAction!saveOrUpdateContractPricingConditions.action',
																{'id':$("#id").val(),
																  'pricingConditions.id':$("#daId").val(),
																  'pricingConditions.item.id':$("#itemId").val(),
																  'pricingConditions.itemName':$("#itemName").val(),
																  'pricingConditions.description':$("#description").val(), 
																  'pricingConditions.supplierId':$("#suppliername").val(), 
																  'pricingConditions.from':$("#minCount").val(),
																  'pricingConditions.to':$("#maxCount").val(),
																  'pricingConditions.price':$("#price").val(),
																  'pricingConditions.discount':$("#discount").val(),
																  'pricingConditions.startTime':$("#startDate").val(),
																  'pricingConditions.endTime':$("#endDate").val()
																},
																function(result){
																	showMessage(result);
																	setTimeout("",1000);
																	$('#contractPricingConditions').datagrid("reload");
																}
														);
													}
												}
						...
						$('#contractPricingConditions').datagrid({
							url: '${vix}/contract/contractInquiryAction!getContractPricingConditionsJson.action?id=${contractInquiry.id}',
							title: '采购定价条件明细',
							width: 'auto',
							height: '450',
							rownumbers : true,
							...
							toolbar:[{
								...
								handler:function(){
									$('#btnsave').linkbutton('enable');
									saveContractPricingConditions('${vix}/contract/contractInquiryAction!goAddContractPricingConditions.action?id=0');
								}
							},'-',{
								id:'btnedit',
								text:'修改',
								iconCls: 'icon-edit',
								handler: function(){
									var row = $('#contractPricingConditions').datagrid("getSelected");	//获取你选择的所有行
									if(row){
										saveContractPricingConditions('${vix}/contract/contractInquiryAction!goAddContractPricingConditions.action?id='+row.id)
									}
								}
							},'-',{
								text:'删除',
								iconCls:'icon-remove',
								handler:function(){
									var rows = $('#contractPricingConditions').datagrid("getSelections");	//获取你选择的所有行	
									//循环所选的行
									for(var i =0;i<rows.length;i++){
										var index = $('#contractPricingConditions').datagrid('getRowIndex',rows[i]);//获取某行的行号
										$('#contractPricingConditions').datagrid('deleteRow',index);	//通过行号移除该行
										$.ajax({url:'${vix}/contract/contractInquiryAction!deleteContractPricingConditionsById.action?id='+rows[i].id,cache: false});
									}
								}
							}]
						});
						...
						<div style="padding: 8px;">
							<table id="contractPricingConditions"></table>
						</div>
					</div>

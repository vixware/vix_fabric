...
function searchForContent(i){
	loadName();
	pager("start","${vix}/contract/contractInquiryAction!goSearchList.action?i="+i+"&contractName="+name,'category');
}

//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/contract/contractInquiryAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/contract/contractInquiryAction!!goSearchList.action?masterContractCoding=" + $('#masterContractCoding').val() 
...
function saleOrderStatus(contractType){
	pager("start","${vix}/contract/contractInquiryAction!goSingleList.action?contractType="+contractType,'category');
}
//最近使用
function leastRecentlyUsed(contractStartTime){
	pager("start","${vix}/contract/contractInquiryAction!goSingleList.action?contractStartTime="+contractStartTime,'category');
}
...
function saveOrUpdate1(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractInquiryAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
...
function saveOrUpdate2(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractInquiryAction!goPurchaseFA.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function saveOrUpdate3(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractInquiryAction!goSalesContract.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function saveOrUpdate4(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractInquiryAction!goContractMarket.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function saveOrUpdate5(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractInquiryAction!goPmContract.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/contract/contractInquiryAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='删除合同查询'/>",function(action){
				var treeNode = zTree.getNodeByTId($("#selectIdTreeId").val());
				if(null != treeNode){
					treeNode.isParent = true;
				}
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/contract/contractInquiryAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
loadName();
//载入分页列表数据
pager("start","${vix}/contract/contractInquiryAction!goSingleList.action?name="+name+"&orderField=id&orderBy=desc",'category');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/contract/contractInquiryAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/contract/contractInquiryAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/contract/contractInquiryAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
	...
	<div class="drop">
		<ul>
			<li><a href="#"><span>新增</span> </a>
				<ul>
					<li><a href="#" onclick="loadContent('${vix}/contract/contractInquiryAction!goSaveOrUpdate.action?contractType=1&pageNo=${pageNo}');">采购合同</a></li>
					<li><a href="#" onclick="loadContent('${vix}/contract/contractInquiryAction!goPurchaseFA.action?contractType=2&pageNo=${pageNo}');">采购框架协议</a></li>
					<li><a href="#" onclick="loadContent('${vix}/contract/contractInquiryAction!goSalesContract.action?contractType=3&pageNo=${pageNo}');">销售合同</a></li>
					<li><a href="#" onclick="loadContent('${vix}/contract/contractInquiryAction!goContractMarket.action?contractType=4&pageNo=${pageNo}');">销售框架协议</a></li>
					<li><a href="#" onclick="loadContent('${vix}/contract/contractInquiryAction!goPmContract.action?contractType=5&pageNo=${pageNo}');">项目合同</a></li>
				</ul></li>
		</ul>
	</div>
</div>
	...
	<div class="box">
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/contract/contractInquiryAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/contract/contractInquiryAction!goSingleList.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/common_listx16.png" alt="" />创建合同</a></li>
				</ul>
			</div>

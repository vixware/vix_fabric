	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/contract/contractInquiryAction!goList.action?pageNo=${pageNo}');"><s:text name="新增销售框架协议" /> </a></li>
			</ul>
		</div>
	</h2>
</div>
			...
			<dl>
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id="firstParty" name="firstParty" value="${contractInquiry.firstParty}" type="text" size="30" /></td>

											<td align="right">乙方：</td>
											<td><input id="secondParty" name="secondParty" value="${contractInquiry.secondParty}" type="text" size="8" class="validate[required] text-input" /> <span style="color: red;">*</span> <input type="hidden" id="secondParty" name="secondParty" value="${contractInquiry.id}" /> <input class="btn" type="button" value="选择信息"
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					<div class="right_menu">
						<ul>
							<li class="current"><a onclick="javascript:tab(7,1,'a',event);"><img src="${vix}/common/img/mail.png" alt="" />合同子项</a></li>
							<li><a onclick="javascript:$('#a2').attr('style','');tab(7,2,'a',event)"><img src="${vix}/common/img/mail.png" alt="" />合同标的</a></li>
							<li><a onclick="javascript:$('#a3').attr('style','');tab(7,3,'a',event)"><img src="${vix}/common/img/mail.png" alt="" />合同预警</a></li>
							<li><a onclick="javascript:$('#a4').attr('style','');tab(7,4,'a',event)"><img src="${vix}/common/img/mail.png" alt="" />合同审批</a></li>
							<li><a onclick="javascript:$('#a5').attr('style','');tab(7,5,'a',event)"><img src="${vix}/common/img/mail.png" alt="" />价格条件</a></li>
							<li><a onclick="javascript:$('#a6').attr('style','');tab(7,6,'a',event)"><img src="${vix}/common/img/mail.png" alt="" />销售定价条件</a></li>
					...
					<div id="a6" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
						<script type="text/javascript">
						function saveContractMarket(url){
							$.ajax({
								  url:url,
								  cache: false,
								  success: function(html){
									  asyncbox.open({
											...
											callback : function(action,returnValue){
												if(action == 'ok'){
													if($('#contractMarketForm').validationEngine('validate')){
														$.post('${vix}/contract/contractInquiryAction!saveOrUpdateContractMarket.action',
																{'id':$("#id").val(),
																  'market.id':$("#daId").val(),
																  'market.supplier':$("#supplier").val(),
																  'market.materialCode':$("#materialCode").val(),
																  'market.purchasingOrganization':$("#purchasingOrganization").val(),
																  'market.descriptions':$("#descriptions").val(),
																  'market.purchasequantityRise':$("#purchasequantityRise").val(),
																  'market.purchasequantityTo':$("#purchasequantityTo").val(),
																  'market.pricingType':$("#pricingType").val(),
																  'market.pricingGrade':$("#pricingGrade").val(),
																  'market.calculationType':$("#calculationType").val(),
																  'market.pricingCategory':$("#pricingCategory").val(),
																  'market.price':$("#price").val(),
																  'market.effectiveTime':$("#effectiveTime").val(),
																  'market.startTime':$("#startTime").val(),
																  'market.endTime':$("#endTime").val()
																},
																function(result){
																	showMessage(result);
																	setTimeout("",1000);
																	$('#contractMarket').datagrid("reload");
																}
														);
													}
												}
						...
						$('#contractMarket').datagrid({
							url: '${vix}/contract/contractInquiryAction!getContractMarketJson.action?id=${contractInquiry.id}',
							title: '销售定价条件明细',
							width: 'auto',
							height: '450',
							rownumbers : true,
							...
							columns:[[
								...
								{field:'materialCode',title:'编码',width:80,align:'center'},
							...
							toolbar:[{
								...
								handler:function(){
									$('#btnsave').linkbutton('enable');
									saveContractMarket('${vix}/contract/contractInquiryAction!goAddContractMarket.action?id=0');
								}
							},'-',{
								id:'btnedit',
								text:'修改',
								iconCls: 'icon-edit',
								handler: function(){
									var row = $('#contractMarket').datagrid("getSelected");	//获取你选择的所有行
									if(row){
										saveContractMarket('${vix}/contract/contractInquiryAction!goAddContractMarket.action?id='+row.id)
									}
								}
							},'-',{
								text:'删除',
								iconCls:'icon-remove',
								handler:function(){
									var rows = $('#contractMarket').datagrid("getSelections");	//获取你选择的所有行	
									//循环所选的行
									for(var i =0;i<rows.length;i++){
										var index = $('#contractMarket').datagrid('getRowIndex',rows[i]);//获取某行的行号
										$('#contractMarket').datagrid('deleteRow',index);	//通过行号移除该行
										$.ajax({url:'${vix}/contract/contractInquiryAction!deleteContractMarketById.action?id='+rows[i].id,cache: false});
									}
								}
							}]
						});

...
function searchForContent(i){
	loadName();
	pager("start","${vix}/contract/contractApprovalAction!goSearchList.action?i="+i+"&contractName="+name,'category');
}

//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/contract/contractApprovalAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/contract/contractApprovalAction!!goSearchList.action?masterContractCoding=" + $('#masterContractCoding').val() 
...
function saleOrderStatus(contractType){
	pager("start","${vix}/contract/contractApprovalAction!goSingleList.action?contractType="+contractType,'category');
}
//最近使用
function leastRecentlyUsed(contractStartTime){
	pager("start","${vix}/contract/contractApprovalAction!goSingleList.action?contractStartTime="+contractStartTime,'category');
}
...
function saveOrUpdate1(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractApprovalAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
...
function saveOrUpdate2(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractApprovalAction!goPurchaseFA.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function saveOrUpdate3(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractApprovalAction!goSalesContract.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function saveOrUpdate4(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractApprovalAction!goContractMarket.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function saveOrUpdate5(id,pageNo){
	$.ajax({
		  url:'${vix}/contract/contractApprovalAction!goPmContract.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function updateIsPublish(id,isPublish){
	var param={'id':id};  
	$.ajax({ 
		url:"${vix}/contract/contractApprovalAction!updateIsPublish.action",
		data:param,
		dataType:"text",
		success:function(data){
			/* alert('修改成功'); */
			loadName();
			pager("start","${vix}/contract/contractApprovalAction!goSingleList.action?name="+name,'category');
		}
		});
}
...
loadName();
//载入分页列表数据
pager("start","${vix}/contract/contractApprovalAction!goSingleList.action?name="+name+"&orderField=id&orderBy=desc",'category');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/contract/contractApprovalAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/contract/contractApprovalAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/contract/contractApprovalAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/contract/contractApprovalAction!goList.action?pageNo=${pageNo}');"><s:text name="合同审批" /> </a></li>
			</ul>
		</div>
	</h2>
</div>
	...
	<div class="box">
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/contract/contractApprovalAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/contract/contractApprovalAction!goSingleList.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/common_listx16.png" alt="" />审批合同</a></li>
				</ul>
			</div>

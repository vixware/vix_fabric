...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:'${vix}/contract/contractCatalogAction!goSaveOrUpdate.action?id='+id+"&parentId="+parentId,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							$.post('${vix}/contract/contractCatalogAction!saveOrUpdate.action',
								 {'contractCatalog.id':$("#id").val(),
								  'contractCatalog.parentCategory.id':$("#parentId").val(),
								  'contractCatalog.name':$("#name").val(),
								  'contractCatalog.status': $(":radio[name=status][checked]").val(),
								  'contractCatalog.memo':$("#memo").val()								 
								},
								function(result){
									asyncbox.success(result,"<s:text name='合同分类'/>",function(action){
										var treeNode = zTree.getNodeByTId($("#selectIdTreeId").val());
										if(null != treeNode){
											treeNode.isParent = true;
										}
										zTree.reAsyncChildNodes(treeNode, "refresh");
										pager("start","${vix}/contract/contractCatalogAction!goSingleList.action?name="+name+"&id="+$("#selectId").val(),'category');
									});
								}
							 );
						}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/contract/contractCatalogAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='message'/>",function(action){
				...
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/contract/contractCatalogAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
function searchForContent(i){
	...
	loadMemo();
	if(i == 0){
		pager("start","${vix}/contract/contractCatalogAction!goSearchList.action?i="+i+"&searchContent="+name,'category');
	}
	else{
		pager("start","${vix}/contract/contractCatalogAction!goSearchList.action?i="+i+"&name="+name+"&memo="+memo,'category');
	}
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/contract/contractCatalogAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/contract/contractCatalogAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
bindSearch();
bindSwitch();
...
//加载顶部工具栏
loadTopDynamicMenuContent('${vix}/contract/contractCatalogAction!goTopDynamicMenuContent.action');
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="合同管理" /> </a></li>
				<li><a href="#"><s:text name="初始设置" /> </a></li>
				<li><a href="#" onclick="loadContent('${vix}/contract/contractCatalogAction!goList.action');"><s:text name="合同分类" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<%-- <ul>
			...
			<li>
				<a href="#"><img src="${vix}/common/img/icon_10.png" /><s:text name="cmn_category"/></a>
			</li>
			<li class="fly_th">
				<a href="#"><img src="${vix}/common/img/icon_10.png" alt="" /><s:text name="cmn_category"/></a>
				<ul>
					<li>
                    	<dl>
		...
		<div class="search_advanced">
			<label><s:text name='名称'/>:<input id="contract_name" name="" type="text" class="int" /></label>
			<label><s:text name='备注'/>:<input id="contract_memo" name="" type="text" class="int" /></label>		
			<label>
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<div class="left_content" style="height: 500px;">
				<div style="padding-left: 4px;">
					<img src="${vix}/common/img/file.png" style="padding-right: 5px;" /><a href="#" onclick="loadRoot();"><s:text name="合同名称" /></a>
				</div>
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/contract/contractCatalogAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/contract/contractCatalogAction!goSingleList.action?parentId="+treeNode.id,"category");
				}

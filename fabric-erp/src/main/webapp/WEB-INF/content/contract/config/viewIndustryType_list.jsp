...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/contract/viewIndustryTypeAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#viewIndustryTypeForm').validationEngine('validate')){
								$.post('${vix}/contract/viewIndustryTypeAction!saveOrUpdate.action',
									 {'viewIndustryType.id':$("#id").val(),
									  'viewIndustryType.code':$("#code").val(),
									  'viewIndustryType.name':$("#name").val(),
									  'viewIndustryType.disabled':$(":radio[name=disabled][checked]").val()
									},
									function(result){
										asyncbox.success(result,"<s:text name='所属行业'/>",function(action){
											pager("current","${vix}/contract/viewIndustryTypeAction!goSingleList.action?name="+name,'currencyType');
										});
									}
								 ); 
			  				}else{
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/contract/viewIndustryTypeAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/contract/viewIndustryTypeAction!goSingleList.action?name="+name,'currencyType');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/contract/viewIndustryTypeAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'currencyType');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/contract/viewIndustryTypeAction!goSingleList.action?name="+name,'currencyType');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">初始设置</a></li>
				<li><a href="#">字典管理</a></li>
				<li><a href="#" onclick="loadContent('${vix}/contract/viewIndustryTypeAction!goList.action');">所属行业</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

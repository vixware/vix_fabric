...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/contract/contractNatureTypeAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#contractNatureTypeForm').validationEngine('validate')){
								$.post('${vix}/contract/contractNatureTypeAction!saveOrUpdate.action',
									 {'contractNatureType.id':$("#id").val(),
									  'contractNatureType.code':$("#code").val(),
									  'contractNatureType.name':$("#name").val(),
									  'contractNatureType.disabled':$(":radio[name=disabled][checked]").val()
									},
									function(result){
										asyncbox.success(result,"<s:text name='合同性质'/>",function(action){
											pager("current","${vix}/contract/contractNatureTypeAction!goSingleList.action?name="+name,'currencyType');
										});
									}
								 ); 
			  				}else{
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/contract/contractNatureTypeAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/contract/contractNatureTypeAction!goSingleList.action?name="+name,'currencyType');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/contract/contractNatureTypeAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'currencyType');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/contract/contractNatureTypeAction!goSingleList.action?name="+name,'currencyType');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">初始设置</a></li>
				<li><a href="#">字典管理</a></li>
				<li><a href="#" onclick="loadContent('${vix}/contract/contractNatureTypeAction!goList.action');">合同性质</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

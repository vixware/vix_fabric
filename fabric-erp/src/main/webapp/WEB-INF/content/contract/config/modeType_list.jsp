...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/contract/modeTypeAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#modeTypeForm').validationEngine('validate')){
								$.post('${vix}/contract/modeTypeAction!saveOrUpdate.action',
									 {'modeType.id':$("#id").val(),
									  'modeType.code':$("#code").val(),
									  'modeType.name':$("#name").val(),
									  'modeType.disabled':$(":radio[name=disabled][checked]").val()
									},
									function(result){
										asyncbox.success(result,"<s:text name='合同履行状态'/>",function(action){
											pager("current","${vix}/contract/modeTypeAction!goSingleList.action?name="+name,'currencyType');
										});
									}
								 ); 
			  				}else{
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/contract/modeTypeAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/contract/modeTypeAction!goSingleList.action?name="+name,'currencyType');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/contract/modeTypeAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'currencyType');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/contract/modeTypeAction!goSingleList.action?name="+name,'currencyType');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">初始设置</a></li>
				<li><a href="#">字典管理</a></li>
				<li><a href="#" onclick="loadContent('${vix}/contract/modeTypeAction!goList.action');">合同履行状态</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

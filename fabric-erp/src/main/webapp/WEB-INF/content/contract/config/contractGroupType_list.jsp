...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/contract/contractGroupTypeAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#contractGroupTypeForm').validationEngine('validate')){
								$.post('${vix}/contract/contractGroupTypeAction!saveOrUpdate.action',
									 {'contractGroupType.id':$("#id").val(),
									  'contractGroupType.code':$("#code").val(),
									  'contractGroupType.name':$("#name").val(),
									  'contractGroupType.disabled':$(":radio[name=disabled][checked]").val()
									},
									function(result){
										asyncbox.success(result,"<s:text name='所属合同组'/>",function(action){
											pager("current","${vix}/contract/contractGroupTypeAction!goSingleList.action?name="+name,'currencyType');
										});
									}
								 ); 
			  				}else{
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/contract/contractGroupTypeAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/contract/contractGroupTypeAction!goSingleList.action?name="+name,'currencyType');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/contract/contractGroupTypeAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'currencyType');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/contract/contractGroupTypeAction!goSingleList.action?name="+name,'currencyType');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">初始设置</a></li>
				<li><a href="#">字典管理</a></li>
				<li><a href="#" onclick="loadContent('${vix}/contract/contractGroupTypeAction!goList.action');">合同类型集合</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/srm/reviewTenderAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
//查看专家评审结果
function showResult(id,pageNo){
	$.ajax({
		  url:'${vix}/srm/reviewTenderAction!goResultList.action?id='+id,
		  cache: false,
		  success: function(html){
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/srm/reviewTenderAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/srm/reviewTenderAction!goSrmTenderList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/srm/reviewTenderAction!goSrmTenderList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/srm/reviewTenderAction!goSrmTenderList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/srm/reviewTenderAction!goSrmTenderList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function searchForContent(i){
	...
	loadPurchasePerson();
	if(i == 0){
		pager("start","${vix}/srm/reviewTenderAction!goSearchList.action?i="+i+"&searchContent="+name,'category');
	}
	else{
		pager("start","${vix}/srm/reviewTenderAction!goSearchList.action?i="+i+"&purchasePerson="+purchasePerson+"&purchaseName="+purchaseName,'category');
	}
}
//状态
function purStatus(status){
	pager("start","${vix}/srm/reviewTenderAction!goSrmTenderList.action?status="+status,'category');
}
//最近使用
function purRecent(rencentDate){
	pager("start","${vix}/srm/reviewTenderAction!goSrmTenderList.action?updateTime="+rencentDate,'category');
}
function goSearch() {
	$.ajax({
...
<div class="sub_menu">
	...
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0,$('#selectId').val());"><span>分配账号</span></a> <a href="#" onclick="showResult(0,$('#selectId').val());"><span>查看评审意见</span></a>
		</p>
	</div>
</div>
		...
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/srm/reviewTenderAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/srm/reviewTenderAction!goSrmTenderList.action?parentId="+treeNode.id,"category");
				}

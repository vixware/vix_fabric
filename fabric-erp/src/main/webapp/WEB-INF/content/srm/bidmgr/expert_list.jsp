...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/srm/expertAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/srm/expertAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/srm/expertAction!goSrmTenderList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/srm/expertAction!goSrmTenderList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/srm/expertAction!goSrmTenderList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/srm/expertAction!goSrmTenderList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function searchForContent(i){
	...
	loadPurchasePerson();
	if(i == 0){
		pager("start","${vix}/srm/expertAction!goSearchList.action?i="+i+"&searchContent="+name,'category');
	}
	else{
		pager("start","${vix}/srm/expertAction!goSearchList.action?i="+i+"&purchasePerson="+purchasePerson+"&purchaseName="+purchaseName,'category');
	}
}
//状态
function purStatus(status){
	pager("start","${vix}/srm/expertAction!goSrmTenderList.action?status="+status,'category');
}
//最近使用
function purRecent(rencentDate){
	pager("start","${vix}/srm/expertAction!goSrmTenderList.action?updateTime="+rencentDate,'category');
}
function goSearch() {
	$.ajax({
...
<div class="sub_menu">
	...
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0,$('#selectId').val());"><span>评标</span></a>
		</p>
	</div>
</div>
	...
	<div class="box">
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/mail.png" alt="" />待评审项目</a></li>
				</ul>
			</div>

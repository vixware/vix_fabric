...
function loadServiceEvaluation(){
	serviceEvaluation = $('#serviceEvaluation').val(); 
	serviceEvaluation=Url.encode(serviceEvaluation);
	serviceEvaluation=Url.encode(serviceEvaluation);
}
...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/srm/supplierEvaluationAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/srm/supplierEvaluationAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/srm/supplierEvaluationAction!goEvaluationList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/srm/supplierEvaluationAction!goEvaluationList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/srm/supplierEvaluationAction!goEvaluationList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/srm/supplierEvaluationAction!goEvaluationList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function searchForContent(i){
	...
	if(i == 0){
		pager("start","${vix}/srm/supplierEvaluationAction!goSearchList.action?i="+i+"&searchContent="+name,'category');
	}
	else{
		pager("start","${vix}/srm/supplierEvaluationAction!goSearchList.action?i="+i+"&serviceEvaluation="+serviceEvaluation+"&supplierName="+supplierName,'category');
	}
}
/*重置搜索*/
...
function srmStatus(status){
	pager("start","${vix}/srm/managementBusinessPartnerAction!goSingleList.action?status="+status,'category');
}
//最近使用
function srmRecent(rencentDate){
	pager("start","${vix}/srm/managementBusinessPartnerAction!goSingleList.action?updateTime="+rencentDate,'category');
}
</script>
<div class="sub_menu">
	<h2>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div>
			<label>搜索内容：<input type="text" class="int" id="nameS"></label> <label> <input id="simpleSearch" name="" type="button" class="btn" value="<s:text name='cmn_search'/>" onclick="searchForContent(0);" /> <input id="simpleReset" name="" type="button" class="btn" value="<s:text name='cmn_reset'/>" onclick="resetForContent(0);" />
			</label> <label> <input type="button" value="高级搜索" class="btn" onclick="goSearch();" /></label>
		</div>
		<div class="search_advanced">
			<label><s:text name='eam_suppliername' />:<input id="supplierName" type="text" class="int" /></label> <label>服务评价:<input id="serviceEvaluation" type="text" class="int" /></label> <label> <input name="" type="button" class="btn" value="<s:text name='cmn_search'/>" onclick="searchForContent(1);" /> <input name="" type="button"
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="supplierEvaluationList" var="c">
	...
	<div class="box">
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/srm/supplierEvaluationAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/srm/supplierEvaluationAction!goEvaluationList.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/mail.png" alt="" /> <s:text name='srm_management_business_partner' /></a></li>
				</ul>
			</div>

...
pager("start","${vix}/srm/managementBusinessPartnerAction!goSubRadioSingleList.action?temp=1","radio");
//排序 
...
function currentPager(tag){
	pager(tag,"${vix}/srm/managementBusinessPartnerAction!goSubRadioSingleList.action?code="+code+"&pageNo=${pageNo}",'radio');
}
...
function searchForContent(i){
	loadCode();
	pager("start","${vix}/srm/managementBusinessPartnerAction!goSubRadioSingleList.action?searchContent="+code,'radio');
}
/*重置搜索*/
...
function srmStatus(status){
	pager("start","${vix}/srm/managementBusinessPartnerAction!goSubRadioSingleList.action?status="+status,'radio');
}
//最近使用
function srmRecent(rencentDate){
	pager("start","${vix}/srm/managementBusinessPartnerAction!goSubRadioSingleList.action?updateTime="+rencentDate,'radio');
}
</script>

...
<script type="text/javascript">
	...
	function deleteEntity(id) {
		asyncbox.confirm('确定要删除该数据么?', '提示信息', function(action) {
			if (action == 'ok') {
				$.ajax({
				url : '${vix}/drp/distributororderManagementAction!deleteById.action?id=' + id,
				cache : false,
				success : function(html) {
					showMessage(html);
					setTimeout("", 1000);
					pager("start", "${vix}/drp/distributororderManagementAction!goSingleList.action", 'salesOrder');
				}
				});
			}
		});
...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<th width="20%"><span style="cursor: pointer;" onclick="orderBy('name');"><s:text name="drp_theme" />&nbsp;<img id="salesOrderImg_name" src="${vix}/common/img/arrow.gif"></span></th>
			<th width="25%"><span style="cursor: pointer;" onclick="orderBy('buyerNick');">要货方&nbsp;<img id="salesOrderImg_buyerNick" src="${vix}/common/img/arrow.gif"></span></th>
			<th width="20%"><span style="cursor: pointer;" onclick="orderBy('createTime');">要货时间&nbsp;<img id="salesOrderImg_createTime" src="${vix}/common/img/arrow.gif"></span></th>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td><a href="#" style="color: gray;" onclick="pophtml('${entity.id}');">${entity.name }</a></td>
				<td><a href="#" style="color: gray;" onclick="pophtml('${entity.id}');">${entity.channelDistributor.name }</a></td>
				<td><a href="#" style="color: gray;" onclick="pophtml('${entity.id}');"><fmt:formatDate value="${entity.deliveryTime }" pattern="yyyy-MM-dd" /></a></td>
				<td>

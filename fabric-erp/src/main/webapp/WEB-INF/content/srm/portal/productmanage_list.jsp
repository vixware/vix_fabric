...
function deleteById(id){
	$.ajax({
		  url:'${vix}/srm/productManagementAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/srm/productManagementAction!goListContent.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/srm/productManagementAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/srm/productManagementAction!goListContent.action?name="+name,'brand');
}
function goSearch() {
	$.ajax({
	...
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				loadcode();
				loadselectname();
				pager("start", "${vix}/srm/productManagementAction!goSingleList.action", 'brand');
			}
		},
...
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#"><img src="img/srm_catalog.png" alt="" /> <s:text name="cmn_supplyChain" /></a></li>
				<li><a href="#"><s:text name="srm_supp_rela_manage" /></a></li>
				<li><a href="#"><s:text name='srm_supplier_portal' /></a></li>
				<li><a href="#"><s:text name='srm_directory_manage' /></a></li>
				<li><a href="#"><s:text name='srm_product_manage' /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

...
function deleteById(id){
	$.ajax({
		  url:'${vix}/srm/contractTemplatesAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/srm/contractTemplatesAction!goListContent.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/srm/contractTemplatesAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/srm/contractTemplatesAction!goListContent.action?name="+name,'brand');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name='srm_contract_templates' /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

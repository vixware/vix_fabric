...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/purchase/purchasingSettlementAction!goSaveOrUpdate.action',
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/purchase/purchasingSettlementAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/purchase/purchasingSettlementAction!goListContent.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/purchase/purchasingSettlementAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/purchase/purchasingSettlementAction!goListContent.action?name="+name,'brand');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name='pur_purchase_settlement' /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0);"><span>新增</span></a> <a href="#"><span><s:text name='pur_cancel_settle' /></span></a> <a href="#" onclick="deleteByIds();"><span><s:text name='cmn_delete' /></span></a> <a href="#"><span><s:text name='pur_audit' /></span></a>
		</p>
	</div>
</div>
<div class="content">

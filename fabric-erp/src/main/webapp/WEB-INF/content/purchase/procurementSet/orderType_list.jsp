...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/purchase/orderTypeAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#orderTypeForm').validationEngine('validate')){
								$.post('${vix}/purchase/orderTypeAction!saveOrUpdate.action', {
										'orderType.id' : $("#newId").val(),
										'orderType.code' : $("#newCode").val(),
										'orderType.name' : $("#newName").val(),
										'updateField' : updateField,
										'orderType.description' : $("#newDescription").val()
										}, function(result){
											showMessage(result);
											setTimeout("",1000);
											//载入分页列表数据
											pager("start","${vix}/purchase/orderTypeAction!goSingleList.action?name="+name,'orderType');
										});
							}else{
			  					return false;
			  				}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/purchase/orderTypeAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
...
function searchForContent(i){
	...
	loadSearchName();
	if(i == 0){
		pager("start","${vix}/purchase/orderTypeAction!goSearchList.action?i="+i+"&searchContent="+name,'orderType');
	}
	else{
		pager("start","${vix}/purchase/orderTypeAction!goSearchList.action?i="+i+"&code="+code+"&searchname="+searchname,'orderType');
	}
}
/*重置搜索*/
...
function purStatus(status){
	pager("start","${vix}/purchase/orderTypeAction!goSingleList.action?status="+status,'orderType');
}
//最近使用
function purRecent(rencentDate){
	pager("start","${vix}/purchase/orderTypeAction!goSingleList.action?updateTime="+rencentDate,'orderType');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/purchase/orderTypeAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'orderType');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/purchase/orderTypeAction!goSingleList.action?name="+name,'orderType');
}
...
<div class="content">
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="orderTypeList" var="p">
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a></span> <span><a class="previous" onclick="currentPager('previous');"></a></span> <em>(<b class="orderTypeInfo"></b> <s:text name='cmn_to' /> <b class="orderTypeTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a></span> <span><a class="end" onclick="currentPager('end');"></a></span>
					</div>
				</div>
				<div id="orderType" class="table" style="overflow-x: auto; width: 100%;"></div>
				<div class="pagelist drop">
					<ul>

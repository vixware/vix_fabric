...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/purchase/issuedATenderInvitationPAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/purchase/issuedATenderInvitationPAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/purchase/issuedATenderInvitationPAction!goListContent.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/purchase/issuedATenderInvitationPAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/purchase/issuedATenderInvitationPAction!goListContent.action?name="+name,'brand');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name='srm_issued_tender_invitation' /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0);"><span><s:text name='cmn_add' /></span></a> <a href="#" onclick="deleteByIds();"><span><s:text name='cmn_delete' /></span></a> <a href="#"><span><s:text name='cmn_report' /></span></a> <a href="#"><span><s:text name='cmn_check' /></span></a> <a href="#"><span><s:text
						name='srm_published_invitation' /></span></a>
		</p>
	</div>
</div>
<div class="content">

...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/purchase/purchaseApproveAction!goShowPurchasePlanPackage.action?id='+id,
		  cache: false,
...
loadName();
//载入分页列表数据
pager("start","${vix}/purchase/purchaseApproveAction!goSingleList.action?name="+name,'purchasePlan');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/purchase/purchaseApproveAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'purchasePlan');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'purchasePlan'){
		pager(tag,"${vix}/purchase/purchaseApproveAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function searchForContent(i){
	...
	loadSupplierName();
	if(i == 0){
		pager("start","${vix}/purchase/purchaseApproveAction!goSingleList.action?i="+i+"&searchContent="+name,'purchasePlan');
	}
	else{
		pager("start","${vix}/purchase/purchaseApproveAction!goSingleList.action?i="+i+"&title="+title+"&supplierName="+supplierName,'purchasePlan');
	}
}
/*重置搜索*/
...
function purStatus(status){
	pager("start","${vix}/purchase/purchaseApproveAction!goSingleList.action?status="+status,'purchasePlan');
}
//最近使用
function purRecent(rencentDate){
	pager("start","${vix}/purchase/purchaseApproveAction!goSingleList.action?updateTime="+rencentDate,'purchasePlan');
}

function showPurchasePlanDetail(id){
	$.ajax({
		  url:'${vix}/purchase/purchaseApproveAction!goPurchasePlanDetail.action?purchasePlanId='+id,
		  cache: false,
		  success: function(html){
		  	$("#mainContent").html(html);
		  }
...
function goSearch() {
	$.ajax({
	url : '${vix}/purchase/purchaseApproveAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start","${vix}/purchase/purchaseApproveAction!goSingleList.action?title="+$('#title').val()+"&supplierName="+$('#supplierName').val(),'tab_content_page');
			}
		},
...
function goShowPurchasePlan(id) {
	$.ajax({
	url : '${vix}/purchase/purchaseApproveAction!goShowPurchasePlan.action?id=' + id,
	cache : false,
	success : function(html) {
		$("#mainContent").html(html);
	}
...
function goPurchasePlanPackage() {
	$.ajax({
		url : '${vix}/purchase/purchaseApproveAction!goCollectPurchasePlan.action',
		cache : false,
		success : function(html) {
			$("#mainContent").html(html);
		}
 ...
 * 计划汇总
 */
function saveOrUpdatePurchasePlanPackage() {
	$.post('${vix}/purchase/purchaseApproveAction!saveOrUpdate.action', {
		'purchasePlanPackage.id' : $("#purchasePlanPackageId").val(),
		'purchasePlanPackage.code' : $("#code").val(),
		'purchasePlanPackage.name' : $("#name").val(),
		'purchasePlanPackage.packageType' : $("#packageType").val(),
		'purchasePlanPackage.creator' : $("#creator").val(),
		'purchasePlanPackage.createTime' : $("#createTime").val(),
		'purchasePlanPackage.planType' : $("#planType").val(),
		'purchasePlanPackage.status' : $("#status").val(),
		'purchasePlanPackage.amount' : $("#amount").val(),
		'purchasePlanPackage.price' : $("#price").val()
		}, function(result) {
			showMessage(result);
			setTimeout("", 1000);
			loadContent('${vix}/purchase/purchaseApproveAction!goList.action');
		});
	};
function subPurchasePlanPackage(id) {
	$.post('${vix}/purchase/purchaseApproveAction!subPurchasePlanPackage.action', {
		'id' : id
		}, function(result) {
			showMessage(result);
			setTimeout("", 1000);
...
<div class="sub_menu">
	...
	<div class="drop">
		<p>
			<a href="#" onclick="goPurchasePlanPackage(0);"><span>汇总</span></a>
		</p>
	</div>
</div>

...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/purchase/purchasePlanAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
//填写计划包
function goWriteTaskPlan(id,pageNo){
	$.ajax({
		  url:'${vix}/purchase/purchasePlanAction!goWriteTaskPlan.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/purchase/purchasePlanAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/purchase/purchasePlanAction!goSingleList.action?name="+name,'purchasePlan');
			});
		  }
	});
}
...
loadName();
//载入分页列表数据
pager("start","${vix}/purchase/purchasePlanAction!goSingleList.action?1=1",'purchasePlan');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/purchase/purchasePlanAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'purchasePlan');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'purchasePlan'){
		pager(tag,"${vix}/purchase/purchasePlanAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function searchForContent(i){
	...
	loadSupplierName();
	if(i == 0){
		pager("start","${vix}/purchase/purchasePlanAction!goSingleList.action?i="+i+"&searchContent="+name,'purchasePlan');
	}
	else{
		pager("start","${vix}/purchase/purchasePlanAction!goSingleList.action?i="+i+"&title="+title+"&supplierName="+supplierName,'purchasePlan');
	}
}
/*重置搜索*/
...
function purStatus(status){
	pager("start","${vix}/purchase/purchasePlanAction!goSingleList.action?status="+status,'purchasePlan');
}
//最近使用
function purRecent(rencentDate){
	pager("start","${vix}/purchase/purchasePlanAction!goSingleList.action?updateTime="+rencentDate,'purchasePlan');
}

function showPurchasePlanDetail(id){
	$.ajax({
		  url:'${vix}/purchase/purchasePlanAction!goPurchasePlanDetail.action?purchasePlanId='+id,
		  cache: false,
		  success: function(html){
		  	$("#mainContent").html(html);
		  }
...
function goSearch() {
	$.ajax({
	url : '${vix}/purchase/purchasePlanAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start","${vix}/purchase/purchasePlanAction!goSingleList.action?title="+$('#title').val()+"&supplierName="+$('#supplierName').val(),'tab_content_page');
			}
		},
...
function goShowPurchasePlan(id) {
	$.ajax({
	url : '${vix}/purchase/purchasePlanAction!goShowPurchasePlan.action?id=' + id,
	cache : false,
	success : function(html) {
		$("#mainContent").html(html);
	}
...
function goPurchasePlanPackage(id) {
	$.ajax({
	url : '${vix}/purchase/purchasePlanAction!goPurchasePlanPackage.action?id=' + id ,
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				$.post('${vix}/purchase/purchaseApproveAction!goPack.action', {
				'purchasePlanPackage.id' : $("#purchasePlanPackageId").val(),
				'purchasePlanPackage.name' : $("#name").val(),
				'purchasePlanPackage.packageType' : $(":radio[name=packageType][checked]").val(),
				'purchasePlanIds' : purchasePlanIds,
				'organdempname' :  $("#organdempname").val(),
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name='pur_purchase_plan' /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#" onclick="goWriteTaskPlan(0);"><span>计划发起</span></a> <a href="#" onclick="saveOrUpdate(0);"><span>计划编写</span></a>
			<!-- <a href="#" onclick="goPurchasePlanPackage(0);"><span>计划上报</span></a> -->
		</p>
	</div>
</div>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div class="untitled">
			...
			<div class="popup">
				<strong> <i class="close"><a href="#"></a></i> <i><a href="#"></a></i> <i><a href="#"></a></i> <b>采购计划</b>
				</strong>
				<p>采购计划帮助</p>
			</div>
		</div>

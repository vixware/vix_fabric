...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/purchase/purchasePackageAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/purchase/purchasePackageAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/purchase/purchasePackageAction!goSingleList.action?name="+name,'purchasePlan');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/purchase/purchasePackageAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'purchasePlan');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'purchasePlan'){
		pager(tag,"${vix}/purchase/purchasePackageAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function searchForContent(i){
	...
	loadSupplierName();
	if(i == 0){
		pager("start","${vix}/purchase/purchasePackageAction!goSingleList.action?i="+i+"&searchContent="+name,'purchasePlan');
	}
	else{
		pager("start","${vix}/purchase/purchasePackageAction!goSingleList.action?i="+i+"&title="+title+"&supplierName="+supplierName,'purchasePlan');
	}
}
/*重置搜索*/
...
function purStatus(status){
	pager("start","${vix}/purchase/purchasePackageAction!goSingleList.action?status="+status,'purchasePlan');
}
//最近使用
function purRecent(rencentDate){
	pager("start","${vix}/purchase/purchasePackageAction!goSingleList.action?updateTime="+rencentDate,'purchasePlan');
}

function showPurchasePlanDetail(id){
	$.ajax({
		  url:'${vix}/purchase/purchasePackageAction!goPurchasePlanDetail.action?purchasePlanId='+id,
		  cache: false,
		  success: function(html){
		  	$("#mainContent").html(html);
		  }
...
function goSearch() {
	$.ajax({
	url : '${vix}/purchase/purchasePackageAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start","${vix}/purchase/purchasePackageAction!goSingleList.action?title="+$('#title').val()+"&supplierName="+$('#supplierName').val(),'tab_content_page');
			}
		},
...
function goShowPurchasePlan(id) {
	$.ajax({
	url : '${vix}/purchase/purchasePackageAction!goShowPurchasePlan.action?id=' + id,
	cache : false,
	success : function(html) {
		$("#mainContent").html(html);
	}
...
function goPurchasePlanPackage(id) {
	$.ajax({
	url : '${vix}/purchase/purchasePackageAction!goPurchasePlanPackage.action?id=' + id ,
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				$.post('${vix}/purchase/purchasePackageAction!goPack.action', {
				...
				'purchasePlanIds' : purchasePlanIds
					...
					showMessage(result);
					setTimeout("", 1000);
					loadContent('${vix}/purchase/purchasePackageAction!goList.action');
				});
			}
		},
		btnsbar : $.btn.OKCANCEL

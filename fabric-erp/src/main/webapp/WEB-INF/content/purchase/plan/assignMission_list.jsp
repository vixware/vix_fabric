...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/purchase/assignMissionAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
loadName();
//载入分页列表数据
pager("start","${vix}/purchase/assignMissionAction!goSingleList.action?name="+name,'purchasePlan');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/purchase/assignMissionAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'purchasePlan');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'purchasePlan'){
		pager(tag,"${vix}/purchase/assignMissionAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function searchForContent(i){
	...
	loadSupplierName();
	if(i == 0){
		pager("start","${vix}/purchase/assignMissionAction!goSingleList.action?i="+i+"&searchContent="+name,'purchasePlan');
	}
	else{
		pager("start","${vix}/purchase/assignMissionAction!goSingleList.action?i="+i+"&title="+title+"&supplierName="+supplierName,'purchasePlan');
	}
}
/*重置搜索*/
...
function purStatus(status){
	pager("start","${vix}/purchase/assignMissionAction!goSingleList.action?status="+status,'purchasePlan');
}
//最近使用
function purRecent(rencentDate){
	pager("start","${vix}/purchase/assignMissionAction!goSingleList.action?updateTime="+rencentDate,'purchasePlan');
}

function showPurchasePlanDetail(id){
	$.ajax({
		  url:'${vix}/purchase/assignMissionAction!goPurchasePlanDetail.action?purchasePlanId='+id,
		  cache: false,
		  success: function(html){
		  	$("#mainContent").html(html);
		  }
...
function goSearch() {
	$.ajax({
	url : '${vix}/purchase/assignMissionAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start","${vix}/purchase/assignMissionAction!goSingleList.action?title="+$('#title').val()+"&supplierName="+$('#supplierName').val(),'tab_content_page');
			}
		},
...
function saveOrUpdate(id) {
	...
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				$.post('${vix}/purchase/assignMissionAction!saveOrUpdate.action', {
				'purchasePlanPackage.id' : $("#purchasePlanPackageId").val(),
				'purchasePlanPackage.name' : $("#purchasePlanPackageName").val(),
				'purchasePlanPackage.creator' : $("#creator").val()
				}, function(result) {
					showMessage(result);
					setTimeout("", 1000);
					loadContent('${vix}/purchase/assignMissionAction!goList.action');
				});
			}
		},
		btnsbar : $.btn.OKCANCEL

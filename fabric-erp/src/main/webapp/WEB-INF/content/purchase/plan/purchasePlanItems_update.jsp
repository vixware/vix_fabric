...
<script type="text/javascript">
	...
	loadName();
	//载入分页列表数据
	pager("start", "${vix}/purchase/purchasePlanAction!goListItemList.action?name=" + name, 'itemPriceManage');
	//排序 
	function orderBy(orderField) {
		...
		pager("start", "${vix}/purchase/purchasePlanAction!goListItemList.action?orderField=" + orderField + "&orderBy=" + orderBy + "&name=" + name, 'itemPriceManage');
	}

	function currentPager(tag) {
		loadName();
		pager(tag, "${vix}/purchase/purchasePlanAction!goListItemList.action?name=" + name + '&companyCode=' + $("#companyCode").val(), 'itemPriceManage');
	}
	function addSalesOrderItem() {
		if ($('#saleOrderItemForm').validationEngine('validate')) {
			$.post('${vix}/purchase/purchasePlanAction!saveOrUpdatePurchasePlanItems.action', {
			'id' : $("#id").val(),
			'purchasePlanItems.id' : $("#oiId").val(),
			'purchasePlanItems.itemCode' : $("#itemCode").val(),
			'purchasePlanItems.itemName' : $("#itemName").val(),
			'purchasePlanItems.specification' : $("#specification").val(),
			'purchasePlanItems.unitcost' : $("#unitcost").val(),
			'purchasePlanItems.amount' : $("#purchasePlanItemsAmount").val(),
			'purchasePlanItems.measureUnit.id' : $("#measureUnitId").val()
			}, function(result) {
				showMessage(result);
				setTimeout("", 1000);
	...
	function searchForContent() {
		...
		pager("start", "${vix}/purchase/purchasePlanAction!goListItemList.action?name=" + name + "&categoryId=" + categoryId, 'itemPriceManage');
	}
	function reset() {
		$("#itemCategoryId").val("");
		$("#itemCategory").val("");
		...
		<div id="right">
			<div class="right_content">
				<form id="saleOrderItemForm">
					<s:hidden id="oiId" name="oiId" value="%{purchasePlanItems.id}" theme="simple" />
					<div class="box order_table" style="line-height: normal;">
						<table>
							...
							<tr height="40">
								<th>规格型号:</th>
								<td><input id="specification" name="specification" value="${purchasePlanItems.specification}" type="text" class="validate[required] text-input" /> <span style="color: red;">*</span></td>
							</tr>
							<tr height="40">
								<th>价格:</th>
							...
							<tr height="40">
								<th>单位:</th>
								<td><s:select id="measureUnitId" headerKey="-1" headerValue="--请选择--" list="%{measureUnitList}" listValue="name" listKey="id" multiple="false" theme="simple" validate="required:true" min="1">
									</s:select><span style="color: red;">*</span></td>
							</tr>
							<tr height="40">
								<th>数量:</th>
								<td><input id="purchasePlanItemsAmount" name="purchasePlanItemsAmount" value="${purchasePlanItems.amount}" type="text" class="validate[required,custom[number]] text-input" /> <span style="color: red;">*</span></td>
							</tr>
						</table>
					</div>

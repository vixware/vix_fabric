...
function saveOrUpdate(id,parentId,purchaseType){
	var url = '${vix}/purchase/purchaseArrivalAction!goSaveOrUpdate.action';
	var data = {};
	if(id != null){
		data.id=id;
	}
	...
	if(purchaseType && purchaseType!=''){
		...
		if(purchaseType=='order'){
			copyUrl = '${vix}/purchase/purchaseArrivalAction!goChoosePurchaseOrders.action?copyTo=arrival';
		}else if(purchaseType=='apply'){
			copyUrl = '${vix}/purchase/purchaseOrderAction!goChoosePurchaseApply.action?copyTo=arrival';
		}
		$.ajax ({
			url : copyUrl,
			cache : false ,
...
function goShowPurchaseArrival(id) {
	$.ajax({
	url : '${vix}/purchase/purchaseArrivalAction!goShowPurchaseArrival.action?id=' + id ,
	cache : false,
	success : function(html) {
...
function goPrintPurchaseArrival(id) {
	$.ajax({
	url : '${vix}/purchase/purchaseArrivalAction!goPrintPurchaseArrival.action?id=' + id,
	cache : false,
	success : function(html) {
		LODOP = getLodop();
...
function goSearch() {
	$.ajax({
	url : '${vix}/purchase/purchaseArrivalAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start","${vix}/purchase/purchaseArrivalAction!goSingleList.action?code="+$('#code').val()+"&name="+$('#name').val()+"&requireDepartment="+$('#requireDepartment').val()+"&supplierName="+$('#supplierName').val()+"&purchasePerson="+$('#purchasePerson').val()+"&contactPerson="+$('#contactPerson').val()+"&postingDate="+$('#postingDate').val()+"&deliveryDate="+$('#deliveryDate').val(),'purchase_order_grid');
			}
		},
		btnsbar : $.btn.OKCANCEL
		});
...
<div class="sub_menu">
	...
	<div class="drop">
		<p>
		<ul>
			<li><a href="#" onclick="saveOrUpdate(0,$('#selectId').val());"><span>新增到货单</span></a>
				<ul>
					<li><a href="#" onclick="saveOrUpdate(0,$('#selectId').val());">普通到货单</a></li>
					<!-- <li><a href="#" onclick="saveOrUpdate(0,$('#selectId').val(),'plan');">来源自采购计划单</a></li> -->
		...
		<div id="right">
			<div id="tab_content" class="tabbable addable_tab">
				<ul id="myTab" class="right_menu nav nav-tabs">
					<li pageId="tab_home" class="current"><a href="#tab_home" page="${vix}/purchase/purchaseArrivalAction!goSingleList.action"> <img src="img/mail.png" alt="" /> 采购到货单
					</a></li>
				</ul>

...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:'${vix}/purchase/checkoutAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							$.post('${vix}/purchase/checkoutAction!saveOrUpdate.action',
								 {'category.id':$("#id").val(),
								...
								function(result){
									asyncbox.success(result,"<s:text name='cmn_message'/>",function(action){
										...
										pager("start","${vix}/purchase/checkoutAction!goListContent.action?name="+name+"&id="+$("#selectId").val(),'category');
									});
								}
							 );
						}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/purchase/checkoutAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/purchase/checkoutAction!goListContent.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/purchase/checkoutAction!goListContent.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/purchase/checkoutAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/purchase/checkoutAction!goListContent.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name='pur_checkout' /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
	...
	<div class="box">
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/purchase/checkoutAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/purchase/checkoutAction!goListContent.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/mail.png" alt="" /> <s:text name='pur_list' /></a></li>
				</ul>
			</div>

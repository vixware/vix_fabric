...
function fixedPrice(){
	...
	$.ajax({
		  url:'${vix}/mdm/item/purchaseItemPriceAction!goFixedPrice.action?priceConditionType=purchase&id='+$("#my_item_page #itemId").val()+"&count="+$("#my_item_page #soiAmount").val()+"&billCreateDate="+$("#my_item_page #requireTime").val()+"&supplierId="+$("#purchaseOrderForm #supplierId").val(),
		  cache: false,
	...
	if($('#saleOrderItemForm').validationEngine('validate')){
		$.post('${vix}/purchase/purchaseOrderAction!saveOrUpdatePurchaseOrderLineItem.action',
			{ 'purchaseItem.id':$("#my_item_page #purchaseApplyItemId").val(),
			  ...
			  'purchaseItem.purchaseOrder.id':$("#id").val(),
...
function chooseItemForPrice(id,code,name,price){
	$.post('${vix}/purchase/purchaseOrderAction!loadPurchaseItemInfo.action',
		{'id':id},
		function(json){
			if(json){

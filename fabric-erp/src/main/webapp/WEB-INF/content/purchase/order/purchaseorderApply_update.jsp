	...
	if($('#purchaseOrderForm').validationEngine('validate')){
		$.post('${vix}/purchase/purchaseOrderAction!saveOrUpdate.action',
				...
				function(result){
					showMessage(result);
					setTimeout("",1000);
					loadContent('${vix}/purchase/purchaseOrderAction!goList.action?menuId=menuOrder');
				}
			 );
	}
}
...
/** 选择采购计划单 */
function choosePurchaseApply (){
	$.ajax ({
			url : '${vix}/purchase/purchaseOrderAction!goChoosePurchaseApply.action' ,
			cache : false ,
			success : function (html){
				asyncbox
						.open ({
						...
						callback : function (action , returnValue){
							if (action == 'ok'){
								if (returnValue != ''){
									$.ajax ({
											url : '${vix}/purchase/purchaseOrderAction!converterApplyToOrder.action?purchaseApplyid='+returnValue+"&id=${purchaseOrder.id}",
											cache : false ,
...
<input type="hidden" id="mark" value="2" />
<div class="content">
		...
		<div class="order">
			<dl>
				<dt id="orderTitleFixd">
					...
					</span> <strong> <b>新增采购订单(来源自采购申请单)</b> <i></i>
					</strong>
				</dt>
				<dd class="clearfix">
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td colspan="3"><font color="red">采购申请单</font> <input class="btn" type="button" value="取单" onclick="choosePurchaseApply();" /></td>
										</tr>
										<tr>
								...
								<dd style="display: none;">
									<table style="border: none;">
										...
										<tr>
											...
											<td><input id="version" name="version" value="${purchaseOrder.version }" type="text" size="30" onchange="fieldChanged(this);" /></td>
										...
										<tr>
											...
											<td><input id="contractTime" name="contractTime" value="<fmt:formatDate value='${purchaseOrder.contractTime }' type='both' pattern='yyyy-MM-dd' />" readonly="readonly" type="text" onchange="fieldChanged(this);" /> <img onclick="showTime('contractTime','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif" width="16" height="22"
												align="absmiddle"></td>
													...
													if(action == 'ok'){
														if($('#orderItemForm').validationEngine('validate')){
															$.post('${vix}/purchase/purchaseOrderAction!saveOrUpdatePurchaseOrderLineItem.action', {
																	...
																	'purchaseOrderLineItem.recieveAddressCode' : $("#newRecieveAddressCode").val()
																	}, function(result){
																		showMessage(result);
																		setTimeout("",1000);
																		$('#dlLineItem').datagrid("reload");

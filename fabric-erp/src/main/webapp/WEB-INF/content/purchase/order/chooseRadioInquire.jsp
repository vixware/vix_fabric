...
pager("start","${vix}/purchase/purchaseOrderAction!goSubRadioInquireList.action?temp=1","radio");
//排序 
...
function searchForContent(i){
	loadCode();
	pager("start","${vix}/purchase/purchaseOrderAction!goSubRadioInquireList.action?searchContent="+code,'radio');
}
/*重置搜索*/
...
function srmStatus(status){
	pager("start","${vix}/purchase/purchaseOrderAction!goSubRadioInquireList.action?status="+status,'radio');
}
//最近使用
function srmRecent(rencentDate){
	pager("start","${vix}/purchase/purchaseOrderAction!goSubRadioInquireList.action?updateTime="+rencentDate,'radio');
}
</script>

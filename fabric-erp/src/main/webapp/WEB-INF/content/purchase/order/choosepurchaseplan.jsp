...
<script type="text/javascript">
	...
	pager ("start" , "${vix}/purchase/purchaseOrderAction!goPurchasePlanList.action?temp=1" , "radio");
	//排序 
	...
	function currentPager (tag){
		pager (tag , "${vix}/template/simpleGridAction!goPurchaseOrderList.action" , 'radio');
	}
	...
	function searchForContent (i){
		loadCode ();
		pager ("start" , "${vix}/purchase/purchaseOrderAction!goPurchasePlanList.action?searchContent=" + code , 'radio');
	}
	/*重置搜索*/
	...
	function srmStatus (status){
		pager ("start" , "${vix}/purchase/purchaseOrderAction!goPurchasePlanList.action?status=" + status , 'radio');
	}
	//最近使用
	function srmRecent (rencentDate){
		pager ("start" , "${vix}/purchase/purchaseOrderAction!goPurchasePlanList.action?updateTime=" + rencentDate , 'radio');
	}
	function chkChange (chk , id){
		if (chk.checked){
			$.returnValue = id;

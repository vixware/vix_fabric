...
function saveOrUpdate(id,parentId,purchaseType){
	var url = '${vix}/purchase/purchaseInboundAction!goSaveOrUpdate.action';
	var data = {};
	if(id!= null){
		data.id=id;
	}
...
function goShowPurchaseInbound(id) {
	$.ajax({
	url : '${vix}/purchase/purchaseInboundAction!goShowPurchaseInbound.action?id=' + id ,
	cache : false,
	success : function(html) {
...
function goPrintPurchaseInbound(id) {
	$.ajax({
	url : '${vix}/purchase/purchaseInboundAction!goPrintPurchaseInbound.action?id=' + id,
	cache : false,
	success : function(html) {
		LODOP = getLodop();
...
function goSearch() {
	$.ajax({
	url : '${vix}/purchase/purchaseInboundAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start","${vix}/purchase/purchaseInboundAction!goSingleList.action?code="+$('#code').val()+"&name="+$('#name').val()+"&supplierName="+$('#supplierName').val()+"&purchasePerson="+$('#purchasePerson').val()+"&postingDate="+$('#postingDate').val()+"&deliveryDate="+$('#deliveryDate').val()+"&createTime="+$('#createTime').val()+"&contactPerson="+$('#contactPerson').val(),'purchase_order_grid');
			}
		},
		btnsbar : $.btn.OKCANCEL
		});
...
<div class="sub_menu">
	...
	<div class="drop">
		<p>
		<ul>
			<li><a href="#" onclick="saveOrUpdate(0,$('#selectId').val());"><span>新增采购入库单</span></a>
				<ul>
					<li><a href="#" onclick="saveOrUpdate(0,$('#selectId').val());">普通采购入库订单</a></li>
		...
		<div id="right">
			<div id="tab_content" class="tabbable addable_tab">
				<ul id="myTab" class="right_menu nav nav-tabs">
					<li pageId="tab_home" class="current"><a href="#tab_home" page="${vix}/purchase/purchaseInboundAction!goSingleList.action"> <img src="img/mail.png" alt="" /> 采购到货单
					</a></li>
				</ul>

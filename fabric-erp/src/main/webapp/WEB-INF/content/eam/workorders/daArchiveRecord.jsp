...
<script src="${vix}/common/core/js/inc_tab_nav.js" type="text/javascript"></script>
...
function goSearchEquipment(){
	...
	_tabShow(searchTabTitle,"${vix}/eam/workOrdersAction!daArchiveRecordList.action?name="+searchKW,'_p_search');
}
...
function saveOrUpdate(id,parentId){
	if(id>0){
		_tabShow('工单信息','${vix}/eam/workOrdersAction!daArchiveRecordDetail.action','_p_saveOrUpdate');
	}
}
	...
	$.ajax({
		  ...
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/eam/workOrdersAction!daArchiveRecordList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/eam/workOrdersAction!daArchiveRecordList.action?name=",'category');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/eam/workOrdersAction!daArchiveRecordList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
...
function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/eam/workOrdersAction!daArchiveRecordList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
<!--
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/eam/workOrdersAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/eam/workOrdersAction!daArchiveRecordList.action?parentId="+treeNode.id,"category");
				}
			...
			<div id="tabNameDiv" class="right_menu nav_tabs">
				<div class="nav_tab">
					<div class="right_menu_tab">
						<ul>
							<li pageId="category" class="current"><a onclick="categoryTab(this)"><img src="img/mail.png" alt="" />工单档案管理</a></li>
						</ul>
					</div>

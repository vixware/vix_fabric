...
function goSearchEquipment(){
	...
	var searchTabTitle = searchKW?'搜索-'+searchKW:'搜索服务合同';
	_tabShow(searchTabTitle,"${vix}/eam/accountManageAction!bxServiceInfoMgrList.action?name="+searchKW,'_p_search');
}
...
function saveOrUpdate(id,parentId){
	...
	if(id>0)
		title = '合同信息';
	var newPageId = _tabShow(title,'${vix}/eam/accountManageAction!bxServiceInfoMgrDetail.action','_p_saveOrUpdate');
}
	...
	$.ajax({
		  ...
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/eam/accountManageAction!bxServiceInfoMgrList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/eam/accountManageAction!bxServiceInfoMgrList.action?name=",'category');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/eam/accountManageAction!bxServiceInfoMgrList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
...
function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/eam/accountManageAction!bxServiceInfoMgrList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div>
			<label><input type="text" class="int more" placeholder="设备编号 或 合同编号" id="nameS"></label> <label> <input name="searchBtn" id="searchBtn" onclick="goSearchEquipment()" type="button" class="btn" value="<s:text name='cmn_search'/>" /> <input name="" type="button" onclick="javascript:return clearSearch()" class="btn"
...
<!--
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/eam/accountManageAction!bxServiceInfoMgrList.action?parentId="+treeNode.id,"category");
				}
			...
			<div id="tabNameDiv" class="right_menu nav_tabs">
				<div class="nav_tab">
					<div class="right_menu_tab">
						<ul>
							<li pageId="category" class="current"><a onclick="categoryTab(this)"><img src="img/mail.png" alt="" />保修服务合同</a></li>
						</ul>
					</div>

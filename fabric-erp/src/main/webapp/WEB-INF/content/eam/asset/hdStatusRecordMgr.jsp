...
function saveOrUpdate(id,parentId){
	...
	var title = '设备活动记录';
	var newPageId = _tabShow(title,'${vix}/eam/accountManageAction!hdStatusRecordMgrDetail.action','_p_saveOrUpdate');
}
	...
	$.ajax({
		  ...
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/eam/accountManageAction!hdStatusRecordMgrList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/eam/accountManageAction!hdStatusRecordMgrList.action?name=",'category');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/eam/accountManageAction!hdStatusRecordMgrList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
...
function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/eam/accountManageAction!hdStatusRecordMgrList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
			...
			<div id="tabNameDiv" class="right_menu nav_tabs">
				<div class="nav_tab">
					<div class="right_menu_tab">
						<ul>
							<li pageId="category" class="current"><a onclick="categoryTab(this)"><img src="img/mail.png" alt="" />设备活动管理</a></li>
						</ul>
					</div>

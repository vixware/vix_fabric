...
function goSearchEquipment(){
	...
	var searchTabTitle = searchKW?'搜索-'+searchKW:'搜索备件';
	_tabShow(searchTabTitle,"${vix}/eam/accountManageAction!sbRepairPartMgrList.action?name="+searchKW,'_p_search');
}
...
function saveOrUpdate(id,parentId){
	...
	if(id>0)
		title = '备件信息';
	var newPageId = _tabShow(title,'${vix}/eam/accountManageAction!sbRepairPartMgrDetail.action','_p_saveOrUpdate');
}
	...
	$.ajax({
		  ...
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/eam/accountManageAction!sbRepairPartMgrList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/eam/accountManageAction!sbRepairPartMgrList.action?name=",'category');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/eam/accountManageAction!sbRepairPartMgrList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
...
function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/eam/accountManageAction!sbRepairPartMgrList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
<!--
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/eam/accountManageAction!sbRepairPartMgrList.action?parentId="+treeNode.id,"category");
				}
			...
			<div id="tabNameDiv" class="right_menu nav_tabs">
				<div class="nav_tab">
					<div class="right_menu_tab">
						<ul>
							<li pageId="category" class="current"><a onclick="categoryTab(this)"><img src="img/mail.png" alt="" />备件管理</a></li>
						</ul>
					</div>

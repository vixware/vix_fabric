...
function goSearchEquipment(){
	...
	_tabShow(searchTabTitle,"${vix}/eam/accountManageAction!gzErrorCodeMgrList.action?name="+searchKW,'_p_search');
}
...
function showError(id,code){
	var title = '新增故障信息';
	var pageId = '_p_saveOrUpdate';
	if(id>0){
		title = '故障-'+code;
		pageId = null;
	}
	var newPageId = _tabShow(title,'${vix}/eam/accountManageAction!gzErrorCodeMgrDetail.action',pageId);
}
	...
	$.ajax({
		  ...
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/eam/accountManageAction!gzErrorCodeMgrList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/eam/accountManageAction!gzErrorCodeMgrList.action?name=",'category');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/eam/accountManageAction!gzErrorCodeMgrList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
...
function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/eam/accountManageAction!gzErrorCodeMgrList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div>
			<label><input type="text" class="int more" placeholder="设备编号 或  故障编码" id="nameS"></label> <label> <input name="searchBtn" id="searchBtn" onclick="goSearchEquipment()" type="button" class="btn" value="<s:text name='cmn_search'/>" /> <input name="" type="button" onclick="javascript:return clearSearch()" class="btn"
				value="<s:text name='cmn_reset'/>" />
			</label> <strong id="lb_search_advanced"><s:text name='cmn_advance_search' /></strong> <label style="margin-left: 30px;"> <input name="" type="button" onclick="showError(0,'');" class="btn" value="<s:text name="cmn_add"/>" />
			</label>
		</div>
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="categoryList" var="c">
				<li><a href="#"
					onclick="showError(${c.id},'${c.name}'>${c.name}</a></li>
        	</s:iterator>
        </ul>
    </div>
	<!-- c_head -->
	<div class="box">
		<div id="left" class="ui-resizable" style="height:500px;">
			...
			<div class="left_content" style="height:500px;">
				<div style="padding-left:4px;"><img src="${vix}/common/img/file.png" style="padding-right:5px;"/><a href="#" onclick="loadRoot();"><s:text name='cmn_rootDirectory'/></a></div>
...
<!--
				...
				var setting = {
					async: {
						enable: true,
						url:"
					${vix}/eam/accountManageAction!findTreeToJson.action",
				...
				function
					...
					pager("start","${vix}/eam/accountManageAction!gzErrorCodeMgrList.action?parentId="+treeNode.id," category");
				}
		...
		<div id="tabNameDiv" class="right_menu nav_tabs">
			<div class="nav_tab">
				<div class="right_menu_tab">
					<ul>
						<li pageId="category" class="current"><a onclick="categoryTab(this)"><img src="img/mail.png" alt="" />故障体系管理</a></li>
					</ul>
				</div>

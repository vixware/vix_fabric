...
function goSearchEquipment(){
	...
	_tabShow(searchTabTitle,"${vix}/eam/faultReportAction!spReviewList.action?name="+searchKW,'_p_search');
}
...
function saveOrUpdate(id,parentId){
	if(id>0){
		_tabShow('缺陷报告详情','${vix}/eam/faultReportAction!spReviewDetail.action','_p_saveOrUpdate');
	}
}
	...
	$.ajax({
		  ...
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/eam/faultReportAction!spReviewList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/eam/faultReportAction!spReviewList.action?name=",'category');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/eam/faultReportAction!spReviewList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
...
function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/eam/faultReportAction!spReviewList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
			...
			<div id="tabNameDiv" class="right_menu nav_tabs">
				<div class="nav_tab">
					<div class="right_menu_tab">
						<ul>
							<li pageId="category" class="current"><a onclick="categoryTab(this)"><img src="img/mail.png" alt="" />故障报告审批</a></li>
						</ul>
					</div>

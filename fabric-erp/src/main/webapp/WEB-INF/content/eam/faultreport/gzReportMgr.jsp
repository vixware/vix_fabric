...
function goSearchEquipment(){
	...
	_tabShow(searchTabTitle,"${vix}/eam/faultReportAction!gzReportMgrList.action?name="+searchKW,'_p_search');
}
...
function saveOrUpdate(id,parentId){
	...
	if(id>0)
		title = '故障报告信息';
	var newPageId = _tabShow(title,'${vix}/eam/faultReportAction!gzReportMgrDetail.action','_p_saveOrUpdate');
}

function deleteById(id){
	$.ajax({
		  url:'${vix}/eam/faultReportAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/eam/faultReportAction!gzReportMgrList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/eam/faultReportAction!gzReportMgrList.action?name=",'category');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/eam/faultReportAction!gzReportMgrList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
...
function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/eam/faultReportAction!gzReportMgrList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">故障与缺陷管理</a></li>
				<li><a href="#">故障缺陷报告/维修通知单</a></li>
			</ul>
		</div>
	</h2>
</div>
...
<!--
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/eam/faultReportAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/eam/faultReportAction!gzReportMgrList.action?parentId="+treeNode.id,"category");
				}
			...
			<div id="tabNameDiv" class="right_menu nav_tabs">
				<div class="nav_tab">
					<div class="right_menu_tab">
						<ul>
							<li pageId="category" class="current"><a onclick="categoryTab(this)"><img src="img/mail.png" alt="" />故障缺陷报告</a></li>
						</ul>
					</div>

...
function goSearchEquipment(){
	...
	_tabShow(searchTabTitle,"${vix}/eam/preMaintainAction!jkInfoMgrList.action?name="+searchKW,'_p_search');
}
...
function saveOrUpdate(id,parentId){
	if(id>0){
		var title = '计划任务信息';
		var newPageId = _tabShow(title,'${vix}/eam/preMaintainAction!jkInfoMgrDetail.action','_p_saveOrUpdate');
	}
}
	...
	$.ajax({
		  ...
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/eam/preMaintainAction!jkInfoMgrList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/eam/preMaintainAction!jkInfoMgrList.action?name=",'category');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/eam/preMaintainAction!jkInfoMgrList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
...
function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/eam/preMaintainAction!jkInfoMgrList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
<!--
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/eam/preMaintainAction!jkInfoMgrList.action?parentId="+treeNode.id,"category");
				}
			...
			<div id="tabNameDiv" class="right_menu nav_tabs">
				<div class="nav_tab">
					<div class="right_menu_tab">
						<ul>
							<li pageId="category" class="current"><a onclick="categoryTab(this)"><img src="img/mail.png" alt="" />计划监控调整</a></li>
						</ul>
					</div>

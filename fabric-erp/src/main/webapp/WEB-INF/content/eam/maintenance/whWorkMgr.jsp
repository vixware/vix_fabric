...
function goSearchEquipment(){
	...
	_tabShow(searchTabTitle,"${vix}/eam/preMaintainAction!whWorkMgrList.action?name="+searchKW,'_p_search');
}
...
function saveOrUpdate(id,parentId){
	...
	if(id>0)
		title = '维护计划信息';
	var newPageId = _tabShow(title,'${vix}/eam/preMaintainAction!whWorkMgrDetail.action','_p_saveOrUpdate');
}
	...
	$.ajax({
		  ...
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/eam/preMaintainAction!whWorkMgrList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/eam/preMaintainAction!whWorkMgrList.action?name=",'category');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/eam/preMaintainAction!whWorkMgrList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
...
function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/eam/preMaintainAction!whWorkMgrList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
<!--
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/eam/preMaintainAction!whWorkMgrList.action?parentId="+treeNode.id,"category");
				}
			...
			<div id="tabNameDiv" class="right_menu nav_tabs">
				<div class="nav_tab">
					<div class="right_menu_tab">
						<ul>
							<li pageId="category" class="current"><a onclick="categoryTab(this)"><img src="img/mail.png" alt="" />维护任务管理</a></li>
						</ul>
					</div>

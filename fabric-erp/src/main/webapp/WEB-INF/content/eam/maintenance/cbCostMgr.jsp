...
function goSearchEquipment(){
	...
	_tabShow(searchTabTitle,"${vix}/eam/preMaintainAction!cbCostMgrList.action?name="+searchKW,'_p_search');
}
...
function saveOrUpdate(id,parentId){
	if(id>0){
		var title = '成本估算';
		var newPageId = _tabShow(title,'${vix}/eam/preMaintainAction!cbCostMgrDetail.action','_p_saveOrUpdate');
	}
}
	...
	$.ajax({
		  ...
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/eam/preMaintainAction!cbCostMgrList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/eam/preMaintainAction!cbCostMgrList.action?name=",'category');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/eam/preMaintainAction!cbCostMgrList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
...
function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/eam/preMaintainAction!cbCostMgrList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
<!--
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/eam/preMaintainAction!cbCostMgrList.action?parentId="+treeNode.id,"category");
				}
			...
			<div id="tabNameDiv" class="right_menu nav_tabs">
				<div class="nav_tab">
					<div class="right_menu_tab">
						<ul>
							<li pageId="category" class="current"><a onclick="categoryTab(this)"><img src="img/mail.png" alt="" />维护成本估算</a></li>
						</ul>
					</div>

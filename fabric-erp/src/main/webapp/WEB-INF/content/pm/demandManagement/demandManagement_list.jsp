...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:'${vix}/pm/demandManagementAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							$.post('${vix}/pm/demandManagementAction!saveOrUpdate.action',
								 {'demandManagement.id':$("#id").val(),
								  'demandManagement.parentCategory.id':$("#parentId").val(),
								  'demandManagement.name':$("#name").val(),
								  'demandManagement.description':$("#description").val(),
								  'demandManagement.serviceCondition':$("#serviceCondition").val(),
								  'demandManagement.consumeRate':$("#consumeRate").val(),
								  'demandManagement.consumeUnit':$("#consumeUnit").val(),
								  'demandManagement.priority':$("#priority").val(),
								  'demandManagement.expectedTime':$("#expectedTime").val(),
								  'demandManagement.review':$("#review").val(),
								  'demandManagement.lifetime':$("#lifetime").val(),
								  'demandManagement.status': $(":radio[name=status][checked]").val(),
								  'demandManagement.memo':$("#memo").val()								 
								},
								function(result){
									asyncbox.success(result,"<s:text name='message'/>",function(action){
										...
										pager("start","${vix}/pm/demandManagementAction!goSingleList.action?name="+name+"&id="+$("#selectId").val(),'category');
									});
								}
							 );
						}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/pm/demandManagementAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='message'/>",function(action){
				...
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/pm/demandManagementAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
function searchForContent(i){
	...
	loadMemo();
	if(i == 0){
		pager("start","${vix}/pm/demandManagementAction!goSearchList.action?i="+i+"&searchContent="+name,'category');
	}
	else{
		pager("start","${vix}/pm/demandManagementAction!goSearchList.action?i="+i+"&name="+name+"&memo="+memo,'category');
	}
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/pm/demandManagementAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/pm/demandManagementAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
bindSearch();
bindSwitch();
...
//加载顶部工具栏
loadTopDynamicMenuContent('${vix}/pm/demandManagementAction!goTopDynamicMenuContent.action');
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="需求管理" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/pm/demandManagementAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/pm/demandManagementAction!goSingleList.action?parentId="+treeNode.id,"category");
				}

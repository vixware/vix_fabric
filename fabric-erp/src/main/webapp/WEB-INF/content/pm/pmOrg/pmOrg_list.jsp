...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:'${vix}/pm/org/pmOrgAction!goSaveOrUpdate.action?id='+id+'&parentIdStr='+parentId,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							var queryString = $('#pmOrgForm').formSerialize(); 
							$.post('${vix}/pm/org/pmOrgAction!saveOrUpdate.action',
								queryString,
								function(result){
									...
									asyncbox.success(msg,"<s:text name='message'/>"); */
									 asyncbox.success(result,"<s:text name='message'/>",function(action){
										 loadContent('${vix}/pm/org/pmOrgAction!goList.action');
										/*  var treeObj = $.fn.zTree.getZTreeObj("tree_root");
										 debugger;
										 treeObj.refresh();
										 alert("vvv"); */
										//zTree.refresh();
										/* vixAjaxSend("${vix}/pm/org/pmOrgAction!findOrgAndUnitTreeToJson.action","a=1",function(data){
											var treeData = $.parseJSON(data);
											zTree = $.fn.zTree.init($("#tree_root"), setting,treeData);
										});  */
										//pager("start","${vix}/pm/org/pmOrgAction!goSingleList.action?businessName="+name+"&treeId="+$("#selectId").val(),'bizOrg');
									}); 
								}
							 );
						}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/pm/org/pmOrgAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			  loadContent('${vix}/pm/org/pmOrgAction!goList.action');
			/* asyncbox.success(html,"<s:text name='message'/>",function(action){
				...
				pager("start","${vix}/pm/org/pmOrgAction!goSingleList.action?businessName="+name+"&id="+$("#selectId").val(),'bizOrg');
			}); */
		  }
	});
}
...
function searchForRightContent(){
	loadName();
	pager("start","${vix}/pm/org/pmOrgAction!goSingleList.action?businessName="+name,'bizOrg');
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/pm/org/pmOrgAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&businessName="+name+"&parentId="+$("#selectId").val(),'bizOrg');
}

function bizOrgPager(tag,entity){
	loadName();
	if(entity == 'bizOrg'){
		pager(tag,"${vix}/pm/org/pmOrgAction!goSingleList.action?businessName="+name+'&parentId='+$('#selectId').val(),entity);
	}
	/* if(entity == 'brand'){
...
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#"><s:text name="供应链" /></a></li>
				<li><a href="#"><s:text name="项目管理" /></a></li>
				<li><a href="#"><s:text name="项目用户" /></a></li>
				<li><a href="#">项目成员</a></li>
			</ul>
		</div>
	</h2>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<ul>
			...
			<li><a href="#"><img src="img/icon_10.png" />
		...
		<div>
			<label><s:text name="cmn_name" /><input type="text" class="int" id="businessName" value=""></label> <label><s:text name="cmn_my_item" /><input type="checkbox" value="" name=""></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForRightContent();" /><input type="button"
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			/* 	var zTree;			
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/pm/org/pmOrgAction!goSingleList.action?id="+treeNode.id,"bizOrg");
				}
				
				
				$(document).ready(function(){
					vixAjaxSendHtml("${vix}/pm/org/pmOrgAction!findTreeToJson.action","a=1",function(data){
						var treeData = $.parseJSON(data);
						zTree = $.fn.zTree.init($("#tree_root"), setting,treeData);
					});
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/pm/org/pmOrgAction!goSingleList.action?treeId="+treeNode.treeId+"&treeType="+treeNode.treeType,"bizOrg");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="bizOrgTab(3,1,'a',event,'bizOrg')"><img src="img/mail.png" alt="" />项目成员</a></li>

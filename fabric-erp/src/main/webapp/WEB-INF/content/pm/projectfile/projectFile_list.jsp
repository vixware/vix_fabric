...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:'${vix}/pm/projectFileAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							$.post('${vix}/pm/projectFileAction!saveOrUpdate.action',
								 {'projectFile.id':$("#id").val(),
								  'projectFile.parentCategory.id':$("#parentId").val(),
								  'projectFile.name':$("#name").val(),
								  'projectFile.linkman':$("#linkman").val(),
								  'projectFile.phone':$("#phone").val(),
								  'projectFile.rank':$("#rank").val(),
								  'projectFile.createDate':$("#createDate").val(),
								  'projectFile.status': $(":radio[name=status][checked]").val(),
								  'projectFile.memo':$("#memo").val()								 
								},
								function(result){
									asyncbox.success(result,"<s:text name='项目文件管理'/>",function(action){
										var treeNode = zTree.getNodeByTId($("#selectIdTreeId").val());
										if(null != treeNode){
											treeNode.isParent = true;
										}
										zTree.reAsyncChildNodes(treeNode, "refresh");
										pager("start","${vix}/pm/projectFileAction!goSingleList.action?name="+name+"&id="+$("#selectId").val(),'category');
									});
								}
							 );
						}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/pm/projectFileAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='message'/>",function(action){
				...
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/pm/projectFileAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
function searchForContent(i){
	...
	loadMemo();
	if(i == 0){
		pager("start","${vix}/pm/projectFileAction!goSearchList.action?i="+i+"&searchContent="+name,'category');
	}
	else{
		pager("start","${vix}/pm/projectFileAction!goSearchList.action?i="+i+"&name="+name+"&memo="+memo,'category');
	}
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/pm/projectFileAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/pm/projectFileAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
bindSearch();
bindSwitch();
...
//加载顶部工具栏
loadTopDynamicMenuContent('${vix}/pm/projectFileAction!goTopDynamicMenuContent.action');
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="项目文件管理" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/pm/projectFileAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/pm/projectFileAction!goSingleList.action?parentId="+treeNode.id,"category");
				}

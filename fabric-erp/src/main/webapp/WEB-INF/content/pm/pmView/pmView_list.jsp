...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/pm/org/pmViewAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
			  ...
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							...
							$("#pmViewForm").validationEngine('validate');
							var queryString = $('#pmViewForm').formSerialize(); 
							$.post('${vix}/pm/org/pmViewAction!saveOrUpdate.action',
								queryString,
								function(result){
									asyncbox.success(result,"<s:text name='message'/>",function(action){
										pager("start","${vix}/pm/org/pmViewAction!goSingleList.action?businessViewName="+name,'pmView');
									});
								}
							 );
						}
		...
		if(action == 'ok'){
			...
			$.post('${vix}/pm/org/pmViewAction!saveOrUpdate.action',
				...
				function(result){
					asyncbox.success(result,"<s:text name='message'/>",function(action){
						if(action == 'ok'){
							loadContent('${vix}/pm/org/pmViewAction!goList.action?title=${title}');
						}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/pm/org/pmViewAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
...
function orderBy(orderField){
	...
	pager("start","${vix}/pm/org/pmViewAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&businessViewName="+name,'pmView');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/pm/org/pmViewAction!goSingleList.action?businessViewName="+name,'pmView');
}
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div>
			<label><s:text name="cmn_name" /><input type="text" class="int" id="businessViewTitle" value="${businessViewTitle}"></label> <label><s:text name="cmn_my_item" /><input type="checkbox" value="" name=""></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn" onclick="searchForContent();" /><input

...
function saveOrUpdate(id){
	$.ajax({
		  url:"${vix}/pm/documentationAction!goSaveOrUpdate.action?id="+id,
		  cache: false,
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#brandForm').validationEngine('validate')){
								$.post('${vix}/pm/documentationAction!saveOrUpdate.action',
									 {'scheduleQueries.id':$("#id").val(),									  
									  'scheduleQueries.startDate':$("#startDate").val(),						  
									  'scheduleQueries.endDate':$("#endDate").val(),						  
									  'scheduleQueries.scheduleContent':$("#scheduleContent").val(),						  
									  'scheduleQueries.explanation':$("#explanation").val(),						  
									  'scheduleQueries.remark':$("#remark").val()						  
									},
									function(result){
										asyncbox.open({
											modal:true,
											...
											html:result,
											callback : function(action){
												pager("current","${vix}/pm/documentationAction!goList.action?name="+name,'brand');
											},
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/pm/documentationAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/pm/documentationAction!goListContent.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/pm/documentationAction!goListContent.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/pm/documentationAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/pm/documentationAction!goListContent.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="文档管理" /></a></li>
				<li><a href="#"><s:text name="文档列表" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0,$('#selectId').val());"><span><s:text name="上传新文件" /></span></a>
		</p>
	</div>
</div>
	...
	<div class="box">
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/pm/documentationAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/pm/documentationAction!goListContent.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/mail.png" alt="" />文档明细</a></li>
				</ul>
			</div>

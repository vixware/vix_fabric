...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:'${vix}/hr/backupResumeDatabaseAciton!goSaveOrUpdate.action?id='+id,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/backupResumeDatabaseAciton!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/hr/backupResumeDatabaseAciton!goListContent.action?name="+name,'category');
			});
		  }
	});
}
...
loadName();
//载入分页列表数据
pager("start","${vix}/hr/backupResumeDatabaseAciton!goListContent.action?name="+name+"&orderField=id&orderBy=desc",'category');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/hr/backupResumeDatabaseAciton!goListContent.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/hr/backupResumeDatabaseAciton!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/hr/backupResumeDatabaseAciton!goListContent.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
/* 加载顶部工具栏 */
loadTopDynamicMenuContent('${vix}/hr/backupResumeDatabaseAciton!goTopDynamicMenuContent.action');
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/hr/backupResumeDatabaseAciton!goList.action');">后备简历库</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div class="search_advanced">
			<label><s:text name="sa_ids" /><input type="text" class="int" name=""></label> <label><s:text name="sa_subjects" /><input type="text" class="int" name=""></label>
		</div>
	</div>
	<div id="number">
	...
	<div class="box">
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/hr/backupResumeDatabaseAciton!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/hr/backupResumeDatabaseAciton!goListContent.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/common_listx16.png" alt="" />后备简历库</a></li>
				</ul>
			</div>

...
var recommendedDepartment ="";
function loadName(){
	name = $('#nameS').val();
	name=Url.encode(name);
	name=Url.encode(name);
}
function loadApplicantsName(){
	applicantsName = $('#hr_applicantsName').val();
	applicantsName = Url.encode(applicantsName);
	applicantsName = Url.encode(applicantsName);
}
...
function loadRecommendedDepartment(){
	recommendedDepartment = $('#hr_recommendedDepartment').val();
	recommendedDepartment = Url.encode(recommendedDepartment);
	recommendedDepartment = Url.encode(recommendedDepartment);
}
...
function searchForContent(i){
	loadName();
	pager("start","${vix}/hr/seniorReAction!goSearchList.action?i="+i+"&applicantsName="+name,'category');
}

//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/hr/seniorReAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/hr/seniorReAction!goSearchList.action?applicantsName=" + $('#applicantsName').val() + "&employmentObjective=" + $('#employmentObjective').val() + "&applicantsDepartment=" + $('#applicantsDepartment').val() + "&recommendedDepartment=" + $('#recommendedDepartment').val(), 'category');
			}
		},
...
//状态
function saleOrderStatus(conclusion){
	pager("start","${vix}/hr/seniorReAction!goSingleList.action?conclusion="+conclusion,'category');
}
//最近使用
function leastRecentlyUsed(createTime){
	pager("start","${vix}/hr/seniorReAction!goSingleList.action?createTime="+createTime,'category');
}
...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:'${vix}/hr/seniorReAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/seniorReAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/hr/seniorReAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
loadName();
//载入分页列表数据
pager("start","${vix}/hr/seniorReAction!goSingleList.action?name="+name+"&orderField=id&orderBy=desc",'category');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/hr/seniorReAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/hr/seniorReAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/hr/seniorReAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/hr/seniorReAction!goList.action');"><s:text name="hr_recruitment_mgs" /></a></li>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<ul>
			...
			<li class="fly"><a href="#"><img alt="" src="img/icon_10.png"> <s:text name="结论" /> </a>
				<ul>
			...
			<li class="fly"><a href="#"><img alt="" src="img/leastRecentlyUsed.png"> <s:text name="最近招聘" /> </a>
				<ul>
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			...
			<s:iterator value="hrSeniorsList" var="c">
				<li><a href="#" onclick="saveOrUpdate(${c.id},$('#selectId').val());"><span style="display: none;">${c.chineseFirstLetter}</span>${c.applicantsName}</a></li>
			</s:iterator>
		</ul>
	</div>
	<!-- c_head -->
		...
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/hr/seniorReAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/hr/seniorReAction!goSingleList.action?parentId="+treeNode.id,"category");
				}

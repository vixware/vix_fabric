...
function loadContact(){
	contact = $('#hr_contact').val();
	contact = Url.encode(contact);
	contact = Url.encode(contact);
}
...
function searchForContent(i){
	loadName();
	pager("start","${vix}/hr/audiManageAction!goSearchList.action?i="+i+"&candidateName="+name,'category');
}

//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/hr/audiManageAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/hr/audiManageAction!goSearchList.action?candidateName=" + $('#candidateName').val() + "&contact=" + $('#contact').val() + "&employmentObjective=" + $('#employmentObjective').val() + "&applicantsDepartment=" + $('#applicantsDepartment').val(), 'category');
			}
		},
...
//状态
function saleOrderStatus(employmentSituation){
	pager("start","${vix}/hr/audiManageAction!goSingleList.action?employmentSituation="+employmentSituation,'category');
}
//最近使用
function leastRecentlyUsed(createTime){
	pager("start","${vix}/hr/audiManageAction!goSingleList.action?createTime="+createTime,'category');
}
...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:'${vix}/hr/audiManageAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/audiManageAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/hr/audiManageAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
loadName();
//载入分页列表数据
pager("start","${vix}/hr/audiManageAction!goSingleList.action?name="+name+"&orderField=id&orderBy=desc",'category');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/hr/audiManageAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/hr/audiManageAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/hr/audiManageAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
/* 加载顶部工具栏 */
loadTopDynamicMenuContent('${vix}/hr/audiManageAction!goTopDynamicMenuContent.action');
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="hr_selection_hired" /></a></li>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<ul>
			...
			<li class="fly"><a href="#"><img alt="" src="img/icon_10.png"> <s:text name="录用情况" /> </a>
				<ul>
					...
					<li><a href="#" onclick="saleOrderStatus('3')"><img alt="" src="img/unaudited.png"> <s:text name="待定" /> </a></li>
				</ul></li>
		...
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/hr/audiManageAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/hr/audiManageAction!goSingleList.action?parentId="+treeNode.id,"category");
				}
			...
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/common_listx16.png" alt="" />
						<s:text name='hr_audition_manage' /></a></li>
				</ul>
			</div>

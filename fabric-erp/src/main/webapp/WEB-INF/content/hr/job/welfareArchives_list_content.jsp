	...
	<tbody>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td><span style="color: gray;">${entity.employeeCode}</span></td>
				<td><span style="color: gray;">${entity.employeeName}</span></td>
				<td><span style="color: gray;">${entity.welfareDepartment}</span></td>
				<td><span style="color: gray;"> <c:if test="${null == entity.whetherIntoAccount }">
						</c:if> <c:if test="${entity.whetherIntoAccount == '1' }">
							是
						</c:if> <c:if test="${entity.whetherIntoAccount == '2' }">
							否
						</c:if>
				</span></td>
				<td><span style="color: gray;"> <c:if test="${null == entity.accountTypes }">
						</c:if> <c:if test="${entity.accountTypes == '1' }">
							已开通
						</c:if> <c:if test="${entity.accountTypes == '2' }">
							未开通
						</c:if>
				</span></td>
				<td><a href="#" style="color: gray;"><fmt:formatDate value="${entity.transferredDate}" type="both" pattern="yyyy-MM-dd" /></a></td>
				<td>
					<div class="untitled" style="position: static; display: inline;">
						<span><img alt="" src="img/icon_untitled.png"></span>
						<div class="popup" style="display: none; top: -7px;">
							<strong> <i class="close"><a href="#" onclick="deleteEntity(${entity.id });"></a></i> <i><a href="#" title="<s:text name='cmn_show'/>"></a></i> <i><a href="#" onclick="saveOrUpdate(${entity.id });" title="<s:text name='cmn_update'/>"></a></i> <!-- 小i标题 --> <b>我是标题</b>
							</strong>
							<!-- 小i内容 -->
							<p>${entity.id }${entity.employeeName }</p>
						</div>
					</div>
				</td>
			</tr>

...
function loadApplicationNumber(){
	applicationNumber = $('#hr_applicationNumber').val();
	applicationNumber = Url.encode(applicationNumber);
	applicationNumber = Url.encode(applicationNumber);
}
...
function loadApplicationDepartment(){
	applicationDepartment = $('#hr_applicationDepartment').val();
	applicationDepartment = Url.encode(applicationDepartment);
	applicationDepartment = Url.encode(applicationDepartment);
}
function loadJobApplication(){
	jobApplication = $('#hr_jobApplication').val();
	jobApplication = Url.encode(jobApplication);
	jobApplication = Url.encode(jobApplication);
}
...
function resetForContent(i){
	if(i == 0){
		...
		$("#hr_applicationNumber").val("");
		$("#hr_applicationName").val("");
		$("#hr_applicationDepartment").val("");
		$("#hr_jobApplication").val("");
	}
}
...
function searchForContent(i){
	loadName();
	pager("start","${vix}/hr/empAAppAction!goSearchList.action?i="+i+"&applicationNumber="+name,'category');
}

//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/hr/empAAppAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/hr/empAAppAction!goSearchList.action?applicationNumber=" + $('#applicationNumber').val() + "&applicationName=" + $('#applicationName').val() + "&applicationDepartment=" + $('#applicationDepartment').val() + "&jobApplication=" + $('#jobApplication').val(), 'category');
			}
		},
...
function saleOrderStatus(approvalStatus){
	pager("start","${vix}/hr/empAAppAction!goSingleList.action?approvalStatus="+approvalStatus,'category');
}
//最近使用
function leastRecentlyUsed(createTime){
	pager("start","${vix}/hr/empAAppAction!goSingleList.action?createTime="+createTime,'category');
}
...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:'${vix}/hr/empAAppAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/empAAppAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/hr/empAAppAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
loadName();
//载入分页列表数据
pager("start","${vix}/hr/empAAppAction!goSingleList.action?name="+name+"&orderField=id&orderBy=desc",'category');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/hr/empAAppAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/hr/empAAppAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/hr/empAAppAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
/* 加载顶部工具栏 */
loadTopDynamicMenuContent('${vix}/hr/empAAppAction!goTopDynamicMenuContent.action');
</script>
<div class="sub_menu">
	<h2>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<ul>
			...
			<li class="fly"><a href="#"><img alt="" src="img/leastRecentlyUsed.png"> <s:text name="最近用人申请" /> </a>
				<ul>
		...
		<!-- 页面左上角按钮部分 -->
		<div>
			<label>申请编号: <input type="text" class="int" id="nameS" style="width: 150px;"> <input type="button" value="搜索" class="btn" onclick="searchForContent(0);" />
		...
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/hr/empAAppAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/hr/empAAppAction!goSingleList.action?parentId="+treeNode.id,"category");
				}
			...
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/common_listx16.png" alt="" />
						<s:text name='hr_emp_an_application' /></a></li>
				</ul>
			</div>

...
function saveOrUpdate(id,parentId){
	<%--
	$.ajax({
		  url:'${vix}/hr/employmentSituationAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							$.post('${vix}/hr/employmentSituationAction!saveOrUpdate.action',
								 {'category.id':$("#id").val(),
								...
								function(result){
									asyncbox.success(result,"<s:text name='cmn_message'/>",function(action){
										...
										pager("start","${vix}/hr/employmentSituationAction!goListContent.action?name="+name+"&id="+$("#selectId").val(),'category');
									});
								}
							 );
						}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/employmentSituationAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/hr/employmentSituationAction!goListContent.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/hr/employmentSituationAction!goListContent.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/hr/employmentSituationAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/hr/employmentSituationAction!goListContent.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
		...
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/hr/employmentSituationAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/hr/employmentSituationAction!goListContent.action?parentId="+treeNode.id,"category");
				}

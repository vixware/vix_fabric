...
function loadSendObj(){
	sendObj = $('#hr_sendObj').val();
	sendObj = Url.encode(sendObj);
	sendObj = Url.encode(sendObj);
}
...
function searchForContent(i){
	loadName();
	pager("start","${vix}/hr/reCollAction!goSearchList.action?i="+i+"&noticeTheName="+name,'category');
}

//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/hr/reCollAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/hr/reCollAction!goSearchList.action?noticeTheName=" + $('#noticeTheName').val() + "&companyOrDepartment=" + $('#companyOrDepartment').val() + "&sendObj=" + $('#sendObj').val() + "&uploadPersonName=" + $('#uploadPersonName').val(), 'category');
			}
		},
...
function leastRecentlyUsed(noticeStartTime){
	pager("start","${vix}/hr/reCollAction!goSingleList.action?noticeStartTime="+noticeStartTime,'category');
}
...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/hr/reCollAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/reCollAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/hr/reCollAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
loadName();
//载入分页列表数据
pager("start","${vix}/hr/reCollAction!goSingleList.action?name="+name+"&orderField=id&orderBy=desc",'category');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/hr/reCollAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/hr/reCollAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/hr/reCollAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
/* 加载顶部工具栏 */
loadTopDynamicMenuContent('${vix}/hr/reCollAction!goTopDynamicMenuContent.action');
</script>
<div class="sub_menu">
	<h2>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<ul>
			...
			<li class="fly"><a href="#"><img alt="" src="img/leastRecentlyUsed.png"> <s:text name="最近开始时间" /> </a>
				<ul>
		...
		<div id="left" class="ui-resizable switch" style="height: 500px; width: 7px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/hr/reCollAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/hr/reCollAction!goSingleList.action?parentId="+treeNode.id,"category");
				}

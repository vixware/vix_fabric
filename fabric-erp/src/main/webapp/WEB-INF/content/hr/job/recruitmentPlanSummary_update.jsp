...
function chooseParentCategory(tag){
	$.ajax({
		  url:'${vix}/hr/recruitmentPlanSummaryAction!goChooseCategory.action',
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
						...
						if(action == 'ok'){
							if(returnValue != undefined){
								var result = returnValue.split(",");
									...
									if(tag==0){
									$("#recruitment").val(result[1]);
									}
									else if (tag==1){
										$("#recruitmentDepartment").val(result[1]);
									}
							}else{
								$("#parentId").val("");
								$("#categoryName").html("");
...
function saveOrUpdateOrder(){
	...
	if($('#replanForm').validationEngine('validate')){
		$.post('${vix}/hr/recruitmentPlanSummaryAction!saveOrUpdate.action',
			{
			...
			function(result){
				showMessage(result);
				setTimeout("",1000);
				loadContent('${vix}/hr/recruitmentPlanSummaryAction!goList.action?menuId=menuOrder');
			}
		 );
	}
}
	...
	if($('#replanForm').validationEngine('validate')){
		$.post('${vix}/hr/recruitmentPlanSummaryAction!saveOrUpdate.action',
			...
			function(result){
				showMessage(result);
				setTimeout("",1000);
				loadContent('${vix}/hr/recruitmentPlanSummaryAction!goSaveOrUpdate.action');
			}
		 );
	}
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/hr/recruitmentPlanSummaryAction!goList.action');">招聘管理</a></li>
				<li><a href="#" onclick="loadContent('${vix}/hr/recruitmentPlanSummaryAction!goList.action');">招聘计划</a></li>
				<li><a href="#" onclick="loadContent('${vix}/hr/recruitmentPlanSummaryAction!goList.action');">汇总</a></li>
			</ul>
		</div>
	</h2>
</div>
			...
			<dl>
				<dt id="orderTitleFixd">
					<span class="no_line"> <a onclick="saveOrUpdateOrder();"><img width="22" height="22" title="保存" src="${vix}/common/img/dt_save.png"></a> <a href="javascript:void(0)" onclick="saveOrAdd()"><img width="22" height="22" title="保存并新增" src="${vix}/common/img/dt_saveandupdate.png" /></a> <a href="#"
						onclick="loadContent('${vix}/hr/recruitmentPlanSummaryAction!goList.action');"><img width="22" height="22" title="返回" src="${vix}/common/img/dt_goback.png"></a>
					</span> <strong><b><s:text name="新增招聘计划汇总" /></b><i></i> </strong>
				</dt>
				<dd class="clearfix">
								...
								<dd style="display: block;">
									<table style="border: none;">
										<tr>
											...
											<td><input id="recruitment" name="recruitment" value="${hrRecruitmentPlansummary.recruitment}" type="text" size="30" class="validate[required] text-input" /> <input class="btn" type="button" value="选择" onclick="chooseParentCategory(0);" />
											<td align="right">发布时间：</td>
											<td><input id="releaseTime" name="releaseTime" value="<fmt:formatDate value='${hrRecruitmentPlansummary.releaseTime }' type='both' pattern='yyyy-MM-dd' />" readonly="readonly" type="text" /> <img onclick="showTime('releaseTime','yyyy-MM-dd')" src="${vix}/common/img/datePicker.gif" width="16" height="22" align="absmiddle"></td>
										</tr>
										<tr>
											<td align="right">招聘部门：</td>
											<td><input id="recruitmentDepartment" name="recruitmentDepartment" value="${hrRecruitmentPlansummary.recruitmentDepartment}" type="text" size="30" class="validate[required] text-input" /> <input class="btn" type="button" value="选择" onclick="chooseParentCategory(1);" /> <span style="color: red;">*</span></td>
											<td align="right">审核人：</td>
											<td><input id="auditPerson" name="parentId" value="${hrRecruitmentPlansummary.auditPerson}" type="text" size="30" class="validate[required] text-input" /> <input type="hidden" id="parentId" name="parentId" value="${hrCategory.parentCategory.id }" /> <input class="btn" type="button" value="选择" onclick="chooseEmployees();" /> <span
												style="color: red;">*</span></td>
										</tr>
										<tr>
											<td align="right">发布状态：</td>
											<td><select id="releaseState" class="validate[required] text-input">
													<option value="">请选择</option>
													<option value="1">发布</option>
													<option value="2">不发布</option>
											</select> <span style="color: red;">*</span></td>
											<td align="right">发布方式：</td>
											<td><select id="releaseMode" class="validate[required] text-input">
													<option value="">请选择</option>
										...
										<tr>
											...
											<td colspan="2"><textarea id="remarks" name="remarks" class="example" rows="2" style="width: 250px">${hrRecruitmentPlansummary.remarks }</textarea></td>
										</tr>
									</table>
									<script type="text/javascript">
				...
				<div class="clearfix" style="background: #FFF; position: relative;">
					...
					<div id="a1" style="position: relative; z-index: 1; background: #FFF;">
						<div style="padding: 8px;">
							<table id="dlLineItem" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="rownumbers:true,pagination: true,iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlLineItemTb',url: '${vix}/hr/recruitmentPlanSummaryAction!getHrRecruitplanJson.action?id=${hrRecruitmentPlansummary.id}'">
								<thead>
									<tr>
							...
							<script type="text/javascript">
								...
								function baddItem(){
									$.ajax({
										  url:'${vix}/hr/recruitmentPlanSummaryAction!goAddHrRecruitplan.action',
										  cache: false,
										  success: function(html){
													...
													callback : function(action,returnValue){
														if(action == 'ok'){
															if(returnValue != undefined){
																...
																$.ajax({
																	  url:'${vix}/hr/recruitmentPlanSummaryAction!saveOrUpdateHrRecruitplan.action?tcId='+rv[0]+"&id=${hrRecruitmentPlansummary.id}",
																	  cache: false,
																	  success: function(html){
																		  showMessage(html);
																		  setTimeout("",1000);
								...
								function beditItem(){
									...
									$.ajax({
										  url:'${vix}/hr/recruitmentPlanSummaryAction!goAddHrRecruitplan.action?lineItemId='+rows.id,
										  cache: false,
										  success: function(html){
											  asyncbox.open({
												 	modal:true,
													...
													callback : function(action){
														if(action == 'ok'){
															if($('#orderItemForm').validationEngine('validate')){
																$.post('${vix}/hr/recruitmentPlanSummaryAction!saveOrUpdateHrRecruitplan.action', {
																		'id' : $("#id").val(),
																		'hrRecruitplan.id' : $("#oiId").val(),
																		'hrRecruitplan.programName' : $("#programName").val(),
																		'hrRecruitplan.org' : $("#org").val(),
																		'hrRecruitplan.responsibleForOrgUnit' : $("#responsibleForOrgUnit").val(),

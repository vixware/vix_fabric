...
function saveOrUpdate(id,parentId){
	...
	--%>
	$.ajax({
		  url:'${vix}/hr/definingSalaryAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/definingSalaryAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(result){
				...
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/hr/definingSalaryAction!goListContent.action?name="+name,'category');
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/hr/definingSalaryAction!goListContent.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/hr/definingSalaryAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/hr/definingSalaryAction!goListContent.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<ul>
			...
			<li><a href="#" onclick="recordSearch();">运算</a></li>
			<li><a href="#" onclick="">清除</a></li>
			<li><a href="#" onclick="">更改</a></li>
		</ul>
		<div>
		...
		<div class="search_advanced">
			<label>名称:<input name="" type="text" class="int" /></label> <label>部门:<input name="" type="text" class="int" /></label>
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/hr/definingSalaryAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/hr/definingSalaryAction!goListContent.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/common_listx16.png" alt="" />工资项管理</a></li>
				</ul>
			</div>

...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/hr/salary/salaryProjectItemAction!goSaveOrUpdate.action?id='+id+"&title="+name,
		  cache: false,
	...
	/*  $.ajax({
		  url:'${vix}/hr/salary/salaryProjectItemAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
					...
					callback : function(action){
						if(action == 'ok'){
							...
							$.post('${vix}/hr/salary/salaryProjectItemAction!saveOrUpdate.action',
								queryString,
								function(result){
									showMessage(result);
									setTimeout("", 1000);
									pager("start","${vix}/hr/salary/salaryProjectItemAction!goSingleList.action?categoryName="+name,'sysDict');
								}
							 );
						}
					},
		...
		if(action == 'ok'){
			...
			$.post('${vix}/hr/salary/salaryProjectItemAction!saveOrUpdate.action',
				...
				function(result){
					showMessage(result);
					setTimeout("", 1000);
					loadContent('${vix}/hr/salary/salaryProjectItemAction!goList.action?title=${title}');
				}
			 );
		}
	});
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/salary/salaryProjectItemAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(result){
			  showMessage(result);
			  setTimeout("", 1000);
				pager("start","${vix}/hr/salary/salaryProjectItemAction!goSingleList.action?title="+name,'role');
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/hr/salary/salaryProjectItemAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&title="+name,'role');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/hr/salary/salaryProjectItemAction!goSingleList.action?title="+name,'role');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name="hrmgr_salary_set_salaryProjectItem" /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">

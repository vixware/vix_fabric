...
function deleteEntity(id){
	asyncbox.confirm('确定要删除该工资类别么?','<s:text name='cmn_message'/>',function(action){
		if(action == 'ok'){
			deleteById(id);
		}
	});
...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<th onclick="orderBy('projectName');" width="55%">类别名称&nbsp; <s:if test="%{pager.orderField == 'projectName' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if> <s:elseif test="%{pager.orderField == 'projectName' && pager.orderBy == 'desc' }">
					<img src="${vix}/common/img/arrow_up.gif">
				</s:elseif> <s:else>
					<img src="${vix}/common/img/arrow.gif">
				</s:else>
			...
			<th onclick="orderBy('projectNote');" width="10%">类别描述&nbsp; <s:if test="%{pager.orderField == 'projectNote' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if> <s:elseif test="%{pager.orderField == 'projectNote' && pager.orderBy == 'desc' }">
					<img src="${vix}/common/img/arrow_up.gif">
				</s:elseif> <s:else>
					<img src="${vix}/common/img/arrow.gif">
				</s:else>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td><s:if test="%{#entity.isAudit=='Y'}">是</s:if> <s:elseif test="%{#entity.isAudit=='N'}">否</s:elseif> <s:else>否</s:else></td>

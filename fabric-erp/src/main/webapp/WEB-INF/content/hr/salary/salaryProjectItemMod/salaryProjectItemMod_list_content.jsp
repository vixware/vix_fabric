...
function deleteEntity(id){
	asyncbox.confirm('确定要删除该工资项么?','<s:text name='cmn_message'/>',function(action){
		if(action == 'ok'){
			deleteById(id);
		}
	});
...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<th onclick="orderBy('name');" width="25%">工资项名称&nbsp; <s:if test="%{pager.orderField == 'name' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
			...
			<th onclick="orderBy('salaryType');" width="10%">类型&nbsp; <s:if test="%{pager.orderField == 'salaryType' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if> <s:elseif test="%{pager.orderField == 'salaryType' && pager.orderBy == 'desc' }">
					<img src="${vix}/common/img/arrow_up.gif">
				</s:elseif> <s:else>
					<img src="${vix}/common/img/arrow.gif">
				</s:else>
			</th>
			<th onclick="orderBy('dotHandler');" width="10%">位数处理&nbsp; <s:if test="%{pager.orderField == 'dotHandler' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if> <s:elseif test="%{pager.orderField == 'dotHandler' && pager.orderBy == 'desc' }">
					<img src="${vix}/common/img/arrow_up.gif">
				</s:elseif> <s:else>
					<img src="${vix}/common/img/arrow.gif">
				</s:else>
			...
			<th onclick="orderBy('isActive');" width="10%">是否启用&nbsp; <s:if test="%{pager.orderField == 'isActive' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if> <s:elseif test="%{pager.orderField == 'isActive' && pager.orderBy == 'desc' }">
					<img src="${vix}/common/img/arrow_up.gif">
				</s:elseif> <s:else>
					<img src="${vix}/common/img/arrow.gif">
				</s:else>
			</th>
			<th onclick="orderBy('defaultValue');" width="20%">初始值&nbsp; <s:if test="%{pager.orderField == 'defaultValue' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if> <s:elseif test="%{pager.orderField == 'defaultValue' && pager.orderBy == 'desc' }">
					<img src="${vix}/common/img/arrow_up.gif">
				</s:elseif> <s:else>
					<img src="${vix}/common/img/arrow.gif">
				</s:else>
			</th>
			<th onclick="orderBy('decutTaxFlag');" width="10%">是否扣税项&nbsp; <s:if test="%{pager.orderField == 'decutTaxFlag' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if> <s:elseif test="%{pager.orderField == 'decutTaxFlag' && pager.orderBy == 'desc' }">
					<img src="${vix}/common/img/arrow_up.gif">
				</s:elseif> <s:else>
					<img src="${vix}/common/img/arrow.gif">
				</s:else>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td><s:if test="%{#entity.decutTaxFlag=='Y'}">是</s:if> <s:elseif test="%{#entity.decutTaxFlag=='N'}">否</s:elseif> <s:else>否</s:else></td>
				<td><s:if test="tag != 'choose'">
					...
					</s:if> <s:if test="#entity.salaryType == @com.vix.core.constant.BizConstant@HR_SAL_ITEMTYPE_JSX">
						<a href="#" onclick="setItemModFormular('${entity.id}');" title="编辑公式"> <img src="${vix}/common/img/icon_edit.png" />
						</a>

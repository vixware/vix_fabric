...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/hr/salary/salaryProjectOptionAction!goSaveOrUpdate.action?id='+id+"&title="+name,
		  cache: false,
	...
	/*  $.ajax({
		  url:'${vix}/hr/salary/salaryProjectOptionAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
						...
						if(action == 'ok'){
							...
							$.post('${vix}/hr/salary/salaryProjectOptionAction!saveOrUpdate.action',
								...
								function(result){
									showMessage(result);
									setTimeout("", 1000);
									pager("start","${vix}/hr/salary/salaryProjectOptionAction!goSingleList.action?categoryName="+name,'sysDict');
								}
							 );
						}
					},
		...
		if(action == 'ok'){
			...
			$.post('${vix}/hr/salary/salaryProjectOptionAction!saveOrUpdate.action',
				...
				function(result){
					showMessage(result);
					setTimeout("", 1000);
					loadContent('${vix}/hr/salary/salaryProjectOptionAction!goList.action?title=${title}');
				}
			 );
		}
	});
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/salary/salaryProjectOptionAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(result){
			  showMessage(result);
			  setTimeout("", 1000);
				pager("start","${vix}/hr/salary/salaryProjectOptionAction!goSingleList.action?title="+name,'role');
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/hr/salary/salaryProjectOptionAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&title="+name,'role');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/hr/salary/salaryProjectOptionAction!goSingleList.action?title="+name,'role');
}

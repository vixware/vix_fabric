...
function saveOrUpdate(id){
	$.ajax({
		  url:'${vix}/hr/salary/salaryProjectItemModAction!goSaveOrUpdate.action?id='+id+"&title="+name,
		  cache: false,
	...
	/*  $.ajax({
		  url:'${vix}/hr/salary/salaryProjectItemModAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
				 	modal:true,
						...
						if(action == 'ok'){
							...
							$.post('${vix}/hr/salary/salaryProjectItemModAction!saveOrUpdate.action',
								...
								function(result){
									showMessage(result);
									setTimeout("", 1000);
									pager("start","${vix}/hr/salary/salaryProjectItemModAction!goSingleList.action?categoryName="+name,'sysDict');
								}
							 );
						}
					},
		...
		if(action == 'ok'){
			...
			$.post('${vix}/hr/salary/salaryProjectItemModAction!saveOrUpdate.action',
				...
				function(result){
					showMessage(result);
					setTimeout("", 1000);
					loadContent('${vix}/hr/salary/salaryProjectItemModAction!goList.action?title=${title}');
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/salary/salaryProjectItemModAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(result){
			  showMessage(result);
				setTimeout("", 1000);
				pager("start","${vix}/hr/salary/salaryProjectItemModAction!goSingleList.action?title="+name,'role');
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/hr/salary/salaryProjectItemModAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&title="+name,'role');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/hr/salary/salaryProjectItemModAction!goSingleList.action?title="+name,'role');
}

/**
 * 进入公式编辑页面
 */
function setItemModFormular(id){
	//alert("编辑公式页面!"+id);
	$.ajax({
		  url:'${vix}/hr/salary/salaryProjectItemModAction!goSaveOrUpdateFormula.action?id='+id,
		  cache: false,
		  success: function(html){
			  $("#mainContent").html(html);
		  }

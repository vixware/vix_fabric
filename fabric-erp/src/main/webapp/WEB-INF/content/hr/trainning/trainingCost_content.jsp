...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<th style="cursor: pointer;" onclick="orderBy('projectTraining');">培训项目&nbsp;<img id="categoryImg_projectTraining" src="${vix}/common/img/arrow.gif">
			</th>
			<th><s:text name="培训类型" /></th>
			<th><s:text name="费用类型" /></th>
			...
			<th style="cursor: pointer;" onclick="orderBy('costAudit');">审核人&nbsp;<img id="categoryImg_costAudit" src="${vix}/common/img/arrow.gif">
			</th>
			<th><s:text name="是否通过审核" /></th>
			<th style="cursor: pointer;" onclick="orderBy('activityStartDate');">提出时间&nbsp;<img id="categoryImg_activityStartDate" src="${vix}/common/img/arrow.gif">
			</th>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td><s:if test="%{#entity.trainingType == 1}">
						<span style="color: red;">内部培训</span>
					</s:if> <s:elseif test="%{#entity.trainingType == 2}">
						<span style="color: green;">公开培训</span>
					</s:elseif></td>
				<td><s:if test="%{#entity.costTypes == 1}">
						<span style="color: red;">公司报销</span>
					</s:if> <s:elseif test="%{#entity.costTypes == 2}">
						<span style="color: green;">个人支付</span>
					</s:elseif></td>
				...
				<td><s:if test="%{#entity.hroughApproval == 0}">
						<span style="color: red;">是</span>
					</s:if> <s:elseif test="%{#entity.hroughApproval == 1}">
						<span style="color: green;">否</span>
					</s:elseif></td>
				<td><span style="color: gray;"><fmt:formatDate value="${entity.costStartDate}" type="both" pattern="yyyy-MM-dd HH:mm:ss" /></a></td>
				<td><span style="color: gray;">${entity.remarks}</span></td>
				<td>
					<div class="untitled" style="position: static; display: inline;">
						<span><img alt="" src="img/icon_untitled.png"></span>
						<div class="popup" style="display: none; top: -7px;">
							<strong> <i class="close"><a href="#" onclick="deleteEntity('${entity.id}');"></a></i> <i><a href="#" onclick="saveOrUpdate('${entity.id}');" title="<s:text name='查看修改'/>"></a></i> <i><a href="#" onclick="saveOrUpdate('${entity.id}');" title="<s:text name='cmn_update'/>"></a></i> <b>${entity.projectTraining}</b>
							</strong>
							<p>${entity.remarks}</p>
						</div>
					</div>

	...
	<div class="table">
		<table class="list">
			<tr class="alt">
				...
				<th onclick="orderBy('channelName');">渠道名称</th>
				<th onclick="orderBy('channelCost');">费用</th>
				<th onclick="orderBy('province');">所在省份</th>
				<th onclick="orderBy('contactInformation');">联系方式</th>
			</tr>
			<s:iterator value="pager.resultList" var="entity" status="s">
				<tr>
					<td><input id="chkBrandId" name="chkBrandId" value="${entity.channelName}" type="radio" onclick="chkChange(this,'${entity.id}',${entity.channelName},'${entity.channelCost}','${entity.province}','${entity.contactInformation}')" /></td>
					<td>${s.count + (pager.pageSize * (pager.pageNo -1))}</td>
					<td>${entity.channelName}</td>
					<td>${entity.channelCost}</td>
					<td>${entity.province}</td>
					<td>${entity.contactInformation}</td>
				</tr>
			</s:iterator>
		</table>
	</div>

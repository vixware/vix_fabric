...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/hr/attendanceCategoryAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#supplierCategoryForm').validationEngine('validate')){
								$.post('${vix}/hr/attendanceCategoryAction!saveOrUpdate.action?parentId='+$("#newScId").val(), {
										...
										'attendanceCategory.categoryName' : $("#newName").val()
											...
											showMessage(result);
											setTimeout("",1000);
											loadContent('${vix}/hr/attendanceCategoryAction!goList.action');
										});
							}else{
			  					return false;
			  				}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/attendanceCategoryAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/hr/attendanceCategoryAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
function searchForContent(i){
	...
	loadContacts();
	if(i == 0){
		pager("start","${vix}/hr/attendanceCategoryAction!goSearchList.action?i="+i+"&searchContent="+code,'category');
	}
	else{
		pager("start","${vix}/hr/attendanceCategoryAction!goSearchList.action?i="+i+"&contacts="+contacts+"&name="+name,'category');
	}
}
/*判断搜索内容是否为空*/
...
loadCode();
//载入分页列表数据
pager("start","${vix}/hr/attendanceCategoryAction!goSingleList.action?code="+code+"&pageNo=${pageNo}",'category');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/hr/attendanceCategoryAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/hr/attendanceCategoryAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadCode();
	if(entity == 'category'){
		pager(tag,"${vix}/hr/attendanceCategoryAction!goSingleList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function srmStatus(status){
	pager("start","${vix}/hr/attendanceCategoryAction!goSingleList.action?status="+status,'category');
}
//最近使用
function srmRecent(rencentDate){
	pager("start","${vix}/hr/attendanceCategoryAction!goSingleList.action?updateTime="+rencentDate,'category');
}
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#">考勤管理</a></li>
				<li><a href="#">考勤设置</a></li>
				<li><a href="#" onclick="loadContent('${vix}/hr/attendanceCategoryAction!goList.action');">考勤类别</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/hr/attendanceCategoryAction!findTreeToJson.action",
						autoParam:["id"]
					},
					callback: {
						onClick: onClick
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/hr/attendanceCategoryAction!goSingleList.action?id="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/common_listx16.png" alt="" />考勤类别</a></li>
				</ul>
			</div>

...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:"${vix}/hr/attendanceRecordAction!goSaveOrUpdate.action?id="+id,
		  cache: false,
					...
					callback : function(action){
						if(action == 'ok'){
							if($('#brandForm').validationEngine('validate')){
								$.post('${vix}/hr/attendanceRecordAction!saveOrUpdate.action',
									 {'brand.id':$("#id").val(),
									  'brand.name':$("#name").val(),
									  'brand.memo':$("#memo").val()
									},
									function(result){
										asyncbox.open({
											...
											html:result,
											callback : function(action){
												pager("current","${vix}/hr/attendanceRecordAction!goSingleList.action?name="+name,'brand');
											},
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/attendanceRecordAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/hr/attendanceRecordAction!goRecordList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/hr/attendanceRecordAction!goRecordList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/hr/attendanceRecordAction!goRecordList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/hr/attendanceRecordAction!goRecordList.action?name="+name+'&parentId='+$('#selectId').val(),entity);
	}
}
...
function recordSearch(){
	...
	month=Url.encode(month);
	pager("start","${vix}/hr/attendanceRecordAction!goRecordList.action?recordYear="+year+"&recordMonth="+month,'category');
}
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/hr/attendanceRecordAction!goList.action');"><s:text name='hr_attendance_record' /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
			...
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/common_listx16.png" alt="" />
						<s:text name="hr_attendance_record" /></a></li>
				</ul>
			</div>

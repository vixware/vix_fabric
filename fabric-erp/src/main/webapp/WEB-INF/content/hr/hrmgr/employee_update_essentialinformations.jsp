...
$(function(){
	...
	function loadEssGrid(){
		...
		$('#dgdA1').datagrid({
			...
			striped: true,
			collapsible:true,
			url:'${vix}/hr/employeesAction!findEssOfPos.action?employeeId='+empId,
			remoteSort: false,
			idField:'id',
			frozenColumns:[[
	...
	function saveOrUpdateEss(){
		...
		$.post('${vix}/hr/employeesAction!saveOrUpdate.action?oId=${entity.organizationUnit.id}',
			queryString,
			function(result){
				...
				asyncbox.success(msg,"<s:text name='保存成功'/>",function (action){
				});
			}
		 );
	}
						...
						<dd style="display: block;">
							<form id="employeeEssForm">
								<table style="border: none;">
									...
									<%-- <tr>
										<td align="right">报销额度：</td>
										<td><input name="expenseAccount" type="text" size="30" value="${entity.expenseAccount}"/></td>
									</tr> --%>
								</table>
							</form>
						</dd>
		...
		<div class="clearfix" style="background: #FFF; position: relative;">
			...
			<div id="a2" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
				<div style="padding: 8px;">
					<table id="dlReceivedAddress" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="rownumbers:true,pagination: true,iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlReceivedAddressTb',url: '${vix}/hr/employeesAction!getLinkmanJson.action?employeeId=${entity.id}'">
						<thead>
							<tr>
					...
					<script type="text/javascript">
							...
							function addcItem(){
								$.ajax({
									  url:'${vix}/hr/employeesAction!goAddLinkman.action?id=${entity.id}',
									  cache: false,
									  success: function(html){
									 	  asyncbox.open({
											 	modal:true,
												...
												callback : function(action){
													if(action == 'ok'){
														if($('#caForm').validationEngine('validate')){
															$.post('${vix}/hr/employeesAction!saveOrUpdateLinkman.action?id=${entity.id}', {
																'linkman.id' : $("#daId").val(),
							...
							function editcItem(){
								...
								$.ajax({
									  url:'${vix}/hr/employeesAction!goAddLinkman.action?id=${entity.id}&linkid='+rows.id,
									  cache: false,
									  success: function(html){
										  asyncbox.open({
											 	modal:true,
							...
							function removecItem(){
								...
								if(row){
									...
									$.ajax({url:'${vix}/hr/employeesAction!deleteLinkmanById.action?linId='+row.id,cache: false});
								}
							}
					</script>
				</div>

...
function searchForContent(){
	loadName();
	pager("start","${vix}/hr/hrmgr/employeesAction!goSingleList.action?empName="+name,'category');
}

//高级搜索
function goSearch() {
	$.ajax({
	url : '${vix}/hr/hrmgr/employeesAction!goSearch.action',
	cache : false,
	success : function(html) {
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/hr/hrmgr/employeesAction!goSearchList.action?empName=" + $('#empName').val() + "&code=" + $('#code').val() + "&oldName=" + $('#oldName').val() + "&idNumber=" + $('#idNumber').val() + "&staffJobNumber=" + $('#staffJobNumber').val(), 'category');
			}
		},
...
function saveOrUpdate(id,parentId){
	if(id==0){
		if(parentId.substring(parentId.length-1,parentId.length)=='O'){
			$.ajax({
				  url:'${vix}/hr/employeesAction!goSaveOrUpdate.action?id='+id+'&parentId='+parentId,
				  cache: false,
				  success: function(html){
		...
		$.ajax({
			  url:'${vix}/hr/employeesAction!goSaveOrUpdate.action?id='+id,
			  cache: false,
			  success: function(html){
				  $("#mainContent").html(html);
			  }
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/employeesAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='message'/>",function(action){
				...
				pager("start","${vix}/hr/employeesAction!goSingleList.action?id="+treeNode.id+"&treeType="+treeNode.treeType,"category");
			});
		  }
	});
}
...
loadName();
//载入分页列表数据
pager("start","${vix}/hr/employeesAction!goSingleList.action?empName="+name+"&orderField=id&orderBy=desc",'category');
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/hr/employeesAction!goSingleList.action?empName=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/hr/employeesAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&empName="+name+"&id="+$("#selectId").val()+"&treeType="+$('#treeType').val(),'category');
}

function categoryPager(tag,entity){
	loadName();
	if(entity == 'category'){
		pager(tag,"${vix}/hr/employeesAction!goSingleList.action?empName="+name+'&id='+$('#selectId').val()+"&treeType="+$('#treeType').val() ,entity);
	}
}
...
/* 加载顶部工具栏 */
loadTopDynamicMenuContent('${vix}/hr/employeesAction!goTopDynamicMenuContent.action');
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#" onclick="loadContent('${vix}/hr/employeesAction!goList.action');">员工管理</a></li>
				<li><a href="#" onclick="loadContent('${vix}/hr/employeesAction!goList.action');">在职管理</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/hr/employeesAction!findOrgAndUnitTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/hr/employeesAction!goSingleList.action?empName="+name+"&id="+treeNode.id+"&treeType="+treeNode.treeType,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><a onclick="categoryTab(3,1,'a',event,'category')"><img src="img/common_listx16.png" alt="" />在职管理</a></li>
				</ul>
			</div>

...
function deleteById(id){
	$.ajax({
		  url:'${vix}/hr/thingsListAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/hr/thingsListAction!goListContent.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/hr/thingsListAction!goListContent.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/hr/thingsListAction!goListContent.action?name="+name,'brand');
}
	...
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"><s:text name='hr_things_list' /></a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0,$('#selectId').val());"><span><s:text name="cmn_add" /></span></a> <a href="#"><span><s:text name='hr_become_full_member' /></span></a> <a href="#"><span><s:text name='hr_particular_changes' /></span></a> <a href="#"><span><s:text name='hr_temporarily_transfer' /></span></a> <a href="#"><span><s:text
						name='hr_ask_for_leave' /></span></a> <a href="#"><span><s:text name='hr_resign' /></span></a> <a href="#"><span><s:text name='hr_dismiss' /></span></a> <a href="#"><span><s:text name='hr_retirement' /></span></a> <a href="#"><span><s:text name='hr_rehire' /></span></a>
		</p>
	</div>
</div>
<div class="content">

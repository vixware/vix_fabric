...
$(function(){
								...
								if(returnValue != undefined){
									...
									var posIds = posId.split(/[P,O,C]/);
										...
										$.post('${vix}/common/org/essOrgPositionAction!saveOrUpdateEssOfOrgPosition.action',
												queryString,
											function(result){
												asyncbox.success(result,"<s:text name='岗位'/>",function(action){
													loadEssGrid();
												}); 
											}
										 );
	...
	function loadEssGrid(){
		...
		$('#dgdA1').datagrid({
			...
			striped: true,
			collapsible:true,
			url:'${vix}/hr/employeeOrgFileAction!findEssOfPos.action?employeeId='+empId,
			remoteSort: false,
			idField:'id',
			frozenColumns:[[
	...
	function saveOrUpdateEss(){
		...
		$.post('${vix}/hr/employeeOrgFileAction!saveOrUpdate.action?oId=${entity.organizationUnit.id}',
			queryString,
			function(result){
		...
		<div class="clearfix" style="background: #FFF; position: relative;">
			...
			<div id="a2" style="width: 100%; visibility: hidden; position: absolute; top: -5000px;">
				<div style="padding: 8px;">
					<table id="dlReceivedAddress" class="easyui-datagrid" style="height: 450px; margin: 6px;" data-options="rownumbers:true,pagination: true,iconCls: 'icon-edit',singleSelect: true,toolbar: '#dlReceivedAddressTb',url: '${vix}/hr/employeeOrgFileAction!getLinkmanJson.action?employeeId=${entity.id}'">
						<thead>
							<tr>
					...
					<script type="text/javascript">
							...
							function addcItem(){
								$.ajax({
									  url:'${vix}/hr/employeeOrgFileAction!goAddLinkman.action?id=${entity.id}',
									  cache: false,
									  success: function(html){
									 	  asyncbox.open({
											 	modal:true,
												...
												callback : function(action){
													if(action == 'ok'){
														if($('#caForm').validationEngine('validate')){
															$.post('${vix}/hr/employeeOrgFileAction!saveOrUpdateLinkman.action?id=${entity.id}', {
																'linkman.id' : $("#daId").val(),
							...
							function editcItem(){
								...
								$.ajax({
									  url:'${vix}/hr/employeeOrgFileAction!goAddLinkman.action?id=${entity.id}&linkid='+rows.id,
									  cache: false,
									  success: function(html){
										  asyncbox.open({
											 	modal:true,
							...
							function removecItem(){
								...
								if(row){
									...
									$.ajax({url:'${vix}/hr/employeeOrgFileAction!deleteLinkmanById.action?linId='+row.id,cache: false});
								}
							}
					</script>
				</div>

...
function deleteEntity(id){
	asyncbox.confirm('确定要删除该岗位么?','提示信息',function(action){
		if(action == 'ok'){
			deleteById(id);
		}
	});
...
<table class="list">
	<tbody>
		<tr class="alt">
			...
			<th onclick="orderBy('posName');" width="30%">名称&nbsp; <s:if test="%{pager.orderField == 'posName' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if> <s:elseif test="%{pager.orderField == 'posName' && pager.orderBy == 'desc' }">
			...
			<th width="10%">编制人数&nbsp;</th>
			<th onclick="orderBy('isChargeDep');" width="15%">是否负责岗位&nbsp; <s:if test="%{pager.orderField == 'isChargeDep' && pager.orderBy == 'asc' }">
					<img src="${vix}/common/img/arrow_down.gif">
				</s:if> <s:elseif test="%{pager.orderField == 'isChargeDep' && pager.orderBy == 'desc' }">
					<img src="${vix}/common/img/arrow_up.gif">
				</s:elseif> <s:else>
					<img src="${vix}/common/img/arrow.gif">
				</s:else>
		...
		<s:iterator value="pager.resultList" var="entity" status="s">
			...
			<tr>
				...
				<td>${entity.personAmount}</td>
				<td><s:if test="%{#entity.isChargeDep.equals(\"1\")}">是</s:if> <s:elseif test="%{#entity.isChargeDep.equals(\"0\")}">否</s:elseif> <s:else>否</s:else></td>
				<td>
					<div class="untitled" style="position: static; display: inline;">
						<span><img alt="" src="img/icon_untitled.png"></span>
						<div class="popup" style="display: none; top: -7px;">
							<strong> <i class="close"><a href="#" onclick="deleteEntity('${entity.id}');" title="删除"><img src="${vix}/common/img/icon_12.png" /></a></i> <i><a href="#" onclick="showPosition('${entity.id}');" title="<s:text name='cmn_show'/>"></a></i> <i><a href="#" onclick="saveOrUpdate('${entity.id}');" title="修改"><img
										src="${vix}/common/img/icon_edit.png" /></a></i> <!-- 小i标题 --> <b>岗位</b>
							</strong>
							<!-- 小i内容 -->
							<p></p>
						</div>

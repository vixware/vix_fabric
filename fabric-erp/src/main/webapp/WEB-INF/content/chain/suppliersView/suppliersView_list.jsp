...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/chain/suppliersViewAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/chain/suppliersViewAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/chain/suppliersViewAction!goSingleList.action?name="+name,'settlementStatement');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/chain/suppliersViewAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'settlementStatement');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/chain/suppliersViewAction!goSingleList.action?name="+name,'settlementStatement');
}
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div class="search_advanced">
			<label><s:text name="cbm_material_coding1" /><input type="text" class="int" name=""> </label> <label><s:text name="cbm_materials1" /><input type="text" class="int" name=""> </label> <label><s:text name="cbm_coupon_number1" /><input type="text" class="int" name=""> </label>
		</div>
	</div>
	<div id="number">
		...
		<div id="right">
			<div class="right_content">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="settlementStatementInfo"></b> <s:text name='cmn_to' /> <b class="settlementStatementTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a> </span> <span><a class="end" onclick="currentPager('end');"></a> </span>
					</div>
				</div>
				<div id="settlementStatement" class="table"></div>
				<div class="pagelist drop">
					<ul>

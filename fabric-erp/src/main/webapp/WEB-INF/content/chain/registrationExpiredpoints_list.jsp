...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/registrationExpiredpointsAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/drp/registrationExpiredpointsAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/drp/registrationExpiredpointsAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/drp/registrationExpiredpointsAction!goSingleList.action?name="+name,'brand');
}
...
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				<li><a href="#"><img src="img/drp_channel.png" alt="" /> <s:text name="drp_registration_expiredpoints" /> </a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
	...
	<div class="drop" id="c_head">
		...
		<div>
			<label><s:text name="ldm_name" /><input type="text" class="int" id="nameS"> </label> <label><s:text name="cmn_my_item" /><input type="checkbox" value="" name=""> </label> <label><input type="button" value="<s:text name='cmn_simple_search'/>" class="btn" onclick="searchForContent();"><input type="button"
				value="<s:text name='cmn_reset'/>" class="btn" name=""> </label> <label> <input type="button" value="高级搜索" class="btn" onclick="goSearch();" /></label>
		</div>

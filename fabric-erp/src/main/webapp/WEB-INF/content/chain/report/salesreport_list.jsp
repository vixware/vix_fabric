...
function searchForContent(tag){
    ...
    pager("start","${vix}/inventory/salesReportAction!goSingleList.action?code="+code+"&name="+name+"&wimTransvouchName="+wimTransvouchName,'wimTransvouch');
}
/* 重置 */
...
function saleOrderStatus(status) {
	pager("start", "${vix}/inventory/salesReportAction!goSingleList.action?status=" + status, 'wimTransvouch');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/inventory/salesReportAction!goSingleList.action?days=" + days, 'wimTransvouch');
}
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/inventory/salesReportAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
loadName();
//载入分页列表数据
pager("start","${vix}/inventory/salesReportAction!goSingleList.action?name="+name,'wimTransvouch');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/inventory/salesReportAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy,'wimTransvouch');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/inventory/salesReportAction!goSingleList.action?name="+name,'wimTransvouch');
}
...
function goPrintAllocateTransfer(id){
	$.ajax({
		  url:'${vix}/inventory/salesReportAction!goPrintAllocateTransfer.action?id='+id,
		  cache: false,
		  success: function(html){
			LODOP=getLodop();  
...
function goShowAllocateTransfer(id,pageNo){
	$.ajax({
		  url:'${vix}/inventory/salesReportAction!goShowAllocateTransfer.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
		  success: function(html){
		  	$("#mainContent").html(html);
		  }
...
function goSearch() {
	$.ajax({
	url : '${vix}/inventory/salesReportAction!goSearch.action',
	cache : false,
	success : function(html) {
		asyncbox.open({
		modal : true,
		...
		callback : function(action) {
			if (action == 'ok') {
				pager("start", "${vix}/inventory/salesReportAction!goSingleList.action?code=" + $('#code').val() + "&name=" + $('#name').val()+ "&itemname=" + $('#itemname').val()+ "&itemcode=" + $('#itemcode').val(), 'wimTransvouch');
			}
		},
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/inventory/salesReportAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
...
function deleteByIds(ids) {
		$.ajax({
		url : '${vix}/inventory/salesReportAction!deleteByIds.action?ids=' + ids,
		cache : false,
		success : function(html) {
			asyncbox.success(html, "提示信息", function(action) {
				pager("start", "${vix}/inventory/salesReportAction!goSingleList.action?name=" + name, 'wimTransvouch');

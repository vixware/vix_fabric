...
loadMenuContent('${vix}/chain/drpMainAction!goMenuContent.action');
var name = "";
function loadName(){
	name = $('#nameS').val();
...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/chain/tranLogAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/chain/tranLogAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/chain/tranLogAction!goSingleList.action?name="+name,'tranLog');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/chain/tranLogAction!goSingleList.action?name=",'tranLog');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/chain/tranLogAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'tranLog');
}
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/chain/tranLogAction!goSingleList.action?name="+name,'tranLog');
}
</script>
<div class="sub_menu">
	<h2>
...
<div class="content">
	...
	<div id="number">
		...
		<ul id="numBox" class="numBox">
			<s:iterator value="tranLogList" var="c">
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/chain/tranLogAction!goSingleList.action?id="+treeNode.id+"&treeType="+treeNode.treeType,"tranLog");
				}
		...
		<div id="right">
			<div class="right_content" id="a1">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="tranLogInfo"></b> <s:text name='cmn_to' /> <b class="tranLogTotalCount"></b>)
						</em> <span><a class="next" onclick="currentPager('next');"></a> </span> <span><a class="end" onclick="currentPager('end');"></a> </span>
					</div>
				</div>
				<div id="tranLog" class="table"></div>
				<div class="pagelist drop">
					<ul>

...
function saveOrUpdate(id){
	if(id==0){
		   return  loadContent('${vix}/chain/pointsDetailAction!goSaveOrUpdate.action');
	}else
		return null;
};
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/chain/pointsDetailAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"提示信息",function(action){
				pager("start","${vix}/chain/pointsDetailAction!goSingleList.action?name="+name,'brand');
			});
		  }
	});
}
...
function orderBy(orderField){
	...
	pager("start","${vix}/chain/pointsDetailAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'brand');
}

bindSearch();
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/chain/pointsDetailAction!goSingleList.action?name="+name,'brand');
}
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div class="search_advanced">
			<label><s:text name="chain_number1" /><input type="text" class="int" name=""> </label> <label><s:text name="cmn_time1" /><input type="text" class="int" name=""> </label>
		</div>
	</div>
	<div id="number">

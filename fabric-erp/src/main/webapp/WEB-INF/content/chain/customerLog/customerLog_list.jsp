...
function saveOrUpdate(id,pageNo){
	$.ajax({
		  url:'${vix}/chain/customerLogAction!goSaveOrUpdate.action?id='+id+"&pageNo="+pageNo,
		  cache: false,
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/chain/customerLogAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				pager("start","${vix}/chain/customerLogAction!goSingleList.action?name="+name,'zoVipCardLog');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/chain/customerLogAction!goSingleList.action?name=",'zoVipCardLog');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/chain/customerLogAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'zoVipCardLog');
}
function currentPager(tag){
	loadName();
	pager(tag,"${vix}/chain/customerLogAction!goSingleList.action?name="+name,'zoVipCardLog');
}
</script>
<div class="sub_menu">
	<h2>
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/chain/customerLogAction!goSingleList.action?id="+treeNode.id+"&treeType="+treeNode.treeType,"zoVipCardLog");
				}
		...
		<div id="right">
			<div class="right_content" id="a1">
				<div class="pagelist drop">
					...
					<div>
						<span><a class="start" onclick="currentPager('start');"></a> </span> <span><a class="previous" onclick="currentPager('previous');"></a> </span> <em>(<b class="zoVipCardLogInfo"></b> <s:text name='cmn_to' /> <b class="zoVipCardLogTotalCount"></b>)

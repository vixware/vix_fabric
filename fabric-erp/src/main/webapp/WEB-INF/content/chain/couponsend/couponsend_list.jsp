...
loadName();
//载入分页列表数据
pager("start","${vix}/chain/couponSendAction!goSingleList.action?1=1",'coupon');
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/chain/couponSendAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name,'coupon');
}

function currentPager(tag){
	loadName();
	pager(tag,"${vix}/chain/couponSendAction!goSingleList.action?name="+name,'coupon');
}
bindSearch();

function chooseCustomerAccount(couponId) {
	$.ajax({
	url : '${vix}/chain/couponSendAction!goChooseCustomerAccount.action?couponId='+couponId,
	cache : false,
	success : function(html) {
		...
		callback : function(action, returnValue) {
			if (action == 'ok') {
				if (returnValue != '') {
					$.ajax({
					url : '${vix}/chain/couponSendAction!bindToCustomerAccount.action?customerAccountIds=' + returnValue + "&couponId=" + $("#couponId").val(),
					cache : false,
					success : function(result) {
						showMessage(result);
						setTimeout("", 1000);
						loadContent('${vix}/chain/couponSendAction!goList.action');
					}
					});
				} else {
					asyncbox.success("请选择会员!", "<s:text name='vix_message'/>");
					return false;
				}
			}
		},
...
function listbystatus(status) {
	pager("start", "${vix}/chain/couponSendAction!goSingleList.action?status=" + status, 'coupon');
}
/* 最近使用 */
function leastRecentlyUsed(days) {
	pager("start", "${vix}/chain/couponSendAction!goSingleList.action?days=" + days, 'coupon');
}
</script>
<div class="sub_menu">
	<h2>
...
<div class="content">
	<div class="drop" id="c_head">
		...
		<div class="search_advanced">
			<label>编码:<input type="text" class="int" name="" id="selectcode" placeholder="编码"></label> <label>名称:<input type="text" class="int" name="" id="selectName" placeholder="名称"></label> <label><input type="button" value="<s:text name='cmn_search'/>" class="btn" name="" onclick="searchForContent();"><input
		...
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">	
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/chain/couponSendAction!goSingleList.action?parentId="+treeNode.id+"&treeType="+treeNode.treeType,"coupon");
				}

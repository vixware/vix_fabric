...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:'${vix}/chain/memberManagelistsAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							$.post('${vix}/chain/memberManagelistsAction!saveOrUpdate.action',
								 {'category.id':$("#id").val(),
								...
								function(result){
									asyncbox.success(result,"<s:text name='cmn_message'/>",function(action){
										...
										pager("start","${vix}/chain/memberManagelistsAction!goSingleList.action?name="+name+"&id="+$("#selectId").val(),'category');
									});
								}
							 );
						}
...
function saveOrUpdate(id){
	...
	if(id==0){
	   return  loadContent('${vix}/chain/memberManagelistsAction!goSaveOrUpdate.action');
}else 
	return null;
};
function deleteById(id){
	$.ajax({
		  url:'${vix}/chain/memberManagelistsAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/chain/memberManagelistsAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/chain/memberManagelistsAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/chain/memberManagelistsAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
bindSearch();
bindSwitch();
...
<div class="sub_menu">
	...
	<div class="drop">
		<p>
			<a href="#" onclick="saveOrUpdate(0);"><span><s:text name='cmn_add' /> </span> </a> <a href="#" onclick="deleteByIds();"><span><s:text name='cmn_delete' /> </span> </a> <a href="#" onclick="saveOrUpdate(0);"><span><s:text name='drp_upgrade' /> </span> </a> <a href="#" onclick="deleteByIds();"><span><s:text name='drp_exchange' /> </span>
			</a> <a href="#" onclick="saveOrUpdate(0);"><span><s:text name='drp_integral_registration' /> </span> </a> <a href="#" onclick="deleteByIds();"><span><s:text name='drp_card_issued' /> </span> </a>
		</p>
	</div>
</div>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div class="search_advanced">
			<label><s:text name="cbm_compellation1" /><input type="text" class="int" name=""> </label><label><s:text name="cmn_time1" /><input type="text" class="int" name=""> </label>
		</div>
	</div>
	<div id="number">
	...
	<div class="box">
		<div id="left" class="ui-resizable" style="height: 500px;">
			...
			<script type="text/javascript">
				...
				var setting = {
					async: {
						enable: true,
						url:"${vix}/chain/memberManagelistsAction!findTreeToJson.action",
				...
				function onClick(event, treeId, treeNode, clickFlag) {
					...
					pager("start","${vix}/chain/memberManagelistsAction!goSingleList.action?parentId="+treeNode.id,"category");
				}
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><img src="img/mail.png" alt="" /> <s:text name="cbm_lists" /></li>
				</ul>
			</div>

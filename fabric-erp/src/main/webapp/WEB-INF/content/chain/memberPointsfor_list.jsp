...
function saveOrUpdate(id,parentId){
	$.ajax({
		  url:'${vix}/drp/memberPointsforAction!goSaveOrUpdate.action?id='+id,
		  cache: false,
		  success: function(html){
			  asyncbox.open({
					...
					callback : function(action){
						if(action == 'ok'){
							$.post('${vix}/drp/memberPointsforAction!saveOrUpdate.action',
								 {'category.id':$("#id").val(),
								...
								function(result){
									asyncbox.success(result,"<s:text name='cmn_message'/>",function(action){
										...
										pager("start","${vix}/drp/memberPointsforAction!goSingleList.action?name="+name+"&id="+$("#selectId").val(),'category');
									});
								}
							 );
						}
...
function deleteById(id){
	$.ajax({
		  url:'${vix}/drp/memberPointsforAction!deleteById.action?id='+id,
		  cache: false,
		  success: function(html){
			asyncbox.success(html,"<s:text name='cmn_message'/>",function(action){
				...
				zTree.reAsyncChildNodes(treeNode, "refresh");
				pager("start","${vix}/drp/memberPointsforAction!goSingleList.action?name="+name,'category');
			});
		  }
	});
}
...
function loadRoot(){
	$('#name').val("");
	$('#selectId').val("");
	pager("start","${vix}/drp/memberPointsforAction!goSingleList.action?name=",'category');
}
//排序 
function orderBy(orderField){
	...
	pager("start","${vix}/drp/memberPointsforAction!goSingleList.action?orderField="+orderField+"&orderBy="+orderBy+"&name="+name+"&parentId="+$("#selectId").val(),'category');
}
bindSearch();
bindSwitch();
...
<div class="sub_menu">
	<h2>
		...
		<div id="breadCrumb" class="breadCrumb module">
			<ul>
				...
				<li><a href="#"> <s:text name="drp_member_points_for" />
				</a></li>
			</ul>
		</div>
	</h2>
	<div class="drop">
		<p>
			<a href="#" onclick="deleteByIds();"><span><s:text name='drp_exchange' /> </span> </a>
		</p>
	</div>
</div>
...
<div class="content">
	<div id="c_head" class=" drop">
		...
		<div class="search_advanced">
			<label><s:text name="drp_coding1" /><input type="text" class="int" name=""> </label><label><s:text name="drp_compellation1" /><input type="text" class="int" name=""> </label>
		</div>
	</div>
	<div id="number">
	...
	<div class="box">
		...
		<div id="right">
			<div class="right_menu">
				<ul>
					<li class="current"><img src="img/mail.png" alt="" /> <s:text name="drp_member_points_for" /></li>
				</ul>
			</div>
